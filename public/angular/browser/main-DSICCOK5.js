var Em = Object.defineProperty,
    Cm = Object.defineProperties;
var Im = Object.getOwnPropertyDescriptors;
var Yl = Object.getOwnPropertySymbols;
var wm = Object.prototype.hasOwnProperty,
    bm = Object.prototype.propertyIsEnumerable;
var Ql = (e, n, t) =>
        n in e
            ? Em(e, n, {
                  enumerable: !0,
                  configurable: !0,
                  writable: !0,
                  value: t,
              })
            : (e[n] = t),
    g = (e, n) => {
        for (var t in (n ||= {})) wm.call(n, t) && Ql(e, t, n[t]);
        if (Yl) for (var t of Yl(n)) bm.call(n, t) && Ql(e, t, n[t]);
        return e;
    },
    B = (e, n) => Cm(e, Im(n));
var ue = null,
    Ko = !1,
    wa = 1,
    Tm = null,
    te = Symbol("SIGNAL");
function S(e) {
    let n = ue;
    return ((ue = e), n);
}
function Jo() {
    return ue;
}
var tn = {
    version: 0,
    lastCleanEpoch: 0,
    dirty: !1,
    producers: void 0,
    producersTail: void 0,
    consumers: void 0,
    consumersTail: void 0,
    recomputing: !1,
    consumerAllowSignalWrites: !1,
    consumerIsAlwaysLive: !1,
    kind: "unknown",
    producerMustRecompute: () => !1,
    producerRecomputeValue: () => {},
    consumerMarkedDirty: () => {},
    consumerOnSignalRead: () => {},
};
function nn(e) {
    if (Ko) throw new Error("");
    if (ue === null) return;
    ue.consumerOnSignalRead(e);
    let n = ue.producersTail;
    if (n !== void 0 && n.producer === e) return;
    let t,
        r = ue.recomputing;
    if (
        r &&
        ((t = n !== void 0 ? n.nextProducer : ue.producers),
        t !== void 0 && t.producer === e)
    ) {
        ((ue.producersTail = t), (t.lastReadVersion = e.version));
        return;
    }
    let o = e.consumersTail;
    if (o !== void 0 && o.consumer === ue && (!r || Mm(o, ue))) return;
    let i = Vn(ue),
        s = {
            producer: e,
            consumer: ue,
            nextProducer: t,
            prevConsumer: o,
            lastReadVersion: e.version,
            nextConsumer: void 0,
        };
    ((ue.producersTail = s),
        n !== void 0 ? (n.nextProducer = s) : (ue.producers = s),
        i && ed(e, s));
}
function Kl() {
    wa++;
}
function rn(e) {
    if (!(Vn(e) && !e.dirty) && !(!e.dirty && e.lastCleanEpoch === wa)) {
        if (!e.producerMustRecompute(e) && !Xo(e)) {
            Un(e);
            return;
        }
        (e.producerRecomputeValue(e), Un(e));
    }
}
function ba(e) {
    if (e.consumers === void 0) return;
    let n = Ko;
    Ko = !0;
    try {
        for (let t = e.consumers; t !== void 0; t = t.nextConsumer) {
            let r = t.consumer;
            r.dirty || Sm(r);
        }
    } finally {
        Ko = n;
    }
}
function Ta() {
    return ue?.consumerAllowSignalWrites !== !1;
}
function Sm(e) {
    ((e.dirty = !0), ba(e), e.consumerMarkedDirty?.(e));
}
function Un(e) {
    ((e.dirty = !1), (e.lastCleanEpoch = wa));
}
function on(e) {
    return (e && Jl(e), S(e));
}
function Jl(e) {
    ((e.producersTail = void 0), (e.recomputing = !0));
}
function Bn(e, n) {
    (S(n), e && Xl(e));
}
function Xl(e) {
    e.recomputing = !1;
    let n = e.producersTail,
        t = n !== void 0 ? n.nextProducer : e.producers;
    if (t !== void 0) {
        if (Vn(e))
            do t = Sa(t);
            while (t !== void 0);
        n !== void 0 ? (n.nextProducer = void 0) : (e.producers = void 0);
    }
}
function Xo(e) {
    for (let n = e.producers; n !== void 0; n = n.nextProducer) {
        let t = n.producer,
            r = n.lastReadVersion;
        if (r !== t.version || (rn(t), r !== t.version)) return !0;
    }
    return !1;
}
function Ar(e) {
    if (Vn(e)) {
        let n = e.producers;
        for (; n !== void 0; ) n = Sa(n);
    }
    ((e.producers = void 0),
        (e.producersTail = void 0),
        (e.consumers = void 0),
        (e.consumersTail = void 0));
}
function ed(e, n) {
    let t = e.consumersTail,
        r = Vn(e);
    if (
        (t !== void 0
            ? ((n.nextConsumer = t.nextConsumer), (t.nextConsumer = n))
            : ((n.nextConsumer = void 0), (e.consumers = n)),
        (n.prevConsumer = t),
        (e.consumersTail = n),
        !r)
    )
        for (let o = e.producers; o !== void 0; o = o.nextProducer)
            ed(o.producer, o);
}
function Sa(e) {
    let n = e.producer,
        t = e.nextProducer,
        r = e.nextConsumer,
        o = e.prevConsumer;
    if (
        ((e.nextConsumer = void 0),
        (e.prevConsumer = void 0),
        r !== void 0 ? (r.prevConsumer = o) : (n.consumersTail = o),
        o !== void 0)
    )
        o.nextConsumer = r;
    else if (((n.consumers = r), !Vn(n))) {
        let i = n.producers;
        for (; i !== void 0; ) i = Sa(i);
    }
    return t;
}
function Vn(e) {
    return e.consumerIsAlwaysLive || e.consumers !== void 0;
}
function xr(e) {
    Tm?.(e);
}
function Mm(e, n) {
    let t = n.producersTail;
    if (t !== void 0) {
        let r = n.producers;
        do {
            if (r === e) return !0;
            if (r === t) break;
            r = r.nextProducer;
        } while (r !== void 0);
    }
    return !1;
}
function Or(e, n) {
    return Object.is(e, n);
}
function ei(e, n) {
    let t = Object.create(_m);
    ((t.computation = e), n !== void 0 && (t.equal = n));
    let r = () => {
        if ((rn(t), nn(t), t.value === rt)) throw t.error;
        return t.value;
    };
    return ((r[te] = t), xr(t), r);
}
var Xt = Symbol("UNSET"),
    en = Symbol("COMPUTING"),
    rt = Symbol("ERRORED"),
    _m = B(g({}, tn), {
        value: Xt,
        dirty: !0,
        error: null,
        equal: Or,
        kind: "computed",
        producerMustRecompute(e) {
            return e.value === Xt || e.value === en;
        },
        producerRecomputeValue(e) {
            if (e.value === en) throw new Error("");
            let n = e.value;
            e.value = en;
            let t = on(e),
                r,
                o = !1;
            try {
                ((r = e.computation()),
                    S(null),
                    (o = n !== Xt && n !== rt && r !== rt && e.equal(n, r)));
            } catch (i) {
                ((r = rt), (e.error = i));
            } finally {
                Bn(e, t);
            }
            if (o) {
                e.value = n;
                return;
            }
            ((e.value = r), e.version++);
        },
    });
function Nm() {
    throw new Error();
}
var td = Nm;
function nd(e) {
    td(e);
}
function Ma(e) {
    td = e;
}
var Rm = null;
function _a(e, n) {
    let t = Object.create(ni);
    ((t.value = e), n !== void 0 && (t.equal = n));
    let r = () => rd(t);
    return ((r[te] = t), xr(t), [r, (s) => sn(t, s), (s) => ti(t, s)]);
}
function rd(e) {
    return (nn(e), e.value);
}
function sn(e, n) {
    (Ta() || nd(e), e.equal(e.value, n) || ((e.value = n), Am(e)));
}
function ti(e, n) {
    (Ta() || nd(e), sn(e, n(e.value)));
}
var ni = B(g({}, tn), { equal: Or, value: void 0, kind: "signal" });
function Am(e) {
    (e.version++, Kl(), ba(e), Rm?.(e));
}
function M(e) {
    return typeof e == "function";
}
function Hn(e) {
    let t = e((r) => {
        (Error.call(r), (r.stack = new Error().stack));
    });
    return (
        (t.prototype = Object.create(Error.prototype)),
        (t.prototype.constructor = t),
        t
    );
}
var ri = Hn(
    (e) =>
        function (t) {
            (e(this),
                (this.message = t
                    ? `${t.length} errors occurred during unsubscription:
${t.map((r, o) => `${o + 1}) ${r.toString()}`).join(`
  `)}`
                    : ""),
                (this.name = "UnsubscriptionError"),
                (this.errors = t));
        },
);
function Pr(e, n) {
    if (e) {
        let t = e.indexOf(n);
        0 <= t && e.splice(t, 1);
    }
}
var ne = class e {
    constructor(n) {
        ((this.initialTeardown = n),
            (this.closed = !1),
            (this._parentage = null),
            (this._finalizers = null));
    }
    unsubscribe() {
        let n;
        if (!this.closed) {
            this.closed = !0;
            let { _parentage: t } = this;
            if (t)
                if (((this._parentage = null), Array.isArray(t)))
                    for (let i of t) i.remove(this);
                else t.remove(this);
            let { initialTeardown: r } = this;
            if (M(r))
                try {
                    r();
                } catch (i) {
                    n = i instanceof ri ? i.errors : [i];
                }
            let { _finalizers: o } = this;
            if (o) {
                this._finalizers = null;
                for (let i of o)
                    try {
                        od(i);
                    } catch (s) {
                        ((n = n ?? []),
                            s instanceof ri
                                ? (n = [...n, ...s.errors])
                                : n.push(s));
                    }
            }
            if (n) throw new ri(n);
        }
    }
    add(n) {
        var t;
        if (n && n !== this)
            if (this.closed) od(n);
            else {
                if (n instanceof e) {
                    if (n.closed || n._hasParent(this)) return;
                    n._addParent(this);
                }
                (this._finalizers =
                    (t = this._finalizers) !== null && t !== void 0
                        ? t
                        : []).push(n);
            }
    }
    _hasParent(n) {
        let { _parentage: t } = this;
        return t === n || (Array.isArray(t) && t.includes(n));
    }
    _addParent(n) {
        let { _parentage: t } = this;
        this._parentage = Array.isArray(t) ? (t.push(n), t) : t ? [t, n] : n;
    }
    _removeParent(n) {
        let { _parentage: t } = this;
        t === n ? (this._parentage = null) : Array.isArray(t) && Pr(t, n);
    }
    remove(n) {
        let { _finalizers: t } = this;
        (t && Pr(t, n), n instanceof e && n._removeParent(this));
    }
};
ne.EMPTY = (() => {
    let e = new ne();
    return ((e.closed = !0), e);
})();
var Na = ne.EMPTY;
function oi(e) {
    return (
        e instanceof ne ||
        (e && "closed" in e && M(e.remove) && M(e.add) && M(e.unsubscribe))
    );
}
function od(e) {
    M(e) ? e() : e.unsubscribe();
}
var He = {
    onUnhandledError: null,
    onStoppedNotification: null,
    Promise: void 0,
    useDeprecatedSynchronousErrorHandling: !1,
    useDeprecatedNextContext: !1,
};
var $n = {
    setTimeout(e, n, ...t) {
        let { delegate: r } = $n;
        return r?.setTimeout
            ? r.setTimeout(e, n, ...t)
            : setTimeout(e, n, ...t);
    },
    clearTimeout(e) {
        let { delegate: n } = $n;
        return (n?.clearTimeout || clearTimeout)(e);
    },
    delegate: void 0,
};
function ii(e) {
    $n.setTimeout(() => {
        let { onUnhandledError: n } = He;
        if (n) n(e);
        else throw e;
    });
}
function kr() {}
var id = Ra("C", void 0, void 0);
function sd(e) {
    return Ra("E", void 0, e);
}
function ad(e) {
    return Ra("N", e, void 0);
}
function Ra(e, n, t) {
    return { kind: e, value: n, error: t };
}
var an = null;
function zn(e) {
    if (He.useDeprecatedSynchronousErrorHandling) {
        let n = !an;
        if ((n && (an = { errorThrown: !1, error: null }), e(), n)) {
            let { errorThrown: t, error: r } = an;
            if (((an = null), t)) throw r;
        }
    } else e();
}
function cd(e) {
    He.useDeprecatedSynchronousErrorHandling &&
        an &&
        ((an.errorThrown = !0), (an.error = e));
}
var cn = class extends ne {
        constructor(n) {
            (super(),
                (this.isStopped = !1),
                n
                    ? ((this.destination = n), oi(n) && n.add(this))
                    : (this.destination = Pm));
        }
        static create(n, t, r) {
            return new Gn(n, t, r);
        }
        next(n) {
            this.isStopped ? xa(ad(n), this) : this._next(n);
        }
        error(n) {
            this.isStopped
                ? xa(sd(n), this)
                : ((this.isStopped = !0), this._error(n));
        }
        complete() {
            this.isStopped
                ? xa(id, this)
                : ((this.isStopped = !0), this._complete());
        }
        unsubscribe() {
            this.closed ||
                ((this.isStopped = !0),
                super.unsubscribe(),
                (this.destination = null));
        }
        _next(n) {
            this.destination.next(n);
        }
        _error(n) {
            try {
                this.destination.error(n);
            } finally {
                this.unsubscribe();
            }
        }
        _complete() {
            try {
                this.destination.complete();
            } finally {
                this.unsubscribe();
            }
        }
    },
    xm = Function.prototype.bind;
function Aa(e, n) {
    return xm.call(e, n);
}
var Oa = class {
        constructor(n) {
            this.partialObserver = n;
        }
        next(n) {
            let { partialObserver: t } = this;
            if (t.next)
                try {
                    t.next(n);
                } catch (r) {
                    si(r);
                }
        }
        error(n) {
            let { partialObserver: t } = this;
            if (t.error)
                try {
                    t.error(n);
                } catch (r) {
                    si(r);
                }
            else si(n);
        }
        complete() {
            let { partialObserver: n } = this;
            if (n.complete)
                try {
                    n.complete();
                } catch (t) {
                    si(t);
                }
        }
    },
    Gn = class extends cn {
        constructor(n, t, r) {
            super();
            let o;
            if (M(n) || !n)
                o = {
                    next: n ?? void 0,
                    error: t ?? void 0,
                    complete: r ?? void 0,
                };
            else {
                let i;
                this && He.useDeprecatedNextContext
                    ? ((i = Object.create(n)),
                      (i.unsubscribe = () => this.unsubscribe()),
                      (o = {
                          next: n.next && Aa(n.next, i),
                          error: n.error && Aa(n.error, i),
                          complete: n.complete && Aa(n.complete, i),
                      }))
                    : (o = n);
            }
            this.destination = new Oa(o);
        }
    };
function si(e) {
    He.useDeprecatedSynchronousErrorHandling ? cd(e) : ii(e);
}
function Om(e) {
    throw e;
}
function xa(e, n) {
    let { onStoppedNotification: t } = He;
    t && $n.setTimeout(() => t(e, n));
}
var Pm = { closed: !0, next: kr, error: Om, complete: kr };
var qn = (typeof Symbol == "function" && Symbol.observable) || "@@observable";
function $e(e) {
    return e;
}
function Pa(...e) {
    return ka(e);
}
function ka(e) {
    return e.length === 0
        ? $e
        : e.length === 1
          ? e[0]
          : function (t) {
                return e.reduce((r, o) => o(r), t);
            };
}
var O = (() => {
    class e {
        constructor(t) {
            t && (this._subscribe = t);
        }
        lift(t) {
            let r = new e();
            return ((r.source = this), (r.operator = t), r);
        }
        subscribe(t, r, o) {
            let i = Fm(t) ? t : new Gn(t, r, o);
            return (
                zn(() => {
                    let { operator: s, source: a } = this;
                    i.add(
                        s
                            ? s.call(i, a)
                            : a
                              ? this._subscribe(i)
                              : this._trySubscribe(i),
                    );
                }),
                i
            );
        }
        _trySubscribe(t) {
            try {
                return this._subscribe(t);
            } catch (r) {
                t.error(r);
            }
        }
        forEach(t, r) {
            return (
                (r = ud(r)),
                new r((o, i) => {
                    let s = new Gn({
                        next: (a) => {
                            try {
                                t(a);
                            } catch (c) {
                                (i(c), s.unsubscribe());
                            }
                        },
                        error: i,
                        complete: o,
                    });
                    this.subscribe(s);
                })
            );
        }
        _subscribe(t) {
            var r;
            return (r = this.source) === null || r === void 0
                ? void 0
                : r.subscribe(t);
        }
        [qn]() {
            return this;
        }
        pipe(...t) {
            return ka(t)(this);
        }
        toPromise(t) {
            return (
                (t = ud(t)),
                new t((r, o) => {
                    let i;
                    this.subscribe(
                        (s) => (i = s),
                        (s) => o(s),
                        () => r(i),
                    );
                })
            );
        }
    }
    return ((e.create = (n) => new e(n)), e);
})();
function ud(e) {
    var n;
    return (n = e ?? He.Promise) !== null && n !== void 0 ? n : Promise;
}
function km(e) {
    return e && M(e.next) && M(e.error) && M(e.complete);
}
function Fm(e) {
    return (e && e instanceof cn) || (km(e) && oi(e));
}
function Lm(e) {
    return M(e?.lift);
}
function L(e) {
    return (n) => {
        if (Lm(n))
            return n.lift(function (t) {
                try {
                    return e(t, this);
                } catch (r) {
                    this.error(r);
                }
            });
        throw new TypeError("Unable to lift unknown Observable type");
    };
}
function V(e, n, t, r, o) {
    return new Fa(e, n, t, r, o);
}
var Fa = class extends cn {
    constructor(n, t, r, o, i, s) {
        (super(n),
            (this.onFinalize = i),
            (this.shouldUnsubscribe = s),
            (this._next = t
                ? function (a) {
                      try {
                          t(a);
                      } catch (c) {
                          n.error(c);
                      }
                  }
                : super._next),
            (this._error = o
                ? function (a) {
                      try {
                          o(a);
                      } catch (c) {
                          n.error(c);
                      } finally {
                          this.unsubscribe();
                      }
                  }
                : super._error),
            (this._complete = r
                ? function () {
                      try {
                          r();
                      } catch (a) {
                          n.error(a);
                      } finally {
                          this.unsubscribe();
                      }
                  }
                : super._complete));
    }
    unsubscribe() {
        var n;
        if (!this.shouldUnsubscribe || this.shouldUnsubscribe()) {
            let { closed: t } = this;
            (super.unsubscribe(),
                !t &&
                    ((n = this.onFinalize) === null ||
                        n === void 0 ||
                        n.call(this)));
        }
    }
};
var ld = Hn(
    (e) =>
        function () {
            (e(this),
                (this.name = "ObjectUnsubscribedError"),
                (this.message = "object unsubscribed"));
        },
);
var re = (() => {
        class e extends O {
            constructor() {
                (super(),
                    (this.closed = !1),
                    (this.currentObservers = null),
                    (this.observers = []),
                    (this.isStopped = !1),
                    (this.hasError = !1),
                    (this.thrownError = null));
            }
            lift(t) {
                let r = new ai(this, this);
                return ((r.operator = t), r);
            }
            _throwIfClosed() {
                if (this.closed) throw new ld();
            }
            next(t) {
                zn(() => {
                    if ((this._throwIfClosed(), !this.isStopped)) {
                        this.currentObservers ||
                            (this.currentObservers = Array.from(
                                this.observers,
                            ));
                        for (let r of this.currentObservers) r.next(t);
                    }
                });
            }
            error(t) {
                zn(() => {
                    if ((this._throwIfClosed(), !this.isStopped)) {
                        ((this.hasError = this.isStopped = !0),
                            (this.thrownError = t));
                        let { observers: r } = this;
                        for (; r.length; ) r.shift().error(t);
                    }
                });
            }
            complete() {
                zn(() => {
                    if ((this._throwIfClosed(), !this.isStopped)) {
                        this.isStopped = !0;
                        let { observers: t } = this;
                        for (; t.length; ) t.shift().complete();
                    }
                });
            }
            unsubscribe() {
                ((this.isStopped = this.closed = !0),
                    (this.observers = this.currentObservers = null));
            }
            get observed() {
                var t;
                return (
                    ((t = this.observers) === null || t === void 0
                        ? void 0
                        : t.length) > 0
                );
            }
            _trySubscribe(t) {
                return (this._throwIfClosed(), super._trySubscribe(t));
            }
            _subscribe(t) {
                return (
                    this._throwIfClosed(),
                    this._checkFinalizedStatuses(t),
                    this._innerSubscribe(t)
                );
            }
            _innerSubscribe(t) {
                let { hasError: r, isStopped: o, observers: i } = this;
                return r || o
                    ? Na
                    : ((this.currentObservers = null),
                      i.push(t),
                      new ne(() => {
                          ((this.currentObservers = null), Pr(i, t));
                      }));
            }
            _checkFinalizedStatuses(t) {
                let { hasError: r, thrownError: o, isStopped: i } = this;
                r ? t.error(o) : i && t.complete();
            }
            asObservable() {
                let t = new O();
                return ((t.source = this), t);
            }
        }
        return ((e.create = (n, t) => new ai(n, t)), e);
    })(),
    ai = class extends re {
        constructor(n, t) {
            (super(), (this.destination = n), (this.source = t));
        }
        next(n) {
            var t, r;
            (r =
                (t = this.destination) === null || t === void 0
                    ? void 0
                    : t.next) === null ||
                r === void 0 ||
                r.call(t, n);
        }
        error(n) {
            var t, r;
            (r =
                (t = this.destination) === null || t === void 0
                    ? void 0
                    : t.error) === null ||
                r === void 0 ||
                r.call(t, n);
        }
        complete() {
            var n, t;
            (t =
                (n = this.destination) === null || n === void 0
                    ? void 0
                    : n.complete) === null ||
                t === void 0 ||
                t.call(n);
        }
        _subscribe(n) {
            var t, r;
            return (r =
                (t = this.source) === null || t === void 0
                    ? void 0
                    : t.subscribe(n)) !== null && r !== void 0
                ? r
                : Na;
        }
    };
var oe = class extends re {
    constructor(n) {
        (super(), (this._value = n));
    }
    get value() {
        return this.getValue();
    }
    _subscribe(n) {
        let t = super._subscribe(n);
        return (!t.closed && n.next(this._value), t);
    }
    getValue() {
        let { hasError: n, thrownError: t, _value: r } = this;
        if (n) throw t;
        return (this._throwIfClosed(), r);
    }
    next(n) {
        super.next((this._value = n));
    }
};
var ie = new O((e) => e.complete());
function dd(e) {
    return e && M(e.schedule);
}
function fd(e) {
    return e[e.length - 1];
}
function pd(e) {
    return M(fd(e)) ? e.pop() : void 0;
}
function St(e) {
    return dd(fd(e)) ? e.pop() : void 0;
}
function gd(e, n, t, r) {
    function o(i) {
        return i instanceof t
            ? i
            : new t(function (s) {
                  s(i);
              });
    }
    return new (t || (t = Promise))(function (i, s) {
        function a(l) {
            try {
                u(r.next(l));
            } catch (d) {
                s(d);
            }
        }
        function c(l) {
            try {
                u(r.throw(l));
            } catch (d) {
                s(d);
            }
        }
        function u(l) {
            l.done ? i(l.value) : o(l.value).then(a, c);
        }
        u((r = r.apply(e, n || [])).next());
    });
}
function hd(e) {
    var n = typeof Symbol == "function" && Symbol.iterator,
        t = n && e[n],
        r = 0;
    if (t) return t.call(e);
    if (e && typeof e.length == "number")
        return {
            next: function () {
                return (
                    e && r >= e.length && (e = void 0),
                    { value: e && e[r++], done: !e }
                );
            },
        };
    throw new TypeError(
        n ? "Object is not iterable." : "Symbol.iterator is not defined.",
    );
}
function un(e) {
    return this instanceof un ? ((this.v = e), this) : new un(e);
}
function md(e, n, t) {
    if (!Symbol.asyncIterator)
        throw new TypeError("Symbol.asyncIterator is not defined.");
    var r = t.apply(e, n || []),
        o,
        i = [];
    return (
        (o = Object.create(
            (typeof AsyncIterator == "function" ? AsyncIterator : Object)
                .prototype,
        )),
        a("next"),
        a("throw"),
        a("return", s),
        (o[Symbol.asyncIterator] = function () {
            return this;
        }),
        o
    );
    function s(f) {
        return function (y) {
            return Promise.resolve(y).then(f, d);
        };
    }
    function a(f, y) {
        r[f] &&
            ((o[f] = function (k) {
                return new Promise(function (C, _) {
                    i.push([f, k, C, _]) > 1 || c(f, k);
                });
            }),
            y && (o[f] = y(o[f])));
    }
    function c(f, y) {
        try {
            u(r[f](y));
        } catch (k) {
            h(i[0][3], k);
        }
    }
    function u(f) {
        f.value instanceof un
            ? Promise.resolve(f.value.v).then(l, d)
            : h(i[0][2], f);
    }
    function l(f) {
        c("next", f);
    }
    function d(f) {
        c("throw", f);
    }
    function h(f, y) {
        (f(y), i.shift(), i.length && c(i[0][0], i[0][1]));
    }
}
function vd(e) {
    if (!Symbol.asyncIterator)
        throw new TypeError("Symbol.asyncIterator is not defined.");
    var n = e[Symbol.asyncIterator],
        t;
    return n
        ? n.call(e)
        : ((e = typeof hd == "function" ? hd(e) : e[Symbol.iterator]()),
          (t = {}),
          r("next"),
          r("throw"),
          r("return"),
          (t[Symbol.asyncIterator] = function () {
              return this;
          }),
          t);
    function r(i) {
        t[i] =
            e[i] &&
            function (s) {
                return new Promise(function (a, c) {
                    ((s = e[i](s)), o(a, c, s.done, s.value));
                });
            };
    }
    function o(i, s, a, c) {
        Promise.resolve(c).then(function (u) {
            i({ value: u, done: a });
        }, s);
    }
}
var ci = (e) => e && typeof e.length == "number" && typeof e != "function";
function ui(e) {
    return M(e?.then);
}
function li(e) {
    return M(e[qn]);
}
function di(e) {
    return Symbol.asyncIterator && M(e?.[Symbol.asyncIterator]);
}
function fi(e) {
    return new TypeError(
        `You provided ${e !== null && typeof e == "object" ? "an invalid object" : `'${e}'`} where a stream was expected. You can provide an Observable, Promise, ReadableStream, Array, AsyncIterable, or Iterable.`,
    );
}
function jm() {
    return typeof Symbol != "function" || !Symbol.iterator
        ? "@@iterator"
        : Symbol.iterator;
}
var pi = jm();
function hi(e) {
    return M(e?.[pi]);
}
function gi(e) {
    return md(this, arguments, function* () {
        let t = e.getReader();
        try {
            for (;;) {
                let { value: r, done: o } = yield un(t.read());
                if (o) return yield un(void 0);
                yield yield un(r);
            }
        } finally {
            t.releaseLock();
        }
    });
}
function mi(e) {
    return M(e?.getReader);
}
function ee(e) {
    if (e instanceof O) return e;
    if (e != null) {
        if (li(e)) return Um(e);
        if (ci(e)) return Bm(e);
        if (ui(e)) return Vm(e);
        if (di(e)) return yd(e);
        if (hi(e)) return Hm(e);
        if (mi(e)) return $m(e);
    }
    throw fi(e);
}
function Um(e) {
    return new O((n) => {
        let t = e[qn]();
        if (M(t.subscribe)) return t.subscribe(n);
        throw new TypeError(
            "Provided object does not correctly implement Symbol.observable",
        );
    });
}
function Bm(e) {
    return new O((n) => {
        for (let t = 0; t < e.length && !n.closed; t++) n.next(e[t]);
        n.complete();
    });
}
function Vm(e) {
    return new O((n) => {
        e.then(
            (t) => {
                n.closed || (n.next(t), n.complete());
            },
            (t) => n.error(t),
        ).then(null, ii);
    });
}
function Hm(e) {
    return new O((n) => {
        for (let t of e) if ((n.next(t), n.closed)) return;
        n.complete();
    });
}
function yd(e) {
    return new O((n) => {
        zm(e, n).catch((t) => n.error(t));
    });
}
function $m(e) {
    return yd(gi(e));
}
function zm(e, n) {
    var t, r, o, i;
    return gd(this, void 0, void 0, function* () {
        try {
            for (t = vd(e); (r = yield t.next()), !r.done; ) {
                let s = r.value;
                if ((n.next(s), n.closed)) return;
            }
        } catch (s) {
            o = { error: s };
        } finally {
            try {
                r && !r.done && (i = t.return) && (yield i.call(t));
            } finally {
                if (o) throw o.error;
            }
        }
        n.complete();
    });
}
function De(e, n, t, r = 0, o = !1) {
    let i = n.schedule(function () {
        (t(), o ? e.add(this.schedule(null, r)) : this.unsubscribe());
    }, r);
    if ((e.add(i), !o)) return i;
}
function vi(e, n = 0) {
    return L((t, r) => {
        t.subscribe(
            V(
                r,
                (o) => De(r, e, () => r.next(o), n),
                () => De(r, e, () => r.complete(), n),
                (o) => De(r, e, () => r.error(o), n),
            ),
        );
    });
}
function yi(e, n = 0) {
    return L((t, r) => {
        r.add(e.schedule(() => t.subscribe(r), n));
    });
}
function Dd(e, n) {
    return ee(e).pipe(yi(n), vi(n));
}
function Ed(e, n) {
    return ee(e).pipe(yi(n), vi(n));
}
function Cd(e, n) {
    return new O((t) => {
        let r = 0;
        return n.schedule(function () {
            r === e.length
                ? t.complete()
                : (t.next(e[r++]), t.closed || this.schedule());
        });
    });
}
function Id(e, n) {
    return new O((t) => {
        let r;
        return (
            De(t, n, () => {
                ((r = e[pi]()),
                    De(
                        t,
                        n,
                        () => {
                            let o, i;
                            try {
                                ({ value: o, done: i } = r.next());
                            } catch (s) {
                                t.error(s);
                                return;
                            }
                            i ? t.complete() : t.next(o);
                        },
                        0,
                        !0,
                    ));
            }),
            () => M(r?.return) && r.return()
        );
    });
}
function Di(e, n) {
    if (!e) throw new Error("Iterable cannot be null");
    return new O((t) => {
        De(t, n, () => {
            let r = e[Symbol.asyncIterator]();
            De(
                t,
                n,
                () => {
                    r.next().then((o) => {
                        o.done ? t.complete() : t.next(o.value);
                    });
                },
                0,
                !0,
            );
        });
    });
}
function wd(e, n) {
    return Di(gi(e), n);
}
function bd(e, n) {
    if (e != null) {
        if (li(e)) return Dd(e, n);
        if (ci(e)) return Cd(e, n);
        if (ui(e)) return Ed(e, n);
        if (di(e)) return Di(e, n);
        if (hi(e)) return Id(e, n);
        if (mi(e)) return wd(e, n);
    }
    throw fi(e);
}
function q(e, n) {
    return n ? bd(e, n) : ee(e);
}
function I(...e) {
    let n = St(e);
    return q(e, n);
}
function La(e, n) {
    let t = M(e) ? e : () => e,
        r = (o) => o.error(t());
    return new O(n ? (o) => n.schedule(r, 0, o) : r);
}
function Ei(e) {
    return !!e && (e instanceof O || (M(e.lift) && M(e.subscribe)));
}
var ln = Hn(
    (e) =>
        function () {
            (e(this),
                (this.name = "EmptyError"),
                (this.message = "no elements in sequence"));
        },
);
function H(e, n) {
    return L((t, r) => {
        let o = 0;
        t.subscribe(
            V(r, (i) => {
                r.next(e.call(n, i, o++));
            }),
        );
    });
}
var { isArray: Gm } = Array;
function qm(e, n) {
    return Gm(n) ? e(...n) : e(n);
}
function Td(e) {
    return H((n) => qm(e, n));
}
var { isArray: Wm } = Array,
    { getPrototypeOf: Zm, prototype: Ym, keys: Qm } = Object;
function Sd(e) {
    if (e.length === 1) {
        let n = e[0];
        if (Wm(n)) return { args: n, keys: null };
        if (Km(n)) {
            let t = Qm(n);
            return { args: t.map((r) => n[r]), keys: t };
        }
    }
    return { args: e, keys: null };
}
function Km(e) {
    return e && typeof e == "object" && Zm(e) === Ym;
}
function Md(e, n) {
    return e.reduce((t, r, o) => ((t[r] = n[o]), t), {});
}
function Ci(...e) {
    let n = St(e),
        t = pd(e),
        { args: r, keys: o } = Sd(e);
    if (r.length === 0) return q([], n);
    let i = new O(Jm(r, n, o ? (s) => Md(o, s) : $e));
    return t ? i.pipe(Td(t)) : i;
}
function Jm(e, n, t = $e) {
    return (r) => {
        _d(
            n,
            () => {
                let { length: o } = e,
                    i = new Array(o),
                    s = o,
                    a = o;
                for (let c = 0; c < o; c++)
                    _d(
                        n,
                        () => {
                            let u = q(e[c], n),
                                l = !1;
                            u.subscribe(
                                V(
                                    r,
                                    (d) => {
                                        ((i[c] = d),
                                            l || ((l = !0), a--),
                                            a || r.next(t(i.slice())));
                                    },
                                    () => {
                                        --s || r.complete();
                                    },
                                ),
                            );
                        },
                        r,
                    );
            },
            r,
        );
    };
}
function _d(e, n, t) {
    e ? De(t, e, n) : n();
}
function Nd(e, n, t, r, o, i, s, a) {
    let c = [],
        u = 0,
        l = 0,
        d = !1,
        h = () => {
            d && !c.length && !u && n.complete();
        },
        f = (k) => (u < r ? y(k) : c.push(k)),
        y = (k) => {
            (i && n.next(k), u++);
            let C = !1;
            ee(t(k, l++)).subscribe(
                V(
                    n,
                    (_) => {
                        (o?.(_), i ? f(_) : n.next(_));
                    },
                    () => {
                        C = !0;
                    },
                    void 0,
                    () => {
                        if (C)
                            try {
                                for (u--; c.length && u < r; ) {
                                    let _ = c.shift();
                                    s ? De(n, s, () => y(_)) : y(_);
                                }
                                h();
                            } catch (_) {
                                n.error(_);
                            }
                    },
                ),
            );
        };
    return (
        e.subscribe(
            V(n, f, () => {
                ((d = !0), h());
            }),
        ),
        () => {
            a?.();
        }
    );
}
function se(e, n, t = 1 / 0) {
    return M(n)
        ? se((r, o) => H((i, s) => n(r, i, o, s))(ee(e(r, o))), t)
        : (typeof n == "number" && (t = n), L((r, o) => Nd(r, o, e, t)));
}
function Wn(e = 1 / 0) {
    return se($e, e);
}
function Rd() {
    return Wn(1);
}
function Zn(...e) {
    return Rd()(q(e, St(e)));
}
function Fr(e) {
    return new O((n) => {
        ee(e()).subscribe(n);
    });
}
function be(e, n) {
    return L((t, r) => {
        let o = 0;
        t.subscribe(V(r, (i) => e.call(n, i, o++) && r.next(i)));
    });
}
function Yn(e) {
    return L((n, t) => {
        let r = null,
            o = !1,
            i;
        ((r = n.subscribe(
            V(t, void 0, void 0, (s) => {
                ((i = ee(e(s, Yn(e)(n)))),
                    r
                        ? (r.unsubscribe(), (r = null), i.subscribe(t))
                        : (o = !0));
            }),
        )),
            o && (r.unsubscribe(), (r = null), i.subscribe(t)));
    });
}
function Mt(e, n) {
    return M(n) ? se(e, n, 1) : se(e, 1);
}
function Ad(e) {
    return L((n, t) => {
        let r = !1;
        n.subscribe(
            V(
                t,
                (o) => {
                    ((r = !0), t.next(o));
                },
                () => {
                    (r || t.next(e), t.complete());
                },
            ),
        );
    });
}
function ft(e) {
    return e <= 0
        ? () => ie
        : L((n, t) => {
              let r = 0;
              n.subscribe(
                  V(t, (o) => {
                      ++r <= e && (t.next(o), e <= r && t.complete());
                  }),
              );
          });
}
function xd(e = Xm) {
    return L((n, t) => {
        let r = !1;
        n.subscribe(
            V(
                t,
                (o) => {
                    ((r = !0), t.next(o));
                },
                () => (r ? t.complete() : t.error(e())),
            ),
        );
    });
}
function Xm() {
    return new ln();
}
function Lr(e) {
    return L((n, t) => {
        try {
            n.subscribe(t);
        } finally {
            t.add(e);
        }
    });
}
function pt(e, n) {
    let t = arguments.length >= 2;
    return (r) =>
        r.pipe(
            e ? be((o, i) => e(o, i, r)) : $e,
            ft(1),
            t ? Ad(n) : xd(() => new ln()),
        );
}
function Ii(e) {
    return e <= 0
        ? () => ie
        : L((n, t) => {
              let r = [];
              n.subscribe(
                  V(
                      t,
                      (o) => {
                          (r.push(o), e < r.length && r.shift());
                      },
                      () => {
                          for (let o of r) t.next(o);
                          t.complete();
                      },
                      void 0,
                      () => {
                          r = null;
                      },
                  ),
              );
          });
}
function ja(...e) {
    let n = St(e);
    return L((t, r) => {
        (n ? Zn(e, t, n) : Zn(e, t)).subscribe(r);
    });
}
function Ee(e, n) {
    return L((t, r) => {
        let o = null,
            i = 0,
            s = !1,
            a = () => s && !o && r.complete();
        t.subscribe(
            V(
                r,
                (c) => {
                    o?.unsubscribe();
                    let u = 0,
                        l = i++;
                    ee(e(c, l)).subscribe(
                        (o = V(
                            r,
                            (d) => r.next(n ? n(c, d, l, u++) : d),
                            () => {
                                ((o = null), a());
                            },
                        )),
                    );
                },
                () => {
                    ((s = !0), a());
                },
            ),
        );
    });
}
function jr(e) {
    return L((n, t) => {
        (ee(e).subscribe(V(t, () => t.complete(), kr)),
            !t.closed && n.subscribe(t));
    });
}
function Ae(e, n, t) {
    let r = M(e) || n || t ? { next: e, error: n, complete: t } : e;
    return r
        ? L((o, i) => {
              var s;
              (s = r.subscribe) === null || s === void 0 || s.call(r);
              let a = !0;
              o.subscribe(
                  V(
                      i,
                      (c) => {
                          var u;
                          ((u = r.next) === null ||
                              u === void 0 ||
                              u.call(r, c),
                              i.next(c));
                      },
                      () => {
                          var c;
                          ((a = !1),
                              (c = r.complete) === null ||
                                  c === void 0 ||
                                  c.call(r),
                              i.complete());
                      },
                      (c) => {
                          var u;
                          ((a = !1),
                              (u = r.error) === null ||
                                  u === void 0 ||
                                  u.call(r, c),
                              i.error(c));
                      },
                      () => {
                          var c, u;
                          (a &&
                              ((c = r.unsubscribe) === null ||
                                  c === void 0 ||
                                  c.call(r)),
                              (u = r.finalize) === null ||
                                  u === void 0 ||
                                  u.call(r));
                      },
                  ),
              );
          })
        : $e;
}
var Ua;
function wi() {
    return Ua;
}
function ot(e) {
    let n = Ua;
    return ((Ua = e), n);
}
var Od = Symbol("NotFound");
function Qn(e) {
    return e === Od || e?.name === "\u0275NotFound";
}
function Ba(e, n, t) {
    let r = Object.create(ev);
    ((r.source = e), (r.computation = n), t != null && (r.equal = t));
    let i = () => {
        if ((rn(r), nn(r), r.value === rt)) throw r.error;
        return r.value;
    };
    return ((i[te] = r), xr(r), i);
}
function Pd(e, n) {
    (rn(e), sn(e, n), Un(e));
}
function kd(e, n) {
    if ((rn(e), e.value === rt)) throw e.error;
    (ti(e, n), Un(e));
}
var ev = B(g({}, tn), {
    value: Xt,
    dirty: !0,
    error: null,
    equal: Or,
    kind: "linkedSignal",
    producerMustRecompute(e) {
        return e.value === Xt || e.value === en;
    },
    producerRecomputeValue(e) {
        if (e.value === en) throw new Error("");
        let n = e.value;
        e.value = en;
        let t = on(e),
            r,
            o = !1;
        try {
            let i = e.source(),
                s = n !== Xt && n !== rt,
                a = s ? { source: e.sourceValue, value: n } : void 0;
            ((r = e.computation(i, a)),
                (e.sourceValue = i),
                S(null),
                (o = s && r !== rt && e.equal(n, r)));
        } catch (i) {
            ((r = rt), (e.error = i));
        } finally {
            Bn(e, t);
        }
        if (o) {
            e.value = n;
            return;
        }
        ((e.value = r), e.version++);
    },
});
function Fd(e) {
    let n = S(null);
    try {
        return e();
    } finally {
        S(n);
    }
}
var Ni =
        "https://angular.dev/best-practices/security#preventing-cross-site-scripting-xss",
    m = class extends Error {
        code;
        constructor(n, t) {
            (super(it(n, t)), (this.code = n));
        }
    };
function rv(e) {
    return `NG0${Math.abs(e)}`;
}
function it(e, n) {
    return `${rv(e)}${n ? ": " + n : ""}`;
}
var zr = globalThis;
function j(e) {
    for (let n in e) if (e[n] === j) return n;
    throw Error("");
}
function Ri(e) {
    if (typeof e == "string") return e;
    if (Array.isArray(e)) return `[${e.map(Ri).join(", ")}]`;
    if (e == null) return "" + e;
    let n = e.overriddenName || e.name;
    if (n) return `${n}`;
    let t = e.toString();
    if (t == null) return "" + t;
    let r = t.indexOf(`
`);
    return r >= 0 ? t.slice(0, r) : t;
}
function tc(e, n) {
    return e ? (n ? `${e} ${n}` : e) : n || "";
}
var ov = j({ __forward_ref__: j });
function Ai(e) {
    return ((e.__forward_ref__ = Ai), e);
}
function Ce(e) {
    return nc(e) ? e() : e;
}
function nc(e) {
    return (
        typeof e == "function" &&
        e.hasOwnProperty(ov) &&
        e.__forward_ref__ === Ai
    );
}
function v(e) {
    return {
        token: e.token,
        providedIn: e.providedIn || null,
        factory: e.factory,
        value: void 0,
    };
}
function gt(e) {
    return { providers: e.providers || [], imports: e.imports || [] };
}
function Gr(e) {
    return iv(e, xi);
}
function rc(e) {
    return Gr(e) !== null;
}
function iv(e, n) {
    return (e.hasOwnProperty(n) && e[n]) || null;
}
function sv(e) {
    let n = e?.[xi] ?? null;
    return n || null;
}
function Ha(e) {
    return e && e.hasOwnProperty(Ti) ? e[Ti] : null;
}
var xi = j({ ɵprov: j }),
    Ti = j({ ɵinj: j }),
    D = class {
        _desc;
        ngMetadataName = "InjectionToken";
        ɵprov;
        constructor(n, t) {
            ((this._desc = n),
                (this.ɵprov = void 0),
                typeof t == "number"
                    ? (this.__NG_ELEMENT_ID__ = t)
                    : t !== void 0 &&
                      (this.ɵprov = v({
                          token: this,
                          providedIn: t.providedIn || "root",
                          factory: t.factory,
                      })));
        }
        get multi() {
            return this;
        }
        toString() {
            return `InjectionToken ${this._desc}`;
        }
    };
function oc(e) {
    return e && !!e.ɵproviders;
}
var ic = j({ ɵcmp: j }),
    sc = j({ ɵdir: j }),
    ac = j({ ɵpipe: j }),
    cc = j({ ɵmod: j }),
    Br = j({ ɵfac: j }),
    hn = j({ __NG_ELEMENT_ID__: j }),
    Ld = j({ __NG_ENV_ID__: j });
function uc(e) {
    return (Oi(e, "@NgModule"), e[cc] || null);
}
function mt(e) {
    return (Oi(e, "@Component"), e[ic] || null);
}
function lc(e) {
    return (Oi(e, "@Directive"), e[sc] || null);
}
function Vd(e) {
    return (Oi(e, "@Pipe"), e[ac] || null);
}
function Oi(e, n) {
    if (e == null) throw new m(-919, !1);
}
function Jn(e) {
    return typeof e == "string" ? e : e == null ? "" : String(e);
}
var Hd = j({ ngErrorCode: j }),
    av = j({ ngErrorMessage: j }),
    cv = j({ ngTokenPath: j });
function dc(e, n) {
    return $d("", -200, n);
}
function Pi(e, n) {
    throw new m(-201, !1);
}
function $d(e, n, t) {
    let r = new m(n, e);
    return ((r[Hd] = n), (r[av] = e), t && (r[cv] = t), r);
}
function uv(e) {
    return e[Hd];
}
var $a;
function zd() {
    return $a;
}
function pe(e) {
    let n = $a;
    return (($a = e), n);
}
function fc(e, n, t) {
    let r = Gr(e);
    if (r && r.providedIn == "root")
        return r.value === void 0 ? (r.value = r.factory()) : r.value;
    if (t & 8) return null;
    if (n !== void 0) return n;
    Pi(e, "");
}
var lv = {},
    dn = lv,
    dv = "__NG_DI_FLAG__",
    za = class {
        injector;
        constructor(n) {
            this.injector = n;
        }
        retrieve(n, t) {
            let r = fn(t) || 0;
            try {
                return this.injector.get(n, r & 8 ? null : dn, r);
            } catch (o) {
                if (Qn(o)) return o;
                throw o;
            }
        }
    };
function fv(e, n = 0) {
    let t = wi();
    if (t === void 0) throw new m(-203, !1);
    if (t === null) return fc(e, void 0, n);
    {
        let r = pv(n),
            o = t.retrieve(e, r);
        if (Qn(o)) {
            if (r.optional) return null;
            throw o;
        }
        return o;
    }
}
function E(e, n = 0) {
    return (zd() || fv)(Ce(e), n);
}
function p(e, n) {
    return E(e, fn(n));
}
function fn(e) {
    return typeof e > "u" || typeof e == "number"
        ? e
        : 0 |
              (e.optional && 8) |
              (e.host && 1) |
              (e.self && 2) |
              (e.skipSelf && 4);
}
function pv(e) {
    return {
        optional: !!(e & 8),
        host: !!(e & 1),
        self: !!(e & 2),
        skipSelf: !!(e & 4),
    };
}
function Ga(e) {
    let n = [];
    for (let t = 0; t < e.length; t++) {
        let r = Ce(e[t]);
        if (Array.isArray(r)) {
            if (r.length === 0) throw new m(900, !1);
            let o,
                i = 0;
            for (let s = 0; s < r.length; s++) {
                let a = r[s],
                    c = hv(a);
                typeof c == "number"
                    ? c === -1
                        ? (o = a.token)
                        : (i |= c)
                    : (o = a);
            }
            n.push(E(o, i));
        } else n.push(E(r));
    }
    return n;
}
function hv(e) {
    return e[dv];
}
function _t(e, n) {
    let t = e.hasOwnProperty(Br);
    return t ? e[Br] : null;
}
function ki(e, n) {
    e.forEach((t) => (Array.isArray(t) ? ki(t, n) : n(t)));
}
function pc(e, n, t) {
    n >= e.length ? e.push(t) : e.splice(n, 0, t);
}
function qr(e, n) {
    return n >= e.length - 1 ? e.pop() : e.splice(n, 1)[0];
}
var gn = {},
    ze = [],
    mn = new D(""),
    hc = new D("", -1),
    gc = new D(""),
    Vr = class {
        get(n, t = dn) {
            if (t === dn) {
                let o = $d("", -201);
                throw ((o.name = "\u0275NotFound"), o);
            }
            return t;
        }
    };
function At(e) {
    return { ɵproviders: e };
}
function Gd(e) {
    return At([{ provide: mn, multi: !0, useValue: e }]);
}
function qd(...e) {
    return { ɵproviders: mc(!0, e), ɵfromNgModule: !0 };
}
function mc(e, ...n) {
    let t = [],
        r = new Set(),
        o,
        i = (s) => {
            t.push(s);
        };
    return (
        ki(n, (s) => {
            let a = s;
            Si(a, i, [], r) && ((o ||= []), o.push(a));
        }),
        o !== void 0 && Wd(o, i),
        t
    );
}
function Wd(e, n) {
    for (let t = 0; t < e.length; t++) {
        let { ngModule: r, providers: o } = e[t];
        vc(o, (i) => {
            n(i, r);
        });
    }
}
function Si(e, n, t, r) {
    if (((e = Ce(e)), !e)) return !1;
    let o = null,
        i = Ha(e),
        s = !i && mt(e);
    if (!i && !s) {
        let c = e.ngModule;
        if (((i = Ha(c)), i)) o = c;
        else return !1;
    } else {
        if (s && !s.standalone) return !1;
        o = e;
    }
    let a = r.has(o);
    if (s) {
        if (a) return !1;
        if ((r.add(o), s.dependencies)) {
            let c =
                typeof s.dependencies == "function"
                    ? s.dependencies()
                    : s.dependencies;
            for (let u of c) Si(u, n, t, r);
        }
    } else if (i) {
        if (i.imports != null && !a) {
            r.add(o);
            let u;
            (ki(i.imports, (l) => {
                Si(l, n, t, r) && ((u ||= []), u.push(l));
            }),
                u !== void 0 && Wd(u, n));
        }
        if (!a) {
            let u = _t(o) || (() => new o());
            (n({ provide: o, useFactory: u, deps: ze }, o),
                n({ provide: gc, useValue: o, multi: !0 }, o),
                n({ provide: mn, useValue: () => E(o), multi: !0 }, o));
        }
        let c = i.providers;
        if (c != null && !a) {
            let u = e;
            vc(c, (l) => {
                n(l, u);
            });
        }
    } else return !1;
    return o !== e && e.providers !== void 0;
}
function vc(e, n) {
    for (let t of e)
        (oc(t) && (t = t.ɵproviders), Array.isArray(t) ? vc(t, n) : n(t));
}
var gv = j({ provide: String, useValue: j });
function Zd(e) {
    return e !== null && typeof e == "object" && gv in e;
}
function mv(e) {
    return !!(e && e.useExisting);
}
function vv(e) {
    return !!(e && e.useFactory);
}
function Mi(e) {
    return typeof e == "function";
}
var Wr = new D(""),
    bi = {},
    jd = {},
    Va;
function Zr() {
    return (Va === void 0 && (Va = new Vr()), Va);
}
var G = class {},
    pn = class extends G {
        parent;
        source;
        scopes;
        records = new Map();
        _ngOnDestroyHooks = new Set();
        _onDestroyHooks = [];
        get destroyed() {
            return this._destroyed;
        }
        _destroyed = !1;
        injectorDefTypes;
        constructor(n, t, r, o) {
            (super(),
                (this.parent = t),
                (this.source = r),
                (this.scopes = o),
                Wa(n, (s) => this.processProvider(s)),
                this.records.set(hc, Kn(void 0, this)),
                o.has("environment") && this.records.set(G, Kn(void 0, this)));
            let i = this.records.get(Wr);
            (i != null &&
                typeof i.value == "string" &&
                this.scopes.add(i.value),
                (this.injectorDefTypes = new Set(
                    this.get(gc, ze, { self: !0 }),
                )));
        }
        retrieve(n, t) {
            let r = fn(t) || 0;
            try {
                return this.get(n, dn, r);
            } catch (o) {
                if (Qn(o)) return o;
                throw o;
            }
        }
        destroy() {
            (Ur(this), (this._destroyed = !0));
            let n = S(null);
            try {
                for (let r of this._ngOnDestroyHooks) r.ngOnDestroy();
                let t = this._onDestroyHooks;
                this._onDestroyHooks = [];
                for (let r of t) r();
            } finally {
                (this.records.clear(),
                    this._ngOnDestroyHooks.clear(),
                    this.injectorDefTypes.clear(),
                    S(n));
            }
        }
        onDestroy(n) {
            return (
                Ur(this),
                this._onDestroyHooks.push(n),
                () => this.removeOnDestroy(n)
            );
        }
        runInContext(n) {
            Ur(this);
            let t = ot(this),
                r = pe(void 0),
                o;
            try {
                return n();
            } finally {
                (ot(t), pe(r));
            }
        }
        get(n, t = dn, r) {
            if ((Ur(this), n.hasOwnProperty(Ld))) return n[Ld](this);
            let o = fn(r),
                i,
                s = ot(this),
                a = pe(void 0);
            try {
                if (!(o & 4)) {
                    let u = this.records.get(n);
                    if (u === void 0) {
                        let l = Iv(n) && Gr(n);
                        (l && this.injectableDefInScope(l)
                            ? (u = Kn(qa(n), bi))
                            : (u = null),
                            this.records.set(n, u));
                    }
                    if (u != null) return this.hydrate(n, u, o);
                }
                let c = o & 2 ? Zr() : this.parent;
                return ((t = o & 8 && t === dn ? null : t), c.get(n, t));
            } catch (c) {
                let u = uv(c);
                throw u === -200 || u === -201 ? new m(u, null) : c;
            } finally {
                (pe(a), ot(s));
            }
        }
        resolveInjectorInitializers() {
            let n = S(null),
                t = ot(this),
                r = pe(void 0),
                o;
            try {
                let i = this.get(mn, ze, { self: !0 });
                for (let s of i) s();
            } finally {
                (ot(t), pe(r), S(n));
            }
        }
        toString() {
            return "R3Injector[...]";
        }
        processProvider(n) {
            n = Ce(n);
            let t = Mi(n) ? n : Ce(n && n.provide),
                r = Dv(n);
            if (!Mi(n) && n.multi === !0) {
                let o = this.records.get(t);
                (o ||
                    ((o = Kn(void 0, bi, !0)),
                    (o.factory = () => Ga(o.multi)),
                    this.records.set(t, o)),
                    (t = n),
                    o.multi.push(n));
            }
            this.records.set(t, r);
        }
        hydrate(n, t, r) {
            let o = S(null);
            try {
                if (t.value === jd) throw dc("");
                return (
                    t.value === bi &&
                        ((t.value = jd), (t.value = t.factory(void 0, r))),
                    typeof t.value == "object" &&
                        t.value &&
                        Cv(t.value) &&
                        this._ngOnDestroyHooks.add(t.value),
                    t.value
                );
            } finally {
                S(o);
            }
        }
        injectableDefInScope(n) {
            if (!n.providedIn) return !1;
            let t = Ce(n.providedIn);
            return typeof t == "string"
                ? t === "any" || this.scopes.has(t)
                : this.injectorDefTypes.has(t);
        }
        removeOnDestroy(n) {
            let t = this._onDestroyHooks.indexOf(n);
            t !== -1 && this._onDestroyHooks.splice(t, 1);
        }
    };
function qa(e) {
    let n = Gr(e),
        t = n !== null ? n.factory : _t(e);
    if (t !== null) return t;
    if (e instanceof D) throw new m(-204, !1);
    if (e instanceof Function) return yv(e);
    throw new m(-204, !1);
}
function yv(e) {
    if (e.length > 0) throw new m(-204, !1);
    let t = sv(e);
    return t !== null ? () => t.factory(e) : () => new e();
}
function Dv(e) {
    if (Zd(e)) return Kn(void 0, e.useValue);
    {
        let n = Yd(e);
        return Kn(n, bi);
    }
}
function Yd(e, n, t) {
    let r;
    if (Mi(e)) {
        let o = Ce(e);
        return _t(o) || qa(o);
    } else if (Zd(e)) r = () => Ce(e.useValue);
    else if (vv(e)) r = () => e.useFactory(...Ga(e.deps || []));
    else if (mv(e))
        r = (o, i) => E(Ce(e.useExisting), i !== void 0 && i & 8 ? 8 : void 0);
    else {
        let o = Ce(e && (e.useClass || e.provide));
        if (Ev(e)) r = () => new o(...Ga(e.deps));
        else return _t(o) || qa(o);
    }
    return r;
}
function Ur(e) {
    if (e.destroyed) throw new m(-205, !1);
}
function Kn(e, n, t = !1) {
    return { factory: e, value: n, multi: t ? [] : void 0 };
}
function Ev(e) {
    return !!e.deps;
}
function Cv(e) {
    return (
        e !== null && typeof e == "object" && typeof e.ngOnDestroy == "function"
    );
}
function Iv(e) {
    return (
        typeof e == "function" ||
        (typeof e == "object" && e.ngMetadataName === "InjectionToken")
    );
}
function Wa(e, n) {
    for (let t of e)
        Array.isArray(t) ? Wa(t, n) : t && oc(t) ? Wa(t.ɵproviders, n) : n(t);
}
function Q(e, n) {
    let t;
    e instanceof pn ? (Ur(e), (t = e)) : (t = new za(e));
    let r,
        o = ot(t),
        i = pe(void 0);
    try {
        return n();
    } finally {
        (ot(o), pe(i));
    }
}
function Qd() {
    return zd() !== void 0 || wi() != null;
}
var qe = 0,
    R = 1,
    w = 2,
    ae = 3,
    xe = 4,
    Oe = 5,
    Yr = 6,
    Xn = 7,
    me = 8,
    xt = 9,
    We = 10,
    K = 11,
    er = 12,
    yc = 13,
    vn = 14,
    Pe = 15,
    yn = 16,
    Dn = 17,
    En = 18,
    Ot = 19,
    Dc = 20,
    ht = 21,
    Fi = 22,
    Qr = 23,
    Te = 24,
    Cn = 25,
    tr = 26,
    ve = 27,
    Kd = 1;
var Pt = 7,
    Kr = 8,
    Jr = 9,
    Ie = 10;
function vt(e) {
    return Array.isArray(e) && typeof e[Kd] == "object";
}
function Ze(e) {
    return Array.isArray(e) && e[Kd] === !0;
}
function Ec(e) {
    return (e.flags & 4) !== 0;
}
function yt(e) {
    return e.componentOffset > -1;
}
function Xr(e) {
    return (e.flags & 1) === 1;
}
function In(e) {
    return !!e.template;
}
function nr(e) {
    return (e[w] & 512) !== 0;
}
function wn(e) {
    return (e[w] & 256) === 256;
}
var Jd = "svg",
    Xd = "math";
function ke(e) {
    for (; Array.isArray(e); ) e = e[qe];
    return e;
}
function ef(e, n) {
    return ke(n[e]);
}
function Ye(e, n) {
    return ke(n[e.index]);
}
function Cc(e, n) {
    return e.data[n];
}
function tf(e, n) {
    return e[n];
}
function Ic(e, n, t, r) {
    (t >= e.data.length && ((e.data[t] = null), (e.blueprint[t] = null)),
        (n[t] = r));
}
function Fe(e, n) {
    let t = n[e];
    return vt(t) ? t : t[qe];
}
function Li(e) {
    return (e[w] & 128) === 128;
}
function nf(e) {
    return Ze(e[ae]);
}
function eo(e, n) {
    return n == null ? null : e[n];
}
function wc(e) {
    e[Dn] = 0;
}
function bc(e) {
    e[w] & 1024 || ((e[w] |= 1024), Li(e) && rr(e));
}
function rf(e, n) {
    for (; e > 0; ) ((n = n[vn]), e--);
    return n;
}
function to(e) {
    return !!(e[w] & 9216 || e[Te]?.dirty);
}
function ji(e) {
    (e[We].changeDetectionScheduler?.notify(8),
        e[w] & 64 && (e[w] |= 1024),
        to(e) && rr(e));
}
function rr(e) {
    e[We].changeDetectionScheduler?.notify(0);
    let n = Nt(e);
    for (; n !== null && !(n[w] & 8192 || ((n[w] |= 8192), !Li(n))); )
        n = Nt(n);
}
function Tc(e, n) {
    if (wn(e)) throw new m(911, !1);
    (e[ht] === null && (e[ht] = []), e[ht].push(n));
}
function of(e, n) {
    if (e[ht] === null) return;
    let t = e[ht].indexOf(n);
    t !== -1 && e[ht].splice(t, 1);
}
function Nt(e) {
    let n = e[ae];
    return Ze(n) ? n[ae] : n;
}
function sf(e) {
    return (e[Xn] ??= []);
}
function af(e) {
    return (e.cleanup ??= []);
}
var A = { lFrame: wf(null), bindingsEnabled: !0, skipHydrationRootTNode: null };
var Za = !1;
function cf() {
    return A.lFrame.elementDepthCount;
}
function uf() {
    A.lFrame.elementDepthCount++;
}
function lf() {
    A.lFrame.elementDepthCount--;
}
function Sc() {
    return A.bindingsEnabled;
}
function df() {
    return A.skipHydrationRootTNode !== null;
}
function ff(e) {
    return A.skipHydrationRootTNode === e;
}
function pf() {
    A.skipHydrationRootTNode = null;
}
function $() {
    return A.lFrame.lView;
}
function Dt() {
    return A.lFrame.tView;
}
function Se() {
    let e = Mc();
    for (; e !== null && e.type === 64; ) e = e.parent;
    return e;
}
function Mc() {
    return A.lFrame.currentTNode;
}
function hf() {
    let e = A.lFrame,
        n = e.currentTNode;
    return e.isParent ? n : n.parent;
}
function or(e, n) {
    let t = A.lFrame;
    ((t.currentTNode = e), (t.isParent = n));
}
function _c() {
    return A.lFrame.isParent;
}
function gf() {
    A.lFrame.isParent = !1;
}
function Nc() {
    return Za;
}
function Rc(e) {
    let n = Za;
    return ((Za = e), n);
}
function Ac() {
    let e = A.lFrame,
        n = e.bindingRootIndex;
    return (
        n === -1 && (n = e.bindingRootIndex = e.tView.bindingStartIndex),
        n
    );
}
function mf() {
    return A.lFrame.bindingIndex;
}
function vf(e) {
    return (A.lFrame.bindingIndex = e);
}
function Ui() {
    return A.lFrame.bindingIndex++;
}
function yf(e) {
    let n = A.lFrame,
        t = n.bindingIndex;
    return ((n.bindingIndex = n.bindingIndex + e), t);
}
function Df() {
    return A.lFrame.inI18n;
}
function Ef(e, n) {
    let t = A.lFrame;
    ((t.bindingIndex = t.bindingRootIndex = e), Bi(n));
}
function Cf() {
    return A.lFrame.currentDirectiveIndex;
}
function Bi(e) {
    A.lFrame.currentDirectiveIndex = e;
}
function xc(e) {
    A.lFrame.currentQueryIndex = e;
}
function wv(e) {
    let n = e[R];
    return n.type === 2 ? n.declTNode : n.type === 1 ? e[Oe] : null;
}
function Oc(e, n, t) {
    if (t & 4) {
        let o = n,
            i = e;
        for (; (o = o.parent), o === null && !(t & 1); )
            if (((o = wv(i)), o === null || ((i = i[vn]), o.type & 10))) break;
        if (o === null) return !1;
        ((n = o), (e = i));
    }
    let r = (A.lFrame = If());
    return ((r.currentTNode = n), (r.lView = e), !0);
}
function Vi(e) {
    let n = If(),
        t = e[R];
    ((A.lFrame = n),
        (n.currentTNode = t.firstChild),
        (n.lView = e),
        (n.tView = t),
        (n.contextLView = e),
        (n.bindingIndex = t.bindingStartIndex),
        (n.inI18n = !1));
}
function If() {
    let e = A.lFrame,
        n = e === null ? null : e.child;
    return n === null ? wf(e) : n;
}
function wf(e) {
    let n = {
        currentTNode: null,
        isParent: !0,
        lView: null,
        tView: null,
        selectedIndex: -1,
        contextLView: null,
        elementDepthCount: 0,
        currentNamespace: null,
        currentDirectiveIndex: -1,
        bindingRootIndex: -1,
        bindingIndex: -1,
        currentQueryIndex: 0,
        parent: e,
        child: null,
        inI18n: !1,
    };
    return (e !== null && (e.child = n), n);
}
function bf() {
    let e = A.lFrame;
    return (
        (A.lFrame = e.parent),
        (e.currentTNode = null),
        (e.lView = null),
        e
    );
}
var Pc = bf;
function Hi() {
    let e = bf();
    ((e.isParent = !0),
        (e.tView = null),
        (e.selectedIndex = -1),
        (e.contextLView = null),
        (e.elementDepthCount = 0),
        (e.currentDirectiveIndex = -1),
        (e.currentNamespace = null),
        (e.bindingRootIndex = -1),
        (e.bindingIndex = -1),
        (e.currentQueryIndex = 0));
}
function Tf(e) {
    return (A.lFrame.contextLView = rf(e, A.lFrame.contextLView))[me];
}
function no() {
    return A.lFrame.selectedIndex;
}
function kt(e) {
    A.lFrame.selectedIndex = e;
}
function kc() {
    let e = A.lFrame;
    return Cc(e.tView, e.selectedIndex);
}
function Sf() {
    return A.lFrame.currentNamespace;
}
var Mf = !0;
function $i() {
    return Mf;
}
function zi(e) {
    Mf = e;
}
function Ya(e, n = null, t = null, r) {
    let o = Fc(e, n, t, r);
    return (o.resolveInjectorInitializers(), o);
}
function Fc(e, n = null, t = null, r, o = new Set()) {
    let i = [t || ze, qd(e)],
        s;
    return new pn(i, n || Zr(), s || null, o);
}
var ge = class e {
        static THROW_IF_NOT_FOUND = dn;
        static NULL = new Vr();
        static create(n, t) {
            if (Array.isArray(n)) return Ya({ name: "" }, t, n, "");
            {
                let r = n.name ?? "";
                return Ya({ name: r }, n.parent, n.providers, r);
            }
        }
        static ɵprov = v({ token: e, providedIn: "any", factory: () => E(hc) });
        static __NG_ELEMENT_ID__ = -1;
    },
    W = new D(""),
    Le = (() => {
        class e {
            static __NG_ELEMENT_ID__ = bv;
            static __NG_ENV_ID__ = (t) => t;
        }
        return e;
    })(),
    Qa = class extends Le {
        _lView;
        constructor(n) {
            (super(), (this._lView = n));
        }
        get destroyed() {
            return wn(this._lView);
        }
        onDestroy(n) {
            let t = this._lView;
            return (Tc(t, n), () => of(t, n));
        }
    };
function bv() {
    return new Qa($());
}
var _f = !1,
    Nf = new D(""),
    Et = (() => {
        class e {
            taskId = 0;
            pendingTasks = new Set();
            destroyed = !1;
            pendingTask = new oe(!1);
            debugTaskTracker = p(Nf, { optional: !0 });
            get hasPendingTasks() {
                return this.destroyed ? !1 : this.pendingTask.value;
            }
            get hasPendingTasksObservable() {
                return this.destroyed
                    ? new O((t) => {
                          (t.next(!1), t.complete());
                      })
                    : this.pendingTask;
            }
            add() {
                !this.hasPendingTasks &&
                    !this.destroyed &&
                    this.pendingTask.next(!0);
                let t = this.taskId++;
                return (
                    this.pendingTasks.add(t),
                    this.debugTaskTracker?.add(t),
                    t
                );
            }
            has(t) {
                return this.pendingTasks.has(t);
            }
            remove(t) {
                (this.pendingTasks.delete(t),
                    this.debugTaskTracker?.remove(t),
                    this.pendingTasks.size === 0 &&
                        this.hasPendingTasks &&
                        this.pendingTask.next(!1));
            }
            ngOnDestroy() {
                (this.pendingTasks.clear(),
                    this.hasPendingTasks && this.pendingTask.next(!1),
                    (this.destroyed = !0),
                    this.pendingTask.unsubscribe());
            }
            static ɵprov = v({
                token: e,
                providedIn: "root",
                factory: () => new e(),
            });
        }
        return e;
    })(),
    Ka = class extends re {
        __isAsync;
        destroyRef = void 0;
        pendingTasks = void 0;
        constructor(n = !1) {
            (super(),
                (this.__isAsync = n),
                Qd() &&
                    ((this.destroyRef = p(Le, { optional: !0 }) ?? void 0),
                    (this.pendingTasks = p(Et, { optional: !0 }) ?? void 0)));
        }
        emit(n) {
            let t = S(null);
            try {
                super.next(n);
            } finally {
                S(t);
            }
        }
        subscribe(n, t, r) {
            let o = n,
                i = t || (() => null),
                s = r;
            if (n && typeof n == "object") {
                let c = n;
                ((o = c.next?.bind(c)),
                    (i = c.error?.bind(c)),
                    (s = c.complete?.bind(c)));
            }
            this.__isAsync &&
                ((i = this.wrapInTimeout(i)),
                o && (o = this.wrapInTimeout(o)),
                s && (s = this.wrapInTimeout(s)));
            let a = super.subscribe({ next: o, error: i, complete: s });
            return (n instanceof ne && n.add(a), a);
        }
        wrapInTimeout(n) {
            return (t) => {
                let r = this.pendingTasks?.add();
                setTimeout(() => {
                    try {
                        n(t);
                    } finally {
                        r !== void 0 && this.pendingTasks?.remove(r);
                    }
                });
            };
        }
    },
    he = Ka;
function _i(...e) {}
function Lc(e) {
    let n, t;
    function r() {
        e = _i;
        try {
            (t !== void 0 &&
                typeof cancelAnimationFrame == "function" &&
                cancelAnimationFrame(t),
                n !== void 0 && clearTimeout(n));
        } catch {}
    }
    return (
        (n = setTimeout(() => {
            (e(), r());
        })),
        typeof requestAnimationFrame == "function" &&
            (t = requestAnimationFrame(() => {
                (e(), r());
            })),
        () => r()
    );
}
function Rf(e) {
    return (
        queueMicrotask(() => e()),
        () => {
            e = _i;
        }
    );
}
var jc = "isAngularZone",
    Hr = jc + "_ID",
    Tv = 0,
    le = class e {
        hasPendingMacrotasks = !1;
        hasPendingMicrotasks = !1;
        isStable = !0;
        onUnstable = new he(!1);
        onMicrotaskEmpty = new he(!1);
        onStable = new he(!1);
        onError = new he(!1);
        constructor(n) {
            let {
                enableLongStackTrace: t = !1,
                shouldCoalesceEventChangeDetection: r = !1,
                shouldCoalesceRunChangeDetection: o = !1,
                scheduleInRootZone: i = _f,
            } = n;
            if (typeof Zone > "u") throw new m(908, !1);
            Zone.assertZonePatched();
            let s = this;
            ((s._nesting = 0),
                (s._outer = s._inner = Zone.current),
                Zone.TaskTrackingZoneSpec &&
                    (s._inner = s._inner.fork(new Zone.TaskTrackingZoneSpec())),
                t &&
                    Zone.longStackTraceZoneSpec &&
                    (s._inner = s._inner.fork(Zone.longStackTraceZoneSpec)),
                (s.shouldCoalesceEventChangeDetection = !o && r),
                (s.shouldCoalesceRunChangeDetection = o),
                (s.callbackScheduled = !1),
                (s.scheduleInRootZone = i),
                _v(s));
        }
        static isInAngularZone() {
            return typeof Zone < "u" && Zone.current.get(jc) === !0;
        }
        static assertInAngularZone() {
            if (!e.isInAngularZone()) throw new m(909, !1);
        }
        static assertNotInAngularZone() {
            if (e.isInAngularZone()) throw new m(909, !1);
        }
        run(n, t, r) {
            return this._inner.run(n, t, r);
        }
        runTask(n, t, r, o) {
            let i = this._inner,
                s = i.scheduleEventTask("NgZoneEvent: " + o, n, Sv, _i, _i);
            try {
                return i.runTask(s, t, r);
            } finally {
                i.cancelTask(s);
            }
        }
        runGuarded(n, t, r) {
            return this._inner.runGuarded(n, t, r);
        }
        runOutsideAngular(n) {
            return this._outer.run(n);
        }
    },
    Sv = {};
function Uc(e) {
    if (e._nesting == 0 && !e.hasPendingMicrotasks && !e.isStable)
        try {
            (e._nesting++, e.onMicrotaskEmpty.emit(null));
        } finally {
            if ((e._nesting--, !e.hasPendingMicrotasks))
                try {
                    e.runOutsideAngular(() => e.onStable.emit(null));
                } finally {
                    e.isStable = !0;
                }
        }
}
function Mv(e) {
    if (e.isCheckStableRunning || e.callbackScheduled) return;
    e.callbackScheduled = !0;
    function n() {
        Lc(() => {
            ((e.callbackScheduled = !1),
                Ja(e),
                (e.isCheckStableRunning = !0),
                Uc(e),
                (e.isCheckStableRunning = !1));
        });
    }
    (e.scheduleInRootZone
        ? Zone.root.run(() => {
              n();
          })
        : e._outer.run(() => {
              n();
          }),
        Ja(e));
}
function _v(e) {
    let n = () => {
            Mv(e);
        },
        t = Tv++;
    e._inner = e._inner.fork({
        name: "angular",
        properties: { [jc]: !0, [Hr]: t, [Hr + t]: !0 },
        onInvokeTask: (r, o, i, s, a, c) => {
            if (Nv(c)) return r.invokeTask(i, s, a, c);
            try {
                return (Ud(e), r.invokeTask(i, s, a, c));
            } finally {
                (((e.shouldCoalesceEventChangeDetection &&
                    s.type === "eventTask") ||
                    e.shouldCoalesceRunChangeDetection) &&
                    n(),
                    Bd(e));
            }
        },
        onInvoke: (r, o, i, s, a, c, u) => {
            try {
                return (Ud(e), r.invoke(i, s, a, c, u));
            } finally {
                (e.shouldCoalesceRunChangeDetection &&
                    !e.callbackScheduled &&
                    !Rv(c) &&
                    n(),
                    Bd(e));
            }
        },
        onHasTask: (r, o, i, s) => {
            (r.hasTask(i, s),
                o === i &&
                    (s.change == "microTask"
                        ? ((e._hasPendingMicrotasks = s.microTask),
                          Ja(e),
                          Uc(e))
                        : s.change == "macroTask" &&
                          (e.hasPendingMacrotasks = s.macroTask)));
        },
        onHandleError: (r, o, i, s) => (
            r.handleError(i, s),
            e.runOutsideAngular(() => e.onError.emit(s)),
            !1
        ),
    });
}
function Ja(e) {
    e._hasPendingMicrotasks ||
    ((e.shouldCoalesceEventChangeDetection ||
        e.shouldCoalesceRunChangeDetection) &&
        e.callbackScheduled === !0)
        ? (e.hasPendingMicrotasks = !0)
        : (e.hasPendingMicrotasks = !1);
}
function Ud(e) {
    (e._nesting++, e.isStable && ((e.isStable = !1), e.onUnstable.emit(null)));
}
function Bd(e) {
    (e._nesting--, Uc(e));
}
var $r = class {
    hasPendingMicrotasks = !1;
    hasPendingMacrotasks = !1;
    isStable = !0;
    onUnstable = new he();
    onMicrotaskEmpty = new he();
    onStable = new he();
    onError = new he();
    run(n, t, r) {
        return n.apply(t, r);
    }
    runGuarded(n, t, r) {
        return n.apply(t, r);
    }
    runOutsideAngular(n) {
        return n();
    }
    runTask(n, t, r, o) {
        return n.apply(t, r);
    }
};
function Nv(e) {
    return Af(e, "__ignore_ng_zone__");
}
function Rv(e) {
    return Af(e, "__scheduler_tick__");
}
function Af(e, n) {
    return !Array.isArray(e) || e.length !== 1 ? !1 : e[0]?.data?.[n] === !0;
}
var Ge = class {
        _console = console;
        handleError(n) {
            this._console.error("ERROR", n);
        }
    },
    Me = new D("", {
        factory: () => {
            let e = p(le),
                n = p(G),
                t;
            return (r) => {
                e.runOutsideAngular(() => {
                    n.destroyed && !t
                        ? setTimeout(() => {
                              throw r;
                          })
                        : ((t ??= n.get(Ge)), t.handleError(r));
                });
            };
        },
    }),
    xf = {
        provide: mn,
        useValue: () => {
            let e = p(Ge, { optional: !0 });
        },
        multi: !0,
    },
    Av = new D("", {
        factory: () => {
            let e = p(W).defaultView;
            if (!e) return;
            let n = p(Me),
                t = (i) => {
                    (n(i.reason), i.preventDefault());
                },
                r = (i) => {
                    (i.error
                        ? n(i.error)
                        : n(new Error(i.message, { cause: i })),
                        i.preventDefault());
                },
                o = () => {
                    (e.addEventListener("unhandledrejection", t),
                        e.addEventListener("error", r));
                };
            (typeof Zone < "u" ? Zone.root.run(o) : o(),
                p(Le).onDestroy(() => {
                    (e.removeEventListener("error", r),
                        e.removeEventListener("unhandledrejection", t));
                }));
        },
    });
function Bc() {
    return At([
        Gd(() => {
            p(Av);
        }),
    ]);
}
function z(e, n) {
    let [t, r, o] = _a(e, n?.equal),
        i = t,
        s = i[te];
    return ((i.set = r), (i.update = o), (i.asReadonly = Gi.bind(i)), i);
}
function Gi() {
    let e = this[te];
    if (e.readonlyFn === void 0) {
        let n = () => this();
        ((n[te] = e), (e.readonlyFn = n));
    }
    return e.readonlyFn;
}
var qi = (() => {
    class e {
        view;
        node;
        constructor(t, r) {
            ((this.view = t), (this.node = r));
        }
        static __NG_ELEMENT_ID__ = xv;
    }
    return e;
})();
function xv() {
    return new qi($(), Se());
}
var Rt = class {},
    ro = new D("", { factory: () => !0 });
var Vc = new D(""),
    oo = (() => {
        class e {
            internalPendingTasks = p(Et);
            scheduler = p(Rt);
            errorHandler = p(Me);
            add() {
                let t = this.internalPendingTasks.add();
                return () => {
                    this.internalPendingTasks.has(t) &&
                        (this.scheduler.notify(11),
                        this.internalPendingTasks.remove(t));
                };
            }
            run(t) {
                let r = this.add();
                t().catch(this.errorHandler).finally(r);
            }
            static ɵprov = v({
                token: e,
                providedIn: "root",
                factory: () => new e(),
            });
        }
        return e;
    })(),
    Hc = (() => {
        class e {
            static ɵprov = v({
                token: e,
                providedIn: "root",
                factory: () => new Xa(),
            });
        }
        return e;
    })(),
    Xa = class {
        dirtyEffectCount = 0;
        queues = new Map();
        add(n) {
            (this.enqueue(n), this.schedule(n));
        }
        schedule(n) {
            n.dirty && this.dirtyEffectCount++;
        }
        remove(n) {
            let t = n.zone,
                r = this.queues.get(t);
            r.has(n) && (r.delete(n), n.dirty && this.dirtyEffectCount--);
        }
        enqueue(n) {
            let t = n.zone;
            this.queues.has(t) || this.queues.set(t, new Set());
            let r = this.queues.get(t);
            r.has(n) || r.add(n);
        }
        flush() {
            for (; this.dirtyEffectCount > 0; ) {
                let n = !1;
                for (let [t, r] of this.queues)
                    t === null
                        ? (n ||= this.flushQueue(r))
                        : (n ||= t.run(() => this.flushQueue(r)));
                n || (this.dirtyEffectCount = 0);
            }
        }
        flushQueue(n) {
            let t = !1;
            for (let r of n)
                r.dirty && (this.dirtyEffectCount--, (t = !0), r.run());
            return t;
        }
    },
    ec = class {
        [te];
        constructor(n) {
            this[te] = n;
        }
        destroy() {
            this[te].destroy();
        }
    };
function fo(e) {
    return { toString: e }.toString();
}
function Hv(e) {
    return typeof e == "function";
}
function sp(e, n, t, r) {
    n !== null ? n.applyValueToInputSignal(n, r) : (e[t] = r);
}
var Ki = class {
        previousValue;
        currentValue;
        firstChange;
        constructor(n, t, r) {
            ((this.previousValue = n),
                (this.currentValue = t),
                (this.firstChange = r));
        }
        isFirstChange() {
            return this.firstChange;
        }
    },
    ur = (() => {
        let e = () => ap;
        return ((e.ngInherit = !0), e);
    })();
function ap(e) {
    return (e.type.prototype.ngOnChanges && (e.setInput = zv), $v);
}
function $v() {
    let e = up(this),
        n = e?.current;
    if (n) {
        let t = e.previous;
        if (t === gn) e.previous = n;
        else for (let r in n) t[r] = n[r];
        ((e.current = null), this.ngOnChanges(n));
    }
}
function zv(e, n, t, r, o) {
    let i = this.declaredInputs[r],
        s = up(e) || Gv(e, { previous: gn, current: null }),
        a = s.current || (s.current = {}),
        c = s.previous,
        u = c[i];
    ((a[i] = new Ki(u && u.currentValue, t, c === gn)), sp(e, n, o, t));
}
var cp = "__ngSimpleChanges__";
function up(e) {
    return e[cp] || null;
}
function Gv(e, n) {
    return (e[cp] = n);
}
var Of = [];
var U = function (e, n = null, t) {
        for (let r = 0; r < Of.length; r++) {
            let o = Of[r];
            o(e, n, t);
        }
    },
    P = (function (e) {
        return (
            (e[(e.TemplateCreateStart = 0)] = "TemplateCreateStart"),
            (e[(e.TemplateCreateEnd = 1)] = "TemplateCreateEnd"),
            (e[(e.TemplateUpdateStart = 2)] = "TemplateUpdateStart"),
            (e[(e.TemplateUpdateEnd = 3)] = "TemplateUpdateEnd"),
            (e[(e.LifecycleHookStart = 4)] = "LifecycleHookStart"),
            (e[(e.LifecycleHookEnd = 5)] = "LifecycleHookEnd"),
            (e[(e.OutputStart = 6)] = "OutputStart"),
            (e[(e.OutputEnd = 7)] = "OutputEnd"),
            (e[(e.BootstrapApplicationStart = 8)] =
                "BootstrapApplicationStart"),
            (e[(e.BootstrapApplicationEnd = 9)] = "BootstrapApplicationEnd"),
            (e[(e.BootstrapComponentStart = 10)] = "BootstrapComponentStart"),
            (e[(e.BootstrapComponentEnd = 11)] = "BootstrapComponentEnd"),
            (e[(e.ChangeDetectionStart = 12)] = "ChangeDetectionStart"),
            (e[(e.ChangeDetectionEnd = 13)] = "ChangeDetectionEnd"),
            (e[(e.ChangeDetectionSyncStart = 14)] = "ChangeDetectionSyncStart"),
            (e[(e.ChangeDetectionSyncEnd = 15)] = "ChangeDetectionSyncEnd"),
            (e[(e.AfterRenderHooksStart = 16)] = "AfterRenderHooksStart"),
            (e[(e.AfterRenderHooksEnd = 17)] = "AfterRenderHooksEnd"),
            (e[(e.ComponentStart = 18)] = "ComponentStart"),
            (e[(e.ComponentEnd = 19)] = "ComponentEnd"),
            (e[(e.DeferBlockStateStart = 20)] = "DeferBlockStateStart"),
            (e[(e.DeferBlockStateEnd = 21)] = "DeferBlockStateEnd"),
            (e[(e.DynamicComponentStart = 22)] = "DynamicComponentStart"),
            (e[(e.DynamicComponentEnd = 23)] = "DynamicComponentEnd"),
            (e[(e.HostBindingsUpdateStart = 24)] = "HostBindingsUpdateStart"),
            (e[(e.HostBindingsUpdateEnd = 25)] = "HostBindingsUpdateEnd"),
            e
        );
    })(P || {});
function qv(e, n, t) {
    let { ngOnChanges: r, ngOnInit: o, ngDoCheck: i } = n.type.prototype;
    if (r) {
        let s = ap(n);
        ((t.preOrderHooks ??= []).push(e, s),
            (t.preOrderCheckHooks ??= []).push(e, s));
    }
    (o && (t.preOrderHooks ??= []).push(0 - e, o),
        i &&
            ((t.preOrderHooks ??= []).push(e, i),
            (t.preOrderCheckHooks ??= []).push(e, i)));
}
function lp(e, n) {
    for (let t = n.directiveStart, r = n.directiveEnd; t < r; t++) {
        let i = e.data[t].type.prototype,
            {
                ngAfterContentInit: s,
                ngAfterContentChecked: a,
                ngAfterViewInit: c,
                ngAfterViewChecked: u,
                ngOnDestroy: l,
            } = i;
        (s && (e.contentHooks ??= []).push(-t, s),
            a &&
                ((e.contentHooks ??= []).push(t, a),
                (e.contentCheckHooks ??= []).push(t, a)),
            c && (e.viewHooks ??= []).push(-t, c),
            u &&
                ((e.viewHooks ??= []).push(t, u),
                (e.viewCheckHooks ??= []).push(t, u)),
            l != null && (e.destroyHooks ??= []).push(t, l));
    }
}
function Zi(e, n, t) {
    dp(e, n, 3, t);
}
function Yi(e, n, t, r) {
    (e[w] & 3) === t && dp(e, n, t, r);
}
function $c(e, n) {
    let t = e[w];
    (t & 3) === n && ((t &= 16383), (t += 1), (e[w] = t));
}
function dp(e, n, t, r) {
    let o = r !== void 0 ? e[Dn] & 65535 : 0,
        i = r ?? -1,
        s = n.length - 1,
        a = 0;
    for (let c = o; c < s; c++)
        if (typeof n[c + 1] == "number") {
            if (((a = n[c]), r != null && a >= r)) break;
        } else
            (n[c] < 0 && (e[Dn] += 65536),
                (a < i || i == -1) &&
                    (Wv(e, t, n, c), (e[Dn] = (e[Dn] & 4294901760) + c + 2)),
                c++);
}
function Pf(e, n) {
    U(P.LifecycleHookStart, e, n);
    let t = S(null);
    try {
        n.call(e);
    } finally {
        (S(t), U(P.LifecycleHookEnd, e, n));
    }
}
function Wv(e, n, t, r) {
    let o = t[r] < 0,
        i = t[r + 1],
        s = o ? -t[r] : t[r],
        a = e[s];
    o
        ? e[w] >> 14 < e[Dn] >> 16 &&
          (e[w] & 3) === n &&
          ((e[w] += 16384), Pf(a, i))
        : Pf(a, i);
}
var sr = -1,
    co = class {
        factory;
        name;
        injectImpl;
        resolving = !1;
        canSeeViewProviders;
        multi;
        componentProviders;
        index;
        providerFactory;
        constructor(n, t, r, o) {
            ((this.factory = n),
                (this.name = o),
                (this.canSeeViewProviders = t),
                (this.injectImpl = r));
        }
    };
function Zv(e) {
    return (e.flags & 8) !== 0;
}
function Yv(e) {
    return (e.flags & 16) !== 0;
}
function Qv(e, n, t) {
    let r = 0;
    for (; r < t.length; ) {
        let o = t[r];
        if (typeof o == "number") {
            if (o !== 0) break;
            r++;
            let i = t[r++],
                s = t[r++],
                a = t[r++];
            e.setAttribute(n, s, a, i);
        } else {
            let i = o,
                s = t[++r];
            (Kv(i) ? e.setProperty(n, i, s) : e.setAttribute(n, i, s), r++);
        }
    }
    return r;
}
function fp(e) {
    return e === 3 || e === 4 || e === 6;
}
function Kv(e) {
    return e.charCodeAt(0) === 64;
}
function cu(e, n) {
    if (!(n === null || n.length === 0))
        if (e === null || e.length === 0) e = n.slice();
        else {
            let t = -1;
            for (let r = 0; r < n.length; r++) {
                let o = n[r];
                typeof o == "number"
                    ? (t = o)
                    : t === 0 ||
                      (t === -1 || t === 2
                          ? kf(e, t, o, null, n[++r])
                          : kf(e, t, o, null, null));
            }
        }
    return e;
}
function kf(e, n, t, r, o) {
    let i = 0,
        s = e.length;
    if (n === -1) s = -1;
    else
        for (; i < e.length; ) {
            let a = e[i++];
            if (typeof a == "number") {
                if (a === n) {
                    s = -1;
                    break;
                } else if (a > n) {
                    s = i - 1;
                    break;
                }
            }
        }
    for (; i < e.length; ) {
        let a = e[i];
        if (typeof a == "number") break;
        if (a === t) {
            o !== null && (e[i + 1] = o);
            return;
        }
        (i++, o !== null && i++);
    }
    (s !== -1 && (e.splice(s, 0, n), (i = s + 1)),
        e.splice(i++, 0, t),
        o !== null && e.splice(i++, 0, o));
}
function pp(e) {
    return e !== sr;
}
function Ji(e) {
    return e & 32767;
}
function Jv(e) {
    return e >> 16;
}
function Xi(e, n) {
    let t = Jv(e),
        r = n;
    for (; t > 0; ) ((r = r[vn]), t--);
    return r;
}
var Zc = !0;
function es(e) {
    let n = Zc;
    return ((Zc = e), n);
}
var Xv = 256,
    hp = Xv - 1,
    gp = 5,
    ey = 0,
    st = {};
function ty(e, n, t) {
    let r;
    (typeof t == "string"
        ? (r = t.charCodeAt(0) || 0)
        : t.hasOwnProperty(hn) && (r = t[hn]),
        r == null && (r = t[hn] = ey++));
    let o = r & hp,
        i = 1 << o;
    n.data[e + (o >> gp)] |= i;
}
function mp(e, n) {
    let t = vp(e, n);
    if (t !== -1) return t;
    let r = n[R];
    r.firstCreatePass &&
        ((e.injectorIndex = n.length),
        zc(r.data, e),
        zc(n, null),
        zc(r.blueprint, null));
    let o = uu(e, n),
        i = e.injectorIndex;
    if (pp(o)) {
        let s = Ji(o),
            a = Xi(o, n),
            c = a[R].data;
        for (let u = 0; u < 8; u++) n[i + u] = a[s + u] | c[s + u];
    }
    return ((n[i + 8] = o), i);
}
function zc(e, n) {
    e.push(0, 0, 0, 0, 0, 0, 0, 0, n);
}
function vp(e, n) {
    return e.injectorIndex === -1 ||
        (e.parent && e.parent.injectorIndex === e.injectorIndex) ||
        n[e.injectorIndex + 8] === null
        ? -1
        : e.injectorIndex;
}
function uu(e, n) {
    if (e.parent && e.parent.injectorIndex !== -1)
        return e.parent.injectorIndex;
    let t = 0,
        r = null,
        o = n;
    for (; o !== null; ) {
        if (((r = Ip(o)), r === null)) return sr;
        if ((t++, (o = o[vn]), r.injectorIndex !== -1))
            return r.injectorIndex | (t << 16);
    }
    return sr;
}
function ny(e, n, t) {
    ty(e, n, t);
}
function ry(e, n) {
    if (n === "class") return e.classes;
    if (n === "style") return e.styles;
    let t = e.attrs;
    if (t) {
        let r = t.length,
            o = 0;
        for (; o < r; ) {
            let i = t[o];
            if (fp(i)) break;
            if (i === 0) o = o + 2;
            else if (typeof i == "number")
                for (o++; o < r && typeof t[o] == "string"; ) o++;
            else {
                if (i === n) return t[o + 1];
                o = o + 2;
            }
        }
    }
    return null;
}
function yp(e, n, t) {
    if (t & 8 || e !== void 0) return e;
    Pi(n, "NodeInjector");
}
function Dp(e, n, t, r) {
    if ((t & 8 && r === void 0 && (r = null), (t & 3) === 0)) {
        let o = e[xt],
            i = pe(void 0);
        try {
            return o ? o.get(n, r, t & 8) : fc(n, r, t & 8);
        } finally {
            pe(i);
        }
    }
    return yp(r, n, t);
}
function Ep(e, n, t, r = 0, o) {
    if (e !== null) {
        if (n[w] & 2048 && !(r & 2)) {
            let s = cy(e, n, t, r, st);
            if (s !== st) return s;
        }
        let i = Cp(e, n, t, r, st);
        if (i !== st) return i;
    }
    return Dp(n, t, r, o);
}
function Cp(e, n, t, r, o) {
    let i = sy(t);
    if (typeof i == "function") {
        if (!Oc(n, e, r)) return r & 1 ? yp(o, t, r) : Dp(n, t, r, o);
        try {
            let s;
            if (((s = i(r)), s == null && !(r & 8))) Pi(t);
            else return s;
        } finally {
            Pc();
        }
    } else if (typeof i == "number") {
        let s = null,
            a = vp(e, n),
            c = sr,
            u = r & 1 ? n[Pe][Oe] : null;
        for (
            (a === -1 || r & 4) &&
            ((c = a === -1 ? uu(e, n) : n[a + 8]),
            c === sr || !Lf(r, !1)
                ? (a = -1)
                : ((s = n[R]), (a = Ji(c)), (n = Xi(c, n))));
            a !== -1;
        ) {
            let l = n[R];
            if (Ff(i, a, l.data)) {
                let d = oy(a, n, t, s, r, u);
                if (d !== st) return d;
            }
            ((c = n[a + 8]),
                c !== sr && Lf(r, n[R].data[a + 8] === u) && Ff(i, a, n)
                    ? ((s = l), (a = Ji(c)), (n = Xi(c, n)))
                    : (a = -1));
        }
    }
    return o;
}
function oy(e, n, t, r, o, i) {
    let s = n[R],
        a = s.data[e + 8],
        c = r == null ? yt(a) && Zc : r != s && (a.type & 3) !== 0,
        u = o & 1 && i === a,
        l = iy(a, s, t, c, u);
    return l !== null ? Yc(n, s, l, a, o) : st;
}
function iy(e, n, t, r, o) {
    let i = e.providerIndexes,
        s = n.data,
        a = i & 1048575,
        c = e.directiveStart,
        u = e.directiveEnd,
        l = i >> 20,
        d = r ? a : a + l,
        h = o ? a + l : u;
    for (let f = d; f < h; f++) {
        let y = s[f];
        if ((f < c && t === y) || (f >= c && y.type === t)) return f;
    }
    if (o) {
        let f = s[c];
        if (f && In(f) && f.type === t) return c;
    }
    return null;
}
function Yc(e, n, t, r, o) {
    let i = e[t],
        s = n.data;
    if (i instanceof co) {
        let a = i;
        if (a.resolving) throw dc("");
        let c = es(a.canSeeViewProviders);
        a.resolving = !0;
        let u = s[t].type || s[t],
            l,
            d = a.injectImpl ? pe(a.injectImpl) : null,
            h = Oc(e, r, 0);
        try {
            ((i = e[t] = a.factory(void 0, o, s, e, r)),
                n.firstCreatePass && t >= r.directiveStart && qv(t, s[t], n));
        } finally {
            (d !== null && pe(d), es(c), (a.resolving = !1), Pc());
        }
    }
    return i;
}
function sy(e) {
    if (typeof e == "string") return e.charCodeAt(0) || 0;
    let n = e.hasOwnProperty(hn) ? e[hn] : void 0;
    return typeof n == "number" ? (n >= 0 ? n & hp : ay) : n;
}
function Ff(e, n, t) {
    let r = 1 << e;
    return !!(t[n + (e >> gp)] & r);
}
function Lf(e, n) {
    return !(e & 2) && !(e & 1 && n);
}
var bn = class {
    _tNode;
    _lView;
    constructor(n, t) {
        ((this._tNode = n), (this._lView = t));
    }
    get(n, t, r) {
        return Ep(this._tNode, this._lView, n, fn(r), t);
    }
};
function ay() {
    return new bn(Se(), $());
}
function po(e) {
    return fo(() => {
        let n = e.prototype.constructor,
            t = n[Br] || Qc(n),
            r = Object.prototype,
            o = Object.getPrototypeOf(e.prototype).constructor;
        for (; o && o !== r; ) {
            let i = o[Br] || Qc(o);
            if (i && i !== t) return i;
            o = Object.getPrototypeOf(o);
        }
        return (i) => new i();
    });
}
function Qc(e) {
    return nc(e)
        ? () => {
              let n = Qc(Ce(e));
              return n && n();
          }
        : _t(e);
}
function cy(e, n, t, r, o) {
    let i = e,
        s = n;
    for (; i !== null && s !== null && s[w] & 2048 && !nr(s); ) {
        let a = Cp(i, s, t, r | 2, st);
        if (a !== st) return a;
        let c = i.parent;
        if (!c) {
            let u = s[Dc];
            if (u) {
                let l = u.get(t, st, r & -5);
                if (l !== st) return l;
            }
            ((c = Ip(s)), (s = s[vn]));
        }
        i = c;
    }
    return o;
}
function Ip(e) {
    let n = e[R],
        t = n.type;
    return t === 2 ? n.declTNode : t === 1 ? e[Oe] : null;
}
function ho(e) {
    return ry(Se(), e);
}
function uy() {
    return us(Se(), $());
}
function us(e, n) {
    return new lr(Ye(e, n));
}
var lr = (() => {
    class e {
        nativeElement;
        constructor(t) {
            this.nativeElement = t;
        }
        static __NG_ELEMENT_ID__ = uy;
    }
    return e;
})();
function wp(e) {
    return (e.flags & 128) === 128;
}
var lu = (function (e) {
        return (
            (e[(e.OnPush = 0)] = "OnPush"),
            (e[(e.Eager = 1)] = "Eager"),
            (e[(e.Default = 1)] = "Default"),
            e
        );
    })(lu || {}),
    bp = new Map(),
    ly = 0;
function dy() {
    return ly++;
}
function fy(e) {
    bp.set(e[Ot], e);
}
function Kc(e) {
    bp.delete(e[Ot]);
}
var jf = "__ngContext__";
function ar(e, n) {
    vt(n) ? ((e[jf] = n[Ot]), fy(n)) : (e[jf] = n);
}
function Tp(e) {
    return Mp(e[er]);
}
function Sp(e) {
    return Mp(e[xe]);
}
function Mp(e) {
    for (; e !== null && !Ze(e); ) e = e[xe];
    return e;
}
var py;
function du(e) {
    py = e;
}
var ls = new D("", { factory: () => hy }),
    hy = "ng";
var ds = new D(""),
    go = new D("", { providedIn: "platform", factory: () => "unknown" });
var mo = new D("", {
    factory: () =>
        p(W).body?.querySelector("[ngCspNonce]")?.getAttribute("ngCspNonce") ||
        null,
});
var _p = !1,
    Np = new D("", { factory: () => _p });
var fu = new D("");
var gy = (e, n, t, r) => {};
function my(e, n, t, r) {
    gy(e, n, t, r);
}
function pu(e) {
    return (e.flags & 32) === 32;
}
var vy = () => null;
function Rp(e, n, t = !1) {
    return vy(e, n, t);
}
function Ap(e, n) {
    let t = e.contentQueries;
    if (t !== null) {
        let r = S(null);
        try {
            for (let o = 0; o < t.length; o += 2) {
                let i = t[o],
                    s = t[o + 1];
                if (s !== -1) {
                    let a = e.data[s];
                    (xc(i), a.contentQueries(2, n[s], s));
                }
            }
        } finally {
            S(r);
        }
    }
}
function Jc(e, n, t) {
    xc(0);
    let r = S(null);
    try {
        n(e, t);
    } finally {
        S(r);
    }
}
function xp(e, n, t) {
    if (Ec(n)) {
        let r = S(null);
        try {
            let o = n.directiveStart,
                i = n.directiveEnd;
            for (let s = o; s < i; s++) {
                let a = e.data[s];
                if (a.contentQueries) {
                    let c = t[s];
                    a.contentQueries(1, c, s);
                }
            }
        } finally {
            S(r);
        }
    }
}
var Ke = (function (e) {
    return (
        (e[(e.Emulated = 0)] = "Emulated"),
        (e[(e.None = 2)] = "None"),
        (e[(e.ShadowDom = 3)] = "ShadowDom"),
        (e[(e.ExperimentalIsolatedShadowDom = 4)] =
            "ExperimentalIsolatedShadowDom"),
        e
    );
})(Ke || {});
var Wi;
function yy() {
    if (Wi === void 0 && ((Wi = null), zr.trustedTypes))
        try {
            Wi = zr.trustedTypes.createPolicy("angular#unsafe-bypass", {
                createHTML: (e) => e,
                createScript: (e) => e,
                createScriptURL: (e) => e,
            });
        } catch {}
    return Wi;
}
function Uf(e) {
    return yy()?.createScriptURL(e) || e;
}
var ts = class {
    changingThisBreaksApplicationSecurity;
    constructor(n) {
        this.changingThisBreaksApplicationSecurity = n;
    }
    toString() {
        return `SafeValue must use [property]=binding: ${this.changingThisBreaksApplicationSecurity} (see ${Ni})`;
    }
};
function fs(e) {
    return e instanceof ts ? e.changingThisBreaksApplicationSecurity : e;
}
function hu(e, n) {
    let t = Op(e);
    if (t != null && t !== n) {
        if (t === "ResourceURL" && n === "URL") return !0;
        throw new Error(`Required a safe ${n}, got a ${t} (see ${Ni})`);
    }
    return t === n;
}
function Op(e) {
    return (e instanceof ts && e.getTypeName()) || null;
}
var Dy = /^(?!javascript:)(?:[a-z0-9+.-]+:|[^&:\/?#]*(?:[\/?#]|$))/i;
function Pp(e) {
    return ((e = String(e)), e.match(Dy) ? e : "unsafe:" + e);
}
function Ey(e, n) {
    return e.createText(n);
}
function Cy(e, n, t) {
    e.setValue(n, t);
}
function kp(e, n, t) {
    return e.createElement(n, t);
}
function ns(e, n, t, r, o) {
    e.insertBefore(n, t, r, o);
}
function Fp(e, n, t) {
    e.appendChild(n, t);
}
function Bf(e, n, t, r, o) {
    r !== null ? ns(e, n, t, r, o) : Fp(e, n, t);
}
function Iy(e, n, t, r) {
    e.removeChild(null, n, t, r);
}
function wy(e, n, t) {
    e.setAttribute(n, "style", t);
}
function by(e, n, t) {
    t === "" ? e.removeAttribute(n, "class") : e.setAttribute(n, "class", t);
}
function Lp(e, n, t) {
    let { mergedAttrs: r, classes: o, styles: i } = t;
    (r !== null && Qv(e, n, r),
        o !== null && by(e, n, o),
        i !== null && wy(e, n, i));
}
var ps = (function (e) {
    return (
        (e[(e.NONE = 0)] = "NONE"),
        (e[(e.HTML = 1)] = "HTML"),
        (e[(e.STYLE = 2)] = "STYLE"),
        (e[(e.SCRIPT = 3)] = "SCRIPT"),
        (e[(e.URL = 4)] = "URL"),
        (e[(e.RESOURCE_URL = 5)] = "RESOURCE_URL"),
        e
    );
})(ps || {});
function hs(e) {
    let n = Up();
    return n ? n.sanitize(ps.URL, e) || "" : hu(e, "URL") ? fs(e) : Pp(Jn(e));
}
function jp(e) {
    let n = Up();
    if (n) return Uf(n.sanitize(ps.RESOURCE_URL, e) || "");
    if (hu(e, "ResourceURL")) return Uf(fs(e));
    throw new m(904, !1);
}
var Ty = {
    embed: { src: !0 },
    frame: { src: !0 },
    iframe: { src: !0 },
    media: { src: !0 },
    script: { src: !0, href: !0, "xlink:href": !0 },
    base: { href: !0 },
    link: { href: !0 },
    object: { data: !0, codebase: !0 },
};
function Sy(e, n) {
    return Ty[e]?.[n] === !0 ? jp : hs;
}
function gu(e, n, t) {
    return Sy(n, t)(e);
}
function Up() {
    let e = $();
    return e && e[We].sanitizer;
}
function Bp(e) {
    return e instanceof Function ? e() : e;
}
function My(e, n, t) {
    let r = e.length;
    for (;;) {
        let o = e.indexOf(n, t);
        if (o === -1) return o;
        if (o === 0 || e.charCodeAt(o - 1) <= 32) {
            let i = n.length;
            if (o + i === r || e.charCodeAt(o + i) <= 32) return o;
        }
        t = o + 1;
    }
}
var Vp = "ng-template";
function _y(e, n, t, r) {
    let o = 0;
    if (r) {
        for (; o < n.length && typeof n[o] == "string"; o += 2)
            if (n[o] === "class" && My(n[o + 1].toLowerCase(), t, 0) !== -1)
                return !0;
    } else if (mu(e)) return !1;
    if (((o = n.indexOf(1, o)), o > -1)) {
        let i;
        for (; ++o < n.length && typeof (i = n[o]) == "string"; )
            if (i.toLowerCase() === t) return !0;
    }
    return !1;
}
function mu(e) {
    return e.type === 4 && e.value !== Vp;
}
function Ny(e, n, t) {
    let r = e.type === 4 && !t ? Vp : e.value;
    return n === r;
}
function Ry(e, n, t) {
    let r = 4,
        o = e.attrs,
        i = o !== null ? Oy(o) : 0,
        s = !1;
    for (let a = 0; a < n.length; a++) {
        let c = n[a];
        if (typeof c == "number") {
            if (!s && !Qe(r) && !Qe(c)) return !1;
            if (s && Qe(c)) continue;
            ((s = !1), (r = c | (r & 1)));
            continue;
        }
        if (!s)
            if (r & 4) {
                if (
                    ((r = 2 | (r & 1)),
                    (c !== "" && !Ny(e, c, t)) || (c === "" && n.length === 1))
                ) {
                    if (Qe(r)) return !1;
                    s = !0;
                }
            } else if (r & 8) {
                if (o === null || !_y(e, o, c, t)) {
                    if (Qe(r)) return !1;
                    s = !0;
                }
            } else {
                let u = n[++a],
                    l = Ay(c, o, mu(e), t);
                if (l === -1) {
                    if (Qe(r)) return !1;
                    s = !0;
                    continue;
                }
                if (u !== "") {
                    let d;
                    if (
                        (l > i ? (d = "") : (d = o[l + 1].toLowerCase()),
                        r & 2 && u !== d)
                    ) {
                        if (Qe(r)) return !1;
                        s = !0;
                    }
                }
            }
    }
    return Qe(r) || s;
}
function Qe(e) {
    return (e & 1) === 0;
}
function Ay(e, n, t, r) {
    if (n === null) return -1;
    let o = 0;
    if (r || !t) {
        let i = !1;
        for (; o < n.length; ) {
            let s = n[o];
            if (s === e) return o;
            if (s === 3 || s === 6) i = !0;
            else if (s === 1 || s === 2) {
                let a = n[++o];
                for (; typeof a == "string"; ) a = n[++o];
                continue;
            } else {
                if (s === 4) break;
                if (s === 0) {
                    o += 4;
                    continue;
                }
            }
            o += i ? 1 : 2;
        }
        return -1;
    } else return Py(n, e);
}
function xy(e, n, t = !1) {
    for (let r = 0; r < n.length; r++) if (Ry(e, n[r], t)) return !0;
    return !1;
}
function Oy(e) {
    for (let n = 0; n < e.length; n++) {
        let t = e[n];
        if (fp(t)) return n;
    }
    return e.length;
}
function Py(e, n) {
    let t = e.indexOf(4);
    if (t > -1)
        for (t++; t < e.length; ) {
            let r = e[t];
            if (typeof r == "number") return -1;
            if (r === n) return t;
            t++;
        }
    return -1;
}
function Vf(e, n) {
    return e ? ":not(" + n.trim() + ")" : n;
}
function ky(e) {
    let n = e[0],
        t = 1,
        r = 2,
        o = "",
        i = !1;
    for (; t < e.length; ) {
        let s = e[t];
        if (typeof s == "string")
            if (r & 2) {
                let a = e[++t];
                o += "[" + s + (a.length > 0 ? '="' + a + '"' : "") + "]";
            } else r & 8 ? (o += "." + s) : r & 4 && (o += " " + s);
        else
            (o !== "" && !Qe(s) && ((n += Vf(i, o)), (o = "")),
                (r = s),
                (i = i || !Qe(r)));
        t++;
    }
    return (o !== "" && (n += Vf(i, o)), n);
}
function Fy(e) {
    return e.map(ky).join(",");
}
function Ly(e) {
    let n = [],
        t = [],
        r = 1,
        o = 2;
    for (; r < e.length; ) {
        let i = e[r];
        if (typeof i == "string")
            o === 2 ? i !== "" && n.push(i, e[++r]) : o === 8 && t.push(i);
        else {
            if (!Qe(o)) break;
            o = i;
        }
        r++;
    }
    return (t.length && n.push(1, ...t), n);
}
var Ct = {};
function vu(e, n, t, r, o, i, s, a, c, u, l) {
    let d = ve + r,
        h = d + o,
        f = jy(d, h),
        y = typeof u == "function" ? u() : u;
    return (f[R] = {
        type: e,
        blueprint: f,
        template: t,
        queries: null,
        viewQuery: a,
        declTNode: n,
        data: f.slice().fill(null, d),
        bindingStartIndex: d,
        expandoStartIndex: h,
        hostBindingOpCodes: null,
        firstCreatePass: !0,
        firstUpdatePass: !0,
        staticViewQueries: !1,
        staticContentQueries: !1,
        preOrderHooks: null,
        preOrderCheckHooks: null,
        contentHooks: null,
        contentCheckHooks: null,
        viewHooks: null,
        viewCheckHooks: null,
        destroyHooks: null,
        cleanup: null,
        contentQueries: null,
        components: null,
        directiveRegistry: typeof i == "function" ? i() : i,
        pipeRegistry: typeof s == "function" ? s() : s,
        firstChild: null,
        schemas: c,
        consts: y,
        incompleteFirstPass: !1,
        ssrId: l,
    });
}
function jy(e, n) {
    let t = [];
    for (let r = 0; r < n; r++) t.push(r < e ? null : Ct);
    return t;
}
function Uy(e) {
    let n = e.tView;
    return n === null || n.incompleteFirstPass
        ? (e.tView = vu(
              1,
              null,
              e.template,
              e.decls,
              e.vars,
              e.directiveDefs,
              e.pipeDefs,
              e.viewQuery,
              e.schemas,
              e.consts,
              e.id,
          ))
        : n;
}
function yu(e, n, t, r, o, i, s, a, c, u, l) {
    let d = n.blueprint.slice();
    return (
        (d[qe] = o),
        (d[w] = r | 4 | 128 | 8 | 64 | 1024),
        (u !== null || (e && e[w] & 2048)) && (d[w] |= 2048),
        wc(d),
        (d[ae] = d[vn] = e),
        (d[me] = t),
        (d[We] = s || (e && e[We])),
        (d[K] = a || (e && e[K])),
        (d[xt] = c || (e && e[xt]) || null),
        (d[Oe] = i),
        (d[Ot] = dy()),
        (d[Yr] = l),
        (d[Dc] = u),
        (d[Pe] = n.type == 2 ? e[Pe] : d),
        d
    );
}
function By(e, n, t) {
    let r = Ye(n, e),
        o = Uy(t),
        i = e[We].rendererFactory,
        s = Du(
            e,
            yu(
                e,
                o,
                null,
                Hp(t),
                r,
                n,
                null,
                i.createRenderer(r, t),
                null,
                null,
                null,
            ),
        );
    return (e[n.index] = s);
}
function Hp(e) {
    let n = 16;
    return (e.signals ? (n = 4096) : e.onPush && (n = 64), n);
}
function $p(e, n, t, r) {
    if (t === 0) return -1;
    let o = n.length;
    for (let i = 0; i < t; i++)
        (n.push(r), e.blueprint.push(r), e.data.push(null));
    return o;
}
function Du(e, n) {
    return (e[er] ? (e[yc][xe] = n) : (e[er] = n), (e[yc] = n), n);
}
function de(e = 1) {
    zp(Dt(), $(), no() + e, !1);
}
function zp(e, n, t, r) {
    if (!r)
        if ((n[w] & 3) === 3) {
            let i = e.preOrderCheckHooks;
            i !== null && Zi(n, i, t);
        } else {
            let i = e.preOrderHooks;
            i !== null && Yi(n, i, 0, t);
        }
    kt(t);
}
var gs = (function (e) {
    return (
        (e[(e.None = 0)] = "None"),
        (e[(e.SignalBased = 1)] = "SignalBased"),
        (e[(e.HasDecoratorInputTransform = 2)] = "HasDecoratorInputTransform"),
        e
    );
})(gs || {});
function Xc(e, n, t, r) {
    let o = S(null);
    try {
        let [i, s, a] = e.inputs[t],
            c = null;
        ((s & gs.SignalBased) !== 0 && (c = n[i][te]),
            c !== null && c.transformFn !== void 0
                ? (r = c.transformFn(r))
                : a !== null && (r = a.call(n, r)),
            e.setInput !== null ? e.setInput(n, c, r, t, i) : sp(n, c, i, r));
    } finally {
        S(o);
    }
}
var Lt = (function (e) {
        return (
            (e[(e.Important = 1)] = "Important"),
            (e[(e.DashCase = 2)] = "DashCase"),
            e
        );
    })(Lt || {}),
    Vy;
function Eu(e, n) {
    return Vy(e, n);
}
var QN =
    typeof document < "u" &&
    typeof document?.documentElement?.getAnimations == "function";
var eu = new WeakMap(),
    io = new WeakSet();
function Hy(e, n) {
    let t = eu.get(e);
    if (!t || t.length === 0) return;
    let r = n.parentNode,
        o = n.previousSibling;
    for (let i = t.length - 1; i >= 0; i--) {
        let s = t[i],
            a = s.parentNode;
        s === n
            ? (t.splice(i, 1),
              io.add(s),
              s.dispatchEvent(
                  new CustomEvent("animationend", { detail: { cancel: !0 } }),
              ))
            : ((o && s === o) || (a && r && a !== r)) &&
              (t.splice(i, 1),
              s.dispatchEvent(
                  new CustomEvent("animationend", { detail: { cancel: !0 } }),
              ),
              s.parentNode?.removeChild(s));
    }
}
function $y(e, n) {
    let t = eu.get(e);
    t ? t.includes(n) || t.push(n) : eu.set(e, [n]);
}
var cr = new Set(),
    ms = (function (e) {
        return (
            (e[(e.CHANGE_DETECTION = 0)] = "CHANGE_DETECTION"),
            (e[(e.AFTER_NEXT_RENDER = 1)] = "AFTER_NEXT_RENDER"),
            e
        );
    })(ms || {}),
    at = new D(""),
    Hf = new Set();
function dr(e) {
    Hf.has(e) ||
        (Hf.add(e),
        performance?.mark?.("mark_feature_usage", { detail: { feature: e } }));
}
var Cu = (() => {
        class e {
            impl = null;
            execute() {
                this.impl?.execute();
            }
            static ɵprov = v({
                token: e,
                providedIn: "root",
                factory: () => new e(),
            });
        }
        return e;
    })(),
    Gp = [0, 1, 2, 3],
    qp = (() => {
        class e {
            ngZone = p(le);
            scheduler = p(Rt);
            errorHandler = p(Ge, { optional: !0 });
            sequences = new Set();
            deferredRegistrations = new Set();
            executing = !1;
            constructor() {
                p(at, { optional: !0 });
            }
            execute() {
                let t = this.sequences.size > 0;
                (t && U(P.AfterRenderHooksStart), (this.executing = !0));
                for (let r of Gp)
                    for (let o of this.sequences)
                        if (!(o.erroredOrDestroyed || !o.hooks[r]))
                            try {
                                o.pipelinedValue =
                                    this.ngZone.runOutsideAngular(() =>
                                        this.maybeTrace(() => {
                                            let i = o.hooks[r];
                                            return i(o.pipelinedValue);
                                        }, o.snapshot),
                                    );
                            } catch (i) {
                                ((o.erroredOrDestroyed = !0),
                                    this.errorHandler?.handleError(i));
                            }
                this.executing = !1;
                for (let r of this.sequences)
                    (r.afterRun(),
                        r.once && (this.sequences.delete(r), r.destroy()));
                for (let r of this.deferredRegistrations) this.sequences.add(r);
                (this.deferredRegistrations.size > 0 &&
                    this.scheduler.notify(7),
                    this.deferredRegistrations.clear(),
                    t && U(P.AfterRenderHooksEnd));
            }
            register(t) {
                let { view: r } = t;
                r !== void 0
                    ? ((r[Cn] ??= []).push(t), rr(r), (r[w] |= 8192))
                    : this.executing
                      ? this.deferredRegistrations.add(t)
                      : this.addSequence(t);
            }
            addSequence(t) {
                (this.sequences.add(t), this.scheduler.notify(7));
            }
            unregister(t) {
                this.executing && this.sequences.has(t)
                    ? ((t.erroredOrDestroyed = !0),
                      (t.pipelinedValue = void 0),
                      (t.once = !0))
                    : (this.sequences.delete(t),
                      this.deferredRegistrations.delete(t));
            }
            maybeTrace(t, r) {
                return r ? r.run(ms.AFTER_NEXT_RENDER, t) : t();
            }
            static ɵprov = v({
                token: e,
                providedIn: "root",
                factory: () => new e(),
            });
        }
        return e;
    })(),
    rs = class {
        impl;
        hooks;
        view;
        once;
        snapshot;
        erroredOrDestroyed = !1;
        pipelinedValue = void 0;
        unregisterOnDestroy;
        constructor(n, t, r, o, i, s = null) {
            ((this.impl = n),
                (this.hooks = t),
                (this.view = r),
                (this.once = o),
                (this.snapshot = s),
                (this.unregisterOnDestroy = i?.onDestroy(() =>
                    this.destroy(),
                )));
        }
        afterRun() {
            ((this.erroredOrDestroyed = !1),
                (this.pipelinedValue = void 0),
                this.snapshot?.dispose(),
                (this.snapshot = null));
        }
        destroy() {
            (this.impl.unregister(this), this.unregisterOnDestroy?.());
            let n = this.view?.[Cn];
            n && (this.view[Cn] = n.filter((t) => t !== this));
        }
    };
function vs(e, n) {
    let t = n?.injector ?? p(ge);
    return (dr("NgAfterNextRender"), Gy(e, t, n, !0));
}
function zy(e) {
    return e instanceof Function
        ? [void 0, void 0, e, void 0]
        : [e.earlyRead, e.write, e.mixedReadWrite, e.read];
}
function Gy(e, n, t, r) {
    let o = n.get(Cu);
    o.impl ??= n.get(qp);
    let i = n.get(at, null, { optional: !0 }),
        s = t?.manualCleanup !== !0 ? n.get(Le) : null,
        a = n.get(qi, null, { optional: !0 }),
        c = new rs(o.impl, zy(e), a?.view, r, s, i?.snapshot(null));
    return (o.impl.register(c), c);
}
var qy = new D("", {
    factory: () => ({
        queue: new Set(),
        isScheduled: !1,
        scheduler: null,
        injector: p(G),
    }),
});
function Wp(e, n, t) {
    let r = e.get(qy);
    if (Array.isArray(n))
        for (let o of n)
            (r.queue.add(o), t?.detachedLeaveAnimationFns?.push(o));
    else (r.queue.add(n), t?.detachedLeaveAnimationFns?.push(n));
    r.scheduler && r.scheduler(e);
}
function Wy(e, n) {
    for (let [t, r] of n) Wp(e, r.animateFns);
}
function $f(e, n, t, r) {
    let o = e?.[tr]?.enter;
    n !== null && o && o.has(t.index) && Wy(r, o);
}
function ir(e, n, t, r, o, i, s, a) {
    if (o != null) {
        let c,
            u = !1;
        Ze(o) ? (c = o) : vt(o) && ((u = !0), (o = o[qe]));
        let l = ke(o);
        (e === 0 && r !== null
            ? ($f(a, r, i, t),
              s == null ? Fp(n, r, l) : ns(n, r, l, s || null, !0))
            : e === 1 && r !== null
              ? ($f(a, r, i, t), ns(n, r, l, s || null, !0), Hy(i, l))
              : e === 2
                ? (a?.[tr]?.leave?.has(i.index) && $y(i, l),
                  io.delete(l),
                  zf(a, i, t, (d) => {
                      if (io.has(l)) {
                          io.delete(l);
                          return;
                      }
                      Iy(n, l, u, d);
                  }))
                : e === 3 &&
                  (io.delete(l),
                  zf(a, i, t, () => {
                      n.destroyNode(l);
                  })),
            c != null && sD(n, e, t, c, i, r, s));
    }
}
function Zy(e, n) {
    (Zp(e, n), (n[qe] = null), (n[Oe] = null));
}
function Yy(e, n, t, r, o, i) {
    ((r[qe] = o), (r[Oe] = n), ys(e, r, t, 1, o, i));
}
function Zp(e, n) {
    (n[We].changeDetectionScheduler?.notify(9), ys(e, n, n[K], 2, null, null));
}
function Qy(e) {
    let n = e[er];
    if (!n) return Gc(e[R], e);
    for (; n; ) {
        let t = null;
        if (vt(n)) t = n[er];
        else {
            let r = n[Ie];
            r && (t = r);
        }
        if (!t) {
            for (; n && !n[xe] && n !== e; )
                (vt(n) && Gc(n[R], n), (n = n[ae]));
            (n === null && (n = e), vt(n) && Gc(n[R], n), (t = n && n[xe]));
        }
        n = t;
    }
}
function Iu(e, n) {
    let t = e[Jr],
        r = t.indexOf(n);
    t.splice(r, 1);
}
function Yp(e, n) {
    if (wn(n)) return;
    let t = n[K];
    (t.destroyNode && ys(e, n, t, 3, null, null), Qy(n));
}
function Gc(e, n) {
    if (wn(n)) return;
    let t = S(null);
    try {
        ((n[w] &= -129),
            (n[w] |= 256),
            n[Te] && Ar(n[Te]),
            Xy(e, n),
            Jy(e, n),
            n[R].type === 1 && n[K].destroy());
        let r = n[yn];
        if (r !== null && Ze(n[ae])) {
            r !== n[ae] && Iu(r, n);
            let o = n[En];
            o !== null && o.detachView(e);
        }
        Kc(n);
    } finally {
        S(t);
    }
}
function zf(e, n, t, r) {
    let o = e?.[tr];
    if (o == null || o.leave == null || !o.leave.has(n.index)) return r(!1);
    (e && cr.add(e[Ot]),
        Wp(
            t,
            () => {
                if (o.leave && o.leave.has(n.index)) {
                    let s = o.leave.get(n.index),
                        a = [];
                    if (s) {
                        for (let c = 0; c < s.animateFns.length; c++) {
                            let u = s.animateFns[c],
                                { promise: l } = u();
                            a.push(l);
                        }
                        o.detachedLeaveAnimationFns = void 0;
                    }
                    ((o.running = Promise.allSettled(a)), Ky(e, r));
                } else (e && cr.delete(e[Ot]), r(!1));
            },
            o,
        ));
}
function Ky(e, n) {
    let t = e[tr]?.running;
    if (t) {
        t.then(() => {
            ((e[tr].running = void 0), cr.delete(e[Ot]), n(!0));
        });
        return;
    }
    n(!1);
}
function Jy(e, n) {
    let t = e.cleanup,
        r = n[Xn];
    if (t !== null)
        for (let s = 0; s < t.length - 1; s += 2)
            if (typeof t[s] == "string") {
                let a = t[s + 3];
                (a >= 0 ? r[a]() : r[-a].unsubscribe(), (s += 2));
            } else {
                let a = r[t[s + 1]];
                t[s].call(a);
            }
    r !== null && (n[Xn] = null);
    let o = n[ht];
    if (o !== null) {
        n[ht] = null;
        for (let s = 0; s < o.length; s++) {
            let a = o[s];
            a();
        }
    }
    let i = n[Qr];
    if (i !== null) {
        n[Qr] = null;
        for (let s of i) s.destroy();
    }
}
function Xy(e, n) {
    let t;
    if (e != null && (t = e.destroyHooks) != null)
        for (let r = 0; r < t.length; r += 2) {
            let o = n[t[r]];
            if (!(o instanceof co)) {
                let i = t[r + 1];
                if (Array.isArray(i))
                    for (let s = 0; s < i.length; s += 2) {
                        let a = o[i[s]],
                            c = i[s + 1];
                        U(P.LifecycleHookStart, a, c);
                        try {
                            c.call(a);
                        } finally {
                            U(P.LifecycleHookEnd, a, c);
                        }
                    }
                else {
                    U(P.LifecycleHookStart, o, i);
                    try {
                        i.call(o);
                    } finally {
                        U(P.LifecycleHookEnd, o, i);
                    }
                }
            }
        }
}
function eD(e, n, t) {
    return tD(e, n.parent, t);
}
function tD(e, n, t) {
    let r = n;
    for (; r !== null && r.type & 168; ) ((n = r), (r = n.parent));
    if (r === null) return t[qe];
    if (yt(r)) {
        let { encapsulation: o } = e.data[r.directiveStart + r.componentOffset];
        if (o === Ke.None || o === Ke.Emulated) return null;
    }
    return Ye(r, t);
}
function nD(e, n, t) {
    return oD(e, n, t);
}
function rD(e, n, t) {
    return e.type & 40 ? Ye(e, t) : null;
}
var oD = rD,
    Gf;
function wu(e, n, t, r) {
    let o = eD(e, r, n),
        i = n[K],
        s = r.parent || n[Oe],
        a = nD(s, r, n);
    if (o != null)
        if (Array.isArray(t))
            for (let c = 0; c < t.length; c++) Bf(i, o, t[c], a, !1);
        else Bf(i, o, t, a, !1);
    Gf !== void 0 && Gf(i, r, n, t, o);
}
function so(e, n) {
    if (n !== null) {
        let t = n.type;
        if (t & 3) return Ye(n, e);
        if (t & 4) return tu(-1, e[n.index]);
        if (t & 8) {
            let r = n.child;
            if (r !== null) return so(e, r);
            {
                let o = e[n.index];
                return Ze(o) ? tu(-1, o) : ke(o);
            }
        } else {
            if (t & 128) return so(e, n.next);
            if (t & 32) return Eu(n, e)() || ke(e[n.index]);
            {
                let r = Qp(e, n);
                if (r !== null) {
                    if (Array.isArray(r)) return r[0];
                    let o = Nt(e[Pe]);
                    return so(o, r);
                } else return so(e, n.next);
            }
        }
    }
    return null;
}
function Qp(e, n) {
    if (n !== null) {
        let r = e[Pe][Oe],
            o = n.projection;
        return r.projection[o];
    }
    return null;
}
function tu(e, n) {
    let t = Ie + e + 1;
    if (t < n.length) {
        let r = n[t],
            o = r[R].firstChild;
        if (o !== null) return so(r, o);
    }
    return n[Pt];
}
function bu(e, n, t, r, o, i, s) {
    for (; t != null; ) {
        let a = r[xt];
        if (t.type === 128) {
            t = t.next;
            continue;
        }
        let c = r[t.index],
            u = t.type;
        if ((s && n === 0 && (c && ar(ke(c), r), (t.flags |= 2)), !pu(t)))
            if (u & 8)
                (bu(e, n, t.child, r, o, i, !1), ir(n, e, a, o, c, t, i, r));
            else if (u & 32) {
                let l = Eu(t, r),
                    d;
                for (; (d = l()); ) ir(n, e, a, o, d, t, i, r);
                ir(n, e, a, o, c, t, i, r);
            } else u & 16 ? iD(e, n, r, t, o, i) : ir(n, e, a, o, c, t, i, r);
        t = s ? t.projectionNext : t.next;
    }
}
function ys(e, n, t, r, o, i) {
    bu(t, r, e.firstChild, n, o, i, !1);
}
function iD(e, n, t, r, o, i) {
    let s = t[Pe],
        c = s[Oe].projection[r.projection];
    if (Array.isArray(c))
        for (let u = 0; u < c.length; u++) {
            let l = c[u];
            ir(n, e, t[xt], o, l, r, i, t);
        }
    else {
        let u = c,
            l = s[ae];
        (wp(r) && (u.flags |= 128), bu(e, n, u, l, o, i, !0));
    }
}
function sD(e, n, t, r, o, i, s) {
    let a = r[Pt],
        c = ke(r);
    a !== c && ir(n, e, t, i, a, o, s);
    for (let u = Ie; u < r.length; u++) {
        let l = r[u];
        ys(l[R], l, e, n, i, a);
    }
}
function Kp(e, n, t, r, o) {
    let i = no(),
        s = r & 2;
    try {
        (kt(-1), s && n.length > ve && zp(e, n, ve, !1));
        let a = s ? P.TemplateUpdateStart : P.TemplateCreateStart;
        (U(a, o, t), t(r, o));
    } finally {
        kt(i);
        let a = s ? P.TemplateUpdateEnd : P.TemplateCreateEnd;
        U(a, o, t);
    }
}
function Tu(e, n, t) {
    (hD(e, n, t), (t.flags & 64) === 64 && gD(e, n, t));
}
function Jp(e, n, t = Ye) {
    let r = n.localNames;
    if (r !== null) {
        let o = n.index + 1;
        for (let i = 0; i < r.length; i += 2) {
            let s = r[i + 1],
                a = s === -1 ? t(n, e) : e[s];
            e[o++] = a;
        }
    }
}
function aD(e, n, t, r) {
    let i =
            r.get(Np, _p) ||
            t === Ke.ShadowDom ||
            t === Ke.ExperimentalIsolatedShadowDom,
        s = e.selectRootElement(n, i);
    return (cD(s), s);
}
function cD(e) {
    uD(e);
}
var uD = () => null;
function lD(e) {
    return e === "class"
        ? "className"
        : e === "for"
          ? "htmlFor"
          : e === "formaction"
            ? "formAction"
            : e === "innerHtml"
              ? "innerHTML"
              : e === "readonly"
                ? "readOnly"
                : e === "tabindex"
                  ? "tabIndex"
                  : e;
}
function dD(e, n, t, r, o, i) {
    let s = n[R];
    if (Su(e, s, n, t, r)) {
        yt(e) && pD(n, e.index);
        return;
    }
    (e.type & 3 && (t = lD(t)), fD(e, n, t, r, o, i));
}
function fD(e, n, t, r, o, i) {
    if (e.type & 3) {
        let s = Ye(e, n);
        ((r = i != null ? i(r, e.value || "", t) : r), o.setProperty(s, t, r));
    } else e.type & 12;
}
function pD(e, n) {
    let t = Fe(n, e);
    t[w] & 16 || (t[w] |= 64);
}
function hD(e, n, t) {
    let r = t.directiveStart,
        o = t.directiveEnd;
    (yt(t) && By(n, t, e.data[r + t.componentOffset]),
        e.firstCreatePass || mp(t, n));
    let i = t.initialInputs;
    for (let s = r; s < o; s++) {
        let a = e.data[s],
            c = Yc(n, e, s, t);
        if ((ar(c, n), i !== null && DD(n, s - r, c, a, t, i), In(a))) {
            let u = Fe(t.index, n);
            u[me] = Yc(n, e, s, t);
        }
    }
}
function gD(e, n, t) {
    let r = t.directiveStart,
        o = t.directiveEnd,
        i = t.index,
        s = Cf();
    try {
        kt(i);
        for (let a = r; a < o; a++) {
            let c = e.data[a],
                u = n[a];
            (Bi(a),
                (c.hostBindings !== null ||
                    c.hostVars !== 0 ||
                    c.hostAttrs !== null) &&
                    mD(c, u));
        }
    } finally {
        (kt(-1), Bi(s));
    }
}
function mD(e, n) {
    e.hostBindings !== null && e.hostBindings(1, n);
}
function Xp(e, n) {
    let t = e.directiveRegistry,
        r = null;
    if (t)
        for (let o = 0; o < t.length; o++) {
            let i = t[o];
            xy(n, i.selectors, !1) &&
                ((r ??= []), In(i) ? r.unshift(i) : r.push(i));
        }
    return r;
}
function vD(e, n, t, r, o, i) {
    let s = Ye(e, n);
    yD(n[K], s, i, e.value, t, r, o);
}
function yD(e, n, t, r, o, i, s) {
    if (i == null) e.removeAttribute(n, o, t);
    else {
        let a = s == null ? Jn(i) : s(i, r || "", o);
        e.setAttribute(n, o, a, t);
    }
}
function DD(e, n, t, r, o, i) {
    let s = i[n];
    if (s !== null)
        for (let a = 0; a < s.length; a += 2) {
            let c = s[a],
                u = s[a + 1];
            Xc(r, t, c, u);
        }
}
function ED(e, n, t, r, o) {
    let i = ve + t,
        s = n[R],
        a = o(s, n, e, r, t);
    ((n[i] = a), or(e, !0));
    let c = e.type === 2;
    return (
        c
            ? (Lp(n[K], a, e), (cf() === 0 || Xr(e)) && ar(a, n), uf())
            : ar(a, n),
        $i() && (!c || !pu(e)) && wu(s, n, a, e),
        e
    );
}
function CD(e) {
    let n = e;
    return (_c() ? gf() : ((n = n.parent), or(n, !1)), n);
}
function ID(e, n) {
    let t = e[xt];
    if (!t) return;
    let r;
    try {
        r = t.get(Me, null);
    } catch {
        r = null;
    }
    r?.(n);
}
function Su(e, n, t, r, o) {
    let i = e.inputs?.[r],
        s = e.hostDirectiveInputs?.[r],
        a = !1;
    if (s)
        for (let c = 0; c < s.length; c += 2) {
            let u = s[c],
                l = s[c + 1],
                d = n.data[u];
            (Xc(d, t[u], l, o), (a = !0));
        }
    if (i)
        for (let c of i) {
            let u = t[c],
                l = n.data[c];
            (Xc(l, u, r, o), (a = !0));
        }
    return a;
}
function wD(e, n) {
    let t = Fe(n, e),
        r = t[R];
    bD(r, t);
    let o = t[qe];
    (o !== null && t[Yr] === null && (t[Yr] = Rp(o, t[xt])),
        U(P.ComponentStart));
    try {
        Mu(r, t, t[me]);
    } finally {
        U(P.ComponentEnd, t[me]);
    }
}
function bD(e, n) {
    for (let t = n.length; t < e.blueprint.length; t++) n.push(e.blueprint[t]);
}
function Mu(e, n, t) {
    Vi(n);
    try {
        let r = e.viewQuery;
        r !== null && Jc(1, r, t);
        let o = e.template;
        (o !== null && Kp(e, n, o, 1, t),
            e.firstCreatePass && (e.firstCreatePass = !1),
            n[En]?.finishViewCreation(e),
            e.staticContentQueries && Ap(e, n),
            e.staticViewQueries && Jc(2, e.viewQuery, t));
        let i = e.components;
        i !== null && TD(n, i);
    } catch (r) {
        throw (
            e.firstCreatePass &&
                ((e.incompleteFirstPass = !0), (e.firstCreatePass = !1)),
            r
        );
    } finally {
        ((n[w] &= -5), Hi());
    }
}
function TD(e, n) {
    for (let t = 0; t < n.length; t++) wD(e, n[t]);
}
function SD(e, n, t, r) {
    let o = S(null);
    try {
        let i = n.tView,
            a = e[w] & 4096 ? 4096 : 16,
            c = yu(
                e,
                i,
                t,
                a,
                null,
                n,
                null,
                null,
                r?.injector ?? null,
                r?.embeddedViewInjector ?? null,
                r?.dehydratedView ?? null,
            ),
            u = e[n.index];
        c[yn] = u;
        let l = e[En];
        return (
            l !== null && (c[En] = l.createEmbeddedView(i)),
            Mu(i, c, t),
            c
        );
    } finally {
        S(o);
    }
}
function qf(e, n) {
    return !n || n.firstChild === null || wp(e);
}
function uo(e, n, t, r, o = !1) {
    for (; t !== null; ) {
        if (t.type === 128) {
            t = o ? t.projectionNext : t.next;
            continue;
        }
        let i = n[t.index];
        (i !== null && r.push(ke(i)), Ze(i) && eh(i, r));
        let s = t.type;
        if (s & 8) uo(e, n, t.child, r);
        else if (s & 32) {
            let a = Eu(t, n),
                c;
            for (; (c = a()); ) r.push(c);
        } else if (s & 16) {
            let a = Qp(n, t);
            if (Array.isArray(a)) r.push(...a);
            else {
                let c = Nt(n[Pe]);
                uo(c[R], c, a, r, !0);
            }
        }
        t = o ? t.projectionNext : t.next;
    }
    return r;
}
function eh(e, n) {
    for (let t = Ie; t < e.length; t++) {
        let r = e[t],
            o = r[R].firstChild;
        o !== null && uo(r[R], r, o, n);
    }
    e[Pt] !== e[qe] && n.push(e[Pt]);
}
function th(e) {
    if (e[Cn] !== null) {
        for (let n of e[Cn]) n.impl.addSequence(n);
        e[Cn].length = 0;
    }
}
var nh = [];
function MD(e) {
    return e[Te] ?? _D(e);
}
function _D(e) {
    let n = nh.pop() ?? Object.create(RD);
    return ((n.lView = e), n);
}
function ND(e) {
    e.lView[Te] !== e && ((e.lView = null), nh.push(e));
}
var RD = B(g({}, tn), {
    consumerIsAlwaysLive: !0,
    kind: "template",
    consumerMarkedDirty: (e) => {
        rr(e.lView);
    },
    consumerOnSignalRead() {
        this.lView[Te] = this;
    },
});
function AD(e) {
    let n = e[Te] ?? Object.create(xD);
    return ((n.lView = e), n);
}
var xD = B(g({}, tn), {
    consumerIsAlwaysLive: !0,
    kind: "template",
    consumerMarkedDirty: (e) => {
        let n = Nt(e.lView);
        for (; n && !rh(n[R]); ) n = Nt(n);
        n && bc(n);
    },
    consumerOnSignalRead() {
        this.lView[Te] = this;
    },
});
function rh(e) {
    return e.type !== 2;
}
function oh(e) {
    if (e[Qr] === null) return;
    let n = !0;
    for (; n; ) {
        let t = !1;
        for (let r of e[Qr])
            r.dirty &&
                ((t = !0),
                r.zone === null || Zone.current === r.zone
                    ? r.run()
                    : r.zone.run(() => r.run()));
        n = t && !!(e[w] & 8192);
    }
}
var OD = 100;
function ih(e, n = 0) {
    let r = e[We].rendererFactory,
        o = !1;
    o || r.begin?.();
    try {
        PD(e, n);
    } finally {
        o || r.end?.();
    }
}
function PD(e, n) {
    let t = Nc();
    try {
        (Rc(!0), nu(e, n));
        let r = 0;
        for (; to(e); ) {
            if (r === OD) throw new m(103, !1);
            (r++, nu(e, 1));
        }
    } finally {
        Rc(t);
    }
}
function kD(e, n, t, r) {
    if (wn(n)) return;
    let o = n[w],
        i = !1,
        s = !1;
    Vi(n);
    let a = !0,
        c = null,
        u = null;
    i ||
        (rh(e)
            ? ((u = MD(n)), (c = on(u)))
            : Jo() === null
              ? ((a = !1), (u = AD(n)), (c = on(u)))
              : n[Te] && (Ar(n[Te]), (n[Te] = null)));
    try {
        (wc(n), vf(e.bindingStartIndex), t !== null && Kp(e, n, t, 2, r));
        let l = (o & 3) === 3;
        if (!i)
            if (l) {
                let f = e.preOrderCheckHooks;
                f !== null && Zi(n, f, null);
            } else {
                let f = e.preOrderHooks;
                (f !== null && Yi(n, f, 0, null), $c(n, 0));
            }
        if (
            (s || FD(n),
            oh(n),
            sh(n, 0),
            e.contentQueries !== null && Ap(e, n),
            !i)
        )
            if (l) {
                let f = e.contentCheckHooks;
                f !== null && Zi(n, f);
            } else {
                let f = e.contentHooks;
                (f !== null && Yi(n, f, 1), $c(n, 1));
            }
        jD(e, n);
        let d = e.components;
        d !== null && ch(n, d, 0);
        let h = e.viewQuery;
        if ((h !== null && Jc(2, h, r), !i))
            if (l) {
                let f = e.viewCheckHooks;
                f !== null && Zi(n, f);
            } else {
                let f = e.viewHooks;
                (f !== null && Yi(n, f, 2), $c(n, 2));
            }
        if ((e.firstUpdatePass === !0 && (e.firstUpdatePass = !1), n[Fi])) {
            for (let f of n[Fi]) f();
            n[Fi] = null;
        }
        i || (th(n), (n[w] &= -73));
    } catch (l) {
        throw (i || rr(n), l);
    } finally {
        (u !== null && (Bn(u, c), a && ND(u)), Hi());
    }
}
function sh(e, n) {
    for (let t = Tp(e); t !== null; t = Sp(t))
        for (let r = Ie; r < t.length; r++) {
            let o = t[r];
            ah(o, n);
        }
}
function FD(e) {
    for (let n = Tp(e); n !== null; n = Sp(n)) {
        if (!(n[w] & 2)) continue;
        let t = n[Jr];
        for (let r = 0; r < t.length; r++) {
            let o = t[r];
            bc(o);
        }
    }
}
function LD(e, n, t) {
    U(P.ComponentStart);
    let r = Fe(n, e);
    try {
        ah(r, t);
    } finally {
        U(P.ComponentEnd, r[me]);
    }
}
function ah(e, n) {
    Li(e) && nu(e, n);
}
function nu(e, n) {
    let r = e[R],
        o = e[w],
        i = e[Te],
        s = !!(n === 0 && o & 16);
    if (
        ((s ||= !!(o & 64 && n === 0)),
        (s ||= !!(o & 1024)),
        (s ||= !!(i?.dirty && Xo(i))),
        (s ||= !1),
        i && (i.dirty = !1),
        (e[w] &= -9217),
        s)
    )
        kD(r, e, r.template, e[me]);
    else if (o & 8192) {
        let a = S(null);
        try {
            (oh(e), sh(e, 1));
            let c = r.components;
            (c !== null && ch(e, c, 1), th(e));
        } finally {
            S(a);
        }
    }
}
function ch(e, n, t) {
    for (let r = 0; r < n.length; r++) LD(e, n[r], t);
}
function jD(e, n) {
    let t = e.hostBindingOpCodes;
    if (t !== null)
        try {
            for (let r = 0; r < t.length; r++) {
                let o = t[r];
                if (o < 0) kt(~o);
                else {
                    let i = o,
                        s = t[++r],
                        a = t[++r];
                    Ef(s, i);
                    let c = n[i];
                    U(P.HostBindingsUpdateStart, c);
                    try {
                        a(2, c);
                    } finally {
                        U(P.HostBindingsUpdateEnd, c);
                    }
                }
            }
        } finally {
            kt(-1);
        }
}
function _u(e, n) {
    let t = Nc() ? 64 : 1088;
    for (e[We].changeDetectionScheduler?.notify(n); e; ) {
        e[w] |= t;
        let r = Nt(e);
        if (nr(e) && !r) return e;
        e = r;
    }
    return null;
}
function uh(e, n, t, r) {
    return [e, !0, 0, n, null, r, null, t, null, null];
}
function UD(e, n, t, r = !0) {
    let o = n[R];
    if ((BD(o, n, e, t), r)) {
        let s = tu(t, e),
            a = n[K],
            c = a.parentNode(e[Pt]);
        c !== null && Yy(o, e[Oe], a, n, c, s);
    }
    let i = n[Yr];
    i !== null && i.firstChild !== null && (i.firstChild = null);
}
function ru(e, n) {
    if (e.length <= Ie) return;
    let t = Ie + n,
        r = e[t];
    if (r) {
        let o = r[yn];
        (o !== null && o !== e && Iu(o, r), n > 0 && (e[t - 1][xe] = r[xe]));
        let i = qr(e, Ie + n);
        Zy(r[R], r);
        let s = i[En];
        (s !== null && s.detachView(i[R]),
            (r[ae] = null),
            (r[xe] = null),
            (r[w] &= -129));
    }
    return r;
}
function BD(e, n, t, r) {
    let o = Ie + r,
        i = t.length;
    (r > 0 && (t[o - 1][xe] = n),
        r < i - Ie
            ? ((n[xe] = t[o]), pc(t, Ie + r, n))
            : (t.push(n), (n[xe] = null)),
        (n[ae] = t));
    let s = n[yn];
    s !== null && t !== s && lh(s, n);
    let a = n[En];
    (a !== null && a.insertView(e), ji(n), (n[w] |= 128));
}
function lh(e, n) {
    let t = e[Jr],
        r = n[ae];
    if (vt(r)) e[w] |= 2;
    else {
        let o = r[ae][Pe];
        n[Pe] !== o && (e[w] |= 2);
    }
    t === null ? (e[Jr] = [n]) : t.push(n);
}
var Ft = class {
    _lView;
    _cdRefInjectingView;
    _appRef = null;
    _attachedToViewContainer = !1;
    exhaustive;
    get rootNodes() {
        let n = this._lView,
            t = n[R];
        return uo(t, n, t.firstChild, []);
    }
    constructor(n, t) {
        ((this._lView = n), (this._cdRefInjectingView = t));
    }
    get context() {
        return this._lView[me];
    }
    set context(n) {
        this._lView[me] = n;
    }
    get destroyed() {
        return wn(this._lView);
    }
    destroy() {
        if (this._appRef) this._appRef.detachView(this);
        else if (this._attachedToViewContainer) {
            let n = this._lView[ae];
            if (Ze(n)) {
                let t = n[Kr],
                    r = t ? t.indexOf(this) : -1;
                r > -1 && (ru(n, r), qr(t, r));
            }
            this._attachedToViewContainer = !1;
        }
        Yp(this._lView[R], this._lView);
    }
    onDestroy(n) {
        Tc(this._lView, n);
    }
    markForCheck() {
        _u(this._cdRefInjectingView || this._lView, 4);
    }
    detach() {
        this._lView[w] &= -129;
    }
    reattach() {
        (ji(this._lView), (this._lView[w] |= 128));
    }
    detectChanges() {
        ((this._lView[w] |= 1024), ih(this._lView));
    }
    checkNoChanges() {}
    attachToViewContainerRef() {
        if (this._appRef) throw new m(902, !1);
        this._attachedToViewContainer = !0;
    }
    detachFromAppRef() {
        this._appRef = null;
        let n = nr(this._lView),
            t = this._lView[yn];
        (t !== null && !n && Iu(t, this._lView),
            Zp(this._lView[R], this._lView));
    }
    attachToAppRef(n) {
        if (this._attachedToViewContainer) throw new m(902, !1);
        this._appRef = n;
        let t = nr(this._lView),
            r = this._lView[yn];
        (r !== null && !t && lh(r, this._lView), ji(this._lView));
    }
};
var vo = (() => {
    class e {
        _declarationLView;
        _declarationTContainer;
        elementRef;
        static __NG_ELEMENT_ID__ = VD;
        constructor(t, r, o) {
            ((this._declarationLView = t),
                (this._declarationTContainer = r),
                (this.elementRef = o));
        }
        get ssrId() {
            return this._declarationTContainer.tView?.ssrId || null;
        }
        createEmbeddedView(t, r) {
            return this.createEmbeddedViewImpl(t, r);
        }
        createEmbeddedViewImpl(t, r, o) {
            let i = SD(this._declarationLView, this._declarationTContainer, t, {
                embeddedViewInjector: r,
                dehydratedView: o,
            });
            return new Ft(i);
        }
    }
    return e;
})();
function VD() {
    return HD(Se(), $());
}
function HD(e, n) {
    return e.type & 4 ? new vo(n, e, us(e, n)) : null;
}
function Nu(e, n, t, r, o) {
    let i = e.data[n];
    if (i === null) ((i = $D(e, n, t, r, o)), Df() && (i.flags |= 32));
    else if (i.type & 64) {
        ((i.type = t), (i.value = r), (i.attrs = o));
        let s = hf();
        i.injectorIndex = s === null ? -1 : s.injectorIndex;
    }
    return (or(i, !0), i);
}
function $D(e, n, t, r, o) {
    let i = Mc(),
        s = _c(),
        a = s ? i : i && i.parent,
        c = (e.data[n] = GD(e, a, t, n, r, o));
    return (zD(e, c, i, s), c);
}
function zD(e, n, t, r) {
    (e.firstChild === null && (e.firstChild = n),
        t !== null &&
            (r
                ? t.child == null && n.parent !== null && (t.child = n)
                : t.next === null && ((t.next = n), (n.prev = t))));
}
function GD(e, n, t, r, o, i) {
    let s = n ? n.injectorIndex : -1,
        a = 0;
    return (
        df() && (a |= 128),
        {
            type: t,
            index: r,
            insertBeforeIndex: null,
            injectorIndex: s,
            directiveStart: -1,
            directiveEnd: -1,
            directiveStylingLast: -1,
            componentOffset: -1,
            controlDirectiveIndex: -1,
            customControlIndex: -1,
            propertyBindings: null,
            flags: a,
            providerIndexes: 0,
            value: o,
            attrs: i,
            mergedAttrs: null,
            localNames: null,
            initialInputs: null,
            inputs: null,
            hostDirectiveInputs: null,
            outputs: null,
            hostDirectiveOutputs: null,
            directiveToIndex: null,
            tView: null,
            next: null,
            prev: null,
            projectionNext: null,
            child: null,
            parent: n,
            projection: null,
            styles: null,
            stylesWithoutHost: null,
            residualStyles: void 0,
            classes: null,
            classesWithoutHost: null,
            residualClasses: void 0,
            classBindings: 0,
            styleBindings: 0,
        }
    );
}
var qD = () => null;
function Wf(e, n) {
    return qD(e, n);
}
var dh = class {},
    Ds = class {},
    ou = class {
        resolveComponentFactory(n) {
            throw new m(917, !1);
        }
    },
    yo = class {
        static NULL = new ou();
    },
    Tn = class {},
    Do = (() => {
        class e {
            destroyNode = null;
            static __NG_ELEMENT_ID__ = () => WD();
        }
        return e;
    })();
function WD() {
    let e = $(),
        n = Se(),
        t = Fe(n.index, e);
    return (vt(t) ? t : e)[K];
}
var fh = (() => {
    class e {
        static ɵprov = v({ token: e, providedIn: "root", factory: () => null });
    }
    return e;
})();
var Qi = {},
    iu = class {
        injector;
        parentInjector;
        constructor(n, t) {
            ((this.injector = n), (this.parentInjector = t));
        }
        get(n, t, r) {
            let o = this.injector.get(n, Qi, r);
            return o !== Qi || t === Qi ? o : this.parentInjector.get(n, t, r);
        }
    };
function Zf(e, n, t) {
    let r = t ? e.styles : null,
        o = t ? e.classes : null,
        i = 0;
    if (n !== null)
        for (let s = 0; s < n.length; s++) {
            let a = n[s];
            if (typeof a == "number") i = a;
            else if (i == 1) o = tc(o, a);
            else if (i == 2) {
                let c = a,
                    u = n[++s];
                r = tc(r, c + ": " + u + ";");
            }
        }
    (t ? (e.styles = r) : (e.stylesWithoutHost = r),
        t ? (e.classes = o) : (e.classesWithoutHost = o));
}
function Z(e, n = 0) {
    let t = $();
    if (t === null) return E(e, n);
    let r = Se();
    return Ep(r, t, Ce(e), n);
}
function Ru() {
    let e = "invalid";
    throw new Error(e);
}
function ph(e, n, t, r, o) {
    let i = r === null ? null : { "": -1 },
        s = o(e, t);
    if (s !== null) {
        let a = s,
            c = null,
            u = null;
        for (let l of s)
            if (l.resolveHostDirectives !== null) {
                [a, c, u] = l.resolveHostDirectives(s);
                break;
            }
        QD(e, n, t, a, i, c, u);
    }
    i !== null && r !== null && ZD(t, r, i);
}
function ZD(e, n, t) {
    let r = (e.localNames = []);
    for (let o = 0; o < n.length; o += 2) {
        let i = t[n[o + 1]];
        if (i == null) throw new m(-301, !1);
        r.push(n[o], i);
    }
}
function YD(e, n, t) {
    ((n.componentOffset = t), (e.components ??= []).push(n.index));
}
function QD(e, n, t, r, o, i, s) {
    let a = r.length,
        c = null;
    for (let h = 0; h < a; h++) {
        let f = r[h];
        (c === null && In(f) && ((c = f), YD(e, t, h)),
            ny(mp(t, n), e, f.type));
    }
    (nE(t, e.data.length, a),
        c?.viewProvidersResolver && c.viewProvidersResolver(c));
    for (let h = 0; h < a; h++) {
        let f = r[h];
        f.providersResolver && f.providersResolver(f);
    }
    let u = !1,
        l = !1,
        d = $p(e, n, a, null);
    a > 0 && (t.directiveToIndex = new Map());
    for (let h = 0; h < a; h++) {
        let f = r[h];
        if (
            ((t.mergedAttrs = cu(t.mergedAttrs, f.hostAttrs)),
            JD(e, t, n, d, f),
            tE(d, f, o),
            s !== null && s.has(f))
        ) {
            let [k, C] = s.get(f);
            t.directiveToIndex.set(f.type, [
                d,
                k + t.directiveStart,
                C + t.directiveStart,
            ]);
        } else (i === null || !i.has(f)) && t.directiveToIndex.set(f.type, d);
        (f.contentQueries !== null && (t.flags |= 4),
            (f.hostBindings !== null ||
                f.hostAttrs !== null ||
                f.hostVars !== 0) &&
                (t.flags |= 64));
        let y = f.type.prototype;
        (!u &&
            (y.ngOnChanges || y.ngOnInit || y.ngDoCheck) &&
            ((e.preOrderHooks ??= []).push(t.index), (u = !0)),
            !l &&
                (y.ngOnChanges || y.ngDoCheck) &&
                ((e.preOrderCheckHooks ??= []).push(t.index), (l = !0)),
            d++);
    }
    KD(e, t, i);
}
function KD(e, n, t) {
    for (let r = n.directiveStart; r < n.directiveEnd; r++) {
        let o = e.data[r];
        if (t === null || !t.has(o))
            (Yf(0, n, o, r), Yf(1, n, o, r), Kf(n, r, !1));
        else {
            let i = t.get(o);
            (Qf(0, n, i, r), Qf(1, n, i, r), Kf(n, r, !0));
        }
    }
}
function Yf(e, n, t, r) {
    let o = e === 0 ? t.inputs : t.outputs;
    for (let i in o)
        if (o.hasOwnProperty(i)) {
            let s;
            (e === 0 ? (s = n.inputs ??= {}) : (s = n.outputs ??= {}),
                (s[i] ??= []),
                s[i].push(r),
                hh(n, i));
        }
}
function Qf(e, n, t, r) {
    let o = e === 0 ? t.inputs : t.outputs;
    for (let i in o)
        if (o.hasOwnProperty(i)) {
            let s = o[i],
                a;
            (e === 0
                ? (a = n.hostDirectiveInputs ??= {})
                : (a = n.hostDirectiveOutputs ??= {}),
                (a[s] ??= []),
                a[s].push(r, i),
                hh(n, s));
        }
}
function hh(e, n) {
    n === "class" ? (e.flags |= 8) : n === "style" && (e.flags |= 16);
}
function Kf(e, n, t) {
    let { attrs: r, inputs: o, hostDirectiveInputs: i } = e;
    if (r === null || (!t && o === null) || (t && i === null) || mu(e)) {
        ((e.initialInputs ??= []), e.initialInputs.push(null));
        return;
    }
    let s = null,
        a = 0;
    for (; a < r.length; ) {
        let c = r[a];
        if (c === 0) {
            a += 4;
            continue;
        } else if (c === 5) {
            a += 2;
            continue;
        } else if (typeof c == "number") break;
        if (!t && o.hasOwnProperty(c)) {
            let u = o[c];
            for (let l of u)
                if (l === n) {
                    ((s ??= []), s.push(c, r[a + 1]));
                    break;
                }
        } else if (t && i.hasOwnProperty(c)) {
            let u = i[c];
            for (let l = 0; l < u.length; l += 2)
                if (u[l] === n) {
                    ((s ??= []), s.push(u[l + 1], r[a + 1]));
                    break;
                }
        }
        a += 2;
    }
    ((e.initialInputs ??= []), e.initialInputs.push(s));
}
function JD(e, n, t, r, o) {
    e.data[r] = o;
    let i = o.factory || (o.factory = _t(o.type, !0)),
        s = new co(i, In(o), Z, null);
    ((e.blueprint[r] = s),
        (t[r] = s),
        XD(e, n, r, $p(e, t, o.hostVars, Ct), o));
}
function XD(e, n, t, r, o) {
    let i = o.hostBindings;
    if (i) {
        let s = e.hostBindingOpCodes;
        s === null && (s = e.hostBindingOpCodes = []);
        let a = ~n.index;
        (eE(s) != a && s.push(a), s.push(t, r, i));
    }
}
function eE(e) {
    let n = e.length;
    for (; n > 0; ) {
        let t = e[--n];
        if (typeof t == "number" && t < 0) return t;
    }
    return 0;
}
function tE(e, n, t) {
    if (t) {
        if (n.exportAs)
            for (let r = 0; r < n.exportAs.length; r++) t[n.exportAs[r]] = e;
        In(n) && (t[""] = e);
    }
}
function nE(e, n, t) {
    ((e.flags |= 1),
        (e.directiveStart = n),
        (e.directiveEnd = n + t),
        (e.providerIndexes = n));
}
function gh(e, n, t, r, o, i, s, a) {
    let c = n[R],
        u = c.consts,
        l = eo(u, s),
        d = Nu(c, e, t, r, l);
    return (
        i && ph(c, n, d, eo(u, a), o),
        (d.mergedAttrs = cu(d.mergedAttrs, d.attrs)),
        d.attrs !== null && Zf(d, d.attrs, !1),
        d.mergedAttrs !== null && Zf(d, d.mergedAttrs, !0),
        c.queries !== null && c.queries.elementStart(c, d),
        d
    );
}
function mh(e, n) {
    (lp(e, n), Ec(n) && e.queries.elementEnd(n));
}
function Au(e) {
    return yh(e)
        ? Array.isArray(e) || (!(e instanceof Map) && Symbol.iterator in e)
        : !1;
}
function vh(e, n) {
    if (Array.isArray(e)) for (let t = 0; t < e.length; t++) n(e[t]);
    else {
        let t = e[Symbol.iterator](),
            r;
        for (; !(r = t.next()).done; ) n(r.value);
    }
}
function yh(e) {
    return e !== null && (typeof e == "function" || typeof e == "object");
}
function Dh(e, n, t) {
    return (e[n] = t);
}
function Sn(e, n, t) {
    if (t === Ct) return !1;
    let r = e[n];
    return Object.is(r, t) ? !1 : ((e[n] = t), !0);
}
function Eh(e, n, t, r) {
    let o = Sn(e, n, t);
    return Sn(e, n + 1, r) || o;
}
function rE(e, n, t, r, o) {
    let i = Eh(e, n, t, r);
    return Sn(e, n + 2, o) || i;
}
function qc(e, n, t) {
    return function r(o) {
        let i = yt(e) ? Fe(e.index, n) : n;
        _u(i, 5);
        let s = n[me],
            a = Jf(n, s, t, o),
            c = r.__ngNextListenerFn__;
        for (; c; ) ((a = Jf(n, s, c, o) && a), (c = c.__ngNextListenerFn__));
        return a;
    };
}
function Jf(e, n, t, r) {
    let o = S(null);
    try {
        return (U(P.OutputStart, n, t), t(r) !== !1);
    } catch (i) {
        return (ID(e, i), !1);
    } finally {
        (U(P.OutputEnd, n, t), S(o));
    }
}
function oE(e, n, t, r, o, i, s, a) {
    let c = Xr(e),
        u = !1,
        l = null;
    if ((!r && c && (l = sE(n, t, i, e.index)), l !== null)) {
        let d = l.__ngLastListenerFn__ || l;
        ((d.__ngNextListenerFn__ = s), (l.__ngLastListenerFn__ = s), (u = !0));
    } else {
        let d = Ye(e, t),
            h = r ? r(d) : d;
        my(t, h, i, a);
        let f = o.listen(h, i, a);
        if (!iE(i)) {
            let y = r ? (k) => r(ke(k[e.index])) : e.index;
            Ch(y, n, t, i, a, f, !1);
        }
    }
    return u;
}
function iE(e) {
    return e.startsWith("animation") || e.startsWith("transition");
}
function sE(e, n, t, r) {
    let o = e.cleanup;
    if (o != null)
        for (let i = 0; i < o.length - 1; i += 2) {
            let s = o[i];
            if (s === t && o[i + 1] === r) {
                let a = n[Xn],
                    c = o[i + 2];
                return a && a.length > c ? a[c] : null;
            }
            typeof s == "string" && (i += 2);
        }
    return null;
}
function Ch(e, n, t, r, o, i, s) {
    let a = n.firstCreatePass ? af(n) : null,
        c = sf(t),
        u = c.length;
    (c.push(o, i), a && a.push(r, e, u, (u + 1) * (s ? -1 : 1)));
}
function Xf(e, n, t, r, o, i) {
    let s = n[t],
        a = n[R],
        u = a.data[t].outputs[r],
        d = s[u].subscribe(i);
    Ch(e.index, a, n, o, i, d, !0);
}
var su = Symbol("BINDING");
function Ih(e) {
    return e.debugInfo?.className || e.type.name || null;
}
var os = class extends yo {
    ngModule;
    constructor(n) {
        (super(), (this.ngModule = n));
    }
    resolveComponentFactory(n) {
        let t = mt(n);
        return new Mn(t, this.ngModule);
    }
};
function aE(e) {
    return Object.keys(e).map((n) => {
        let [t, r, o] = e[n],
            i = {
                propName: t,
                templateName: n,
                isSignal: (r & gs.SignalBased) !== 0,
            };
        return (o && (i.transform = o), i);
    });
}
function cE(e) {
    return Object.keys(e).map((n) => ({ propName: e[n], templateName: n }));
}
function uE(e, n, t) {
    let r = n instanceof G ? n : n?.injector;
    return (
        r &&
            e.getStandaloneInjector !== null &&
            (r = e.getStandaloneInjector(r) || r),
        r ? new iu(t, r) : t
    );
}
function lE(e) {
    let n = e.get(Tn, null);
    if (n === null) throw new m(407, !1);
    let t = e.get(fh, null),
        r = e.get(Rt, null),
        o = e.get(at, null, { optional: !0 });
    return {
        rendererFactory: n,
        sanitizer: t,
        changeDetectionScheduler: r,
        ngReflect: !1,
        tracingService: o,
    };
}
function dE(e, n) {
    let t = wh(e);
    return kp(n, t, t === "svg" ? Jd : t === "math" ? Xd : null);
}
function wh(e) {
    return (e.selectors[0][0] || "div").toLowerCase();
}
var Mn = class extends Ds {
    componentDef;
    ngModule;
    selector;
    componentType;
    ngContentSelectors;
    isBoundToModule;
    cachedInputs = null;
    cachedOutputs = null;
    get inputs() {
        return (
            (this.cachedInputs ??= aE(this.componentDef.inputs)),
            this.cachedInputs
        );
    }
    get outputs() {
        return (
            (this.cachedOutputs ??= cE(this.componentDef.outputs)),
            this.cachedOutputs
        );
    }
    constructor(n, t) {
        (super(),
            (this.componentDef = n),
            (this.ngModule = t),
            (this.componentType = n.type),
            (this.selector = Fy(n.selectors)),
            (this.ngContentSelectors = n.ngContentSelectors ?? []),
            (this.isBoundToModule = !!t));
    }
    create(n, t, r, o, i, s) {
        U(P.DynamicComponentStart);
        let a = S(null);
        try {
            let c = this.componentDef,
                u = uE(c, o || this.ngModule, n),
                l = lE(u),
                d = l.tracingService;
            return d && d.componentCreate
                ? d.componentCreate(Ih(c), () =>
                      this.createComponentRef(l, u, t, r, i, s),
                  )
                : this.createComponentRef(l, u, t, r, i, s);
        } finally {
            S(a);
        }
    }
    createComponentRef(n, t, r, o, i, s) {
        let a = this.componentDef,
            c = fE(o, a, s, i),
            u = n.rendererFactory.createRenderer(null, a),
            l = o ? aD(u, o, a.encapsulation, t) : dE(a, u),
            d =
                s?.some(ep) ||
                i?.some((y) => typeof y != "function" && y.bindings.some(ep)),
            h = yu(
                null,
                c,
                null,
                512 | Hp(a),
                null,
                null,
                n,
                u,
                t,
                null,
                Rp(l, t, !0),
            );
        ((h[ve] = l), Vi(h));
        let f = null;
        try {
            let y = gh(ve, h, 2, "#host", () => c.directiveRegistry, !0, 0);
            (Lp(u, l, y),
                ar(l, h),
                Tu(c, h, y),
                xp(c, y, h),
                mh(c, y),
                r !== void 0 && hE(y, this.ngContentSelectors, r),
                (f = Fe(y.index, h)),
                (h[me] = f[me]),
                Mu(c, h, null));
        } catch (y) {
            throw (f !== null && Kc(f), Kc(h), y);
        } finally {
            (U(P.DynamicComponentEnd), Hi());
        }
        return new is(this.componentType, h, !!d);
    }
};
function fE(e, n, t, r) {
    let o = e ? ["ng-version", "21.2.9"] : Ly(n.selectors[0]),
        i = null,
        s = null,
        a = 0;
    if (t)
        for (let l of t)
            ((a += l[su].requiredVars),
                l.create && ((l.targetIdx = 0), (i ??= []).push(l)),
                l.update && ((l.targetIdx = 0), (s ??= []).push(l)));
    if (r)
        for (let l = 0; l < r.length; l++) {
            let d = r[l];
            if (typeof d != "function")
                for (let h of d.bindings) {
                    a += h[su].requiredVars;
                    let f = l + 1;
                    (h.create && ((h.targetIdx = f), (i ??= []).push(h)),
                        h.update && ((h.targetIdx = f), (s ??= []).push(h)));
                }
        }
    let c = [n];
    if (r)
        for (let l of r) {
            let d = typeof l == "function" ? l : l.type,
                h = lc(d);
            c.push(h);
        }
    return vu(0, null, pE(i, s), 1, a, c, null, null, null, [o], null);
}
function pE(e, n) {
    return !e && !n
        ? null
        : (t) => {
              if (t & 1 && e) for (let r of e) r.create();
              if (t & 2 && n) for (let r of n) r.update();
          };
}
function ep(e) {
    let n = e[su].kind;
    return n === "input" || n === "twoWay";
}
var is = class extends dh {
    _rootLView;
    _hasInputBindings;
    instance;
    hostView;
    changeDetectorRef;
    componentType;
    location;
    previousInputValues = null;
    _tNode;
    constructor(n, t, r) {
        (super(),
            (this._rootLView = t),
            (this._hasInputBindings = r),
            (this._tNode = Cc(t[R], ve)),
            (this.location = us(this._tNode, t)),
            (this.instance = Fe(this._tNode.index, t)[me]),
            (this.hostView = this.changeDetectorRef = new Ft(t, void 0)),
            (this.componentType = n));
    }
    setInput(n, t) {
        this._hasInputBindings;
        let r = this._tNode;
        if (
            ((this.previousInputValues ??= new Map()),
            this.previousInputValues.has(n) &&
                Object.is(this.previousInputValues.get(n), t))
        )
            return;
        let o = this._rootLView,
            i = Su(r, o[R], o, n, t);
        this.previousInputValues.set(n, t);
        let s = Fe(r.index, o);
        _u(s, 1);
    }
    get injector() {
        return new bn(this._tNode, this._rootLView);
    }
    destroy() {
        this.hostView.destroy();
    }
    onDestroy(n) {
        this.hostView.onDestroy(n);
    }
};
function hE(e, n, t) {
    let r = (e.projection = []);
    for (let o = 0; o < n.length; o++) {
        let i = t[o];
        r.push(i != null && i.length ? Array.from(i) : null);
    }
}
var Nn = (() => {
    class e {
        static __NG_ELEMENT_ID__ = gE;
    }
    return e;
})();
function gE() {
    let e = Se();
    return mE(e, $());
}
var au = class e extends Nn {
    _lContainer;
    _hostTNode;
    _hostLView;
    constructor(n, t, r) {
        (super(),
            (this._lContainer = n),
            (this._hostTNode = t),
            (this._hostLView = r));
    }
    get element() {
        return us(this._hostTNode, this._hostLView);
    }
    get injector() {
        return new bn(this._hostTNode, this._hostLView);
    }
    get parentInjector() {
        let n = uu(this._hostTNode, this._hostLView);
        if (pp(n)) {
            let t = Xi(n, this._hostLView),
                r = Ji(n),
                o = t[R].data[r + 8];
            return new bn(o, t);
        } else return new bn(null, this._hostLView);
    }
    clear() {
        for (; this.length > 0; ) this.remove(this.length - 1);
    }
    get(n) {
        let t = tp(this._lContainer);
        return (t !== null && t[n]) || null;
    }
    get length() {
        return this._lContainer.length - Ie;
    }
    createEmbeddedView(n, t, r) {
        let o, i;
        typeof r == "number"
            ? (o = r)
            : r != null && ((o = r.index), (i = r.injector));
        let s = Wf(this._lContainer, n.ssrId),
            a = n.createEmbeddedViewImpl(t || {}, i, s);
        return (this.insertImpl(a, o, qf(this._hostTNode, s)), a);
    }
    createComponent(n, t, r, o, i, s, a) {
        let c = n && !Hv(n),
            u;
        if (c) u = t;
        else {
            let C = t || {};
            ((u = C.index),
                (r = C.injector),
                (o = C.projectableNodes),
                (i = C.environmentInjector || C.ngModuleRef),
                (s = C.directives),
                (a = C.bindings));
        }
        let l = c ? n : new Mn(mt(n)),
            d = r || this.parentInjector;
        if (!i && l.ngModule == null) {
            let _ = (c ? d : this.parentInjector).get(G, null);
            _ && (i = _);
        }
        let h = mt(l.componentType ?? {}),
            f = Wf(this._lContainer, h?.id ?? null),
            y = f?.firstChild ?? null,
            k = l.create(d, o, y, i, s, a);
        return (this.insertImpl(k.hostView, u, qf(this._hostTNode, f)), k);
    }
    insert(n, t) {
        return this.insertImpl(n, t, !0);
    }
    insertImpl(n, t, r) {
        let o = n._lView;
        if (nf(o)) {
            let a = this.indexOf(n);
            if (a !== -1) this.detach(a);
            else {
                let c = o[ae],
                    u = new e(c, c[Oe], c[ae]);
                u.detach(u.indexOf(n));
            }
        }
        let i = this._adjustIndex(t),
            s = this._lContainer;
        return (
            UD(s, o, i, r),
            n.attachToViewContainerRef(),
            pc(Wc(s), i, n),
            n
        );
    }
    move(n, t) {
        return this.insert(n, t);
    }
    indexOf(n) {
        let t = tp(this._lContainer);
        return t !== null ? t.indexOf(n) : -1;
    }
    remove(n) {
        let t = this._adjustIndex(n, -1),
            r = ru(this._lContainer, t);
        r && (qr(Wc(this._lContainer), t), Yp(r[R], r));
    }
    detach(n) {
        let t = this._adjustIndex(n, -1),
            r = ru(this._lContainer, t);
        return r && qr(Wc(this._lContainer), t) != null ? new Ft(r) : null;
    }
    _adjustIndex(n, t = 0) {
        return n ?? this.length + t;
    }
};
function tp(e) {
    return e[Kr];
}
function Wc(e) {
    return e[Kr] || (e[Kr] = []);
}
function mE(e, n) {
    let t,
        r = n[e.index];
    return (
        Ze(r) ? (t = r) : ((t = uh(r, n, null, e)), (n[e.index] = t), Du(n, t)),
        yE(t, n, e, r),
        new au(t, e, n)
    );
}
function vE(e, n) {
    let t = e[K],
        r = t.createComment(""),
        o = Ye(n, e),
        i = t.parentNode(o);
    return (ns(t, i, r, t.nextSibling(o), !1), r);
}
var yE = CE,
    DE = () => !1;
function EE(e, n, t) {
    return DE(e, n, t);
}
function CE(e, n, t, r) {
    if (e[Pt]) return;
    let o;
    (t.type & 8 ? (o = ke(r)) : (o = vE(n, t)), (e[Pt] = o));
}
var _n = class {},
    Es = class {};
var ss = class extends _n {
        ngModuleType;
        _parent;
        _bootstrapComponents = [];
        _r3Injector;
        instance;
        destroyCbs = [];
        componentFactoryResolver = new os(this);
        constructor(n, t, r, o = !0) {
            (super(), (this.ngModuleType = n), (this._parent = t));
            let i = uc(n);
            ((this._bootstrapComponents = Bp(i.bootstrap)),
                (this._r3Injector = Fc(
                    n,
                    t,
                    [
                        { provide: _n, useValue: this },
                        {
                            provide: yo,
                            useValue: this.componentFactoryResolver,
                        },
                        ...r,
                    ],
                    Ri(n),
                    new Set(["environment"]),
                )),
                o && this.resolveInjectorInitializers());
        }
        resolveInjectorInitializers() {
            (this._r3Injector.resolveInjectorInitializers(),
                (this.instance = this._r3Injector.get(this.ngModuleType)));
        }
        get injector() {
            return this._r3Injector;
        }
        destroy() {
            let n = this._r3Injector;
            (!n.destroyed && n.destroy(),
                this.destroyCbs.forEach((t) => t()),
                (this.destroyCbs = null));
        }
        onDestroy(n) {
            this.destroyCbs.push(n);
        }
    },
    as = class extends Es {
        moduleType;
        constructor(n) {
            (super(), (this.moduleType = n));
        }
        create(n) {
            return new ss(this.moduleType, n, []);
        }
    };
var lo = class extends _n {
    injector;
    componentFactoryResolver = new os(this);
    instance = null;
    constructor(n) {
        super();
        let t = new pn(
            [
                ...n.providers,
                { provide: _n, useValue: this },
                { provide: yo, useValue: this.componentFactoryResolver },
            ],
            n.parent || Zr(),
            n.debugName,
            new Set(["environment"]),
        );
        ((this.injector = t),
            n.runEnvironmentInitializers && t.resolveInjectorInitializers());
    }
    destroy() {
        this.injector.destroy();
    }
    onDestroy(n) {
        this.injector.onDestroy(n);
    }
};
function fr(e, n, t = null) {
    return new lo({
        providers: e,
        parent: n,
        debugName: t,
        runEnvironmentInitializers: !0,
    }).injector;
}
var IE = (() => {
    class e {
        _injector;
        cachedInjectors = new Map();
        constructor(t) {
            this._injector = t;
        }
        getOrCreateStandaloneInjector(t) {
            if (!t.standalone) return null;
            if (!this.cachedInjectors.has(t)) {
                let r = mc(!1, t.type),
                    o = r.length > 0 ? fr([r], this._injector, "") : null;
                this.cachedInjectors.set(t, o);
            }
            return this.cachedInjectors.get(t);
        }
        ngOnDestroy() {
            try {
                for (let t of this.cachedInjectors.values())
                    t !== null && t.destroy();
            } finally {
                this.cachedInjectors.clear();
            }
        }
        static ɵprov = v({
            token: e,
            providedIn: "environment",
            factory: () => new e(E(G)),
        });
    }
    return e;
})();
function Je(e) {
    return fo(() => {
        let n = bh(e),
            t = B(g({}, n), {
                decls: e.decls,
                vars: e.vars,
                template: e.template,
                consts: e.consts || null,
                ngContentSelectors: e.ngContentSelectors,
                onPush: e.changeDetection === lu.OnPush,
                directiveDefs: null,
                pipeDefs: null,
                dependencies: (n.standalone && e.dependencies) || null,
                getStandaloneInjector: n.standalone
                    ? (o) => o.get(IE).getOrCreateStandaloneInjector(t)
                    : null,
                getExternalStyles: null,
                signals: e.signals ?? !1,
                data: e.data || {},
                encapsulation: e.encapsulation || Ke.Emulated,
                styles: e.styles || ze,
                _: null,
                schemas: e.schemas || null,
                tView: null,
                id: "",
            });
        (n.standalone && dr("NgStandalone"), Th(t));
        let r = e.dependencies;
        return (
            (t.directiveDefs = np(r, wE)),
            (t.pipeDefs = np(r, Vd)),
            (t.id = SE(t)),
            t
        );
    });
}
function wE(e) {
    return mt(e) || lc(e);
}
function jt(e) {
    return fo(() => ({
        type: e.type,
        bootstrap: e.bootstrap || ze,
        declarations: e.declarations || ze,
        imports: e.imports || ze,
        exports: e.exports || ze,
        transitiveCompileScopes: null,
        schemas: e.schemas || null,
        id: e.id || null,
    }));
}
function bE(e, n) {
    if (e == null) return gn;
    let t = {};
    for (let r in e)
        if (e.hasOwnProperty(r)) {
            let o = e[r],
                i,
                s,
                a,
                c;
            (Array.isArray(o)
                ? ((a = o[0]), (i = o[1]), (s = o[2] ?? i), (c = o[3] || null))
                : ((i = o), (s = o), (a = gs.None), (c = null)),
                (t[i] = [r, a, c]),
                (n[i] = s));
        }
    return t;
}
function TE(e) {
    if (e == null) return gn;
    let n = {};
    for (let t in e) e.hasOwnProperty(t) && (n[e[t]] = t);
    return n;
}
function Ut(e) {
    return fo(() => {
        let n = bh(e);
        return (Th(n), n);
    });
}
function xu(e) {
    return {
        type: e.type,
        name: e.name,
        factory: null,
        pure: e.pure !== !1,
        standalone: e.standalone ?? !0,
        onDestroy: e.type.prototype.ngOnDestroy || null,
    };
}
function bh(e) {
    let n = {};
    return {
        type: e.type,
        providersResolver: null,
        viewProvidersResolver: null,
        factory: null,
        hostBindings: e.hostBindings || null,
        hostVars: e.hostVars || 0,
        hostAttrs: e.hostAttrs || null,
        contentQueries: e.contentQueries || null,
        declaredInputs: n,
        inputConfig: e.inputs || gn,
        exportAs: e.exportAs || null,
        standalone: e.standalone ?? !0,
        signals: e.signals === !0,
        selectors: e.selectors || ze,
        viewQuery: e.viewQuery || null,
        features: e.features || null,
        setInput: null,
        resolveHostDirectives: null,
        hostDirectives: null,
        controlDef: null,
        inputs: bE(e.inputs, n),
        outputs: TE(e.outputs),
        debugInfo: null,
    };
}
function Th(e) {
    e.features?.forEach((n) => n(e));
}
function np(e, n) {
    return e
        ? () => {
              let t = typeof e == "function" ? e() : e,
                  r = [];
              for (let o of t) {
                  let i = n(o);
                  i !== null && r.push(i);
              }
              return r;
          }
        : null;
}
function SE(e) {
    let n = 0,
        t = typeof e.consts == "function" ? "" : e.consts,
        r = [
            e.selectors,
            e.ngContentSelectors,
            e.hostVars,
            e.hostAttrs,
            t,
            e.vars,
            e.decls,
            e.encapsulation,
            e.standalone,
            e.signals,
            e.exportAs,
            JSON.stringify(e.inputs),
            JSON.stringify(e.outputs),
            Object.getOwnPropertyNames(e.type.prototype),
            !!e.contentQueries,
            !!e.viewQuery,
        ];
    for (let i of r.join("|")) n = (Math.imul(31, n) + i.charCodeAt(0)) << 0;
    return ((n += 2147483648), "c" + n);
}
function ME(e, n, t, r, o, i, s, a) {
    if (t.firstCreatePass) {
        e.mergedAttrs = cu(e.mergedAttrs, e.attrs);
        let l = (e.tView = vu(
            2,
            e,
            o,
            i,
            s,
            t.directiveRegistry,
            t.pipeRegistry,
            null,
            t.schemas,
            t.consts,
            null,
        ));
        t.queries !== null &&
            (t.queries.template(t, e),
            (l.queries = t.queries.embeddedTView(e)));
    }
    (a && (e.flags |= a), or(e, !1));
    let c = NE(t, n, e, r);
    ($i() && wu(t, n, c, e), ar(c, n));
    let u = uh(c, n, c, e);
    ((n[r + ve] = u), Du(n, u), EE(u, e, n));
}
function _E(e, n, t, r, o, i, s, a, c, u, l) {
    let d = t + ve,
        h;
    return (
        n.firstCreatePass
            ? ((h = Nu(n, d, 4, s || null, a || null)),
              Sc() && ph(n, e, h, eo(n.consts, u), Xp),
              lp(n, h))
            : (h = n.data[d]),
        ME(h, e, n, t, r, o, i, c),
        Xr(h) && Tu(n, e, h),
        u != null && Jp(e, h, l),
        h
    );
}
function Bt(e, n, t, r, o, i, s, a) {
    let c = $(),
        u = Dt(),
        l = eo(u.consts, i);
    return (_E(c, u, e, n, t, r, o, l, void 0, s, a), Bt);
}
var NE = RE;
function RE(e, n, t, r) {
    return (zi(!0), n[K].createComment(""));
}
var Ou = (() => {
    class e {
        log(t) {
            console.log(t);
        }
        warn(t) {
            console.warn(t);
        }
        static ɵfac = function (r) {
            return new (r || e)();
        };
        static ɵprov = v({ token: e, factory: e.ɵfac, providedIn: "platform" });
    }
    return e;
})();
var Pu = new D("");
function pr(e) {
    return !!e && typeof e.then == "function";
}
function ku(e) {
    return !!e && typeof e.subscribe == "function";
}
var Fu = new D("");
function Cs(e) {
    return At([{ provide: Fu, multi: !0, useValue: e }]);
}
var Lu = (() => {
        class e {
            resolve;
            reject;
            initialized = !1;
            done = !1;
            donePromise = new Promise((t, r) => {
                ((this.resolve = t), (this.reject = r));
            });
            appInits = p(Fu, { optional: !0 }) ?? [];
            injector = p(ge);
            constructor() {}
            runInitializers() {
                if (this.initialized) return;
                let t = [];
                for (let o of this.appInits) {
                    let i = Q(this.injector, o);
                    if (pr(i)) t.push(i);
                    else if (ku(i)) {
                        let s = new Promise((a, c) => {
                            i.subscribe({ complete: a, error: c });
                        });
                        t.push(s);
                    }
                }
                let r = () => {
                    ((this.done = !0), this.resolve());
                };
                (Promise.all(t)
                    .then(() => {
                        r();
                    })
                    .catch((o) => {
                        this.reject(o);
                    }),
                    t.length === 0 && r(),
                    (this.initialized = !0));
            }
            static ɵfac = function (r) {
                return new (r || e)();
            };
            static ɵprov = v({ token: e, factory: e.ɵfac, providedIn: "root" });
        }
        return e;
    })(),
    Eo = new D("");
function Sh() {
    Ma(() => {
        let e = "";
        throw new m(600, e);
    });
}
function Mh(e) {
    return e.isBoundToModule;
}
var AE = 10;
var hr = (() => {
    class e {
        _runningTick = !1;
        _destroyed = !1;
        _destroyListeners = [];
        _views = [];
        internalErrorHandler = p(Me);
        afterRenderManager = p(Cu);
        zonelessEnabled = p(ro);
        rootEffectScheduler = p(Hc);
        dirtyFlags = 0;
        tracingSnapshot = null;
        allTestViews = new Set();
        autoDetectTestViews = new Set();
        includeAllTestViews = !1;
        afterTick = new re();
        get allViews() {
            return [
                ...(this.includeAllTestViews
                    ? this.allTestViews
                    : this.autoDetectTestViews
                ).keys(),
                ...this._views,
            ];
        }
        get destroyed() {
            return this._destroyed;
        }
        componentTypes = [];
        components = [];
        internalPendingTask = p(Et);
        get isStable() {
            return this.internalPendingTask.hasPendingTasksObservable.pipe(
                H((t) => !t),
            );
        }
        constructor() {
            p(at, { optional: !0 });
        }
        whenStable() {
            let t;
            return new Promise((r) => {
                t = this.isStable.subscribe({
                    next: (o) => {
                        o && r();
                    },
                });
            }).finally(() => {
                t.unsubscribe();
            });
        }
        _injector = p(G);
        _rendererFactory = null;
        get injector() {
            return this._injector;
        }
        bootstrap(t, r) {
            return this.bootstrapImpl(t, r);
        }
        bootstrapImpl(t, r, o = ge.NULL) {
            return this._injector.get(le).run(() => {
                U(P.BootstrapComponentStart);
                let s = t instanceof Ds;
                if (!this._injector.get(Lu).done) {
                    let y = "";
                    throw new m(405, y);
                }
                let c;
                (s
                    ? (c = t)
                    : (c = this._injector.get(yo).resolveComponentFactory(t)),
                    this.componentTypes.push(c.componentType));
                let u = Mh(c) ? void 0 : this._injector.get(_n),
                    l = r || c.selector,
                    d = c.create(o, [], l, u),
                    h = d.location.nativeElement,
                    f = d.injector.get(Pu, null);
                return (
                    f?.registerApplication(h),
                    d.onDestroy(() => {
                        (this.detachView(d.hostView),
                            ao(this.components, d),
                            f?.unregisterApplication(h));
                    }),
                    this._loadComponent(d),
                    U(P.BootstrapComponentEnd, d),
                    d
                );
            });
        }
        tick() {
            (this.zonelessEnabled || (this.dirtyFlags |= 1), this._tick());
        }
        _tick() {
            (U(P.ChangeDetectionStart),
                this.tracingSnapshot !== null
                    ? this.tracingSnapshot.run(
                          ms.CHANGE_DETECTION,
                          this.tickImpl,
                      )
                    : this.tickImpl());
        }
        tickImpl = () => {
            if (this._runningTick)
                throw (U(P.ChangeDetectionEnd), new m(101, !1));
            let t = S(null);
            try {
                ((this._runningTick = !0), this.synchronize());
            } finally {
                ((this._runningTick = !1),
                    this.tracingSnapshot?.dispose(),
                    (this.tracingSnapshot = null),
                    S(t),
                    this.afterTick.next(),
                    U(P.ChangeDetectionEnd));
            }
        };
        synchronize() {
            this._rendererFactory === null &&
                !this._injector.destroyed &&
                (this._rendererFactory = this._injector.get(Tn, null, {
                    optional: !0,
                }));
            let t = 0;
            for (; this.dirtyFlags !== 0 && t++ < AE; ) {
                U(P.ChangeDetectionSyncStart);
                try {
                    this.synchronizeOnce();
                } finally {
                    U(P.ChangeDetectionSyncEnd);
                }
            }
        }
        synchronizeOnce() {
            this.dirtyFlags & 16 &&
                ((this.dirtyFlags &= -17), this.rootEffectScheduler.flush());
            let t = !1;
            if (this.dirtyFlags & 7) {
                let r = !!(this.dirtyFlags & 1);
                ((this.dirtyFlags &= -8), (this.dirtyFlags |= 8));
                for (let { _lView: o } of this.allViews) {
                    if (!r && !to(o)) continue;
                    let i = r && !this.zonelessEnabled ? 0 : 1;
                    (ih(o, i), (t = !0));
                }
                if (
                    ((this.dirtyFlags &= -5),
                    this.syncDirtyFlagsWithViews(),
                    this.dirtyFlags & 23)
                )
                    return;
            }
            (t ||
                (this._rendererFactory?.begin?.(),
                this._rendererFactory?.end?.()),
                this.dirtyFlags & 8 &&
                    ((this.dirtyFlags &= -9),
                    this.afterRenderManager.execute()),
                this.syncDirtyFlagsWithViews());
        }
        syncDirtyFlagsWithViews() {
            if (this.allViews.some(({ _lView: t }) => to(t))) {
                this.dirtyFlags |= 2;
                return;
            } else this.dirtyFlags &= -8;
        }
        attachView(t) {
            let r = t;
            (this._views.push(r), r.attachToAppRef(this));
        }
        detachView(t) {
            let r = t;
            (ao(this._views, r), r.detachFromAppRef());
        }
        _loadComponent(t) {
            this.attachView(t.hostView);
            try {
                this.tick();
            } catch (o) {
                this.internalErrorHandler(o);
            }
            (this.components.push(t),
                this._injector.get(Eo, []).forEach((o) => o(t)));
        }
        ngOnDestroy() {
            if (!this._destroyed)
                try {
                    (this._destroyListeners.forEach((t) => t()),
                        this._views.slice().forEach((t) => t.destroy()));
                } finally {
                    ((this._destroyed = !0),
                        (this._views = []),
                        (this._destroyListeners = []));
                }
        }
        onDestroy(t) {
            return (
                this._destroyListeners.push(t),
                () => ao(this._destroyListeners, t)
            );
        }
        destroy() {
            if (this._destroyed) throw new m(406, !1);
            let t = this._injector;
            t.destroy && !t.destroyed && t.destroy();
        }
        get viewCount() {
            return this._views.length;
        }
        static ɵfac = function (r) {
            return new (r || e)();
        };
        static ɵprov = v({ token: e, factory: e.ɵfac, providedIn: "root" });
    }
    return e;
})();
function ao(e, n) {
    let t = e.indexOf(n);
    t > -1 && e.splice(t, 1);
}
function Is(e, n, t, r) {
    let o = $(),
        i = Ui();
    if (Sn(o, i, n)) {
        let s = Dt(),
            a = kc();
        vD(a, o, e, n, t, r);
    }
    return Is;
}
function Xe(e, n, t) {
    let r = $(),
        o = Ui();
    if (Sn(r, o, n)) {
        let i = Dt(),
            s = kc();
        dD(s, r, e, n, r[K], t);
    }
    return Xe;
}
function rp(e, n, t, r, o) {
    Su(n, e, t, o ? "class" : "style", r);
}
function b(e, n, t, r) {
    let o = $(),
        i = o[R],
        s = e + ve,
        a = i.firstCreatePass ? gh(s, o, 2, n, Xp, Sc(), t, r) : i.data[s];
    if (yt(a)) {
        let c = o[We].tracingService;
        if (c && c.componentCreate) {
            let u = i.data[a.directiveStart + a.componentOffset];
            return c.componentCreate(Ih(u), () => (op(e, n, o, a, r), b));
        }
    }
    return (op(e, n, o, a, r), b);
}
function op(e, n, t, r, o) {
    if ((ED(r, t, e, n, xE), Xr(r))) {
        let i = t[R];
        (Tu(i, t, r), xp(i, r, t));
    }
    o != null && Jp(t, r);
}
function N() {
    let e = Dt(),
        n = Se(),
        t = CD(n);
    return (
        e.firstCreatePass && mh(e, t),
        ff(t) && pf(),
        lf(),
        t.classesWithoutHost != null &&
            Zv(t) &&
            rp(e, t, $(), t.classesWithoutHost, !0),
        t.stylesWithoutHost != null &&
            Yv(t) &&
            rp(e, t, $(), t.stylesWithoutHost, !1),
        N
    );
}
function ye(e, n, t, r) {
    return (b(e, n, t, r), N(), ye);
}
var xE = (e, n, t, r, o) => (zi(!0), kp(n[K], r, Sf()));
var Co = "en-US";
var OE = Co;
function _h(e) {
    typeof e == "string" && (OE = e.toLowerCase().replace(/_/g, "-"));
}
function ws(e, n, t) {
    let r = $(),
        o = Dt(),
        i = Se();
    return (PE(o, r, r[K], i, e, n, t), ws);
}
function PE(e, n, t, r, o, i, s) {
    let a = !0,
        c = null;
    if (
        ((r.type & 3 || s) &&
            ((c ??= qc(r, n, i)), oE(r, e, n, s, t, o, i, c) && (a = !1)),
        a)
    ) {
        let u = r.outputs?.[o],
            l = r.hostDirectiveOutputs?.[o];
        if (l && l.length)
            for (let d = 0; d < l.length; d += 2) {
                let h = l[d],
                    f = l[d + 1];
                ((c ??= qc(r, n, i)), Xf(r, n, h, f, o, c));
            }
        if (u && u.length)
            for (let d of u) ((c ??= qc(r, n, i)), Xf(r, n, d, o, o, c));
    }
}
function bs(e = 1) {
    return Tf(e);
}
function x(e, n = "") {
    let t = $(),
        r = Dt(),
        o = e + ve,
        i = r.firstCreatePass ? Nu(r, o, 1, n, null) : r.data[o],
        s = kE(r, t, i, n);
    ((t[o] = s), $i() && wu(r, t, s, i), or(i, !1));
}
var kE = (e, n, t, r) => (zi(!0), Ey(n[K], r));
function FE(e, n, t, r = "") {
    return Sn(e, Ui(), t) ? n + Jn(t) + r : Ct;
}
function LE(e, n, t, r, o, i = "") {
    let s = mf(),
        a = Eh(e, s, t, o);
    return (yf(2), a ? n + Jn(t) + r + Jn(o) + i : Ct);
}
function ct(e) {
    return (ju("", e), ct);
}
function ju(e, n, t) {
    let r = $(),
        o = FE(r, e, n, t);
    return (o !== Ct && Nh(r, no(), o), ju);
}
function Ts(e, n, t, r, o) {
    let i = $(),
        s = LE(i, e, n, t, r, o);
    return (s !== Ct && Nh(i, no(), s), Ts);
}
function Nh(e, n, t) {
    let r = ef(n, e);
    Cy(e[K], r, t);
}
function Uu(e, n, t) {
    return jE($(), Ac(), e, n, t);
}
function Rh(e, n) {
    let t = e[n];
    return t === Ct ? void 0 : t;
}
function jE(e, n, t, r, o, i) {
    let s = n + t;
    return Sn(e, s, o) ? Dh(e, s + 1, i ? r.call(i, o) : r(o)) : Rh(e, s + 1);
}
function UE(e, n, t, r, o, i, s, a) {
    let c = n + t;
    return rE(e, c, o, i, s)
        ? Dh(e, c + 3, a ? r.call(a, o, i, s) : r(o, i, s))
        : Rh(e, c + 3);
}
function Bu(e, n) {
    let t = Dt(),
        r,
        o = e + ve;
    t.firstCreatePass
        ? ((r = BE(n, t.pipeRegistry)),
          (t.data[o] = r),
          r.onDestroy && (t.destroyHooks ??= []).push(o, r.onDestroy))
        : (r = t.data[o]);
    let i = r.factory || (r.factory = _t(r.type, !0)),
        s,
        a = pe(Z);
    try {
        let c = es(!1),
            u = i();
        return (es(c), Ic(t, $(), o, u), u);
    } finally {
        pe(a);
    }
}
function BE(e, n) {
    if (n)
        for (let t = n.length - 1; t >= 0; t--) {
            let r = n[t];
            if (e === r.name) return r;
        }
}
function Vu(e, n, t, r, o) {
    let i = e + ve,
        s = $(),
        a = tf(s, i);
    return VE(s, i)
        ? UE(s, Ac(), n, a.transform, t, r, o, a)
        : a.transform(t, r, o);
}
function VE(e, n) {
    return e[R].data[n].pure;
}
var cs = class {
        ngModuleFactory;
        componentFactories;
        constructor(n, t) {
            ((this.ngModuleFactory = n), (this.componentFactories = t));
        }
    },
    Hu = (() => {
        class e {
            compileModuleSync(t) {
                return new as(t);
            }
            compileModuleAsync(t) {
                return Promise.resolve(this.compileModuleSync(t));
            }
            compileModuleAndAllComponentsSync(t) {
                let r = this.compileModuleSync(t),
                    o = uc(t),
                    i = Bp(o.declarations).reduce((s, a) => {
                        let c = mt(a);
                        return (c && s.push(new Mn(c)), s);
                    }, []);
                return new cs(r, i);
            }
            compileModuleAndAllComponentsAsync(t) {
                return Promise.resolve(
                    this.compileModuleAndAllComponentsSync(t),
                );
            }
            clearCache() {}
            clearCacheFor(t) {}
            getModuleId(t) {}
            static ɵfac = function (r) {
                return new (r || e)();
            };
            static ɵprov = v({ token: e, factory: e.ɵfac, providedIn: "root" });
        }
        return e;
    })();
var Ah = (() => {
    class e {
        applicationErrorHandler = p(Me);
        appRef = p(hr);
        taskService = p(Et);
        ngZone = p(le);
        zonelessEnabled = p(ro);
        tracing = p(at, { optional: !0 });
        zoneIsDefined = typeof Zone < "u" && !!Zone.root.run;
        schedulerTickApplyArgs = [{ data: { __scheduler_tick__: !0 } }];
        subscriptions = new ne();
        angularZoneId = this.zoneIsDefined ? this.ngZone._inner?.get(Hr) : null;
        scheduleInRootZone =
            !this.zonelessEnabled &&
            this.zoneIsDefined &&
            (p(Vc, { optional: !0 }) ?? !1);
        cancelScheduledCallback = null;
        useMicrotaskScheduler = !1;
        runningTick = !1;
        pendingRenderTaskId = null;
        constructor() {
            (this.subscriptions.add(
                this.appRef.afterTick.subscribe(() => {
                    let t = this.taskService.add();
                    if (
                        !this.runningTick &&
                        (this.cleanup(),
                        !this.zonelessEnabled ||
                            this.appRef.includeAllTestViews)
                    ) {
                        this.taskService.remove(t);
                        return;
                    }
                    (this.switchToMicrotaskScheduler(),
                        this.taskService.remove(t));
                }),
            ),
                this.subscriptions.add(
                    this.ngZone.onUnstable.subscribe(() => {
                        this.runningTick || this.cleanup();
                    }),
                ));
        }
        switchToMicrotaskScheduler() {
            this.ngZone.runOutsideAngular(() => {
                let t = this.taskService.add();
                ((this.useMicrotaskScheduler = !0),
                    queueMicrotask(() => {
                        ((this.useMicrotaskScheduler = !1),
                            this.taskService.remove(t));
                    }));
            });
        }
        notify(t) {
            if (!this.zonelessEnabled && t === 5) return;
            switch (t) {
                case 0: {
                    this.appRef.dirtyFlags |= 2;
                    break;
                }
                case 3:
                case 2:
                case 4:
                case 5:
                case 1: {
                    this.appRef.dirtyFlags |= 4;
                    break;
                }
                case 6: {
                    this.appRef.dirtyFlags |= 2;
                    break;
                }
                case 12: {
                    this.appRef.dirtyFlags |= 16;
                    break;
                }
                case 13: {
                    this.appRef.dirtyFlags |= 2;
                    break;
                }
                case 11:
                    break;
                default:
                    this.appRef.dirtyFlags |= 8;
            }
            if (
                ((this.appRef.tracingSnapshot =
                    this.tracing?.snapshot(this.appRef.tracingSnapshot) ??
                    null),
                !this.shouldScheduleTick())
            )
                return;
            let r = this.useMicrotaskScheduler ? Rf : Lc;
            ((this.pendingRenderTaskId = this.taskService.add()),
                this.scheduleInRootZone
                    ? (this.cancelScheduledCallback = Zone.root.run(() =>
                          r(() => this.tick()),
                      ))
                    : (this.cancelScheduledCallback =
                          this.ngZone.runOutsideAngular(() =>
                              r(() => this.tick()),
                          )));
        }
        shouldScheduleTick() {
            return !(
                this.appRef.destroyed ||
                this.pendingRenderTaskId !== null ||
                this.runningTick ||
                this.appRef._runningTick ||
                (!this.zonelessEnabled &&
                    this.zoneIsDefined &&
                    Zone.current.get(Hr + this.angularZoneId))
            );
        }
        tick() {
            if (this.runningTick || this.appRef.destroyed) return;
            if (this.appRef.dirtyFlags === 0) {
                this.cleanup();
                return;
            }
            !this.zonelessEnabled &&
                this.appRef.dirtyFlags & 7 &&
                (this.appRef.dirtyFlags |= 1);
            let t = this.taskService.add();
            try {
                this.ngZone.run(
                    () => {
                        ((this.runningTick = !0), this.appRef._tick());
                    },
                    void 0,
                    this.schedulerTickApplyArgs,
                );
            } catch (r) {
                this.applicationErrorHandler(r);
            } finally {
                (this.taskService.remove(t), this.cleanup());
            }
        }
        ngOnDestroy() {
            (this.subscriptions.unsubscribe(), this.cleanup());
        }
        cleanup() {
            if (
                ((this.runningTick = !1),
                this.cancelScheduledCallback?.(),
                (this.cancelScheduledCallback = null),
                this.pendingRenderTaskId !== null)
            ) {
                let t = this.pendingRenderTaskId;
                ((this.pendingRenderTaskId = null), this.taskService.remove(t));
            }
        }
        static ɵfac = function (r) {
            return new (r || e)();
        };
        static ɵprov = v({ token: e, factory: e.ɵfac, providedIn: "root" });
    }
    return e;
})();
function xh() {
    return [
        { provide: Rt, useExisting: Ah },
        { provide: le, useClass: $r },
        { provide: ro, useValue: !0 },
    ];
}
function HE() {
    return (typeof $localize < "u" && $localize.locale) || Co;
}
var Ss = new D("", {
    factory: () => p(Ss, { optional: !0, skipSelf: !0 }) || HE(),
});
function Y(e) {
    return Fd(e);
}
function Vt(e, n) {
    return ei(e, n?.equal);
}
var zE = (e) => e;
function $u(e, n) {
    if (typeof e == "function") {
        let t = Ba(e, zE, n?.equal);
        return Oh(t, n?.debugName);
    } else {
        let t = Ba(e.source, e.computation, e.equal);
        return Oh(t, e.debugName);
    }
}
function Oh(e, n) {
    let t = e[te],
        r = e;
    return (
        (r.set = (o) => Pd(t, o)),
        (r.update = (o) => kd(t, o)),
        (r.asReadonly = Gi.bind(e)),
        r
    );
}
var jh = Symbol("InputSignalNode#UNSET"),
    uC = B(g({}, ni), {
        transformFn: void 0,
        applyValueToInputSignal(e, n) {
            sn(e, n);
        },
    });
function Uh(e, n) {
    let t = Object.create(uC);
    ((t.value = e), (t.transformFn = n?.transform));
    function r() {
        if ((nn(t), t.value === jh)) {
            let o = null;
            throw new m(-950, o);
        }
        return t.value;
    }
    return ((r[te] = t), r);
}
var _s = class {
    attributeName;
    constructor(n) {
        this.attributeName = n;
    }
    __NG_ELEMENT_ID__ = () => ho(this.attributeName);
    toString() {
        return `HostAttributeToken ${this.attributeName}`;
    }
};
function Ph(e, n) {
    return Uh(e, n);
}
function lC(e) {
    return Uh(jh, e);
}
var Bh = ((Ph.required = lC), Ph);
var zu = new D(""),
    dC = new D("");
function Io(e) {
    return !e.moduleRef;
}
function fC(e) {
    let n = Io(e) ? e.r3Injector : e.moduleRef.injector,
        t = n.get(le);
    return t.run(() => {
        Io(e)
            ? e.r3Injector.resolveInjectorInitializers()
            : e.moduleRef.resolveInjectorInitializers();
        let r = n.get(Me),
            o;
        if (
            (t.runOutsideAngular(() => {
                o = t.onError.subscribe({ next: r });
            }),
            Io(e))
        ) {
            let i = () => n.destroy(),
                s = e.platformInjector.get(zu);
            (s.add(i),
                n.onDestroy(() => {
                    (o.unsubscribe(), s.delete(i));
                }));
        } else {
            let i = () => e.moduleRef.destroy(),
                s = e.platformInjector.get(zu);
            (s.add(i),
                e.moduleRef.onDestroy(() => {
                    (ao(e.allPlatformModules, e.moduleRef),
                        o.unsubscribe(),
                        s.delete(i));
                }));
        }
        return hC(r, t, () => {
            let i = n.get(Et),
                s = i.add(),
                a = n.get(Lu);
            return (
                a.runInitializers(),
                a.donePromise
                    .then(() => {
                        let c = n.get(Ss, Co);
                        if ((_h(c || Co), !n.get(dC, !0)))
                            return Io(e)
                                ? n.get(hr)
                                : (e.allPlatformModules.push(e.moduleRef),
                                  e.moduleRef);
                        if (Io(e)) {
                            let l = n.get(hr);
                            return (
                                e.rootComponent !== void 0 &&
                                    l.bootstrap(e.rootComponent),
                                l
                            );
                        } else
                            return (
                                pC?.(e.moduleRef, e.allPlatformModules),
                                e.moduleRef
                            );
                    })
                    .finally(() => {
                        i.remove(s);
                    })
            );
        });
    });
}
var pC;
function hC(e, n, t) {
    try {
        let r = t();
        return pr(r)
            ? r.catch((o) => {
                  throw (n.runOutsideAngular(() => e(o)), o);
              })
            : r;
    } catch (r) {
        throw (n.runOutsideAngular(() => e(r)), r);
    }
}
var Ms = null;
function gC(e = [], n) {
    return ge.create({
        name: n,
        providers: [
            { provide: Wr, useValue: "platform" },
            { provide: zu, useValue: new Set([() => (Ms = null)]) },
            ...e,
        ],
    });
}
function mC(e = []) {
    if (Ms) return Ms;
    let n = gC(e);
    return ((Ms = n), Sh(), vC(n), n);
}
function vC(e) {
    let n = e.get(ds, null);
    Q(e, () => {
        n?.forEach((t) => t());
    });
}
var yC = 1e4;
var kF = yC - 1e3;
var wo = (() => {
    class e {
        static __NG_ELEMENT_ID__ = DC;
    }
    return e;
})();
function DC(e) {
    return EC(Se(), $(), (e & 16) === 16);
}
function EC(e, n, t) {
    if (yt(e) && !t) {
        let r = Fe(e.index, n);
        return new Ft(r, r);
    } else if (e.type & 175) {
        let r = n[Pe];
        return new Ft(r, n);
    }
    return null;
}
var Gu = class {
        supports(n) {
            return Au(n);
        }
        create(n) {
            return new qu(n);
        }
    },
    CC = (e, n) => n,
    qu = class {
        length = 0;
        collection;
        _linkedRecords = null;
        _unlinkedRecords = null;
        _previousItHead = null;
        _itHead = null;
        _itTail = null;
        _additionsHead = null;
        _additionsTail = null;
        _movesHead = null;
        _movesTail = null;
        _removalsHead = null;
        _removalsTail = null;
        _identityChangesHead = null;
        _identityChangesTail = null;
        _trackByFn;
        constructor(n) {
            this._trackByFn = n || CC;
        }
        forEachItem(n) {
            let t;
            for (t = this._itHead; t !== null; t = t._next) n(t);
        }
        forEachOperation(n) {
            let t = this._itHead,
                r = this._removalsHead,
                o = 0,
                i = null;
            for (; t || r; ) {
                let s = !r || (t && t.currentIndex < kh(r, o, i)) ? t : r,
                    a = kh(s, o, i),
                    c = s.currentIndex;
                if (s === r) (o--, (r = r._nextRemoved));
                else if (((t = t._next), s.previousIndex == null)) o++;
                else {
                    i || (i = []);
                    let u = a - o,
                        l = c - o;
                    if (u != l) {
                        for (let h = 0; h < u; h++) {
                            let f = h < i.length ? i[h] : (i[h] = 0),
                                y = f + h;
                            l <= y && y < u && (i[h] = f + 1);
                        }
                        let d = s.previousIndex;
                        i[d] = l - u;
                    }
                }
                a !== c && n(s, a, c);
            }
        }
        forEachPreviousItem(n) {
            let t;
            for (t = this._previousItHead; t !== null; t = t._nextPrevious)
                n(t);
        }
        forEachAddedItem(n) {
            let t;
            for (t = this._additionsHead; t !== null; t = t._nextAdded) n(t);
        }
        forEachMovedItem(n) {
            let t;
            for (t = this._movesHead; t !== null; t = t._nextMoved) n(t);
        }
        forEachRemovedItem(n) {
            let t;
            for (t = this._removalsHead; t !== null; t = t._nextRemoved) n(t);
        }
        forEachIdentityChange(n) {
            let t;
            for (
                t = this._identityChangesHead;
                t !== null;
                t = t._nextIdentityChange
            )
                n(t);
        }
        diff(n) {
            if ((n == null && (n = []), !Au(n))) throw new m(900, !1);
            return this.check(n) ? this : null;
        }
        onDestroy() {}
        check(n) {
            this._reset();
            let t = this._itHead,
                r = !1,
                o,
                i,
                s;
            if (Array.isArray(n)) {
                this.length = n.length;
                for (let a = 0; a < this.length; a++)
                    ((i = n[a]),
                        (s = this._trackByFn(a, i)),
                        t === null || !Object.is(t.trackById, s)
                            ? ((t = this._mismatch(t, i, s, a)), (r = !0))
                            : (r && (t = this._verifyReinsertion(t, i, s, a)),
                              Object.is(t.item, i) ||
                                  this._addIdentityChange(t, i)),
                        (t = t._next));
            } else
                ((o = 0),
                    vh(n, (a) => {
                        ((s = this._trackByFn(o, a)),
                            t === null || !Object.is(t.trackById, s)
                                ? ((t = this._mismatch(t, a, s, o)), (r = !0))
                                : (r &&
                                      (t = this._verifyReinsertion(t, a, s, o)),
                                  Object.is(t.item, a) ||
                                      this._addIdentityChange(t, a)),
                            (t = t._next),
                            o++);
                    }),
                    (this.length = o));
            return (this._truncate(t), (this.collection = n), this.isDirty);
        }
        get isDirty() {
            return (
                this._additionsHead !== null ||
                this._movesHead !== null ||
                this._removalsHead !== null ||
                this._identityChangesHead !== null
            );
        }
        _reset() {
            if (this.isDirty) {
                let n;
                for (
                    n = this._previousItHead = this._itHead;
                    n !== null;
                    n = n._next
                )
                    n._nextPrevious = n._next;
                for (n = this._additionsHead; n !== null; n = n._nextAdded)
                    n.previousIndex = n.currentIndex;
                for (
                    this._additionsHead = this._additionsTail = null,
                        n = this._movesHead;
                    n !== null;
                    n = n._nextMoved
                )
                    n.previousIndex = n.currentIndex;
                ((this._movesHead = this._movesTail = null),
                    (this._removalsHead = this._removalsTail = null),
                    (this._identityChangesHead = this._identityChangesTail =
                        null));
            }
        }
        _mismatch(n, t, r, o) {
            let i;
            return (
                n === null
                    ? (i = this._itTail)
                    : ((i = n._prev), this._remove(n)),
                (n =
                    this._unlinkedRecords === null
                        ? null
                        : this._unlinkedRecords.get(r, null)),
                n !== null
                    ? (Object.is(n.item, t) || this._addIdentityChange(n, t),
                      this._reinsertAfter(n, i, o))
                    : ((n =
                          this._linkedRecords === null
                              ? null
                              : this._linkedRecords.get(r, o)),
                      n !== null
                          ? (Object.is(n.item, t) ||
                                this._addIdentityChange(n, t),
                            this._moveAfter(n, i, o))
                          : (n = this._addAfter(new Wu(t, r), i, o))),
                n
            );
        }
        _verifyReinsertion(n, t, r, o) {
            let i =
                this._unlinkedRecords === null
                    ? null
                    : this._unlinkedRecords.get(r, null);
            return (
                i !== null
                    ? (n = this._reinsertAfter(i, n._prev, o))
                    : n.currentIndex != o &&
                      ((n.currentIndex = o), this._addToMoves(n, o)),
                n
            );
        }
        _truncate(n) {
            for (; n !== null; ) {
                let t = n._next;
                (this._addToRemovals(this._unlink(n)), (n = t));
            }
            (this._unlinkedRecords !== null && this._unlinkedRecords.clear(),
                this._additionsTail !== null &&
                    (this._additionsTail._nextAdded = null),
                this._movesTail !== null && (this._movesTail._nextMoved = null),
                this._itTail !== null && (this._itTail._next = null),
                this._removalsTail !== null &&
                    (this._removalsTail._nextRemoved = null),
                this._identityChangesTail !== null &&
                    (this._identityChangesTail._nextIdentityChange = null));
        }
        _reinsertAfter(n, t, r) {
            this._unlinkedRecords !== null && this._unlinkedRecords.remove(n);
            let o = n._prevRemoved,
                i = n._nextRemoved;
            return (
                o === null ? (this._removalsHead = i) : (o._nextRemoved = i),
                i === null ? (this._removalsTail = o) : (i._prevRemoved = o),
                this._insertAfter(n, t, r),
                this._addToMoves(n, r),
                n
            );
        }
        _moveAfter(n, t, r) {
            return (
                this._unlink(n),
                this._insertAfter(n, t, r),
                this._addToMoves(n, r),
                n
            );
        }
        _addAfter(n, t, r) {
            return (
                this._insertAfter(n, t, r),
                this._additionsTail === null
                    ? (this._additionsTail = this._additionsHead = n)
                    : (this._additionsTail = this._additionsTail._nextAdded =
                          n),
                n
            );
        }
        _insertAfter(n, t, r) {
            let o = t === null ? this._itHead : t._next;
            return (
                (n._next = o),
                (n._prev = t),
                o === null ? (this._itTail = n) : (o._prev = n),
                t === null ? (this._itHead = n) : (t._next = n),
                this._linkedRecords === null &&
                    (this._linkedRecords = new Ns()),
                this._linkedRecords.put(n),
                (n.currentIndex = r),
                n
            );
        }
        _remove(n) {
            return this._addToRemovals(this._unlink(n));
        }
        _unlink(n) {
            this._linkedRecords !== null && this._linkedRecords.remove(n);
            let t = n._prev,
                r = n._next;
            return (
                t === null ? (this._itHead = r) : (t._next = r),
                r === null ? (this._itTail = t) : (r._prev = t),
                n
            );
        }
        _addToMoves(n, t) {
            return (
                n.previousIndex === t ||
                    (this._movesTail === null
                        ? (this._movesTail = this._movesHead = n)
                        : (this._movesTail = this._movesTail._nextMoved = n)),
                n
            );
        }
        _addToRemovals(n) {
            return (
                this._unlinkedRecords === null &&
                    (this._unlinkedRecords = new Ns()),
                this._unlinkedRecords.put(n),
                (n.currentIndex = null),
                (n._nextRemoved = null),
                this._removalsTail === null
                    ? ((this._removalsTail = this._removalsHead = n),
                      (n._prevRemoved = null))
                    : ((n._prevRemoved = this._removalsTail),
                      (this._removalsTail = this._removalsTail._nextRemoved =
                          n)),
                n
            );
        }
        _addIdentityChange(n, t) {
            return (
                (n.item = t),
                this._identityChangesTail === null
                    ? (this._identityChangesTail = this._identityChangesHead =
                          n)
                    : (this._identityChangesTail =
                          this._identityChangesTail._nextIdentityChange =
                              n),
                n
            );
        }
    },
    Wu = class {
        item;
        trackById;
        currentIndex = null;
        previousIndex = null;
        _nextPrevious = null;
        _prev = null;
        _next = null;
        _prevDup = null;
        _nextDup = null;
        _prevRemoved = null;
        _nextRemoved = null;
        _nextAdded = null;
        _nextMoved = null;
        _nextIdentityChange = null;
        constructor(n, t) {
            ((this.item = n), (this.trackById = t));
        }
    },
    Zu = class {
        _head = null;
        _tail = null;
        add(n) {
            this._head === null
                ? ((this._head = this._tail = n),
                  (n._nextDup = null),
                  (n._prevDup = null))
                : ((this._tail._nextDup = n),
                  (n._prevDup = this._tail),
                  (n._nextDup = null),
                  (this._tail = n));
        }
        get(n, t) {
            let r;
            for (r = this._head; r !== null; r = r._nextDup)
                if (
                    (t === null || t <= r.currentIndex) &&
                    Object.is(r.trackById, n)
                )
                    return r;
            return null;
        }
        remove(n) {
            let t = n._prevDup,
                r = n._nextDup;
            return (
                t === null ? (this._head = r) : (t._nextDup = r),
                r === null ? (this._tail = t) : (r._prevDup = t),
                this._head === null
            );
        }
    },
    Ns = class {
        map = new Map();
        put(n) {
            let t = n.trackById,
                r = this.map.get(t);
            (r || ((r = new Zu()), this.map.set(t, r)), r.add(n));
        }
        get(n, t) {
            let r = n,
                o = this.map.get(r);
            return o ? o.get(n, t) : null;
        }
        remove(n) {
            let t = n.trackById;
            return (this.map.get(t).remove(n) && this.map.delete(t), n);
        }
        get isEmpty() {
            return this.map.size === 0;
        }
        clear() {
            this.map.clear();
        }
    };
function kh(e, n, t) {
    let r = e.previousIndex;
    if (r === null) return r;
    let o = 0;
    return (t && r < t.length && (o = t[r]), r + n + o);
}
function Fh() {
    return new Yu([new Gu()]);
}
var Yu = (() => {
    class e {
        factories;
        static ɵprov = v({ token: e, providedIn: "root", factory: Fh });
        constructor(t) {
            this.factories = t;
        }
        static create(t, r) {
            if (r != null) {
                let o = r.factories.slice();
                t = t.concat(o);
            }
            return new e(t);
        }
        static extend(t) {
            return {
                provide: e,
                useFactory: () => {
                    let r = p(e, { optional: !0, skipSelf: !0 });
                    return e.create(t, r || Fh());
                },
            };
        }
        find(t) {
            let r = this.factories.find((o) => o.supports(t));
            if (r != null) return r;
            throw new m(901, !1);
        }
    }
    return e;
})();
function Vh(e) {
    let {
        rootComponent: n,
        appProviders: t,
        platformProviders: r,
        platformRef: o,
    } = e;
    U(P.BootstrapApplicationStart);
    try {
        let i = o?.injector ?? mC(r),
            s = [xh(), xf, ...(t || [])],
            a = new lo({
                providers: s,
                parent: i,
                debugName: "",
                runEnvironmentInitializers: !1,
            });
        return fC({
            r3Injector: a.injector,
            platformInjector: i,
            rootComponent: n,
        });
    } catch (i) {
        return Promise.reject(i);
    } finally {
        U(P.BootstrapApplicationEnd);
    }
}
function bo(e) {
    return typeof e == "boolean" ? e : e != null && e !== "false";
}
function Hh(e) {
    let n = mt(e);
    if (!n) return null;
    let t = new Mn(n);
    return {
        get selector() {
            return t.selector;
        },
        get type() {
            return t.componentType;
        },
        get inputs() {
            return t.inputs;
        },
        get outputs() {
            return t.outputs;
        },
        get ngContentSelectors() {
            return t.ngContentSelectors;
        },
        get isStandalone() {
            return n.standalone;
        },
        get isSignal() {
            return n.signals;
        },
    };
}
var $h = null;
function It() {
    return $h;
}
function Qu(e) {
    $h ??= e;
}
var To = class {},
    Rn = (() => {
        class e {
            historyGo(t) {
                throw new Error("");
            }
            static ɵfac = function (r) {
                return new (r || e)();
            };
            static ɵprov = v({
                token: e,
                factory: () => p(zh),
                providedIn: "platform",
            });
        }
        return e;
    })(),
    Ku = new D(""),
    zh = (() => {
        class e extends Rn {
            _location;
            _history;
            _doc = p(W);
            constructor() {
                (super(),
                    (this._location = window.location),
                    (this._history = window.history));
            }
            getBaseHrefFromDOM() {
                return It().getBaseHref(this._doc);
            }
            onPopState(t) {
                let r = It().getGlobalEventTarget(this._doc, "window");
                return (
                    r.addEventListener("popstate", t, !1),
                    () => r.removeEventListener("popstate", t)
                );
            }
            onHashChange(t) {
                let r = It().getGlobalEventTarget(this._doc, "window");
                return (
                    r.addEventListener("hashchange", t, !1),
                    () => r.removeEventListener("hashchange", t)
                );
            }
            get href() {
                return this._location.href;
            }
            get protocol() {
                return this._location.protocol;
            }
            get hostname() {
                return this._location.hostname;
            }
            get port() {
                return this._location.port;
            }
            get pathname() {
                return this._location.pathname;
            }
            get search() {
                return this._location.search;
            }
            get hash() {
                return this._location.hash;
            }
            set pathname(t) {
                this._location.pathname = t;
            }
            pushState(t, r, o) {
                this._history.pushState(t, r, o);
            }
            replaceState(t, r, o) {
                this._history.replaceState(t, r, o);
            }
            forward() {
                this._history.forward();
            }
            back() {
                this._history.back();
            }
            historyGo(t = 0) {
                this._history.go(t);
            }
            getState() {
                return this._history.state;
            }
            static ɵfac = function (r) {
                return new (r || e)();
            };
            static ɵprov = v({
                token: e,
                factory: () => new e(),
                providedIn: "platform",
            });
        }
        return e;
    })();
function As(e, n) {
    return e
        ? n
            ? e.endsWith("/")
                ? n.startsWith("/")
                    ? e + n.slice(1)
                    : e + n
                : n.startsWith("/")
                  ? e + n
                  : `${e}/${n}`
            : e
        : n;
}
function Gh(e) {
    let n = e.search(/#|\?|$/);
    return e[n - 1] === "/" ? e.slice(0, n - 1) + e.slice(n) : e;
}
function et(e) {
    return e && e[0] !== "?" ? `?${e}` : e;
}
var tt = (() => {
        class e {
            historyGo(t) {
                throw new Error("");
            }
            static ɵfac = function (r) {
                return new (r || e)();
            };
            static ɵprov = v({
                token: e,
                factory: () => p(Os),
                providedIn: "root",
            });
        }
        return e;
    })(),
    xs = new D(""),
    Os = (() => {
        class e extends tt {
            _platformLocation;
            _baseHref;
            _removeListenerFns = [];
            constructor(t, r) {
                (super(),
                    (this._platformLocation = t),
                    (this._baseHref =
                        r ??
                        this._platformLocation.getBaseHrefFromDOM() ??
                        p(W).location?.origin ??
                        ""));
            }
            ngOnDestroy() {
                for (; this._removeListenerFns.length; )
                    this._removeListenerFns.pop()();
            }
            onPopState(t) {
                this._removeListenerFns.push(
                    this._platformLocation.onPopState(t),
                    this._platformLocation.onHashChange(t),
                );
            }
            getBaseHref() {
                return this._baseHref;
            }
            prepareExternalUrl(t) {
                return As(this._baseHref, t);
            }
            path(t = !1) {
                let r =
                        this._platformLocation.pathname +
                        et(this._platformLocation.search),
                    o = this._platformLocation.hash;
                return o && t ? `${r}${o}` : r;
            }
            pushState(t, r, o, i) {
                let s = this.prepareExternalUrl(o + et(i));
                this._platformLocation.pushState(t, r, s);
            }
            replaceState(t, r, o, i) {
                let s = this.prepareExternalUrl(o + et(i));
                this._platformLocation.replaceState(t, r, s);
            }
            forward() {
                this._platformLocation.forward();
            }
            back() {
                this._platformLocation.back();
            }
            getState() {
                return this._platformLocation.getState();
            }
            historyGo(t = 0) {
                this._platformLocation.historyGo?.(t);
            }
            static ɵfac = function (r) {
                return new (r || e)(E(Rn), E(xs, 8));
            };
            static ɵprov = v({ token: e, factory: e.ɵfac, providedIn: "root" });
        }
        return e;
    })();
var Ht = (() => {
    class e {
        _subject = new re();
        _basePath;
        _locationStrategy;
        _urlChangeListeners = [];
        _urlChangeSubscription = null;
        constructor(t) {
            this._locationStrategy = t;
            let r = this._locationStrategy.getBaseHref();
            ((this._basePath = bC(Gh(qh(r)))),
                this._locationStrategy.onPopState((o) => {
                    this._subject.next({
                        url: this.path(!0),
                        pop: !0,
                        state: o.state,
                        type: o.type,
                    });
                }));
        }
        ngOnDestroy() {
            (this._urlChangeSubscription?.unsubscribe(),
                (this._urlChangeListeners = []));
        }
        path(t = !1) {
            return this.normalize(this._locationStrategy.path(t));
        }
        getState() {
            return this._locationStrategy.getState();
        }
        isCurrentPathEqualTo(t, r = "") {
            return this.path() == this.normalize(t + et(r));
        }
        normalize(t) {
            return e.stripTrailingSlash(wC(this._basePath, qh(t)));
        }
        prepareExternalUrl(t) {
            return (
                t && t[0] !== "/" && (t = "/" + t),
                this._locationStrategy.prepareExternalUrl(t)
            );
        }
        go(t, r = "", o = null) {
            (this._locationStrategy.pushState(o, "", t, r),
                this._notifyUrlChangeListeners(
                    this.prepareExternalUrl(t + et(r)),
                    o,
                ));
        }
        replaceState(t, r = "", o = null) {
            (this._locationStrategy.replaceState(o, "", t, r),
                this._notifyUrlChangeListeners(
                    this.prepareExternalUrl(t + et(r)),
                    o,
                ));
        }
        forward() {
            this._locationStrategy.forward();
        }
        back() {
            this._locationStrategy.back();
        }
        historyGo(t = 0) {
            this._locationStrategy.historyGo?.(t);
        }
        onUrlChange(t) {
            return (
                this._urlChangeListeners.push(t),
                (this._urlChangeSubscription ??= this.subscribe((r) => {
                    this._notifyUrlChangeListeners(r.url, r.state);
                })),
                () => {
                    let r = this._urlChangeListeners.indexOf(t);
                    (this._urlChangeListeners.splice(r, 1),
                        this._urlChangeListeners.length === 0 &&
                            (this._urlChangeSubscription?.unsubscribe(),
                            (this._urlChangeSubscription = null)));
                }
            );
        }
        _notifyUrlChangeListeners(t = "", r) {
            this._urlChangeListeners.forEach((o) => o(t, r));
        }
        subscribe(t, r, o) {
            return this._subject.subscribe({
                next: t,
                error: r ?? void 0,
                complete: o ?? void 0,
            });
        }
        static normalizeQueryParams = et;
        static joinWithSlash = As;
        static stripTrailingSlash = Gh;
        static ɵfac = function (r) {
            return new (r || e)(E(tt));
        };
        static ɵprov = v({ token: e, factory: () => IC(), providedIn: "root" });
    }
    return e;
})();
function IC() {
    return new Ht(E(tt));
}
function wC(e, n) {
    if (!e || !n.startsWith(e)) return n;
    let t = n.substring(e.length);
    return t === "" || ["/", ";", "?", "#"].includes(t[0]) ? t : n;
}
function qh(e) {
    return e.replace(/\/index.html$/, "");
}
function bC(e) {
    if (new RegExp("^(https?:)?//").test(e)) {
        let [, t] = e.split(/\/\/[^\/]+/);
        return t;
    }
    return e;
}
var Ju = (() => {
    class e extends tt {
        _platformLocation;
        _baseHref = "";
        _removeListenerFns = [];
        constructor(t, r) {
            (super(),
                (this._platformLocation = t),
                r != null && (this._baseHref = r));
        }
        ngOnDestroy() {
            for (; this._removeListenerFns.length; )
                this._removeListenerFns.pop()();
        }
        onPopState(t) {
            this._removeListenerFns.push(
                this._platformLocation.onPopState(t),
                this._platformLocation.onHashChange(t),
            );
        }
        getBaseHref() {
            return this._baseHref;
        }
        path(t = !1) {
            let r = this._platformLocation.hash ?? "#";
            return r.length > 0 ? r.substring(1) : r;
        }
        prepareExternalUrl(t) {
            let r = As(this._baseHref, t);
            return r.length > 0 ? "#" + r : r;
        }
        pushState(t, r, o, i) {
            let s =
                this.prepareExternalUrl(o + et(i)) ||
                this._platformLocation.pathname;
            this._platformLocation.pushState(t, r, s);
        }
        replaceState(t, r, o, i) {
            let s =
                this.prepareExternalUrl(o + et(i)) ||
                this._platformLocation.pathname;
            this._platformLocation.replaceState(t, r, s);
        }
        forward() {
            this._platformLocation.forward();
        }
        back() {
            this._platformLocation.back();
        }
        getState() {
            return this._platformLocation.getState();
        }
        historyGo(t = 0) {
            this._platformLocation.historyGo?.(t);
        }
        static ɵfac = function (r) {
            return new (r || e)(E(Rn), E(xs, 8));
        };
        static ɵprov = v({ token: e, factory: e.ɵfac });
    }
    return e;
})();
var Ps = class {
        $implicit;
        ngForOf;
        index;
        count;
        constructor(n, t, r, o) {
            ((this.$implicit = n),
                (this.ngForOf = t),
                (this.index = r),
                (this.count = o));
        }
        get first() {
            return this.index === 0;
        }
        get last() {
            return this.index === this.count - 1;
        }
        get even() {
            return this.index % 2 === 0;
        }
        get odd() {
            return !this.even;
        }
    },
    Fs = (() => {
        class e {
            _viewContainer;
            _template;
            _differs;
            set ngForOf(t) {
                ((this._ngForOf = t), (this._ngForOfDirty = !0));
            }
            set ngForTrackBy(t) {
                this._trackByFn = t;
            }
            get ngForTrackBy() {
                return this._trackByFn;
            }
            _ngForOf = null;
            _ngForOfDirty = !0;
            _differ = null;
            _trackByFn;
            constructor(t, r, o) {
                ((this._viewContainer = t),
                    (this._template = r),
                    (this._differs = o));
            }
            set ngForTemplate(t) {
                t && (this._template = t);
            }
            ngDoCheck() {
                if (this._ngForOfDirty) {
                    this._ngForOfDirty = !1;
                    let t = this._ngForOf;
                    !this._differ &&
                        t &&
                        (this._differ = this._differs
                            .find(t)
                            .create(this.ngForTrackBy));
                }
                if (this._differ) {
                    let t = this._differ.diff(this._ngForOf);
                    t && this._applyChanges(t);
                }
            }
            _applyChanges(t) {
                let r = this._viewContainer;
                t.forEachOperation((o, i, s) => {
                    if (o.previousIndex == null)
                        r.createEmbeddedView(
                            this._template,
                            new Ps(o.item, this._ngForOf, -1, -1),
                            s === null ? void 0 : s,
                        );
                    else if (s == null) r.remove(i === null ? void 0 : i);
                    else if (i !== null) {
                        let a = r.get(i);
                        (r.move(a, s), Wh(a, o));
                    }
                });
                for (let o = 0, i = r.length; o < i; o++) {
                    let a = r.get(o).context;
                    ((a.index = o), (a.count = i), (a.ngForOf = this._ngForOf));
                }
                t.forEachIdentityChange((o) => {
                    let i = r.get(o.currentIndex);
                    Wh(i, o);
                });
            }
            static ngTemplateContextGuard(t, r) {
                return !0;
            }
            static ɵfac = function (r) {
                return new (r || e)(Z(Nn), Z(vo), Z(Yu));
            };
            static ɵdir = Ut({
                type: e,
                selectors: [["", "ngFor", "", "ngForOf", ""]],
                inputs: {
                    ngForOf: "ngForOf",
                    ngForTrackBy: "ngForTrackBy",
                    ngForTemplate: "ngForTemplate",
                },
            });
        }
        return e;
    })();
function Wh(e, n) {
    e.context.$implicit = n.item;
}
var So = (() => {
        class e {
            _viewContainer;
            _context = new ks();
            _thenTemplateRef = null;
            _elseTemplateRef = null;
            _thenViewRef = null;
            _elseViewRef = null;
            constructor(t, r) {
                ((this._viewContainer = t), (this._thenTemplateRef = r));
            }
            set ngIf(t) {
                ((this._context.$implicit = this._context.ngIf = t),
                    this._updateView());
            }
            set ngIfThen(t) {
                (Zh(t, !1),
                    (this._thenTemplateRef = t),
                    (this._thenViewRef = null),
                    this._updateView());
            }
            set ngIfElse(t) {
                (Zh(t, !1),
                    (this._elseTemplateRef = t),
                    (this._elseViewRef = null),
                    this._updateView());
            }
            _updateView() {
                this._context.$implicit
                    ? this._thenViewRef ||
                      (this._viewContainer.clear(),
                      (this._elseViewRef = null),
                      this._thenTemplateRef &&
                          (this._thenViewRef =
                              this._viewContainer.createEmbeddedView(
                                  this._thenTemplateRef,
                                  this._context,
                              )))
                    : this._elseViewRef ||
                      (this._viewContainer.clear(),
                      (this._thenViewRef = null),
                      this._elseTemplateRef &&
                          (this._elseViewRef =
                              this._viewContainer.createEmbeddedView(
                                  this._elseTemplateRef,
                                  this._context,
                              )));
            }
            static ngIfUseIfTypeGuard;
            static ngTemplateGuard_ngIf;
            static ngTemplateContextGuard(t, r) {
                return !0;
            }
            static ɵfac = function (r) {
                return new (r || e)(Z(Nn), Z(vo));
            };
            static ɵdir = Ut({
                type: e,
                selectors: [["", "ngIf", ""]],
                inputs: {
                    ngIf: "ngIf",
                    ngIfThen: "ngIfThen",
                    ngIfElse: "ngIfElse",
                },
            });
        }
        return e;
    })(),
    ks = class {
        $implicit = null;
        ngIf = null;
    };
function Zh(e, n) {
    if (e && !e.createEmbeddedView) throw new m(2020, !1);
}
function TC(e, n) {
    return new m(2100, !1);
}
var Xu = (() => {
    class e {
        transform(t, r, o) {
            if (t == null) return null;
            if (!(typeof t == "string" || Array.isArray(t))) throw TC(e, t);
            return t.slice(r, o);
        }
        static ɵfac = function (r) {
            return new (r || e)();
        };
        static ɵpipe = xu({ name: "slice", type: e, pure: !1 });
    }
    return e;
})();
var $t = (() => {
    class e {
        static ɵfac = function (r) {
            return new (r || e)();
        };
        static ɵmod = jt({ type: e });
        static ɵinj = gt({});
    }
    return e;
})();
function Ls(e, n) {
    n = encodeURIComponent(n);
    for (let t of e.split(";")) {
        let r = t.indexOf("="),
            [o, i] = r == -1 ? [t, ""] : [t.slice(0, r), t.slice(r + 1)];
        if (o.trim() === n) return decodeURIComponent(i);
    }
    return null;
}
var An = class {};
var Yh = "browser";
var tl = (() => {
        class e {
            static ɵprov = v({
                token: e,
                providedIn: "root",
                factory: () => new el(p(W), window),
            });
        }
        return e;
    })(),
    el = class {
        document;
        window;
        offset = () => [0, 0];
        constructor(n, t) {
            ((this.document = n), (this.window = t));
        }
        setOffset(n) {
            Array.isArray(n) ? (this.offset = () => n) : (this.offset = n);
        }
        getScrollPosition() {
            return [this.window.scrollX, this.window.scrollY];
        }
        scrollToPosition(n, t) {
            this.window.scrollTo(B(g({}, t), { left: n[0], top: n[1] }));
        }
        scrollToAnchor(n, t) {
            let r = SC(this.document, n);
            r && (this.scrollToElement(r, t), r.focus());
        }
        setHistoryScrollRestoration(n) {
            try {
                this.window.history.scrollRestoration = n;
            } catch {
                console.warn(it(2400, !1));
            }
        }
        scrollToElement(n, t) {
            let r = n.getBoundingClientRect(),
                o = r.left + this.window.pageXOffset,
                i = r.top + this.window.pageYOffset,
                s = this.offset();
            this.window.scrollTo(
                B(g({}, t), { left: o - s[0], top: i - s[1] }),
            );
        }
    };
function SC(e, n) {
    let t = e.getElementById(n) || e.getElementsByName(n)[0];
    if (t) return t;
    if (
        typeof e.createTreeWalker == "function" &&
        e.body &&
        typeof e.body.attachShadow == "function"
    ) {
        let r = e.createTreeWalker(e.body, NodeFilter.SHOW_ELEMENT),
            o = r.currentNode;
        for (; o; ) {
            let i = o.shadowRoot;
            if (i) {
                let s = i.getElementById(n) || i.querySelector(`[name="${n}"]`);
                if (s) return s;
            }
            o = r.nextNode();
        }
    }
    return null;
}
var Mo = class {
        _doc;
        constructor(n) {
            this._doc = n;
        }
        manager;
    },
    js = (() => {
        class e extends Mo {
            constructor(t) {
                super(t);
            }
            supports(t) {
                return !0;
            }
            addEventListener(t, r, o, i) {
                return (
                    t.addEventListener(r, o, i),
                    () => this.removeEventListener(t, r, o, i)
                );
            }
            removeEventListener(t, r, o, i) {
                return t.removeEventListener(r, o, i);
            }
            static ɵfac = function (r) {
                return new (r || e)(E(W));
            };
            static ɵprov = v({ token: e, factory: e.ɵfac });
        }
        return e;
    })(),
    Vs = new D(""),
    sl = (() => {
        class e {
            _zone;
            _plugins;
            _eventNameToPlugin = new Map();
            constructor(t, r) {
                ((this._zone = r),
                    t.forEach((s) => {
                        s.manager = this;
                    }));
                let o = t.filter((s) => !(s instanceof js));
                this._plugins = o.slice().reverse();
                let i = t.find((s) => s instanceof js);
                i && this._plugins.push(i);
            }
            addEventListener(t, r, o, i) {
                return this._findPluginFor(r).addEventListener(t, r, o, i);
            }
            getZone() {
                return this._zone;
            }
            _findPluginFor(t) {
                let r = this._eventNameToPlugin.get(t);
                if (r) return r;
                if (((r = this._plugins.find((i) => i.supports(t))), !r))
                    throw new m(5101, !1);
                return (this._eventNameToPlugin.set(t, r), r);
            }
            static ɵfac = function (r) {
                return new (r || e)(E(Vs), E(le));
            };
            static ɵprov = v({ token: e, factory: e.ɵfac });
        }
        return e;
    })(),
    rl = "ng-app-id";
function Qh(e) {
    for (let n of e) n.remove();
}
function Kh(e, n) {
    let t = n.createElement("style");
    return ((t.textContent = e), t);
}
function MC(e, n, t, r) {
    let o = e.head?.querySelectorAll(`style[${rl}="${n}"],link[${rl}="${n}"]`);
    if (o)
        for (let i of o)
            (i.removeAttribute(rl),
                i instanceof HTMLLinkElement
                    ? r.set(i.href.slice(i.href.lastIndexOf("/") + 1), {
                          usage: 0,
                          elements: [i],
                      })
                    : i.textContent &&
                      t.set(i.textContent, { usage: 0, elements: [i] }));
}
function il(e, n) {
    let t = n.createElement("link");
    return (t.setAttribute("rel", "stylesheet"), t.setAttribute("href", e), t);
}
var al = (() => {
        class e {
            doc;
            appId;
            nonce;
            inline = new Map();
            external = new Map();
            hosts = new Set();
            constructor(t, r, o, i = {}) {
                ((this.doc = t),
                    (this.appId = r),
                    (this.nonce = o),
                    MC(t, r, this.inline, this.external),
                    this.hosts.add(t.head));
            }
            addStyles(t, r) {
                for (let o of t) this.addUsage(o, this.inline, Kh);
                r?.forEach((o) => this.addUsage(o, this.external, il));
            }
            removeStyles(t, r) {
                for (let o of t) this.removeUsage(o, this.inline);
                r?.forEach((o) => this.removeUsage(o, this.external));
            }
            addUsage(t, r, o) {
                let i = r.get(t);
                i
                    ? i.usage++
                    : r.set(t, {
                          usage: 1,
                          elements: [...this.hosts].map((s) =>
                              this.addElement(s, o(t, this.doc)),
                          ),
                      });
            }
            removeUsage(t, r) {
                let o = r.get(t);
                o && (o.usage--, o.usage <= 0 && (Qh(o.elements), r.delete(t)));
            }
            ngOnDestroy() {
                for (let [, { elements: t }] of [
                    ...this.inline,
                    ...this.external,
                ])
                    Qh(t);
                this.hosts.clear();
            }
            addHost(t) {
                this.hosts.add(t);
                for (let [r, { elements: o }] of this.inline)
                    o.push(this.addElement(t, Kh(r, this.doc)));
                for (let [r, { elements: o }] of this.external)
                    o.push(this.addElement(t, il(r, this.doc)));
            }
            removeHost(t) {
                this.hosts.delete(t);
            }
            addElement(t, r) {
                return (
                    this.nonce && r.setAttribute("nonce", this.nonce),
                    t.appendChild(r)
                );
            }
            static ɵfac = function (r) {
                return new (r || e)(E(W), E(ls), E(mo, 8), E(go));
            };
            static ɵprov = v({ token: e, factory: e.ɵfac });
        }
        return e;
    })(),
    ol = {
        svg: "http://www.w3.org/2000/svg",
        xhtml: "http://www.w3.org/1999/xhtml",
        xlink: "http://www.w3.org/1999/xlink",
        xml: "http://www.w3.org/XML/1998/namespace",
        xmlns: "http://www.w3.org/2000/xmlns/",
        math: "http://www.w3.org/1998/Math/MathML",
    },
    cl = /%COMP%/g;
var Xh = "%COMP%",
    _C = `_nghost-${Xh}`,
    NC = `_ngcontent-${Xh}`,
    RC = !0,
    AC = new D("", { factory: () => RC });
function xC(e) {
    return NC.replace(cl, e);
}
function OC(e) {
    return _C.replace(cl, e);
}
function eg(e, n) {
    return n.map((t) => t.replace(cl, e));
}
var ul = (() => {
        class e {
            eventManager;
            sharedStylesHost;
            appId;
            removeStylesOnCompDestroy;
            doc;
            ngZone;
            nonce;
            tracingService;
            rendererByCompId = new Map();
            defaultRenderer;
            constructor(t, r, o, i, s, a, c = null, u = null) {
                ((this.eventManager = t),
                    (this.sharedStylesHost = r),
                    (this.appId = o),
                    (this.removeStylesOnCompDestroy = i),
                    (this.doc = s),
                    (this.ngZone = a),
                    (this.nonce = c),
                    (this.tracingService = u),
                    (this.defaultRenderer = new _o(
                        t,
                        s,
                        a,
                        this.tracingService,
                    )));
            }
            createRenderer(t, r) {
                if (!t || !r) return this.defaultRenderer;
                let o = this.getOrCreateRenderer(t, r);
                return (
                    o instanceof Bs
                        ? o.applyToHost(t)
                        : o instanceof No && o.applyStyles(),
                    o
                );
            }
            getOrCreateRenderer(t, r) {
                let o = this.rendererByCompId,
                    i = o.get(r.id);
                if (!i) {
                    let s = this.doc,
                        a = this.ngZone,
                        c = this.eventManager,
                        u = this.sharedStylesHost,
                        l = this.removeStylesOnCompDestroy,
                        d = this.tracingService;
                    switch (r.encapsulation) {
                        case Ke.Emulated:
                            i = new Bs(c, u, r, this.appId, l, s, a, d);
                            break;
                        case Ke.ShadowDom:
                            return new Us(c, t, r, s, a, this.nonce, d, u);
                        case Ke.ExperimentalIsolatedShadowDom:
                            return new Us(c, t, r, s, a, this.nonce, d);
                        default:
                            i = new No(c, u, r, l, s, a, d);
                            break;
                    }
                    o.set(r.id, i);
                }
                return i;
            }
            ngOnDestroy() {
                this.rendererByCompId.clear();
            }
            componentReplaced(t) {
                this.rendererByCompId.delete(t);
            }
            static ɵfac = function (r) {
                return new (r || e)(
                    E(sl),
                    E(al),
                    E(ls),
                    E(AC),
                    E(W),
                    E(le),
                    E(mo),
                    E(at, 8),
                );
            };
            static ɵprov = v({ token: e, factory: e.ɵfac });
        }
        return e;
    })(),
    _o = class {
        eventManager;
        doc;
        ngZone;
        tracingService;
        data = Object.create(null);
        throwOnSyntheticProps = !0;
        constructor(n, t, r, o) {
            ((this.eventManager = n),
                (this.doc = t),
                (this.ngZone = r),
                (this.tracingService = o));
        }
        destroy() {}
        destroyNode = null;
        createElement(n, t) {
            return t
                ? this.doc.createElementNS(ol[t] || t, n)
                : this.doc.createElement(n);
        }
        createComment(n) {
            return this.doc.createComment(n);
        }
        createText(n) {
            return this.doc.createTextNode(n);
        }
        appendChild(n, t) {
            (Jh(n) ? n.content : n).appendChild(t);
        }
        insertBefore(n, t, r) {
            n && (Jh(n) ? n.content : n).insertBefore(t, r);
        }
        removeChild(n, t) {
            t.remove();
        }
        selectRootElement(n, t) {
            let r = typeof n == "string" ? this.doc.querySelector(n) : n;
            if (!r) throw new m(-5104, !1);
            return (t || (r.textContent = ""), r);
        }
        parentNode(n) {
            return n.parentNode;
        }
        nextSibling(n) {
            return n.nextSibling;
        }
        setAttribute(n, t, r, o) {
            if (o) {
                t = o + ":" + t;
                let i = ol[o];
                i ? n.setAttributeNS(i, t, r) : n.setAttribute(t, r);
            } else n.setAttribute(t, r);
        }
        removeAttribute(n, t, r) {
            if (r) {
                let o = ol[r];
                o ? n.removeAttributeNS(o, t) : n.removeAttribute(`${r}:${t}`);
            } else n.removeAttribute(t);
        }
        addClass(n, t) {
            n.classList.add(t);
        }
        removeClass(n, t) {
            n.classList.remove(t);
        }
        setStyle(n, t, r, o) {
            o & (Lt.DashCase | Lt.Important)
                ? n.style.setProperty(t, r, o & Lt.Important ? "important" : "")
                : (n.style[t] = r);
        }
        removeStyle(n, t, r) {
            r & Lt.DashCase ? n.style.removeProperty(t) : (n.style[t] = "");
        }
        setProperty(n, t, r) {
            n != null && (n[t] = r);
        }
        setValue(n, t) {
            n.nodeValue = t;
        }
        listen(n, t, r, o) {
            if (
                typeof n == "string" &&
                ((n = It().getGlobalEventTarget(this.doc, n)), !n)
            )
                throw new m(5102, !1);
            let i = this.decoratePreventDefault(r);
            return (
                this.tracingService?.wrapEventListener &&
                    (i = this.tracingService.wrapEventListener(n, t, i)),
                this.eventManager.addEventListener(n, t, i, o)
            );
        }
        decoratePreventDefault(n) {
            return (t) => {
                if (t === "__ngUnwrap__") return n;
                n(t) === !1 && t.preventDefault();
            };
        }
    };
function Jh(e) {
    return e.tagName === "TEMPLATE" && e.content !== void 0;
}
var Us = class extends _o {
        hostEl;
        sharedStylesHost;
        shadowRoot;
        constructor(n, t, r, o, i, s, a, c) {
            (super(n, o, i, a),
                (this.hostEl = t),
                (this.sharedStylesHost = c),
                (this.shadowRoot = t.attachShadow({ mode: "open" })),
                this.sharedStylesHost &&
                    this.sharedStylesHost.addHost(this.shadowRoot));
            let u = r.styles;
            u = eg(r.id, u);
            for (let d of u) {
                let h = document.createElement("style");
                (s && h.setAttribute("nonce", s),
                    (h.textContent = d),
                    this.shadowRoot.appendChild(h));
            }
            let l = r.getExternalStyles?.();
            if (l)
                for (let d of l) {
                    let h = il(d, o);
                    (s && h.setAttribute("nonce", s),
                        this.shadowRoot.appendChild(h));
                }
        }
        nodeOrShadowRoot(n) {
            return n === this.hostEl ? this.shadowRoot : n;
        }
        appendChild(n, t) {
            return super.appendChild(this.nodeOrShadowRoot(n), t);
        }
        insertBefore(n, t, r) {
            return super.insertBefore(this.nodeOrShadowRoot(n), t, r);
        }
        removeChild(n, t) {
            return super.removeChild(null, t);
        }
        parentNode(n) {
            return this.nodeOrShadowRoot(
                super.parentNode(this.nodeOrShadowRoot(n)),
            );
        }
        destroy() {
            this.sharedStylesHost &&
                this.sharedStylesHost.removeHost(this.shadowRoot);
        }
    },
    No = class extends _o {
        sharedStylesHost;
        removeStylesOnCompDestroy;
        styles;
        styleUrls;
        constructor(n, t, r, o, i, s, a, c) {
            (super(n, i, s, a),
                (this.sharedStylesHost = t),
                (this.removeStylesOnCompDestroy = o));
            let u = r.styles;
            ((this.styles = c ? eg(c, u) : u),
                (this.styleUrls = r.getExternalStyles?.(c)));
        }
        applyStyles() {
            this.sharedStylesHost.addStyles(this.styles, this.styleUrls);
        }
        destroy() {
            this.removeStylesOnCompDestroy &&
                cr.size === 0 &&
                this.sharedStylesHost.removeStyles(this.styles, this.styleUrls);
        }
    },
    Bs = class extends No {
        contentAttr;
        hostAttr;
        constructor(n, t, r, o, i, s, a, c) {
            let u = o + "-" + r.id;
            (super(n, t, r, i, s, a, c, u),
                (this.contentAttr = xC(u)),
                (this.hostAttr = OC(u)));
        }
        applyToHost(n) {
            (this.applyStyles(), this.setAttribute(n, this.hostAttr, ""));
        }
        createElement(n, t) {
            let r = super.createElement(n, t);
            return (super.setAttribute(r, this.contentAttr, ""), r);
        }
    };
var Hs = class e extends To {
        supportsDOMEvents = !0;
        static makeCurrent() {
            Qu(new e());
        }
        onAndCancel(n, t, r, o) {
            return (
                n.addEventListener(t, r, o),
                () => {
                    n.removeEventListener(t, r, o);
                }
            );
        }
        dispatchEvent(n, t) {
            n.dispatchEvent(t);
        }
        remove(n) {
            n.remove();
        }
        createElement(n, t) {
            return ((t = t || this.getDefaultDocument()), t.createElement(n));
        }
        createHtmlDocument() {
            return document.implementation.createHTMLDocument("fakeTitle");
        }
        getDefaultDocument() {
            return document;
        }
        isElementNode(n) {
            return n.nodeType === Node.ELEMENT_NODE;
        }
        isShadowRoot(n) {
            return n instanceof DocumentFragment;
        }
        getGlobalEventTarget(n, t) {
            return t === "window"
                ? window
                : t === "document"
                  ? n
                  : t === "body"
                    ? n.body
                    : null;
        }
        getBaseHref(n) {
            let t = PC();
            return t == null ? null : kC(t);
        }
        resetBaseElement() {
            Ro = null;
        }
        getUserAgent() {
            return window.navigator.userAgent;
        }
        getCookie(n) {
            return Ls(document.cookie, n);
        }
    },
    Ro = null;
function PC() {
    return (
        (Ro = Ro || document.head.querySelector("base")),
        Ro ? Ro.getAttribute("href") : null
    );
}
function kC(e) {
    return new URL(e, document.baseURI).pathname;
}
var FC = (() => {
        class e {
            build() {
                return new XMLHttpRequest();
            }
            static ɵfac = function (r) {
                return new (r || e)();
            };
            static ɵprov = v({ token: e, factory: e.ɵfac });
        }
        return e;
    })(),
    tg = ["alt", "control", "meta", "shift"],
    LC = {
        "\b": "Backspace",
        "	": "Tab",
        "\x7F": "Delete",
        "\x1B": "Escape",
        Del: "Delete",
        Esc: "Escape",
        Left: "ArrowLeft",
        Right: "ArrowRight",
        Up: "ArrowUp",
        Down: "ArrowDown",
        Menu: "ContextMenu",
        Scroll: "ScrollLock",
        Win: "OS",
    },
    jC = {
        alt: (e) => e.altKey,
        control: (e) => e.ctrlKey,
        meta: (e) => e.metaKey,
        shift: (e) => e.shiftKey,
    },
    ng = (() => {
        class e extends Mo {
            constructor(t) {
                super(t);
            }
            supports(t) {
                return e.parseEventName(t) != null;
            }
            addEventListener(t, r, o, i) {
                let s = e.parseEventName(r),
                    a = e.eventCallback(s.fullKey, o, this.manager.getZone());
                return this.manager
                    .getZone()
                    .runOutsideAngular(() =>
                        It().onAndCancel(t, s.domEventName, a, i),
                    );
            }
            static parseEventName(t) {
                let r = t.toLowerCase().split("."),
                    o = r.shift();
                if (r.length === 0 || !(o === "keydown" || o === "keyup"))
                    return null;
                let i = e._normalizeKey(r.pop()),
                    s = "",
                    a = r.indexOf("code");
                if (
                    (a > -1 && (r.splice(a, 1), (s = "code.")),
                    tg.forEach((u) => {
                        let l = r.indexOf(u);
                        l > -1 && (r.splice(l, 1), (s += u + "."));
                    }),
                    (s += i),
                    r.length != 0 || i.length === 0)
                )
                    return null;
                let c = {};
                return ((c.domEventName = o), (c.fullKey = s), c);
            }
            static matchEventFullKeyCode(t, r) {
                let o = LC[t.key] || t.key,
                    i = "";
                return (
                    r.indexOf("code.") > -1 && ((o = t.code), (i = "code.")),
                    o == null || !o
                        ? !1
                        : ((o = o.toLowerCase()),
                          o === " " ? (o = "space") : o === "." && (o = "dot"),
                          tg.forEach((s) => {
                              if (s !== o) {
                                  let a = jC[s];
                                  a(t) && (i += s + ".");
                              }
                          }),
                          (i += o),
                          i === r)
                );
            }
            static eventCallback(t, r, o) {
                return (i) => {
                    e.matchEventFullKeyCode(i, t) && o.runGuarded(() => r(i));
                };
            }
            static _normalizeKey(t) {
                return t === "esc" ? "escape" : t;
            }
            static ɵfac = function (r) {
                return new (r || e)(E(W));
            };
            static ɵprov = v({ token: e, factory: e.ɵfac });
        }
        return e;
    })();
async function ll(e, n, t) {
    let r = g({ rootComponent: e }, UC(n, t));
    return Vh(r);
}
function UC(e, n) {
    return {
        platformRef: n?.platformRef,
        appProviders: [...zC, ...(e?.providers ?? [])],
        platformProviders: $C,
    };
}
function BC() {
    Hs.makeCurrent();
}
function VC() {
    return new Ge();
}
function HC() {
    return (du(document), document);
}
var $C = [
    { provide: go, useValue: Yh },
    { provide: ds, useValue: BC, multi: !0 },
    { provide: W, useFactory: HC },
];
var zC = [
    { provide: Wr, useValue: "root" },
    { provide: Ge, useFactory: VC },
    { provide: Vs, useClass: js, multi: !0 },
    { provide: Vs, useClass: ng, multi: !0 },
    ul,
    al,
    sl,
    { provide: Tn, useExisting: ul },
    { provide: An, useClass: FC },
    [],
];
var zt = class e {
    headers;
    normalizedNames = new Map();
    lazyInit;
    lazyUpdate = null;
    constructor(n) {
        n
            ? typeof n == "string"
                ? (this.lazyInit = () => {
                      ((this.headers = new Map()),
                          n
                              .split(
                                  `
`,
                              )
                              .forEach((t) => {
                                  let r = t.indexOf(":");
                                  if (r > 0) {
                                      let o = t.slice(0, r),
                                          i = t.slice(r + 1).trim();
                                      this.addHeaderEntry(o, i);
                                  }
                              }));
                  })
                : typeof Headers < "u" && n instanceof Headers
                  ? ((this.headers = new Map()),
                    n.forEach((t, r) => {
                        this.addHeaderEntry(r, t);
                    }))
                  : (this.lazyInit = () => {
                        ((this.headers = new Map()),
                            Object.entries(n).forEach(([t, r]) => {
                                this.setHeaderEntries(t, r);
                            }));
                    })
            : (this.headers = new Map());
    }
    has(n) {
        return (this.init(), this.headers.has(n.toLowerCase()));
    }
    get(n) {
        this.init();
        let t = this.headers.get(n.toLowerCase());
        return t && t.length > 0 ? t[0] : null;
    }
    keys() {
        return (this.init(), Array.from(this.normalizedNames.values()));
    }
    getAll(n) {
        return (this.init(), this.headers.get(n.toLowerCase()) || null);
    }
    append(n, t) {
        return this.clone({ name: n, value: t, op: "a" });
    }
    set(n, t) {
        return this.clone({ name: n, value: t, op: "s" });
    }
    delete(n, t) {
        return this.clone({ name: n, value: t, op: "d" });
    }
    maybeSetNormalizedName(n, t) {
        this.normalizedNames.has(t) || this.normalizedNames.set(t, n);
    }
    init() {
        this.lazyInit &&
            (this.lazyInit instanceof e
                ? this.copyFrom(this.lazyInit)
                : this.lazyInit(),
            (this.lazyInit = null),
            this.lazyUpdate &&
                (this.lazyUpdate.forEach((n) => this.applyUpdate(n)),
                (this.lazyUpdate = null)));
    }
    copyFrom(n) {
        (n.init(),
            Array.from(n.headers.keys()).forEach((t) => {
                (this.headers.set(t, n.headers.get(t)),
                    this.normalizedNames.set(t, n.normalizedNames.get(t)));
            }));
    }
    clone(n) {
        let t = new e();
        return (
            (t.lazyInit =
                this.lazyInit && this.lazyInit instanceof e
                    ? this.lazyInit
                    : this),
            (t.lazyUpdate = (this.lazyUpdate || []).concat([n])),
            t
        );
    }
    applyUpdate(n) {
        let t = n.name.toLowerCase();
        switch (n.op) {
            case "a":
            case "s":
                let r = n.value;
                if ((typeof r == "string" && (r = [r]), r.length === 0)) return;
                this.maybeSetNormalizedName(n.name, t);
                let o = (n.op === "a" ? this.headers.get(t) : void 0) || [];
                (o.push(...r), this.headers.set(t, o));
                break;
            case "d":
                let i = n.value;
                if (!i)
                    (this.headers.delete(t), this.normalizedNames.delete(t));
                else {
                    let s = this.headers.get(t);
                    if (!s) return;
                    ((s = s.filter((a) => i.indexOf(a) === -1)),
                        s.length === 0
                            ? (this.headers.delete(t),
                              this.normalizedNames.delete(t))
                            : this.headers.set(t, s));
                }
                break;
        }
    }
    addHeaderEntry(n, t) {
        let r = n.toLowerCase();
        (this.maybeSetNormalizedName(n, r),
            this.headers.has(r)
                ? this.headers.get(r).push(t)
                : this.headers.set(r, [t]));
    }
    setHeaderEntries(n, t) {
        let r = (Array.isArray(t) ? t : [t]).map((i) => i.toString()),
            o = n.toLowerCase();
        (this.headers.set(o, r), this.maybeSetNormalizedName(n, o));
    }
    forEach(n) {
        (this.init(),
            Array.from(this.normalizedNames.keys()).forEach((t) =>
                n(this.normalizedNames.get(t), this.headers.get(t)),
            ));
    }
};
var fl = class {
        map = new Map();
        set(n, t) {
            return (this.map.set(n, t), this);
        }
        get(n) {
            return (
                this.map.has(n) || this.map.set(n, n.defaultValue()),
                this.map.get(n)
            );
        }
        delete(n) {
            return (this.map.delete(n), this);
        }
        has(n) {
            return this.map.has(n);
        }
        keys() {
            return this.map.keys();
        }
    },
    pl = class {
        encodeKey(n) {
            return rg(n);
        }
        encodeValue(n) {
            return rg(n);
        }
        decodeKey(n) {
            return decodeURIComponent(n);
        }
        decodeValue(n) {
            return decodeURIComponent(n);
        }
    };
function GC(e, n) {
    let t = new Map();
    return (
        e.length > 0 &&
            e
                .replace(/^\?/, "")
                .split("&")
                .forEach((o) => {
                    let i = o.indexOf("="),
                        [s, a] =
                            i == -1
                                ? [n.decodeKey(o), ""]
                                : [
                                      n.decodeKey(o.slice(0, i)),
                                      n.decodeValue(o.slice(i + 1)),
                                  ],
                        c = t.get(s) || [];
                    (c.push(a), t.set(s, c));
                }),
        t
    );
}
var qC = /%(\d[a-f0-9])/gi,
    WC = {
        40: "@",
        "3A": ":",
        24: "$",
        "2C": ",",
        "3B": ";",
        "3D": "=",
        "3F": "?",
        "2F": "/",
    };
function rg(e) {
    return encodeURIComponent(e).replace(qC, (n, t) => WC[t] ?? n);
}
function $s(e) {
    return `${e}`;
}
var wt = class e {
    map;
    encoder;
    updates = null;
    cloneFrom = null;
    constructor(n = {}) {
        if (((this.encoder = n.encoder || new pl()), n.fromString)) {
            if (n.fromObject) throw new m(2805, !1);
            this.map = GC(n.fromString, this.encoder);
        } else
            n.fromObject
                ? ((this.map = new Map()),
                  Object.keys(n.fromObject).forEach((t) => {
                      let r = n.fromObject[t],
                          o = Array.isArray(r) ? r.map($s) : [$s(r)];
                      this.map.set(t, o);
                  }))
                : (this.map = null);
    }
    has(n) {
        return (this.init(), this.map.has(n));
    }
    get(n) {
        this.init();
        let t = this.map.get(n);
        return t ? t[0] : null;
    }
    getAll(n) {
        return (this.init(), this.map.get(n) || null);
    }
    keys() {
        return (this.init(), Array.from(this.map.keys()));
    }
    append(n, t) {
        return this.clone({ param: n, value: t, op: "a" });
    }
    appendAll(n) {
        let t = [];
        return (
            Object.keys(n).forEach((r) => {
                let o = n[r];
                Array.isArray(o)
                    ? o.forEach((i) => {
                          t.push({ param: r, value: i, op: "a" });
                      })
                    : t.push({ param: r, value: o, op: "a" });
            }),
            this.clone(t)
        );
    }
    set(n, t) {
        return this.clone({ param: n, value: t, op: "s" });
    }
    delete(n, t) {
        return this.clone({ param: n, value: t, op: "d" });
    }
    toString() {
        return (
            this.init(),
            this.keys()
                .map((n) => {
                    let t = this.encoder.encodeKey(n);
                    return this.map
                        .get(n)
                        .map((r) => t + "=" + this.encoder.encodeValue(r))
                        .join("&");
                })
                .filter((n) => n !== "")
                .join("&")
        );
    }
    clone(n) {
        let t = new e({ encoder: this.encoder });
        return (
            (t.cloneFrom = this.cloneFrom || this),
            (t.updates = (this.updates || []).concat(n)),
            t
        );
    }
    init() {
        (this.map === null && (this.map = new Map()),
            this.cloneFrom !== null &&
                (this.cloneFrom.init(),
                this.cloneFrom
                    .keys()
                    .forEach((n) => this.map.set(n, this.cloneFrom.map.get(n))),
                this.updates.forEach((n) => {
                    switch (n.op) {
                        case "a":
                        case "s":
                            let t =
                                (n.op === "a"
                                    ? this.map.get(n.param)
                                    : void 0) || [];
                            (t.push($s(n.value)), this.map.set(n.param, t));
                            break;
                        case "d":
                            if (n.value !== void 0) {
                                let r = this.map.get(n.param) || [],
                                    o = r.indexOf($s(n.value));
                                (o !== -1 && r.splice(o, 1),
                                    r.length > 0
                                        ? this.map.set(n.param, r)
                                        : this.map.delete(n.param));
                            } else {
                                this.map.delete(n.param);
                                break;
                            }
                    }
                }),
                (this.cloneFrom = this.updates = null)));
    }
};
function ZC(e) {
    switch (e) {
        case "DELETE":
        case "GET":
        case "HEAD":
        case "OPTIONS":
        case "JSONP":
            return !1;
        default:
            return !0;
    }
}
function og(e) {
    return typeof ArrayBuffer < "u" && e instanceof ArrayBuffer;
}
function ig(e) {
    return typeof Blob < "u" && e instanceof Blob;
}
function sg(e) {
    return typeof FormData < "u" && e instanceof FormData;
}
function YC(e) {
    return typeof URLSearchParams < "u" && e instanceof URLSearchParams;
}
var ag = "Content-Type",
    cg = "Accept",
    ug = "text/plain",
    lg = "application/json",
    QC = `${lg}, ${ug}, */*`,
    gr = class e {
        url;
        body = null;
        headers;
        context;
        reportProgress = !1;
        withCredentials = !1;
        credentials;
        keepalive = !1;
        cache;
        priority;
        mode;
        redirect;
        referrer;
        integrity;
        referrerPolicy;
        responseType = "json";
        method;
        params;
        urlWithParams;
        transferCache;
        timeout;
        constructor(n, t, r, o) {
            ((this.url = t), (this.method = n.toUpperCase()));
            let i;
            if (
                (ZC(this.method) || o
                    ? ((this.body = r !== void 0 ? r : null), (i = o))
                    : (i = r),
                i)
            ) {
                if (
                    ((this.reportProgress = !!i.reportProgress),
                    (this.withCredentials = !!i.withCredentials),
                    (this.keepalive = !!i.keepalive),
                    i.responseType && (this.responseType = i.responseType),
                    i.headers && (this.headers = i.headers),
                    i.context && (this.context = i.context),
                    i.params && (this.params = i.params),
                    i.priority && (this.priority = i.priority),
                    i.cache && (this.cache = i.cache),
                    i.credentials && (this.credentials = i.credentials),
                    typeof i.timeout == "number")
                ) {
                    if (i.timeout < 1 || !Number.isInteger(i.timeout))
                        throw new m(2822, "");
                    this.timeout = i.timeout;
                }
                (i.mode && (this.mode = i.mode),
                    i.redirect && (this.redirect = i.redirect),
                    i.integrity && (this.integrity = i.integrity),
                    i.referrer && (this.referrer = i.referrer),
                    i.referrerPolicy &&
                        (this.referrerPolicy = i.referrerPolicy),
                    (this.transferCache = i.transferCache));
            }
            if (
                ((this.headers ??= new zt()),
                (this.context ??= new fl()),
                !this.params)
            )
                ((this.params = new wt()), (this.urlWithParams = t));
            else {
                let s = this.params.toString();
                if (s.length === 0) this.urlWithParams = t;
                else {
                    let a = t.indexOf("?"),
                        c = a === -1 ? "?" : a < t.length - 1 ? "&" : "";
                    this.urlWithParams = t + c + s;
                }
            }
        }
        serializeBody() {
            return this.body === null
                ? null
                : typeof this.body == "string" ||
                    og(this.body) ||
                    ig(this.body) ||
                    sg(this.body) ||
                    YC(this.body)
                  ? this.body
                  : this.body instanceof wt
                    ? this.body.toString()
                    : typeof this.body == "object" ||
                        typeof this.body == "boolean" ||
                        Array.isArray(this.body)
                      ? JSON.stringify(this.body)
                      : this.body.toString();
        }
        detectContentTypeHeader() {
            return this.body === null || sg(this.body)
                ? null
                : ig(this.body)
                  ? this.body.type || null
                  : og(this.body)
                    ? null
                    : typeof this.body == "string"
                      ? ug
                      : this.body instanceof wt
                        ? "application/x-www-form-urlencoded;charset=UTF-8"
                        : typeof this.body == "object" ||
                            typeof this.body == "number" ||
                            typeof this.body == "boolean"
                          ? lg
                          : null;
        }
        clone(n = {}) {
            let t = n.method || this.method,
                r = n.url || this.url,
                o = n.responseType || this.responseType,
                i = n.keepalive ?? this.keepalive,
                s = n.priority || this.priority,
                a = n.cache || this.cache,
                c = n.mode || this.mode,
                u = n.redirect || this.redirect,
                l = n.credentials || this.credentials,
                d = n.referrer || this.referrer,
                h = n.integrity || this.integrity,
                f = n.referrerPolicy || this.referrerPolicy,
                y = n.transferCache ?? this.transferCache,
                k = n.timeout ?? this.timeout,
                C = n.body !== void 0 ? n.body : this.body,
                _ = n.withCredentials ?? this.withCredentials,
                ce = n.reportProgress ?? this.reportProgress,
                Ve = n.headers || this.headers,
                X = n.params || this.params,
                Nr = n.context ?? this.context;
            return (
                n.setHeaders !== void 0 &&
                    (Ve = Object.keys(n.setHeaders).reduce(
                        (Rr, Jt) => Rr.set(Jt, n.setHeaders[Jt]),
                        Ve,
                    )),
                n.setParams &&
                    (X = Object.keys(n.setParams).reduce(
                        (Rr, Jt) => Rr.set(Jt, n.setParams[Jt]),
                        X,
                    )),
                new e(t, r, C, {
                    params: X,
                    headers: Ve,
                    context: Nr,
                    reportProgress: ce,
                    responseType: o,
                    withCredentials: _,
                    transferCache: y,
                    keepalive: i,
                    cache: a,
                    priority: s,
                    timeout: k,
                    mode: c,
                    redirect: u,
                    credentials: l,
                    referrer: d,
                    integrity: h,
                    referrerPolicy: f,
                })
            );
        }
    },
    xn = (function (e) {
        return (
            (e[(e.Sent = 0)] = "Sent"),
            (e[(e.UploadProgress = 1)] = "UploadProgress"),
            (e[(e.ResponseHeader = 2)] = "ResponseHeader"),
            (e[(e.DownloadProgress = 3)] = "DownloadProgress"),
            (e[(e.Response = 4)] = "Response"),
            (e[(e.User = 5)] = "User"),
            e
        );
    })(xn || {}),
    Ao = class {
        headers;
        status;
        statusText;
        url;
        ok;
        type;
        redirected;
        responseType;
        constructor(n, t = 200, r = "OK") {
            ((this.headers = n.headers || new zt()),
                (this.status = n.status !== void 0 ? n.status : t),
                (this.statusText = n.statusText || r),
                (this.url = n.url || null),
                (this.redirected = n.redirected),
                (this.responseType = n.responseType),
                (this.ok = this.status >= 200 && this.status < 300));
        }
    },
    hl = class e extends Ao {
        constructor(n = {}) {
            super(n);
        }
        type = xn.ResponseHeader;
        clone(n = {}) {
            return new e({
                headers: n.headers || this.headers,
                status: n.status !== void 0 ? n.status : this.status,
                statusText: n.statusText || this.statusText,
                url: n.url || this.url || void 0,
            });
        }
    },
    xo = class e extends Ao {
        body;
        constructor(n = {}) {
            (super(n), (this.body = n.body !== void 0 ? n.body : null));
        }
        type = xn.Response;
        clone(n = {}) {
            return new e({
                body: n.body !== void 0 ? n.body : this.body,
                headers: n.headers || this.headers,
                status: n.status !== void 0 ? n.status : this.status,
                statusText: n.statusText || this.statusText,
                url: n.url || this.url || void 0,
                redirected: n.redirected ?? this.redirected,
                responseType: n.responseType ?? this.responseType,
            });
        }
    },
    mr = class extends Ao {
        name = "HttpErrorResponse";
        message;
        error;
        ok = !1;
        constructor(n) {
            (super(n, 0, "Unknown Error"),
                this.status >= 200 && this.status < 300
                    ? (this.message = `Http failure during parsing for ${n.url || "(unknown url)"}`)
                    : (this.message = `Http failure response for ${n.url || "(unknown url)"}: ${n.status} ${n.statusText}`),
                (this.error = n.error || null));
        }
    },
    KC = 200,
    JC = 204;
var XC = /^\)\]\}',?\n/;
var eI = (() => {
    class e {
        xhrFactory;
        tracingService = p(at, { optional: !0 });
        constructor(t) {
            this.xhrFactory = t;
        }
        maybePropagateTrace(t) {
            return this.tracingService?.propagate
                ? this.tracingService.propagate(t)
                : t;
        }
        handle(t) {
            if (t.method === "JSONP") throw new m(-2800, !1);
            let r = this.xhrFactory;
            return I(null).pipe(
                Ee(
                    () =>
                        new O((i) => {
                            let s = r.build();
                            if (
                                (s.open(t.method, t.urlWithParams),
                                t.withCredentials && (s.withCredentials = !0),
                                t.headers.forEach((C, _) =>
                                    s.setRequestHeader(C, _.join(",")),
                                ),
                                t.headers.has(cg) || s.setRequestHeader(cg, QC),
                                !t.headers.has(ag))
                            ) {
                                let C = t.detectContentTypeHeader();
                                C !== null && s.setRequestHeader(ag, C);
                            }
                            if (
                                (t.timeout && (s.timeout = t.timeout),
                                t.responseType)
                            ) {
                                let C = t.responseType.toLowerCase();
                                s.responseType = C !== "json" ? C : "text";
                            }
                            let a = t.serializeBody(),
                                c = null,
                                u = () => {
                                    if (c !== null) return c;
                                    let C = s.statusText || "OK",
                                        _ = new zt(s.getAllResponseHeaders()),
                                        ce = s.responseURL || t.url;
                                    return (
                                        (c = new hl({
                                            headers: _,
                                            status: s.status,
                                            statusText: C,
                                            url: ce,
                                        })),
                                        c
                                    );
                                },
                                l = this.maybePropagateTrace(() => {
                                    let {
                                            headers: C,
                                            status: _,
                                            statusText: ce,
                                            url: Ve,
                                        } = u(),
                                        X = null;
                                    (_ !== JC &&
                                        (X =
                                            typeof s.response > "u"
                                                ? s.responseText
                                                : s.response),
                                        _ === 0 && (_ = X ? KC : 0));
                                    let Nr = _ >= 200 && _ < 300;
                                    if (
                                        t.responseType === "json" &&
                                        typeof X == "string"
                                    ) {
                                        let Rr = X;
                                        X = X.replace(XC, "");
                                        try {
                                            X = X !== "" ? JSON.parse(X) : null;
                                        } catch (Jt) {
                                            ((X = Rr),
                                                Nr &&
                                                    ((Nr = !1),
                                                    (X = {
                                                        error: Jt,
                                                        text: X,
                                                    })));
                                        }
                                    }
                                    Nr
                                        ? (i.next(
                                              new xo({
                                                  body: X,
                                                  headers: C,
                                                  status: _,
                                                  statusText: ce,
                                                  url: Ve || void 0,
                                              }),
                                          ),
                                          i.complete())
                                        : i.error(
                                              new mr({
                                                  error: X,
                                                  headers: C,
                                                  status: _,
                                                  statusText: ce,
                                                  url: Ve || void 0,
                                              }),
                                          );
                                }),
                                d = this.maybePropagateTrace((C) => {
                                    let { url: _ } = u(),
                                        ce = new mr({
                                            error: C,
                                            status: s.status || 0,
                                            statusText:
                                                s.statusText || "Unknown Error",
                                            url: _ || void 0,
                                        });
                                    i.error(ce);
                                }),
                                h = d;
                            t.timeout &&
                                (h = this.maybePropagateTrace((C) => {
                                    let { url: _ } = u(),
                                        ce = new mr({
                                            error: new DOMException(
                                                "Request timed out",
                                                "TimeoutError",
                                            ),
                                            status: s.status || 0,
                                            statusText:
                                                s.statusText ||
                                                "Request timeout",
                                            url: _ || void 0,
                                        });
                                    i.error(ce);
                                }));
                            let f = !1,
                                y = this.maybePropagateTrace((C) => {
                                    f || (i.next(u()), (f = !0));
                                    let _ = {
                                        type: xn.DownloadProgress,
                                        loaded: C.loaded,
                                    };
                                    (C.lengthComputable && (_.total = C.total),
                                        t.responseType === "text" &&
                                            s.responseText &&
                                            (_.partialText = s.responseText),
                                        i.next(_));
                                }),
                                k = this.maybePropagateTrace((C) => {
                                    let _ = {
                                        type: xn.UploadProgress,
                                        loaded: C.loaded,
                                    };
                                    (C.lengthComputable && (_.total = C.total),
                                        i.next(_));
                                });
                            return (
                                s.addEventListener("load", l),
                                s.addEventListener("error", d),
                                s.addEventListener("timeout", h),
                                s.addEventListener("abort", d),
                                t.reportProgress &&
                                    (s.addEventListener("progress", y),
                                    a !== null &&
                                        s.upload &&
                                        s.upload.addEventListener(
                                            "progress",
                                            k,
                                        )),
                                s.send(a),
                                i.next({ type: xn.Sent }),
                                () => {
                                    (s.removeEventListener("error", d),
                                        s.removeEventListener("abort", d),
                                        s.removeEventListener("load", l),
                                        s.removeEventListener("timeout", h),
                                        t.reportProgress &&
                                            (s.removeEventListener(
                                                "progress",
                                                y,
                                            ),
                                            a !== null &&
                                                s.upload &&
                                                s.upload.removeEventListener(
                                                    "progress",
                                                    k,
                                                )),
                                        s.readyState !== s.DONE && s.abort());
                                }
                            );
                        }),
                ),
            );
        }
        static ɵfac = function (r) {
            return new (r || e)(E(An));
        };
        static ɵprov = v({ token: e, factory: e.ɵfac, providedIn: "root" });
    }
    return e;
})();
function tI(e, n) {
    return n(e);
}
function nI(e, n, t) {
    return (r, o) => Q(t, () => n(r, (i) => e(i, o)));
}
var rI = new D("", { factory: () => [] }),
    dg = new D(""),
    oI = new D("", { factory: () => !0 });
var iI = (() => {
    class e {
        static ɵfac = function (r) {
            return new (r || e)();
        };
        static ɵprov = v({
            token: e,
            factory: function (r) {
                let o = null;
                return (r ? (o = new (r || e)()) : (o = E(eI)), o);
            },
            providedIn: "root",
        });
    }
    return e;
})();
var sI = (() => {
        class e {
            backend;
            injector;
            chain = null;
            pendingTasks = p(oo);
            contributeToStability = p(oI);
            constructor(t, r) {
                ((this.backend = t), (this.injector = r));
            }
            handle(t) {
                if (this.chain === null) {
                    let r = Array.from(
                        new Set([
                            ...this.injector.get(rI),
                            ...this.injector.get(dg, []),
                        ]),
                    );
                    this.chain = r.reduceRight(
                        (o, i) => nI(o, i, this.injector),
                        tI,
                    );
                }
                if (this.contributeToStability) {
                    let r = this.pendingTasks.add();
                    return this.chain(t, (o) => this.backend.handle(o)).pipe(
                        Lr(r),
                    );
                } else return this.chain(t, (r) => this.backend.handle(r));
            }
            static ɵfac = function (r) {
                return new (r || e)(E(iI), E(G));
            };
            static ɵprov = v({ token: e, factory: e.ɵfac, providedIn: "root" });
        }
        return e;
    })(),
    aI = (() => {
        class e {
            static ɵfac = function (r) {
                return new (r || e)();
            };
            static ɵprov = v({
                token: e,
                factory: function (r) {
                    let o = null;
                    return (r ? (o = new (r || e)()) : (o = E(sI)), o);
                },
                providedIn: "root",
            });
        }
        return e;
    })();
function dl(e, n) {
    return {
        body: n,
        headers: e.headers,
        context: e.context,
        observe: e.observe,
        params: e.params,
        reportProgress: e.reportProgress,
        responseType: e.responseType,
        withCredentials: e.withCredentials,
        credentials: e.credentials,
        transferCache: e.transferCache,
        timeout: e.timeout,
        keepalive: e.keepalive,
        priority: e.priority,
        cache: e.cache,
        mode: e.mode,
        redirect: e.redirect,
        integrity: e.integrity,
        referrer: e.referrer,
        referrerPolicy: e.referrerPolicy,
    };
}
var gl = (() => {
    class e {
        handler;
        constructor(t) {
            this.handler = t;
        }
        request(t, r, o = {}) {
            let i;
            if (t instanceof gr) i = t;
            else {
                let c;
                o.headers instanceof zt
                    ? (c = o.headers)
                    : (c = new zt(o.headers));
                let u;
                (o.params &&
                    (o.params instanceof wt
                        ? (u = o.params)
                        : (u = new wt({ fromObject: o.params }))),
                    (i = new gr(t, r, o.body !== void 0 ? o.body : null, {
                        headers: c,
                        context: o.context,
                        params: u,
                        reportProgress: o.reportProgress,
                        responseType: o.responseType || "json",
                        withCredentials: o.withCredentials,
                        transferCache: o.transferCache,
                        keepalive: o.keepalive,
                        priority: o.priority,
                        cache: o.cache,
                        mode: o.mode,
                        redirect: o.redirect,
                        credentials: o.credentials,
                        referrer: o.referrer,
                        referrerPolicy: o.referrerPolicy,
                        integrity: o.integrity,
                        timeout: o.timeout,
                    })));
            }
            let s = I(i).pipe(Mt((c) => this.handler.handle(c)));
            if (t instanceof gr || o.observe === "events") return s;
            let a = s.pipe(be((c) => c instanceof xo));
            switch (o.observe || "body") {
                case "body":
                    switch (i.responseType) {
                        case "arraybuffer":
                            return a.pipe(
                                H((c) => {
                                    if (
                                        c.body !== null &&
                                        !(c.body instanceof ArrayBuffer)
                                    )
                                        throw new m(2806, !1);
                                    return c.body;
                                }),
                            );
                        case "blob":
                            return a.pipe(
                                H((c) => {
                                    if (
                                        c.body !== null &&
                                        !(c.body instanceof Blob)
                                    )
                                        throw new m(2807, !1);
                                    return c.body;
                                }),
                            );
                        case "text":
                            return a.pipe(
                                H((c) => {
                                    if (
                                        c.body !== null &&
                                        typeof c.body != "string"
                                    )
                                        throw new m(2808, !1);
                                    return c.body;
                                }),
                            );
                        default:
                            return a.pipe(H((c) => c.body));
                    }
                case "response":
                    return a;
                default:
                    throw new m(2809, !1);
            }
        }
        delete(t, r = {}) {
            return this.request("DELETE", t, r);
        }
        get(t, r = {}) {
            return this.request("GET", t, r);
        }
        head(t, r = {}) {
            return this.request("HEAD", t, r);
        }
        jsonp(t, r) {
            return this.request("JSONP", t, {
                params: new wt().append(r, "JSONP_CALLBACK"),
                observe: "body",
                responseType: "json",
            });
        }
        options(t, r = {}) {
            return this.request("OPTIONS", t, r);
        }
        patch(t, r, o = {}) {
            return this.request("PATCH", t, dl(o, r));
        }
        post(t, r, o = {}) {
            return this.request("POST", t, dl(o, r));
        }
        put(t, r, o = {}) {
            return this.request("PUT", t, dl(o, r));
        }
        static ɵfac = function (r) {
            return new (r || e)(E(aI));
        };
        static ɵprov = v({ token: e, factory: e.ɵfac, providedIn: "root" });
    }
    return e;
})();
var fg = (() => {
    class e {
        _doc;
        constructor(t) {
            this._doc = t;
        }
        getTitle() {
            return this._doc.title;
        }
        setTitle(t) {
            this._doc.title = t || "";
        }
        static ɵfac = function (r) {
            return new (r || e)(E(W));
        };
        static ɵprov = v({ token: e, factory: e.ɵfac, providedIn: "root" });
    }
    return e;
})();
var T = "primary",
    zo = Symbol("RouteTitle"),
    El = class {
        params;
        constructor(n) {
            this.params = n || {};
        }
        has(n) {
            return Object.prototype.hasOwnProperty.call(this.params, n);
        }
        get(n) {
            if (this.has(n)) {
                let t = this.params[n];
                return Array.isArray(t) ? t[0] : t;
            }
            return null;
        }
        getAll(n) {
            if (this.has(n)) {
                let t = this.params[n];
                return Array.isArray(t) ? t : [t];
            }
            return [];
        }
        get keys() {
            return Object.keys(this.params);
        }
    };
function Pn(e) {
    return new El(e);
}
function ml(e, n, t) {
    for (let r = 0; r < e.length; r++) {
        let o = e[r],
            i = n[r];
        if (o[0] === ":") t[o.substring(1)] = i;
        else if (o !== i.path) return !1;
    }
    return !0;
}
function Eg(e, n, t) {
    let r = t.path.split("/"),
        o = r.indexOf("**");
    if (o === -1) {
        if (
            r.length > e.length ||
            (t.pathMatch === "full" && (n.hasChildren() || r.length < e.length))
        )
            return null;
        let c = {},
            u = e.slice(0, r.length);
        return ml(r, u, c) ? { consumed: u, posParams: c } : null;
    }
    if (o !== r.lastIndexOf("**")) return null;
    let i = r.slice(0, o),
        s = r.slice(o + 1);
    if (
        i.length + s.length > e.length ||
        (t.pathMatch === "full" && n.hasChildren() && t.path !== "**")
    )
        return null;
    let a = {};
    return !ml(i, e.slice(0, i.length), a) ||
        !ml(s, e.slice(e.length - s.length), a)
        ? null
        : { consumed: e, posParams: a };
}
function Ys(e) {
    return new Promise((n, t) => {
        e.pipe(pt()).subscribe({ next: (r) => n(r), error: (r) => t(r) });
    });
}
function lI(e, n) {
    if (e.length !== n.length) return !1;
    for (let t = 0; t < e.length; ++t) if (!ut(e[t], n[t])) return !1;
    return !0;
}
function ut(e, n) {
    let t = e ? Cl(e) : void 0,
        r = n ? Cl(n) : void 0;
    if (!t || !r || t.length != r.length) return !1;
    let o;
    for (let i = 0; i < t.length; i++)
        if (((o = t[i]), !Cg(e[o], n[o]))) return !1;
    return !0;
}
function Cl(e) {
    return [...Object.keys(e), ...Object.getOwnPropertySymbols(e)];
}
function Cg(e, n) {
    if (Array.isArray(e) && Array.isArray(n)) {
        if (e.length !== n.length) return !1;
        let t = [...e].sort(),
            r = [...n].sort();
        return t.every((o, i) => r[i] === o);
    } else return e === n;
}
function dI(e) {
    return e.length > 0 ? e[e.length - 1] : null;
}
function Fn(e) {
    return Ei(e) ? e : pr(e) ? q(Promise.resolve(e)) : I(e);
}
function Ig(e) {
    return Ei(e) ? Ys(e) : Promise.resolve(e);
}
var fI = { exact: Tg, subset: Sg },
    wg = { exact: pI, subset: hI, ignored: () => !0 },
    bg = {
        paths: "exact",
        fragment: "ignored",
        matrixParams: "ignored",
        queryParams: "exact",
    },
    Il = {
        paths: "subset",
        fragment: "ignored",
        matrixParams: "ignored",
        queryParams: "subset",
    };
function pg(e, n, t) {
    return (
        fI[t.paths](e.root, n.root, t.matrixParams) &&
        wg[t.queryParams](e.queryParams, n.queryParams) &&
        !(t.fragment === "exact" && e.fragment !== n.fragment)
    );
}
function pI(e, n) {
    return ut(e, n);
}
function Tg(e, n, t) {
    if (
        !On(e.segments, n.segments) ||
        !qs(e.segments, n.segments, t) ||
        e.numberOfChildren !== n.numberOfChildren
    )
        return !1;
    for (let r in n.children)
        if (!e.children[r] || !Tg(e.children[r], n.children[r], t)) return !1;
    return !0;
}
function hI(e, n) {
    return (
        Object.keys(n).length <= Object.keys(e).length &&
        Object.keys(n).every((t) => Cg(e[t], n[t]))
    );
}
function Sg(e, n, t) {
    return Mg(e, n, n.segments, t);
}
function Mg(e, n, t, r) {
    if (e.segments.length > t.length) {
        let o = e.segments.slice(0, t.length);
        return !(!On(o, t) || n.hasChildren() || !qs(o, t, r));
    } else if (e.segments.length === t.length) {
        if (!On(e.segments, t) || !qs(e.segments, t, r)) return !1;
        for (let o in n.children)
            if (!e.children[o] || !Sg(e.children[o], n.children[o], r))
                return !1;
        return !0;
    } else {
        let o = t.slice(0, e.segments.length),
            i = t.slice(e.segments.length);
        return !On(e.segments, o) || !qs(e.segments, o, r) || !e.children[T]
            ? !1
            : Mg(e.children[T], n, i, r);
    }
}
function qs(e, n, t) {
    return n.every((r, o) => wg[t](e[o].parameters, r.parameters));
}
var Ne = class {
        root;
        queryParams;
        fragment;
        _queryParamMap;
        constructor(n = new F([], {}), t = {}, r = null) {
            ((this.root = n), (this.queryParams = t), (this.fragment = r));
        }
        get queryParamMap() {
            return (
                (this._queryParamMap ??= Pn(this.queryParams)),
                this._queryParamMap
            );
        }
        toString() {
            return vI.serialize(this);
        }
    },
    F = class {
        segments;
        children;
        parent = null;
        constructor(n, t) {
            ((this.segments = n),
                (this.children = t),
                Object.values(t).forEach((r) => (r.parent = this)));
        }
        hasChildren() {
            return this.numberOfChildren > 0;
        }
        get numberOfChildren() {
            return Object.keys(this.children).length;
        }
        toString() {
            return Ws(this);
        }
    },
    Gt = class {
        path;
        parameters;
        _parameterMap;
        constructor(n, t) {
            ((this.path = n), (this.parameters = t));
        }
        get parameterMap() {
            return (
                (this._parameterMap ??= Pn(this.parameters)),
                this._parameterMap
            );
        }
        toString() {
            return Ng(this);
        }
    };
function gI(e, n) {
    return On(e, n) && e.every((t, r) => ut(t.parameters, n[r].parameters));
}
function On(e, n) {
    return e.length !== n.length ? !1 : e.every((t, r) => t.path === n[r].path);
}
function mI(e, n) {
    let t = [];
    return (
        Object.entries(e.children).forEach(([r, o]) => {
            r === T && (t = t.concat(n(o, r)));
        }),
        Object.entries(e.children).forEach(([r, o]) => {
            r !== T && (t = t.concat(n(o, r)));
        }),
        t
    );
}
var Zt = (() => {
        class e {
            static ɵfac = function (r) {
                return new (r || e)();
            };
            static ɵprov = v({
                token: e,
                factory: () => new Tt(),
                providedIn: "root",
            });
        }
        return e;
    })(),
    Tt = class {
        parse(n) {
            let t = new bl(n);
            return new Ne(
                t.parseRootSegment(),
                t.parseQueryParams(),
                t.parseFragment(),
            );
        }
        serialize(n) {
            let t = `/${Oo(n.root, !0)}`,
                r = EI(n.queryParams),
                o = typeof n.fragment == "string" ? `#${yI(n.fragment)}` : "";
            return `${t}${r}${o}`;
        }
    },
    vI = new Tt();
function Ws(e) {
    return e.segments.map((n) => Ng(n)).join("/");
}
function Oo(e, n) {
    if (!e.hasChildren()) return Ws(e);
    if (n) {
        let t = e.children[T] ? Oo(e.children[T], !1) : "",
            r = [];
        return (
            Object.entries(e.children).forEach(([o, i]) => {
                o !== T && r.push(`${o}:${Oo(i, !1)}`);
            }),
            r.length > 0 ? `${t}(${r.join("//")})` : t
        );
    } else {
        let t = mI(e, (r, o) =>
            o === T ? [Oo(e.children[T], !1)] : [`${o}:${Oo(r, !1)}`],
        );
        return Object.keys(e.children).length === 1 && e.children[T] != null
            ? `${Ws(e)}/${t[0]}`
            : `${Ws(e)}/(${t.join("//")})`;
    }
}
function _g(e) {
    return encodeURIComponent(e)
        .replace(/%40/g, "@")
        .replace(/%3A/gi, ":")
        .replace(/%24/g, "$")
        .replace(/%2C/gi, ",");
}
function zs(e) {
    return _g(e).replace(/%3B/gi, ";");
}
function yI(e) {
    return encodeURI(e);
}
function wl(e) {
    return _g(e)
        .replace(/\(/g, "%28")
        .replace(/\)/g, "%29")
        .replace(/%26/gi, "&");
}
function Zs(e) {
    return decodeURIComponent(e);
}
function hg(e) {
    return Zs(e.replace(/\+/g, "%20"));
}
function Ng(e) {
    return `${wl(e.path)}${DI(e.parameters)}`;
}
function DI(e) {
    return Object.entries(e)
        .map(([n, t]) => `;${wl(n)}=${wl(t)}`)
        .join("");
}
function EI(e) {
    let n = Object.entries(e)
        .map(([t, r]) =>
            Array.isArray(r)
                ? r.map((o) => `${zs(t)}=${zs(o)}`).join("&")
                : `${zs(t)}=${zs(r)}`,
        )
        .filter((t) => t);
    return n.length ? `?${n.join("&")}` : "";
}
var CI = /^[^\/()?;#]+/;
function vl(e) {
    let n = e.match(CI);
    return n ? n[0] : "";
}
var II = /^[^\/()?;=#]+/;
function wI(e) {
    let n = e.match(II);
    return n ? n[0] : "";
}
var bI = /^[^=?&#]+/;
function TI(e) {
    let n = e.match(bI);
    return n ? n[0] : "";
}
var SI = /^[^&#]+/;
function MI(e) {
    let n = e.match(SI);
    return n ? n[0] : "";
}
var bl = class {
    url;
    remaining;
    constructor(n) {
        ((this.url = n), (this.remaining = n));
    }
    parseRootSegment() {
        for (; this.consumeOptional("/"); );
        return this.remaining === "" ||
            this.peekStartsWith("?") ||
            this.peekStartsWith("#")
            ? new F([], {})
            : new F([], this.parseChildren());
    }
    parseQueryParams() {
        let n = {};
        if (this.consumeOptional("?"))
            do this.parseQueryParam(n);
            while (this.consumeOptional("&"));
        return n;
    }
    parseFragment() {
        return this.consumeOptional("#")
            ? decodeURIComponent(this.remaining)
            : null;
    }
    parseChildren(n = 0) {
        if (n > 50) throw new m(4010, !1);
        if (this.remaining === "") return {};
        this.consumeOptional("/");
        let t = [];
        for (
            this.peekStartsWith("(") || t.push(this.parseSegment());
            this.peekStartsWith("/") &&
            !this.peekStartsWith("//") &&
            !this.peekStartsWith("/(");
        )
            (this.capture("/"), t.push(this.parseSegment()));
        let r = {};
        this.peekStartsWith("/(") &&
            (this.capture("/"), (r = this.parseParens(!0, n)));
        let o = {};
        return (
            this.peekStartsWith("(") && (o = this.parseParens(!1, n)),
            (t.length > 0 || Object.keys(r).length > 0) && (o[T] = new F(t, r)),
            o
        );
    }
    parseSegment() {
        let n = vl(this.remaining);
        if (n === "" && this.peekStartsWith(";")) throw new m(4009, !1);
        return (this.capture(n), new Gt(Zs(n), this.parseMatrixParams()));
    }
    parseMatrixParams() {
        let n = {};
        for (; this.consumeOptional(";"); ) this.parseParam(n);
        return n;
    }
    parseParam(n) {
        let t = wI(this.remaining);
        if (!t) return;
        this.capture(t);
        let r = "";
        if (this.consumeOptional("=")) {
            let o = vl(this.remaining);
            o && ((r = o), this.capture(r));
        }
        n[Zs(t)] = Zs(r);
    }
    parseQueryParam(n) {
        let t = TI(this.remaining);
        if (!t) return;
        this.capture(t);
        let r = "";
        if (this.consumeOptional("=")) {
            let s = MI(this.remaining);
            s && ((r = s), this.capture(r));
        }
        let o = hg(t),
            i = hg(r);
        if (n.hasOwnProperty(o)) {
            let s = n[o];
            (Array.isArray(s) || ((s = [s]), (n[o] = s)), s.push(i));
        } else n[o] = i;
    }
    parseParens(n, t) {
        let r = {};
        for (
            this.capture("(");
            !this.consumeOptional(")") && this.remaining.length > 0;
        ) {
            let o = vl(this.remaining),
                i = this.remaining[o.length];
            if (i !== "/" && i !== ")" && i !== ";") throw new m(4010, !1);
            let s;
            o.indexOf(":") > -1
                ? ((s = o.slice(0, o.indexOf(":"))),
                  this.capture(s),
                  this.capture(":"))
                : n && (s = T);
            let a = this.parseChildren(t + 1);
            ((r[s ?? T] =
                Object.keys(a).length === 1 && a[T] ? a[T] : new F([], a)),
                this.consumeOptional("//"));
        }
        return r;
    }
    peekStartsWith(n) {
        return this.remaining.startsWith(n);
    }
    consumeOptional(n) {
        return this.peekStartsWith(n)
            ? ((this.remaining = this.remaining.substring(n.length)), !0)
            : !1;
    }
    capture(n) {
        if (!this.consumeOptional(n)) throw new m(4011, !1);
    }
};
function Rg(e) {
    return e.segments.length > 0 ? new F([], { [T]: e }) : e;
}
function Ag(e) {
    let n = {};
    for (let [r, o] of Object.entries(e.children)) {
        let i = Ag(o);
        if (r === T && i.segments.length === 0 && i.hasChildren())
            for (let [s, a] of Object.entries(i.children)) n[s] = a;
        else (i.segments.length > 0 || i.hasChildren()) && (n[r] = i);
    }
    let t = new F(e.segments, n);
    return _I(t);
}
function _I(e) {
    if (e.numberOfChildren === 1 && e.children[T]) {
        let n = e.children[T];
        return new F(e.segments.concat(n.segments), n.children);
    }
    return e;
}
function qt(e) {
    return e instanceof Ne;
}
function xg(e, n, t = null, r = null, o = new Tt()) {
    let i = Og(e);
    return Pg(i, n, t, r, o);
}
function Og(e) {
    let n;
    function t(i) {
        let s = {};
        for (let c of i.children) {
            let u = t(c);
            s[c.outlet] = u;
        }
        let a = new F(i.url, s);
        return (i === e && (n = a), a);
    }
    let r = t(e.root),
        o = Rg(r);
    return n ?? o;
}
function Pg(e, n, t, r, o) {
    let i = e;
    for (; i.parent; ) i = i.parent;
    if (n.length === 0) return yl(i, i, i, t, r, o);
    let s = NI(n);
    if (s.toRoot()) return yl(i, i, new F([], {}), t, r, o);
    let a = RI(s, i, e),
        c = a.processChildren
            ? ko(a.segmentGroup, a.index, s.commands)
            : Fg(a.segmentGroup, a.index, s.commands);
    return yl(i, a.segmentGroup, c, t, r, o);
}
function Qs(e) {
    return typeof e == "object" && e != null && !e.outlets && !e.segmentPath;
}
function Lo(e) {
    return typeof e == "object" && e != null && e.outlets;
}
function gg(e, n, t) {
    e ||= "\u0275";
    let r = new Ne();
    return (
        (r.queryParams = { [e]: n }),
        t.parse(t.serialize(r)).queryParams[e]
    );
}
function yl(e, n, t, r, o, i) {
    let s = {};
    for (let [u, l] of Object.entries(r ?? {}))
        s[u] = Array.isArray(l) ? l.map((d) => gg(u, d, i)) : gg(u, l, i);
    let a;
    e === n ? (a = t) : (a = kg(e, n, t));
    let c = Rg(Ag(a));
    return new Ne(c, s, o);
}
function kg(e, n, t) {
    let r = {};
    return (
        Object.entries(e.children).forEach(([o, i]) => {
            i === n ? (r[o] = t) : (r[o] = kg(i, n, t));
        }),
        new F(e.segments, r)
    );
}
var Ks = class {
    isAbsolute;
    numberOfDoubleDots;
    commands;
    constructor(n, t, r) {
        if (
            ((this.isAbsolute = n),
            (this.numberOfDoubleDots = t),
            (this.commands = r),
            n && r.length > 0 && Qs(r[0]))
        )
            throw new m(4003, !1);
        let o = r.find(Lo);
        if (o && o !== dI(r)) throw new m(4004, !1);
    }
    toRoot() {
        return (
            this.isAbsolute &&
            this.commands.length === 1 &&
            this.commands[0] == "/"
        );
    }
};
function NI(e) {
    if (typeof e[0] == "string" && e.length === 1 && e[0] === "/")
        return new Ks(!0, 0, e);
    let n = 0,
        t = !1,
        r = e.reduce((o, i, s) => {
            if (typeof i == "object" && i != null) {
                if (i.outlets) {
                    let a = {};
                    return (
                        Object.entries(i.outlets).forEach(([c, u]) => {
                            a[c] = typeof u == "string" ? u.split("/") : u;
                        }),
                        [...o, { outlets: a }]
                    );
                }
                if (i.segmentPath) return [...o, i.segmentPath];
            }
            return typeof i != "string"
                ? [...o, i]
                : s === 0
                  ? (i.split("/").forEach((a, c) => {
                        (c == 0 && a === ".") ||
                            (c == 0 && a === ""
                                ? (t = !0)
                                : a === ".."
                                  ? n++
                                  : a != "" && o.push(a));
                    }),
                    o)
                  : [...o, i];
        }, []);
    return new Ks(t, n, r);
}
var yr = class {
    segmentGroup;
    processChildren;
    index;
    constructor(n, t, r) {
        ((this.segmentGroup = n), (this.processChildren = t), (this.index = r));
    }
};
function RI(e, n, t) {
    if (e.isAbsolute) return new yr(n, !0, 0);
    if (!t) return new yr(n, !1, NaN);
    if (t.parent === null) return new yr(t, !0, 0);
    let r = Qs(e.commands[0]) ? 0 : 1,
        o = t.segments.length - 1 + r;
    return AI(t, o, e.numberOfDoubleDots);
}
function AI(e, n, t) {
    let r = e,
        o = n,
        i = t;
    for (; i > o; ) {
        if (((i -= o), (r = r.parent), !r)) throw new m(4005, !1);
        o = r.segments.length;
    }
    return new yr(r, !1, o - i);
}
function xI(e) {
    return Lo(e[0]) ? e[0].outlets : { [T]: e };
}
function Fg(e, n, t) {
    if (((e ??= new F([], {})), e.segments.length === 0 && e.hasChildren()))
        return ko(e, n, t);
    let r = OI(e, n, t),
        o = t.slice(r.commandIndex);
    if (r.match && r.pathIndex < e.segments.length) {
        let i = new F(e.segments.slice(0, r.pathIndex), {});
        return (
            (i.children[T] = new F(e.segments.slice(r.pathIndex), e.children)),
            ko(i, 0, o)
        );
    } else
        return r.match && o.length === 0
            ? new F(e.segments, {})
            : r.match && !e.hasChildren()
              ? Tl(e, n, t)
              : r.match
                ? ko(e, 0, o)
                : Tl(e, n, t);
}
function ko(e, n, t) {
    if (t.length === 0) return new F(e.segments, {});
    {
        let r = xI(t),
            o = {};
        if (
            Object.keys(r).some((i) => i !== T) &&
            e.children[T] &&
            e.numberOfChildren === 1 &&
            e.children[T].segments.length === 0
        ) {
            let i = ko(e.children[T], n, t);
            return new F(e.segments, i.children);
        }
        return (
            Object.entries(r).forEach(([i, s]) => {
                (typeof s == "string" && (s = [s]),
                    s !== null && (o[i] = Fg(e.children[i], n, s)));
            }),
            Object.entries(e.children).forEach(([i, s]) => {
                r[i] === void 0 && (o[i] = s);
            }),
            new F(e.segments, o)
        );
    }
}
function OI(e, n, t) {
    let r = 0,
        o = n,
        i = { match: !1, pathIndex: 0, commandIndex: 0 };
    for (; o < e.segments.length; ) {
        if (r >= t.length) return i;
        let s = e.segments[o],
            a = t[r];
        if (Lo(a)) break;
        let c = `${a}`,
            u = r < t.length - 1 ? t[r + 1] : null;
        if (o > 0 && c === void 0) break;
        if (c && u && typeof u == "object" && u.outlets === void 0) {
            if (!vg(c, u, s)) return i;
            r += 2;
        } else {
            if (!vg(c, {}, s)) return i;
            r++;
        }
        o++;
    }
    return { match: !0, pathIndex: o, commandIndex: r };
}
function Tl(e, n, t) {
    let r = e.segments.slice(0, n),
        o = 0;
    for (; o < t.length; ) {
        let i = t[o];
        if (Lo(i)) {
            let c = PI(i.outlets);
            return new F(r, c);
        }
        if (o === 0 && Qs(t[0])) {
            let c = e.segments[n];
            (r.push(new Gt(c.path, mg(t[0]))), o++);
            continue;
        }
        let s = Lo(i) ? i.outlets[T] : `${i}`,
            a = o < t.length - 1 ? t[o + 1] : null;
        s && a && Qs(a)
            ? (r.push(new Gt(s, mg(a))), (o += 2))
            : (r.push(new Gt(s, {})), o++);
    }
    return new F(r, {});
}
function PI(e) {
    let n = {};
    return (
        Object.entries(e).forEach(([t, r]) => {
            (typeof r == "string" && (r = [r]),
                r !== null && (n[t] = Tl(new F([], {}), 0, r)));
        }),
        n
    );
}
function mg(e) {
    let n = {};
    return (Object.entries(e).forEach(([t, r]) => (n[t] = `${r}`)), n);
}
function vg(e, n, t) {
    return e == t.path && ut(n, t.parameters);
}
var Dr = "imperative",
    J = (function (e) {
        return (
            (e[(e.NavigationStart = 0)] = "NavigationStart"),
            (e[(e.NavigationEnd = 1)] = "NavigationEnd"),
            (e[(e.NavigationCancel = 2)] = "NavigationCancel"),
            (e[(e.NavigationError = 3)] = "NavigationError"),
            (e[(e.RoutesRecognized = 4)] = "RoutesRecognized"),
            (e[(e.ResolveStart = 5)] = "ResolveStart"),
            (e[(e.ResolveEnd = 6)] = "ResolveEnd"),
            (e[(e.GuardsCheckStart = 7)] = "GuardsCheckStart"),
            (e[(e.GuardsCheckEnd = 8)] = "GuardsCheckEnd"),
            (e[(e.RouteConfigLoadStart = 9)] = "RouteConfigLoadStart"),
            (e[(e.RouteConfigLoadEnd = 10)] = "RouteConfigLoadEnd"),
            (e[(e.ChildActivationStart = 11)] = "ChildActivationStart"),
            (e[(e.ChildActivationEnd = 12)] = "ChildActivationEnd"),
            (e[(e.ActivationStart = 13)] = "ActivationStart"),
            (e[(e.ActivationEnd = 14)] = "ActivationEnd"),
            (e[(e.Scroll = 15)] = "Scroll"),
            (e[(e.NavigationSkipped = 16)] = "NavigationSkipped"),
            e
        );
    })(J || {}),
    Re = class {
        id;
        url;
        constructor(n, t) {
            ((this.id = n), (this.url = t));
        }
    },
    Wt = class extends Re {
        type = J.NavigationStart;
        navigationTrigger;
        restoredState;
        constructor(n, t, r = "imperative", o = null) {
            (super(n, t),
                (this.navigationTrigger = r),
                (this.restoredState = o));
        }
        toString() {
            return `NavigationStart(id: ${this.id}, url: '${this.url}')`;
        }
    },
    Ue = class extends Re {
        urlAfterRedirects;
        type = J.NavigationEnd;
        constructor(n, t, r) {
            (super(n, t), (this.urlAfterRedirects = r));
        }
        toString() {
            return `NavigationEnd(id: ${this.id}, url: '${this.url}', urlAfterRedirects: '${this.urlAfterRedirects}')`;
        }
    },
    fe = (function (e) {
        return (
            (e[(e.Redirect = 0)] = "Redirect"),
            (e[(e.SupersededByNewNavigation = 1)] =
                "SupersededByNewNavigation"),
            (e[(e.NoDataFromResolver = 2)] = "NoDataFromResolver"),
            (e[(e.GuardRejected = 3)] = "GuardRejected"),
            (e[(e.Aborted = 4)] = "Aborted"),
            e
        );
    })(fe || {}),
    Cr = (function (e) {
        return (
            (e[(e.IgnoredSameUrlNavigation = 0)] = "IgnoredSameUrlNavigation"),
            (e[(e.IgnoredByUrlHandlingStrategy = 1)] =
                "IgnoredByUrlHandlingStrategy"),
            e
        );
    })(Cr || {}),
    je = class extends Re {
        reason;
        code;
        type = J.NavigationCancel;
        constructor(n, t, r, o) {
            (super(n, t), (this.reason = r), (this.code = o));
        }
        toString() {
            return `NavigationCancel(id: ${this.id}, url: '${this.url}')`;
        }
    };
function Lg(e) {
    return (
        e instanceof je &&
        (e.code === fe.Redirect || e.code === fe.SupersededByNewNavigation)
    );
}
var lt = class extends Re {
        reason;
        code;
        type = J.NavigationSkipped;
        constructor(n, t, r, o) {
            (super(n, t), (this.reason = r), (this.code = o));
        }
    },
    kn = class extends Re {
        error;
        target;
        type = J.NavigationError;
        constructor(n, t, r, o) {
            (super(n, t), (this.error = r), (this.target = o));
        }
        toString() {
            return `NavigationError(id: ${this.id}, url: '${this.url}', error: ${this.error})`;
        }
    },
    jo = class extends Re {
        urlAfterRedirects;
        state;
        type = J.RoutesRecognized;
        constructor(n, t, r, o) {
            (super(n, t), (this.urlAfterRedirects = r), (this.state = o));
        }
        toString() {
            return `RoutesRecognized(id: ${this.id}, url: '${this.url}', urlAfterRedirects: '${this.urlAfterRedirects}', state: ${this.state})`;
        }
    },
    Js = class extends Re {
        urlAfterRedirects;
        state;
        type = J.GuardsCheckStart;
        constructor(n, t, r, o) {
            (super(n, t), (this.urlAfterRedirects = r), (this.state = o));
        }
        toString() {
            return `GuardsCheckStart(id: ${this.id}, url: '${this.url}', urlAfterRedirects: '${this.urlAfterRedirects}', state: ${this.state})`;
        }
    },
    Xs = class extends Re {
        urlAfterRedirects;
        state;
        shouldActivate;
        type = J.GuardsCheckEnd;
        constructor(n, t, r, o, i) {
            (super(n, t),
                (this.urlAfterRedirects = r),
                (this.state = o),
                (this.shouldActivate = i));
        }
        toString() {
            return `GuardsCheckEnd(id: ${this.id}, url: '${this.url}', urlAfterRedirects: '${this.urlAfterRedirects}', state: ${this.state}, shouldActivate: ${this.shouldActivate})`;
        }
    },
    ea = class extends Re {
        urlAfterRedirects;
        state;
        type = J.ResolveStart;
        constructor(n, t, r, o) {
            (super(n, t), (this.urlAfterRedirects = r), (this.state = o));
        }
        toString() {
            return `ResolveStart(id: ${this.id}, url: '${this.url}', urlAfterRedirects: '${this.urlAfterRedirects}', state: ${this.state})`;
        }
    },
    ta = class extends Re {
        urlAfterRedirects;
        state;
        type = J.ResolveEnd;
        constructor(n, t, r, o) {
            (super(n, t), (this.urlAfterRedirects = r), (this.state = o));
        }
        toString() {
            return `ResolveEnd(id: ${this.id}, url: '${this.url}', urlAfterRedirects: '${this.urlAfterRedirects}', state: ${this.state})`;
        }
    },
    na = class {
        route;
        type = J.RouteConfigLoadStart;
        constructor(n) {
            this.route = n;
        }
        toString() {
            return `RouteConfigLoadStart(path: ${this.route.path})`;
        }
    },
    ra = class {
        route;
        type = J.RouteConfigLoadEnd;
        constructor(n) {
            this.route = n;
        }
        toString() {
            return `RouteConfigLoadEnd(path: ${this.route.path})`;
        }
    },
    oa = class {
        snapshot;
        type = J.ChildActivationStart;
        constructor(n) {
            this.snapshot = n;
        }
        toString() {
            return `ChildActivationStart(path: '${(this.snapshot.routeConfig && this.snapshot.routeConfig.path) || ""}')`;
        }
    },
    ia = class {
        snapshot;
        type = J.ChildActivationEnd;
        constructor(n) {
            this.snapshot = n;
        }
        toString() {
            return `ChildActivationEnd(path: '${(this.snapshot.routeConfig && this.snapshot.routeConfig.path) || ""}')`;
        }
    },
    sa = class {
        snapshot;
        type = J.ActivationStart;
        constructor(n) {
            this.snapshot = n;
        }
        toString() {
            return `ActivationStart(path: '${(this.snapshot.routeConfig && this.snapshot.routeConfig.path) || ""}')`;
        }
    },
    aa = class {
        snapshot;
        type = J.ActivationEnd;
        constructor(n) {
            this.snapshot = n;
        }
        toString() {
            return `ActivationEnd(path: '${(this.snapshot.routeConfig && this.snapshot.routeConfig.path) || ""}')`;
        }
    },
    Ir = class {
        routerEvent;
        position;
        anchor;
        scrollBehavior;
        type = J.Scroll;
        constructor(n, t, r, o) {
            ((this.routerEvent = n),
                (this.position = t),
                (this.anchor = r),
                (this.scrollBehavior = o));
        }
        toString() {
            let n = this.position
                ? `${this.position[0]}, ${this.position[1]}`
                : null;
            return `Scroll(anchor: '${this.anchor}', position: '${n}')`;
        }
    },
    wr = class {},
    Uo = class {},
    br = class {
        url;
        navigationBehaviorOptions;
        constructor(n, t) {
            ((this.url = n), (this.navigationBehaviorOptions = t));
        }
    };
function kI(e) {
    return !(e instanceof wr) && !(e instanceof br) && !(e instanceof Uo);
}
var ca = class {
        rootInjector;
        outlet = null;
        route = null;
        children;
        attachRef = null;
        get injector() {
            return (
                this.route?.snapshot._environmentInjector ?? this.rootInjector
            );
        }
        constructor(n) {
            ((this.rootInjector = n),
                (this.children = new Ln(this.rootInjector)));
        }
    },
    Ln = (() => {
        class e {
            rootInjector;
            contexts = new Map();
            constructor(t) {
                this.rootInjector = t;
            }
            onChildOutletCreated(t, r) {
                let o = this.getOrCreateContext(t);
                ((o.outlet = r), this.contexts.set(t, o));
            }
            onChildOutletDestroyed(t) {
                let r = this.getContext(t);
                r && ((r.outlet = null), (r.attachRef = null));
            }
            onOutletDeactivated() {
                let t = this.contexts;
                return ((this.contexts = new Map()), t);
            }
            onOutletReAttached(t) {
                this.contexts = t;
            }
            getOrCreateContext(t) {
                let r = this.getContext(t);
                return (
                    r ||
                        ((r = new ca(this.rootInjector)),
                        this.contexts.set(t, r)),
                    r
                );
            }
            getContext(t) {
                return this.contexts.get(t) || null;
            }
            static ɵfac = function (r) {
                return new (r || e)(E(G));
            };
            static ɵprov = v({ token: e, factory: e.ɵfac, providedIn: "root" });
        }
        return e;
    })(),
    ua = class {
        _root;
        constructor(n) {
            this._root = n;
        }
        get root() {
            return this._root.value;
        }
        parent(n) {
            let t = this.pathFromRoot(n);
            return t.length > 1 ? t[t.length - 2] : null;
        }
        children(n) {
            let t = Sl(n, this._root);
            return t ? t.children.map((r) => r.value) : [];
        }
        firstChild(n) {
            let t = Sl(n, this._root);
            return t && t.children.length > 0 ? t.children[0].value : null;
        }
        siblings(n) {
            let t = Ml(n, this._root);
            return t.length < 2
                ? []
                : t[t.length - 2].children
                      .map((o) => o.value)
                      .filter((o) => o !== n);
        }
        pathFromRoot(n) {
            return Ml(n, this._root).map((t) => t.value);
        }
    };
function Sl(e, n) {
    if (e === n.value) return n;
    for (let t of n.children) {
        let r = Sl(e, t);
        if (r) return r;
    }
    return null;
}
function Ml(e, n) {
    if (e === n.value) return [n];
    for (let t of n.children) {
        let r = Ml(e, t);
        if (r.length) return (r.unshift(n), r);
    }
    return [];
}
var _e = class {
    value;
    children;
    constructor(n, t) {
        ((this.value = n), (this.children = t));
    }
    toString() {
        return `TreeNode(${this.value})`;
    }
};
function vr(e) {
    let n = {};
    return (e && e.children.forEach((t) => (n[t.value.outlet] = t)), n);
}
var Bo = class extends ua {
    snapshot;
    constructor(n, t) {
        (super(n), (this.snapshot = t), Fl(this, n));
    }
    toString() {
        return this.snapshot.toString();
    }
};
function jg(e, n) {
    let t = FI(e, n),
        r = new oe([new Gt("", {})]),
        o = new oe({}),
        i = new oe({}),
        s = new oe({}),
        a = new oe(""),
        c = new Be(r, o, s, a, i, T, e, t.root);
    return ((c.snapshot = t.root), new Bo(new _e(c, []), t));
}
function FI(e, n) {
    let t = {},
        r = {},
        o = {},
        s = new Tr([], t, o, "", r, T, e, null, {}, n);
    return new Vo("", new _e(s, []));
}
var Be = class {
    urlSubject;
    paramsSubject;
    queryParamsSubject;
    fragmentSubject;
    dataSubject;
    outlet;
    component;
    snapshot;
    _futureSnapshot;
    _routerState;
    _paramMap;
    _queryParamMap;
    title;
    url;
    params;
    queryParams;
    fragment;
    data;
    constructor(n, t, r, o, i, s, a, c) {
        ((this.urlSubject = n),
            (this.paramsSubject = t),
            (this.queryParamsSubject = r),
            (this.fragmentSubject = o),
            (this.dataSubject = i),
            (this.outlet = s),
            (this.component = a),
            (this._futureSnapshot = c),
            (this.title = this.dataSubject?.pipe(H((u) => u[zo])) ?? I(void 0)),
            (this.url = n),
            (this.params = t),
            (this.queryParams = r),
            (this.fragment = o),
            (this.data = i));
    }
    get routeConfig() {
        return this._futureSnapshot.routeConfig;
    }
    get root() {
        return this._routerState.root;
    }
    get parent() {
        return this._routerState.parent(this);
    }
    get firstChild() {
        return this._routerState.firstChild(this);
    }
    get children() {
        return this._routerState.children(this);
    }
    get pathFromRoot() {
        return this._routerState.pathFromRoot(this);
    }
    get paramMap() {
        return (
            (this._paramMap ??= this.params.pipe(H((n) => Pn(n)))),
            this._paramMap
        );
    }
    get queryParamMap() {
        return (
            (this._queryParamMap ??= this.queryParams.pipe(H((n) => Pn(n)))),
            this._queryParamMap
        );
    }
    toString() {
        return this.snapshot
            ? this.snapshot.toString()
            : `Future(${this._futureSnapshot})`;
    }
};
function kl(e, n, t = "emptyOnly") {
    let r,
        { routeConfig: o } = e;
    return (
        n !== null &&
        (t === "always" ||
            o?.path === "" ||
            (!n.component && !n.routeConfig?.loadComponent))
            ? (r = {
                  params: g(g({}, n.params), e.params),
                  data: g(g({}, n.data), e.data),
                  resolve: g(
                      g(g(g({}, e.data), n.data), o?.data),
                      e._resolvedData,
                  ),
              })
            : (r = {
                  params: g({}, e.params),
                  data: g({}, e.data),
                  resolve: g(g({}, e.data), e._resolvedData ?? {}),
              }),
        o && Bg(o) && (r.resolve[zo] = o.title),
        r
    );
}
var Tr = class {
        url;
        params;
        queryParams;
        fragment;
        data;
        outlet;
        component;
        routeConfig;
        _resolve;
        _resolvedData;
        _routerState;
        _paramMap;
        _queryParamMap;
        _environmentInjector;
        get title() {
            return this.data?.[zo];
        }
        constructor(n, t, r, o, i, s, a, c, u, l) {
            ((this.url = n),
                (this.params = t),
                (this.queryParams = r),
                (this.fragment = o),
                (this.data = i),
                (this.outlet = s),
                (this.component = a),
                (this.routeConfig = c),
                (this._resolve = u),
                (this._environmentInjector = l));
        }
        get root() {
            return this._routerState.root;
        }
        get parent() {
            return this._routerState.parent(this);
        }
        get firstChild() {
            return this._routerState.firstChild(this);
        }
        get children() {
            return this._routerState.children(this);
        }
        get pathFromRoot() {
            return this._routerState.pathFromRoot(this);
        }
        get paramMap() {
            return ((this._paramMap ??= Pn(this.params)), this._paramMap);
        }
        get queryParamMap() {
            return (
                (this._queryParamMap ??= Pn(this.queryParams)),
                this._queryParamMap
            );
        }
        toString() {
            let n = this.url.map((r) => r.toString()).join("/"),
                t = this.routeConfig ? this.routeConfig.path : "";
            return `Route(url:'${n}', path:'${t}')`;
        }
    },
    Vo = class extends ua {
        url;
        constructor(n, t) {
            (super(t), (this.url = n), Fl(this, t));
        }
        toString() {
            return Ug(this._root);
        }
    };
function Fl(e, n) {
    ((n.value._routerState = e), n.children.forEach((t) => Fl(e, t)));
}
function Ug(e) {
    let n =
        e.children.length > 0 ? ` { ${e.children.map(Ug).join(", ")} } ` : "";
    return `${e.value}${n}`;
}
function Dl(e) {
    if (e.snapshot) {
        let n = e.snapshot,
            t = e._futureSnapshot;
        ((e.snapshot = t),
            ut(n.queryParams, t.queryParams) ||
                e.queryParamsSubject.next(t.queryParams),
            n.fragment !== t.fragment && e.fragmentSubject.next(t.fragment),
            ut(n.params, t.params) || e.paramsSubject.next(t.params),
            lI(n.url, t.url) || e.urlSubject.next(t.url),
            ut(n.data, t.data) || e.dataSubject.next(t.data));
    } else
        ((e.snapshot = e._futureSnapshot),
            e.dataSubject.next(e._futureSnapshot.data));
}
function _l(e, n) {
    let t = ut(e.params, n.params) && gI(e.url, n.url),
        r = !e.parent != !n.parent;
    return t && !r && (!e.parent || _l(e.parent, n.parent));
}
function Bg(e) {
    return typeof e.title == "string" || e.title === null;
}
var Vg = new D(""),
    Go = (() => {
        class e {
            activated = null;
            get activatedComponentRef() {
                return this.activated;
            }
            _activatedRoute = null;
            name = T;
            activateEvents = new he();
            deactivateEvents = new he();
            attachEvents = new he();
            detachEvents = new he();
            routerOutletData = Bh();
            parentContexts = p(Ln);
            location = p(Nn);
            changeDetector = p(wo);
            inputBinder = p(qo, { optional: !0 });
            supportsBindingToComponentInputs = !0;
            ngOnChanges(t) {
                if (t.name) {
                    let { firstChange: r, previousValue: o } = t.name;
                    if (r) return;
                    (this.isTrackedInParentContexts(o) &&
                        (this.deactivate(),
                        this.parentContexts.onChildOutletDestroyed(o)),
                        this.initializeOutletWithName());
                }
            }
            ngOnDestroy() {
                (this.isTrackedInParentContexts(this.name) &&
                    this.parentContexts.onChildOutletDestroyed(this.name),
                    this.inputBinder?.unsubscribeFromRouteData(this));
            }
            isTrackedInParentContexts(t) {
                return this.parentContexts.getContext(t)?.outlet === this;
            }
            ngOnInit() {
                this.initializeOutletWithName();
            }
            initializeOutletWithName() {
                if (
                    (this.parentContexts.onChildOutletCreated(this.name, this),
                    this.activated)
                )
                    return;
                let t = this.parentContexts.getContext(this.name);
                t?.route &&
                    (t.attachRef
                        ? this.attach(t.attachRef, t.route)
                        : this.activateWith(t.route, t.injector));
            }
            get isActivated() {
                return !!this.activated;
            }
            get component() {
                if (!this.activated) throw new m(4012, !1);
                return this.activated.instance;
            }
            get activatedRoute() {
                if (!this.activated) throw new m(4012, !1);
                return this._activatedRoute;
            }
            get activatedRouteData() {
                return this._activatedRoute
                    ? this._activatedRoute.snapshot.data
                    : {};
            }
            detach() {
                if (!this.activated) throw new m(4012, !1);
                this.location.detach();
                let t = this.activated;
                return (
                    (this.activated = null),
                    (this._activatedRoute = null),
                    this.detachEvents.emit(t.instance),
                    t
                );
            }
            attach(t, r) {
                ((this.activated = t),
                    (this._activatedRoute = r),
                    this.location.insert(t.hostView),
                    this.inputBinder?.bindActivatedRouteToOutletComponent(this),
                    this.attachEvents.emit(t.instance));
            }
            deactivate() {
                if (this.activated) {
                    let t = this.component;
                    (this.activated.destroy(),
                        (this.activated = null),
                        (this._activatedRoute = null),
                        this.deactivateEvents.emit(t));
                }
            }
            activateWith(t, r) {
                if (this.isActivated) throw new m(4013, !1);
                this._activatedRoute = t;
                let o = this.location,
                    s = t.snapshot.component,
                    a = this.parentContexts.getOrCreateContext(
                        this.name,
                    ).children,
                    c = new Nl(t, a, o.injector, this.routerOutletData);
                ((this.activated = o.createComponent(s, {
                    index: o.length,
                    injector: c,
                    environmentInjector: r,
                })),
                    this.changeDetector.markForCheck(),
                    this.inputBinder?.bindActivatedRouteToOutletComponent(this),
                    this.activateEvents.emit(this.activated.instance));
            }
            static ɵfac = function (r) {
                return new (r || e)();
            };
            static ɵdir = Ut({
                type: e,
                selectors: [["router-outlet"]],
                inputs: {
                    name: "name",
                    routerOutletData: [1, "routerOutletData"],
                },
                outputs: {
                    activateEvents: "activate",
                    deactivateEvents: "deactivate",
                    attachEvents: "attach",
                    detachEvents: "detach",
                },
                exportAs: ["outlet"],
                features: [ur],
            });
        }
        return e;
    })(),
    Nl = class {
        route;
        childContexts;
        parent;
        outletData;
        constructor(n, t, r, o) {
            ((this.route = n),
                (this.childContexts = t),
                (this.parent = r),
                (this.outletData = o));
        }
        get(n, t) {
            return n === Be
                ? this.route
                : n === Ln
                  ? this.childContexts
                  : n === Vg
                    ? this.outletData
                    : this.parent.get(n, t);
        }
    },
    qo = new D(""),
    Ll = (() => {
        class e {
            outletDataSubscriptions = new Map();
            bindActivatedRouteToOutletComponent(t) {
                (this.unsubscribeFromRouteData(t),
                    this.subscribeToRouteData(t));
            }
            unsubscribeFromRouteData(t) {
                (this.outletDataSubscriptions.get(t)?.unsubscribe(),
                    this.outletDataSubscriptions.delete(t));
            }
            subscribeToRouteData(t) {
                let { activatedRoute: r } = t,
                    o = Ci([r.queryParams, r.params, r.data])
                        .pipe(
                            Ee(
                                ([i, s, a], c) => (
                                    (a = g(g(g({}, i), s), a)),
                                    c === 0 ? I(a) : Promise.resolve(a)
                                ),
                            ),
                        )
                        .subscribe((i) => {
                            if (
                                !t.isActivated ||
                                !t.activatedComponentRef ||
                                t.activatedRoute !== r ||
                                r.component === null
                            ) {
                                this.unsubscribeFromRouteData(t);
                                return;
                            }
                            let s = Hh(r.component);
                            if (!s) {
                                this.unsubscribeFromRouteData(t);
                                return;
                            }
                            for (let { templateName: a } of s.inputs)
                                t.activatedComponentRef.setInput(a, i[a]);
                        });
                this.outletDataSubscriptions.set(t, o);
            }
            static ɵfac = function (r) {
                return new (r || e)();
            };
            static ɵprov = v({ token: e, factory: e.ɵfac });
        }
        return e;
    })(),
    jl = (() => {
        class e {
            static ɵfac = function (r) {
                return new (r || e)();
            };
            static ɵcmp = Je({
                type: e,
                selectors: [["ng-component"]],
                exportAs: ["emptyRouterOutlet"],
                decls: 1,
                vars: 0,
                template: function (r, o) {
                    r & 1 && ye(0, "router-outlet");
                },
                dependencies: [Go],
                encapsulation: 2,
            });
        }
        return e;
    })();
function Ul(e) {
    let n = e.children && e.children.map(Ul),
        t = n ? B(g({}, e), { children: n }) : g({}, e);
    return (
        !t.component &&
            !t.loadComponent &&
            (n || t.loadChildren) &&
            t.outlet &&
            t.outlet !== T &&
            (t.component = jl),
        t
    );
}
function LI(e, n, t) {
    let r = Ho(e, n._root, t ? t._root : void 0);
    return new Bo(r, n);
}
function Ho(e, n, t) {
    if (t && e.shouldReuseRoute(n.value, t.value.snapshot)) {
        let r = t.value;
        r._futureSnapshot = n.value;
        let o = jI(e, n, t);
        return new _e(r, o);
    } else {
        if (e.shouldAttach(n.value)) {
            let i = e.retrieve(n.value);
            if (i !== null) {
                let s = i.route;
                return (
                    (s.value._futureSnapshot = n.value),
                    (s.children = n.children.map((a) => Ho(e, a))),
                    s
                );
            }
        }
        let r = UI(n.value),
            o = n.children.map((i) => Ho(e, i));
        return new _e(r, o);
    }
}
function jI(e, n, t) {
    return n.children.map((r) => {
        for (let o of t.children)
            if (e.shouldReuseRoute(r.value, o.value.snapshot))
                return Ho(e, r, o);
        return Ho(e, r);
    });
}
function UI(e) {
    return new Be(
        new oe(e.url),
        new oe(e.params),
        new oe(e.queryParams),
        new oe(e.fragment),
        new oe(e.data),
        e.outlet,
        e.component,
        e,
    );
}
var Sr = class {
        redirectTo;
        navigationBehaviorOptions;
        constructor(n, t) {
            ((this.redirectTo = n), (this.navigationBehaviorOptions = t));
        }
    },
    Hg = "ngNavigationCancelingError";
function la(e, n) {
    let { redirectTo: t, navigationBehaviorOptions: r } = qt(n)
            ? { redirectTo: n, navigationBehaviorOptions: void 0 }
            : n,
        o = $g(!1, fe.Redirect);
    return ((o.url = t), (o.navigationBehaviorOptions = r), o);
}
function $g(e, n) {
    let t = new Error(`NavigationCancelingError: ${e || ""}`);
    return ((t[Hg] = !0), (t.cancellationCode = n), t);
}
function BI(e) {
    return zg(e) && qt(e.url);
}
function zg(e) {
    return !!e && e[Hg];
}
var Rl = class {
        routeReuseStrategy;
        futureState;
        currState;
        forwardEvent;
        inputBindingEnabled;
        constructor(n, t, r, o, i) {
            ((this.routeReuseStrategy = n),
                (this.futureState = t),
                (this.currState = r),
                (this.forwardEvent = o),
                (this.inputBindingEnabled = i));
        }
        activate(n) {
            let t = this.futureState._root,
                r = this.currState ? this.currState._root : null;
            (this.deactivateChildRoutes(t, r, n),
                Dl(this.futureState.root),
                this.activateChildRoutes(t, r, n));
        }
        deactivateChildRoutes(n, t, r) {
            let o = vr(t);
            (n.children.forEach((i) => {
                let s = i.value.outlet;
                (this.deactivateRoutes(i, o[s], r), delete o[s]);
            }),
                Object.values(o).forEach((i) => {
                    this.deactivateRouteAndItsChildren(i, r);
                }));
        }
        deactivateRoutes(n, t, r) {
            let o = n.value,
                i = t ? t.value : null;
            if (o === i)
                if (o.component) {
                    let s = r.getContext(o.outlet);
                    s && this.deactivateChildRoutes(n, t, s.children);
                } else this.deactivateChildRoutes(n, t, r);
            else i && this.deactivateRouteAndItsChildren(t, r);
        }
        deactivateRouteAndItsChildren(n, t) {
            n.value.component &&
            this.routeReuseStrategy.shouldDetach(n.value.snapshot)
                ? this.detachAndStoreRouteSubtree(n, t)
                : this.deactivateRouteAndOutlet(n, t);
        }
        detachAndStoreRouteSubtree(n, t) {
            let r = t.getContext(n.value.outlet),
                o = r && n.value.component ? r.children : t,
                i = vr(n);
            for (let s of Object.values(i))
                this.deactivateRouteAndItsChildren(s, o);
            if (r && r.outlet) {
                let s = r.outlet.detach(),
                    a = r.children.onOutletDeactivated();
                this.routeReuseStrategy.store(n.value.snapshot, {
                    componentRef: s,
                    route: n,
                    contexts: a,
                });
            }
        }
        deactivateRouteAndOutlet(n, t) {
            let r = t.getContext(n.value.outlet),
                o = r && n.value.component ? r.children : t,
                i = vr(n);
            for (let s of Object.values(i))
                this.deactivateRouteAndItsChildren(s, o);
            r &&
                (r.outlet &&
                    (r.outlet.deactivate(), r.children.onOutletDeactivated()),
                (r.attachRef = null),
                (r.route = null));
        }
        activateChildRoutes(n, t, r) {
            let o = vr(t);
            (n.children.forEach((i) => {
                (this.activateRoutes(i, o[i.value.outlet], r),
                    this.forwardEvent(new aa(i.value.snapshot)));
            }),
                n.children.length &&
                    this.forwardEvent(new ia(n.value.snapshot)));
        }
        activateRoutes(n, t, r) {
            let o = n.value,
                i = t ? t.value : null;
            if ((Dl(o), o === i))
                if (o.component) {
                    let s = r.getOrCreateContext(o.outlet);
                    this.activateChildRoutes(n, t, s.children);
                } else this.activateChildRoutes(n, t, r);
            else if (o.component) {
                let s = r.getOrCreateContext(o.outlet);
                if (this.routeReuseStrategy.shouldAttach(o.snapshot)) {
                    let a = this.routeReuseStrategy.retrieve(o.snapshot);
                    (this.routeReuseStrategy.store(o.snapshot, null),
                        s.children.onOutletReAttached(a.contexts),
                        (s.attachRef = a.componentRef),
                        (s.route = a.route.value),
                        s.outlet &&
                            s.outlet.attach(a.componentRef, a.route.value),
                        Dl(a.route.value),
                        this.activateChildRoutes(n, null, s.children));
                } else
                    ((s.attachRef = null),
                        (s.route = o),
                        s.outlet && s.outlet.activateWith(o, s.injector),
                        this.activateChildRoutes(n, null, s.children));
            } else this.activateChildRoutes(n, null, r);
        }
    },
    da = class {
        path;
        route;
        constructor(n) {
            ((this.path = n), (this.route = this.path[this.path.length - 1]));
        }
    },
    Er = class {
        component;
        route;
        constructor(n, t) {
            ((this.component = n), (this.route = t));
        }
    };
function VI(e, n, t) {
    let r = e._root,
        o = n ? n._root : null;
    return Po(r, o, t, [r.value]);
}
function HI(e) {
    let n = e.routeConfig ? e.routeConfig.canActivateChild : null;
    return !n || n.length === 0 ? null : { node: e, guards: n };
}
function _r(e, n) {
    let t = Symbol(),
        r = n.get(e, t);
    return r === t ? (typeof e == "function" && !rc(e) ? e : n.get(e)) : r;
}
function Po(
    e,
    n,
    t,
    r,
    o = { canDeactivateChecks: [], canActivateChecks: [] },
) {
    let i = vr(n);
    return (
        e.children.forEach((s) => {
            ($I(s, i[s.value.outlet], t, r.concat([s.value]), o),
                delete i[s.value.outlet]);
        }),
        Object.entries(i).forEach(([s, a]) => Fo(a, t.getContext(s), o)),
        o
    );
}
function $I(
    e,
    n,
    t,
    r,
    o = { canDeactivateChecks: [], canActivateChecks: [] },
) {
    let i = e.value,
        s = n ? n.value : null,
        a = t ? t.getContext(e.value.outlet) : null;
    if (s && i.routeConfig === s.routeConfig) {
        let c = zI(s, i, i.routeConfig.runGuardsAndResolvers);
        (c
            ? o.canActivateChecks.push(new da(r))
            : ((i.data = s.data), (i._resolvedData = s._resolvedData)),
            i.component
                ? Po(e, n, a ? a.children : null, r, o)
                : Po(e, n, t, r, o),
            c &&
                a &&
                a.outlet &&
                a.outlet.isActivated &&
                o.canDeactivateChecks.push(new Er(a.outlet.component, s)));
    } else
        (s && Fo(n, a, o),
            o.canActivateChecks.push(new da(r)),
            i.component
                ? Po(e, null, a ? a.children : null, r, o)
                : Po(e, null, t, r, o));
    return o;
}
function zI(e, n, t) {
    if (typeof t == "function") return Q(n._environmentInjector, () => t(e, n));
    switch (t) {
        case "pathParamsChange":
            return !On(e.url, n.url);
        case "pathParamsOrQueryParamsChange":
            return !On(e.url, n.url) || !ut(e.queryParams, n.queryParams);
        case "always":
            return !0;
        case "paramsOrQueryParamsChange":
            return !_l(e, n) || !ut(e.queryParams, n.queryParams);
        default:
            return !_l(e, n);
    }
}
function Fo(e, n, t) {
    let r = vr(e),
        o = e.value;
    (Object.entries(r).forEach(([i, s]) => {
        o.component
            ? n
                ? Fo(s, n.children.getContext(i), t)
                : Fo(s, null, t)
            : Fo(s, n, t);
    }),
        o.component
            ? n && n.outlet && n.outlet.isActivated
                ? t.canDeactivateChecks.push(new Er(n.outlet.component, o))
                : t.canDeactivateChecks.push(new Er(null, o))
            : t.canDeactivateChecks.push(new Er(null, o)));
}
function Wo(e) {
    return typeof e == "function";
}
function GI(e) {
    return typeof e == "boolean";
}
function qI(e) {
    return e && Wo(e.canLoad);
}
function WI(e) {
    return e && Wo(e.canActivate);
}
function ZI(e) {
    return e && Wo(e.canActivateChild);
}
function YI(e) {
    return e && Wo(e.canDeactivate);
}
function QI(e) {
    return e && Wo(e.canMatch);
}
function Gg(e) {
    return e instanceof ln || e?.name === "EmptyError";
}
var Gs = Symbol("INITIAL_VALUE");
function Mr() {
    return Ee((e) =>
        Ci(e.map((n) => n.pipe(ft(1), ja(Gs)))).pipe(
            H((n) => {
                for (let t of n)
                    if (t !== !0) {
                        if (t === Gs) return Gs;
                        if (t === !1 || KI(t)) return t;
                    }
                return !0;
            }),
            be((n) => n !== Gs),
            ft(1),
        ),
    );
}
function KI(e) {
    return qt(e) || e instanceof Sr;
}
function qg(e) {
    return e.aborted
        ? I(void 0).pipe(ft(1))
        : new O((n) => {
              let t = () => {
                  (n.next(), n.complete());
              };
              return (
                  e.addEventListener("abort", t),
                  () => e.removeEventListener("abort", t)
              );
          });
}
function Wg(e) {
    return jr(qg(e));
}
function JI(e) {
    return se((n) => {
        let {
            targetSnapshot: t,
            currentSnapshot: r,
            guards: { canActivateChecks: o, canDeactivateChecks: i },
        } = n;
        return i.length === 0 && o.length === 0
            ? I(B(g({}, n), { guardsResult: !0 }))
            : XI(i, t, r).pipe(
                  se((s) => (s && GI(s) ? ew(t, o, e) : I(s))),
                  H((s) => B(g({}, n), { guardsResult: s })),
              );
    });
}
function XI(e, n, t) {
    return q(e).pipe(
        se((r) => iw(r.component, r.route, t, n)),
        pt((r) => r !== !0, !0),
    );
}
function ew(e, n, t) {
    return q(n).pipe(
        Mt((r) =>
            Zn(
                nw(r.route.parent, t),
                tw(r.route, t),
                ow(e, r.path),
                rw(e, r.route),
            ),
        ),
        pt((r) => r !== !0, !0),
    );
}
function tw(e, n) {
    return (e !== null && n && n(new sa(e)), I(!0));
}
function nw(e, n) {
    return (e !== null && n && n(new oa(e)), I(!0));
}
function rw(e, n) {
    let t = n.routeConfig ? n.routeConfig.canActivate : null;
    if (!t || t.length === 0) return I(!0);
    let r = t.map((o) =>
        Fr(() => {
            let i = n._environmentInjector,
                s = _r(o, i),
                a = WI(s) ? s.canActivate(n, e) : Q(i, () => s(n, e));
            return Fn(a).pipe(pt());
        }),
    );
    return I(r).pipe(Mr());
}
function ow(e, n) {
    let t = n[n.length - 1],
        o = n
            .slice(0, n.length - 1)
            .reverse()
            .map((i) => HI(i))
            .filter((i) => i !== null)
            .map((i) =>
                Fr(() => {
                    let s = i.guards.map((a) => {
                        let c = i.node._environmentInjector,
                            u = _r(a, c),
                            l = ZI(u)
                                ? u.canActivateChild(t, e)
                                : Q(c, () => u(t, e));
                        return Fn(l).pipe(pt());
                    });
                    return I(s).pipe(Mr());
                }),
            );
    return I(o).pipe(Mr());
}
function iw(e, n, t, r) {
    let o = n && n.routeConfig ? n.routeConfig.canDeactivate : null;
    if (!o || o.length === 0) return I(!0);
    let i = o.map((s) => {
        let a = n._environmentInjector,
            c = _r(s, a),
            u = YI(c) ? c.canDeactivate(e, n, t, r) : Q(a, () => c(e, n, t, r));
        return Fn(u).pipe(pt());
    });
    return I(i).pipe(Mr());
}
function sw(e, n, t, r, o) {
    let i = n.canLoad;
    if (i === void 0 || i.length === 0) return I(!0);
    let s = i.map((a) => {
        let c = _r(a, e),
            u = qI(c) ? c.canLoad(n, t) : Q(e, () => c(n, t)),
            l = Fn(u);
        return o ? l.pipe(Wg(o)) : l;
    });
    return I(s).pipe(Mr(), Zg(r));
}
function Zg(e) {
    return Pa(
        Ae((n) => {
            if (typeof n != "boolean") throw la(e, n);
        }),
        H((n) => n === !0),
    );
}
function aw(e, n, t, r, o, i) {
    let s = n.canMatch;
    if (!s || s.length === 0) return I(!0);
    let a = s.map((c) => {
        let u = _r(c, e),
            l = QI(u) ? u.canMatch(n, t, o) : Q(e, () => u(n, t, o));
        return Fn(l).pipe(Wg(i));
    });
    return I(a).pipe(Mr(), Zg(r));
}
var bt = class e extends Error {
        segmentGroup;
        constructor(n) {
            (super(),
                (this.segmentGroup = n || null),
                Object.setPrototypeOf(this, e.prototype));
        }
    },
    $o = class e extends Error {
        urlTree;
        constructor(n) {
            (super(),
                (this.urlTree = n),
                Object.setPrototypeOf(this, e.prototype));
        }
    };
function cw(e) {
    throw new m(4e3, !1);
}
function uw(e) {
    throw $g(!1, fe.GuardRejected);
}
var Al = class {
    urlSerializer;
    urlTree;
    constructor(n, t) {
        ((this.urlSerializer = n), (this.urlTree = t));
    }
    async lineralizeSegments(n, t) {
        let r = [],
            o = t.root;
        for (;;) {
            if (((r = r.concat(o.segments)), o.numberOfChildren === 0))
                return r;
            if (o.numberOfChildren > 1 || !o.children[T])
                throw cw(`${n.redirectTo}`);
            o = o.children[T];
        }
    }
    async applyRedirectCommands(n, t, r, o, i) {
        let s = await lw(t, o, i);
        if (s instanceof Ne) throw new $o(s);
        let a = this.applyRedirectCreateUrlTree(
            s,
            this.urlSerializer.parse(s),
            n,
            r,
        );
        if (s[0] === "/") throw new $o(a);
        return a;
    }
    applyRedirectCreateUrlTree(n, t, r, o) {
        let i = this.createSegmentGroup(n, t.root, r, o);
        return new Ne(
            i,
            this.createQueryParams(t.queryParams, this.urlTree.queryParams),
            t.fragment,
        );
    }
    createQueryParams(n, t) {
        let r = {};
        return (
            Object.entries(n).forEach(([o, i]) => {
                if (typeof i == "string" && i[0] === ":") {
                    let a = i.substring(1);
                    r[o] = t[a];
                } else r[o] = i;
            }),
            r
        );
    }
    createSegmentGroup(n, t, r, o) {
        let i = this.createSegments(n, t.segments, r, o),
            s = {};
        return (
            Object.entries(t.children).forEach(([a, c]) => {
                s[a] = this.createSegmentGroup(n, c, r, o);
            }),
            new F(i, s)
        );
    }
    createSegments(n, t, r, o) {
        return t.map((i) =>
            i.path[0] === ":"
                ? this.findPosParam(n, i, o)
                : this.findOrReturn(i, r),
        );
    }
    findPosParam(n, t, r) {
        let o = r[t.path.substring(1)];
        if (!o) throw new m(4001, !1);
        return o;
    }
    findOrReturn(n, t) {
        let r = 0;
        for (let o of t) {
            if (o.path === n.path) return (t.splice(r), o);
            r++;
        }
        return n;
    }
};
function lw(e, n, t) {
    if (typeof e == "string") return Promise.resolve(e);
    let r = e;
    return Ys(Fn(Q(t, () => r(n))));
}
function dw(e, n) {
    return (
        e.providers &&
            !e._injector &&
            (e._injector = fr(e.providers, n, `Route: ${e.path}`)),
        e._injector ?? n
    );
}
function nt(e) {
    return e.outlet || T;
}
function fw(e, n) {
    let t = e.filter((r) => nt(r) === n);
    return (t.push(...e.filter((r) => nt(r) !== n)), t);
}
var xl = {
    matched: !1,
    consumedSegments: [],
    remainingSegments: [],
    parameters: {},
    positionalParamSegments: {},
};
function Yg(e) {
    return {
        routeConfig: e.routeConfig,
        url: e.url,
        params: e.params,
        queryParams: e.queryParams,
        fragment: e.fragment,
        data: e.data,
        outlet: e.outlet,
        title: e.title,
        paramMap: e.paramMap,
        queryParamMap: e.queryParamMap,
    };
}
function pw(e, n, t, r, o, i, s) {
    let a = Qg(e, n, t);
    if (!a.matched) return I(a);
    let c = Yg(i(a));
    return (
        (r = dw(n, r)),
        aw(r, n, t, o, c, s).pipe(H((u) => (u === !0 ? a : g({}, xl))))
    );
}
function Qg(e, n, t) {
    if (n.path === "")
        return n.pathMatch === "full" && (e.hasChildren() || t.length > 0)
            ? g({}, xl)
            : {
                  matched: !0,
                  consumedSegments: [],
                  remainingSegments: t,
                  parameters: {},
                  positionalParamSegments: {},
              };
    let o = (n.matcher || Eg)(t, e, n);
    if (!o) return g({}, xl);
    let i = {};
    Object.entries(o.posParams ?? {}).forEach(([a, c]) => {
        i[a] = c.path;
    });
    let s =
        o.consumed.length > 0
            ? g(g({}, i), o.consumed[o.consumed.length - 1].parameters)
            : i;
    return {
        matched: !0,
        consumedSegments: o.consumed,
        remainingSegments: t.slice(o.consumed.length),
        parameters: s,
        positionalParamSegments: o.posParams ?? {},
    };
}
function yg(e, n, t, r, o) {
    return t.length > 0 && mw(e, t, r, o)
        ? {
              segmentGroup: new F(n, gw(r, new F(t, e.children))),
              slicedSegments: [],
          }
        : t.length === 0 && vw(e, t, r)
          ? {
                segmentGroup: new F(e.segments, hw(e, t, r, e.children)),
                slicedSegments: t,
            }
          : { segmentGroup: new F(e.segments, e.children), slicedSegments: t };
}
function hw(e, n, t, r) {
    let o = {};
    for (let i of t)
        if (pa(e, n, i) && !r[nt(i)]) {
            let s = new F([], {});
            o[nt(i)] = s;
        }
    return g(g({}, r), o);
}
function gw(e, n) {
    let t = {};
    t[T] = n;
    for (let r of e)
        if (r.path === "" && nt(r) !== T) {
            let o = new F([], {});
            t[nt(r)] = o;
        }
    return t;
}
function mw(e, n, t, r) {
    return t.some((o) =>
        !pa(e, n, o) || !(nt(o) !== T) ? !1 : !(r !== void 0 && nt(o) === r),
    );
}
function vw(e, n, t) {
    return t.some((r) => pa(e, n, r));
}
function pa(e, n, t) {
    return (e.hasChildren() || n.length > 0) && t.pathMatch === "full"
        ? !1
        : t.path === "";
}
function yw(e, n, t) {
    return n.length === 0 && !e.children[t];
}
var Ol = class {};
async function Dw(e, n, t, r, o, i, s = "emptyOnly", a) {
    return new Pl(e, n, t, r, o, s, i, a).recognize();
}
var Ew = 31,
    Pl = class {
        injector;
        configLoader;
        rootComponentType;
        config;
        urlTree;
        paramsInheritanceStrategy;
        urlSerializer;
        abortSignal;
        applyRedirects;
        absoluteRedirectCount = 0;
        allowRedirects = !0;
        constructor(n, t, r, o, i, s, a, c) {
            ((this.injector = n),
                (this.configLoader = t),
                (this.rootComponentType = r),
                (this.config = o),
                (this.urlTree = i),
                (this.paramsInheritanceStrategy = s),
                (this.urlSerializer = a),
                (this.abortSignal = c),
                (this.applyRedirects = new Al(
                    this.urlSerializer,
                    this.urlTree,
                )));
        }
        noMatchError(n) {
            return new m(4002, `'${n.segmentGroup}'`);
        }
        async recognize() {
            let n = yg(this.urlTree.root, [], [], this.config).segmentGroup,
                { children: t, rootSnapshot: r } = await this.match(n),
                o = new _e(r, t),
                i = new Vo("", o),
                s = xg(r, [], this.urlTree.queryParams, this.urlTree.fragment);
            return (
                (s.queryParams = this.urlTree.queryParams),
                (i.url = this.urlSerializer.serialize(s)),
                { state: i, tree: s }
            );
        }
        async match(n) {
            let t = new Tr(
                [],
                Object.freeze({}),
                Object.freeze(g({}, this.urlTree.queryParams)),
                this.urlTree.fragment,
                Object.freeze({}),
                T,
                this.rootComponentType,
                null,
                {},
                this.injector,
            );
            try {
                return {
                    children: await this.processSegmentGroup(
                        this.injector,
                        this.config,
                        n,
                        T,
                        t,
                    ),
                    rootSnapshot: t,
                };
            } catch (r) {
                if (r instanceof $o)
                    return (
                        (this.urlTree = r.urlTree),
                        this.match(r.urlTree.root)
                    );
                throw r instanceof bt ? this.noMatchError(r) : r;
            }
        }
        async processSegmentGroup(n, t, r, o, i) {
            if (r.segments.length === 0 && r.hasChildren())
                return this.processChildren(n, t, r, i);
            let s = await this.processSegment(n, t, r, r.segments, o, !0, i);
            return s instanceof _e ? [s] : [];
        }
        async processChildren(n, t, r, o) {
            let i = [];
            for (let c of Object.keys(r.children))
                c === "primary" ? i.unshift(c) : i.push(c);
            let s = [];
            for (let c of i) {
                let u = r.children[c],
                    l = fw(t, c),
                    d = await this.processSegmentGroup(n, l, u, c, o);
                s.push(...d);
            }
            let a = Kg(s);
            return (Cw(a), a);
        }
        async processSegment(n, t, r, o, i, s, a) {
            for (let c of t)
                try {
                    return await this.processSegmentAgainstRoute(
                        c._injector ?? n,
                        t,
                        c,
                        r,
                        o,
                        i,
                        s,
                        a,
                    );
                } catch (u) {
                    if (u instanceof bt || Gg(u)) continue;
                    throw u;
                }
            if (yw(r, o, i)) return new Ol();
            throw new bt(r);
        }
        async processSegmentAgainstRoute(n, t, r, o, i, s, a, c) {
            if (nt(r) !== s && (s === T || !pa(o, i, r))) throw new bt(o);
            if (r.redirectTo === void 0)
                return this.matchSegmentAgainstRoute(n, o, r, i, s, c);
            if (this.allowRedirects && a)
                return this.expandSegmentAgainstRouteUsingRedirect(
                    n,
                    o,
                    t,
                    r,
                    i,
                    s,
                    c,
                );
            throw new bt(o);
        }
        async expandSegmentAgainstRouteUsingRedirect(n, t, r, o, i, s, a) {
            let {
                matched: c,
                parameters: u,
                consumedSegments: l,
                positionalParamSegments: d,
                remainingSegments: h,
            } = Qg(t, o, i);
            if (!c) throw new bt(t);
            typeof o.redirectTo == "string" &&
                o.redirectTo[0] === "/" &&
                (this.absoluteRedirectCount++,
                this.absoluteRedirectCount > Ew && (this.allowRedirects = !1));
            let f = this.createSnapshot(n, o, i, u, a);
            if (this.abortSignal.aborted)
                throw new Error(this.abortSignal.reason);
            let y = await this.applyRedirects.applyRedirectCommands(
                    l,
                    o.redirectTo,
                    d,
                    Yg(f),
                    n,
                ),
                k = await this.applyRedirects.lineralizeSegments(o, y);
            return this.processSegment(n, r, t, k.concat(h), s, !1, a);
        }
        createSnapshot(n, t, r, o, i) {
            let s = new Tr(
                    r,
                    o,
                    Object.freeze(g({}, this.urlTree.queryParams)),
                    this.urlTree.fragment,
                    ww(t),
                    nt(t),
                    t.component ?? t._loadedComponent ?? null,
                    t,
                    bw(t),
                    n,
                ),
                a = kl(s, i, this.paramsInheritanceStrategy);
            return (
                (s.params = Object.freeze(a.params)),
                (s.data = Object.freeze(a.data)),
                s
            );
        }
        async matchSegmentAgainstRoute(n, t, r, o, i, s) {
            if (this.abortSignal.aborted)
                throw new Error(this.abortSignal.reason);
            let a = (Ve) =>
                    this.createSnapshot(
                        n,
                        r,
                        Ve.consumedSegments,
                        Ve.parameters,
                        s,
                    ),
                c = await Ys(
                    pw(t, r, o, n, this.urlSerializer, a, this.abortSignal),
                );
            if ((r.path === "**" && (t.children = {}), !c?.matched))
                throw new bt(t);
            n = r._injector ?? n;
            let { routes: u } = await this.getChildConfig(n, r, o),
                l = r._loadedInjector ?? n,
                {
                    parameters: d,
                    consumedSegments: h,
                    remainingSegments: f,
                } = c,
                y = this.createSnapshot(n, r, h, d, s),
                { segmentGroup: k, slicedSegments: C } = yg(t, h, f, u, i);
            if (C.length === 0 && k.hasChildren()) {
                let Ve = await this.processChildren(l, u, k, y);
                return new _e(y, Ve);
            }
            if (u.length === 0 && C.length === 0) return new _e(y, []);
            let _ = nt(r) === i,
                ce = await this.processSegment(l, u, k, C, _ ? T : i, !0, y);
            return new _e(y, ce instanceof _e ? [ce] : []);
        }
        async getChildConfig(n, t, r) {
            if (t.children) return { routes: t.children, injector: n };
            if (t.loadChildren) {
                if (t._loadedRoutes !== void 0) {
                    let i = t._loadedNgModuleFactory;
                    return (
                        i &&
                            !t._loadedInjector &&
                            (t._loadedInjector = i.create(n).injector),
                        { routes: t._loadedRoutes, injector: t._loadedInjector }
                    );
                }
                if (this.abortSignal.aborted)
                    throw new Error(this.abortSignal.reason);
                if (
                    await Ys(sw(n, t, r, this.urlSerializer, this.abortSignal))
                ) {
                    let i = await this.configLoader.loadChildren(n, t);
                    return (
                        (t._loadedRoutes = i.routes),
                        (t._loadedInjector = i.injector),
                        (t._loadedNgModuleFactory = i.factory),
                        i
                    );
                }
                throw uw(t);
            }
            return { routes: [], injector: n };
        }
    };
function Cw(e) {
    e.sort((n, t) =>
        n.value.outlet === T
            ? -1
            : t.value.outlet === T
              ? 1
              : n.value.outlet.localeCompare(t.value.outlet),
    );
}
function Iw(e) {
    let n = e.value.routeConfig;
    return n && n.path === "";
}
function Kg(e) {
    let n = [],
        t = new Set();
    for (let r of e) {
        if (!Iw(r)) {
            n.push(r);
            continue;
        }
        let o = n.find((i) => r.value.routeConfig === i.value.routeConfig);
        o !== void 0 ? (o.children.push(...r.children), t.add(o)) : n.push(r);
    }
    for (let r of t) {
        let o = Kg(r.children);
        n.push(new _e(r.value, o));
    }
    return n.filter((r) => !t.has(r));
}
function ww(e) {
    return e.data || {};
}
function bw(e) {
    return e.resolve || {};
}
function Tw(e, n, t, r, o, i, s) {
    return se(async (a) => {
        let { state: c, tree: u } = await Dw(
            e,
            n,
            t,
            r,
            a.extractedUrl,
            o,
            i,
            s,
        );
        return B(g({}, a), { targetSnapshot: c, urlAfterRedirects: u });
    });
}
function Sw(e) {
    return se((n) => {
        let {
            targetSnapshot: t,
            guards: { canActivateChecks: r },
        } = n;
        if (!r.length) return I(n);
        let o = new Set(r.map((a) => a.route)),
            i = new Set();
        for (let a of o) if (!i.has(a)) for (let c of Jg(a)) i.add(c);
        let s = 0;
        return q(i).pipe(
            Mt((a) =>
                o.has(a)
                    ? Mw(a, t, e)
                    : ((a.data = kl(a, a.parent, e).resolve), I(void 0)),
            ),
            Ae(() => s++),
            Ii(1),
            se((a) => (s === i.size ? I(n) : ie)),
        );
    });
}
function Jg(e) {
    let n = e.children.map((t) => Jg(t)).flat();
    return [e, ...n];
}
function Mw(e, n, t) {
    let r = e.routeConfig,
        o = e._resolve;
    return (
        r?.title !== void 0 && !Bg(r) && (o[zo] = r.title),
        Fr(
            () => (
                (e.data = kl(e, e.parent, t).resolve),
                _w(o, e, n).pipe(
                    H(
                        (i) => (
                            (e._resolvedData = i),
                            (e.data = g(g({}, e.data), i)),
                            null
                        ),
                    ),
                )
            ),
        )
    );
}
function _w(e, n, t) {
    let r = Cl(e);
    if (r.length === 0) return I({});
    let o = {};
    return q(r).pipe(
        se((i) =>
            Nw(e[i], n, t).pipe(
                pt(),
                Ae((s) => {
                    if (s instanceof Sr) throw la(new Tt(), s);
                    o[i] = s;
                }),
            ),
        ),
        Ii(1),
        H(() => o),
        Yn((i) => (Gg(i) ? ie : La(i))),
    );
}
function Nw(e, n, t) {
    let r = n._environmentInjector,
        o = _r(e, r),
        i = o.resolve ? o.resolve(n, t) : Q(r, () => o(n, t));
    return Fn(i);
}
function Dg(e) {
    return Ee((n) => {
        let t = e(n);
        return t ? q(t).pipe(H(() => n)) : I(n);
    });
}
var Bl = (() => {
        class e {
            buildTitle(t) {
                let r,
                    o = t.root;
                for (; o !== void 0; )
                    ((r = this.getResolvedTitleForRoute(o) ?? r),
                        (o = o.children.find((i) => i.outlet === T)));
                return r;
            }
            getResolvedTitleForRoute(t) {
                return t.data[zo];
            }
            static ɵfac = function (r) {
                return new (r || e)();
            };
            static ɵprov = v({
                token: e,
                factory: () => p(Xg),
                providedIn: "root",
            });
        }
        return e;
    })(),
    Xg = (() => {
        class e extends Bl {
            title;
            constructor(t) {
                (super(), (this.title = t));
            }
            updateTitle(t) {
                let r = this.buildTitle(t);
                r !== void 0 && this.title.setTitle(r);
            }
            static ɵfac = function (r) {
                return new (r || e)(E(fg));
            };
            static ɵprov = v({ token: e, factory: e.ɵfac, providedIn: "root" });
        }
        return e;
    })(),
    Yt = new D("", { factory: () => ({}) }),
    jn = new D(""),
    ha = (() => {
        class e {
            componentLoaders = new WeakMap();
            childrenLoaders = new WeakMap();
            onLoadStartListener;
            onLoadEndListener;
            compiler = p(Hu);
            async loadComponent(t, r) {
                if (this.componentLoaders.get(r))
                    return this.componentLoaders.get(r);
                if (r._loadedComponent)
                    return Promise.resolve(r._loadedComponent);
                this.onLoadStartListener && this.onLoadStartListener(r);
                let o = (async () => {
                    try {
                        let i = await Ig(Q(t, () => r.loadComponent())),
                            s = await nm(tm(i));
                        return (
                            this.onLoadEndListener && this.onLoadEndListener(r),
                            (r._loadedComponent = s),
                            s
                        );
                    } finally {
                        this.componentLoaders.delete(r);
                    }
                })();
                return (this.componentLoaders.set(r, o), o);
            }
            loadChildren(t, r) {
                if (this.childrenLoaders.get(r))
                    return this.childrenLoaders.get(r);
                if (r._loadedRoutes)
                    return Promise.resolve({
                        routes: r._loadedRoutes,
                        injector: r._loadedInjector,
                    });
                this.onLoadStartListener && this.onLoadStartListener(r);
                let o = (async () => {
                    try {
                        let i = await em(
                            r,
                            this.compiler,
                            t,
                            this.onLoadEndListener,
                        );
                        return (
                            (r._loadedRoutes = i.routes),
                            (r._loadedInjector = i.injector),
                            (r._loadedNgModuleFactory = i.factory),
                            i
                        );
                    } finally {
                        this.childrenLoaders.delete(r);
                    }
                })();
                return (this.childrenLoaders.set(r, o), o);
            }
            static ɵfac = function (r) {
                return new (r || e)();
            };
            static ɵprov = v({ token: e, factory: e.ɵfac, providedIn: "root" });
        }
        return e;
    })();
async function em(e, n, t, r) {
    let o = await Ig(Q(t, () => e.loadChildren())),
        i = await nm(tm(o)),
        s;
    (i instanceof Es || Array.isArray(i)
        ? (s = i)
        : (s = await n.compileModuleAsync(i)),
        r && r(e));
    let a,
        c,
        u = !1,
        l;
    return (
        Array.isArray(s)
            ? ((c = s), (u = !0))
            : ((a = s.create(t).injector),
              (l = s),
              (c = a.get(jn, [], { optional: !0, self: !0 }).flat())),
        { routes: c.map(Ul), injector: a, factory: l }
    );
}
function Rw(e) {
    return e && typeof e == "object" && "default" in e;
}
function tm(e) {
    return Rw(e) ? e.default : e;
}
async function nm(e) {
    return e;
}
var ga = (() => {
        class e {
            static ɵfac = function (r) {
                return new (r || e)();
            };
            static ɵprov = v({
                token: e,
                factory: () => p(Aw),
                providedIn: "root",
            });
        }
        return e;
    })(),
    Aw = (() => {
        class e {
            shouldProcessUrl(t) {
                return !0;
            }
            extract(t) {
                return t;
            }
            merge(t, r) {
                return t;
            }
            static ɵfac = function (r) {
                return new (r || e)();
            };
            static ɵprov = v({ token: e, factory: e.ɵfac, providedIn: "root" });
        }
        return e;
    })(),
    Vl = new D(""),
    Hl = new D("");
function rm(e, n, t) {
    let r = e.get(Hl),
        o = e.get(W);
    if (!o.startViewTransition || r.skipNextTransition)
        return ((r.skipNextTransition = !1), new Promise((u) => setTimeout(u)));
    let i,
        s = new Promise((u) => {
            i = u;
        }),
        a = o.startViewTransition(() => (i(), xw(e)));
    (a.updateCallbackDone.catch((u) => {}),
        a.ready.catch((u) => {}),
        a.finished.catch((u) => {}));
    let { onViewTransitionCreated: c } = r;
    return (c && Q(e, () => c({ transition: a, from: n, to: t })), s);
}
function xw(e) {
    return new Promise((n) => {
        vs({ read: () => setTimeout(n) }, { injector: e });
    });
}
var Ow = () => {},
    $l = new D(""),
    ma = (() => {
        class e {
            currentNavigation = z(null, { equal: () => !1 });
            currentTransition = null;
            lastSuccessfulNavigation = z(null);
            events = new re();
            transitionAbortWithErrorSubject = new re();
            configLoader = p(ha);
            environmentInjector = p(G);
            destroyRef = p(Le);
            urlSerializer = p(Zt);
            rootContexts = p(Ln);
            location = p(Ht);
            inputBindingEnabled = p(qo, { optional: !0 }) !== null;
            titleStrategy = p(Bl);
            options = p(Yt, { optional: !0 }) || {};
            paramsInheritanceStrategy =
                this.options.paramsInheritanceStrategy || "emptyOnly";
            urlHandlingStrategy = p(ga);
            createViewTransition = p(Vl, { optional: !0 });
            navigationErrorHandler = p($l, { optional: !0 });
            navigationId = 0;
            get hasRequestedNavigation() {
                return this.navigationId !== 0;
            }
            transitions;
            afterPreactivation = () => I(void 0);
            rootComponentType = null;
            destroyed = !1;
            constructor() {
                let t = (o) => this.events.next(new na(o)),
                    r = (o) => this.events.next(new ra(o));
                ((this.configLoader.onLoadEndListener = r),
                    (this.configLoader.onLoadStartListener = t),
                    this.destroyRef.onDestroy(() => {
                        this.destroyed = !0;
                    }));
            }
            complete() {
                this.transitions?.complete();
            }
            handleNavigationRequest(t) {
                let r = ++this.navigationId;
                Y(() => {
                    this.transitions?.next(
                        B(g({}, t), {
                            extractedUrl: this.urlHandlingStrategy.extract(
                                t.rawUrl,
                            ),
                            targetSnapshot: null,
                            targetRouterState: null,
                            guards: {
                                canActivateChecks: [],
                                canDeactivateChecks: [],
                            },
                            guardsResult: null,
                            id: r,
                            routesRecognizeHandler: {},
                            beforeActivateHandler: {},
                        }),
                    );
                });
            }
            setupNavigations(t) {
                return (
                    (this.transitions = new oe(null)),
                    this.transitions.pipe(
                        be((r) => r !== null),
                        Ee((r) => {
                            let o = !1,
                                i = new AbortController(),
                                s = () =>
                                    !o && this.currentTransition?.id === r.id;
                            return I(r).pipe(
                                Ee((a) => {
                                    if (this.navigationId > r.id)
                                        return (
                                            this.cancelNavigationTransition(
                                                r,
                                                "",
                                                fe.SupersededByNewNavigation,
                                            ),
                                            ie
                                        );
                                    this.currentTransition = r;
                                    let c = this.lastSuccessfulNavigation();
                                    this.currentNavigation.set({
                                        id: a.id,
                                        initialUrl: a.rawUrl,
                                        extractedUrl: a.extractedUrl,
                                        targetBrowserUrl:
                                            typeof a.extras.browserUrl ==
                                            "string"
                                                ? this.urlSerializer.parse(
                                                      a.extras.browserUrl,
                                                  )
                                                : a.extras.browserUrl,
                                        trigger: a.source,
                                        extras: a.extras,
                                        previousNavigation: c
                                            ? B(g({}, c), {
                                                  previousNavigation: null,
                                              })
                                            : null,
                                        abort: () => i.abort(),
                                        routesRecognizeHandler:
                                            a.routesRecognizeHandler,
                                        beforeActivateHandler:
                                            a.beforeActivateHandler,
                                    });
                                    let u =
                                            !t.navigated ||
                                            this.isUpdatingInternalState() ||
                                            this.isUpdatedBrowserUrl(),
                                        l =
                                            a.extras.onSameUrlNavigation ??
                                            t.onSameUrlNavigation;
                                    if (!u && l !== "reload")
                                        return (
                                            this.events.next(
                                                new lt(
                                                    a.id,
                                                    this.urlSerializer.serialize(
                                                        a.rawUrl,
                                                    ),
                                                    "",
                                                    Cr.IgnoredSameUrlNavigation,
                                                ),
                                            ),
                                            a.resolve(!1),
                                            ie
                                        );
                                    if (
                                        this.urlHandlingStrategy.shouldProcessUrl(
                                            a.rawUrl,
                                        )
                                    )
                                        return I(a).pipe(
                                            Ee(
                                                (d) => (
                                                    this.events.next(
                                                        new Wt(
                                                            d.id,
                                                            this.urlSerializer.serialize(
                                                                d.extractedUrl,
                                                            ),
                                                            d.source,
                                                            d.restoredState,
                                                        ),
                                                    ),
                                                    d.id !== this.navigationId
                                                        ? ie
                                                        : Promise.resolve(d)
                                                ),
                                            ),
                                            Tw(
                                                this.environmentInjector,
                                                this.configLoader,
                                                this.rootComponentType,
                                                t.config,
                                                this.urlSerializer,
                                                this.paramsInheritanceStrategy,
                                                i.signal,
                                            ),
                                            Ae((d) => {
                                                ((r.targetSnapshot =
                                                    d.targetSnapshot),
                                                    (r.urlAfterRedirects =
                                                        d.urlAfterRedirects),
                                                    this.currentNavigation.update(
                                                        (h) => (
                                                            (h.finalUrl =
                                                                d.urlAfterRedirects),
                                                            h
                                                        ),
                                                    ),
                                                    this.events.next(new Uo()));
                                            }),
                                            Ee((d) =>
                                                q(
                                                    r.routesRecognizeHandler
                                                        .deferredHandle ??
                                                        I(void 0),
                                                ).pipe(H(() => d)),
                                            ),
                                            Ae(() => {
                                                let d = new jo(
                                                    a.id,
                                                    this.urlSerializer.serialize(
                                                        a.extractedUrl,
                                                    ),
                                                    this.urlSerializer.serialize(
                                                        a.urlAfterRedirects,
                                                    ),
                                                    a.targetSnapshot,
                                                );
                                                this.events.next(d);
                                            }),
                                        );
                                    if (
                                        u &&
                                        this.urlHandlingStrategy.shouldProcessUrl(
                                            a.currentRawUrl,
                                        )
                                    ) {
                                        let {
                                                id: d,
                                                extractedUrl: h,
                                                source: f,
                                                restoredState: y,
                                                extras: k,
                                            } = a,
                                            C = new Wt(
                                                d,
                                                this.urlSerializer.serialize(h),
                                                f,
                                                y,
                                            );
                                        this.events.next(C);
                                        let _ = jg(
                                            this.rootComponentType,
                                            this.environmentInjector,
                                        ).snapshot;
                                        return (
                                            (this.currentTransition = r =
                                                B(g({}, a), {
                                                    targetSnapshot: _,
                                                    urlAfterRedirects: h,
                                                    extras: B(g({}, k), {
                                                        skipLocationChange: !1,
                                                        replaceUrl: !1,
                                                    }),
                                                })),
                                            this.currentNavigation.update(
                                                (ce) => ((ce.finalUrl = h), ce),
                                            ),
                                            I(r)
                                        );
                                    } else
                                        return (
                                            this.events.next(
                                                new lt(
                                                    a.id,
                                                    this.urlSerializer.serialize(
                                                        a.extractedUrl,
                                                    ),
                                                    "",
                                                    Cr.IgnoredByUrlHandlingStrategy,
                                                ),
                                            ),
                                            a.resolve(!1),
                                            ie
                                        );
                                }),
                                H((a) => {
                                    let c = new Js(
                                        a.id,
                                        this.urlSerializer.serialize(
                                            a.extractedUrl,
                                        ),
                                        this.urlSerializer.serialize(
                                            a.urlAfterRedirects,
                                        ),
                                        a.targetSnapshot,
                                    );
                                    return (
                                        this.events.next(c),
                                        (this.currentTransition = r =
                                            B(g({}, a), {
                                                guards: VI(
                                                    a.targetSnapshot,
                                                    a.currentSnapshot,
                                                    this.rootContexts,
                                                ),
                                            })),
                                        r
                                    );
                                }),
                                JI((a) => this.events.next(a)),
                                Ee((a) => {
                                    if (
                                        ((r.guardsResult = a.guardsResult),
                                        a.guardsResult &&
                                            typeof a.guardsResult != "boolean")
                                    )
                                        throw la(
                                            this.urlSerializer,
                                            a.guardsResult,
                                        );
                                    let c = new Xs(
                                        a.id,
                                        this.urlSerializer.serialize(
                                            a.extractedUrl,
                                        ),
                                        this.urlSerializer.serialize(
                                            a.urlAfterRedirects,
                                        ),
                                        a.targetSnapshot,
                                        !!a.guardsResult,
                                    );
                                    if ((this.events.next(c), !s())) return ie;
                                    if (!a.guardsResult)
                                        return (
                                            this.cancelNavigationTransition(
                                                a,
                                                "",
                                                fe.GuardRejected,
                                            ),
                                            ie
                                        );
                                    if (a.guards.canActivateChecks.length === 0)
                                        return I(a);
                                    let u = new ea(
                                        a.id,
                                        this.urlSerializer.serialize(
                                            a.extractedUrl,
                                        ),
                                        this.urlSerializer.serialize(
                                            a.urlAfterRedirects,
                                        ),
                                        a.targetSnapshot,
                                    );
                                    if ((this.events.next(u), !s())) return ie;
                                    let l = !1;
                                    return I(a).pipe(
                                        Sw(this.paramsInheritanceStrategy),
                                        Ae({
                                            next: () => {
                                                l = !0;
                                                let d = new ta(
                                                    a.id,
                                                    this.urlSerializer.serialize(
                                                        a.extractedUrl,
                                                    ),
                                                    this.urlSerializer.serialize(
                                                        a.urlAfterRedirects,
                                                    ),
                                                    a.targetSnapshot,
                                                );
                                                this.events.next(d);
                                            },
                                            complete: () => {
                                                l ||
                                                    this.cancelNavigationTransition(
                                                        a,
                                                        "",
                                                        fe.NoDataFromResolver,
                                                    );
                                            },
                                        }),
                                    );
                                }),
                                Dg((a) => {
                                    let c = (l) => {
                                            let d = [];
                                            if (l.routeConfig?._loadedComponent)
                                                l.component =
                                                    l.routeConfig?._loadedComponent;
                                            else if (
                                                l.routeConfig?.loadComponent
                                            ) {
                                                let h = l._environmentInjector;
                                                d.push(
                                                    this.configLoader
                                                        .loadComponent(
                                                            h,
                                                            l.routeConfig,
                                                        )
                                                        .then((f) => {
                                                            l.component = f;
                                                        }),
                                                );
                                            }
                                            for (let h of l.children)
                                                d.push(...c(h));
                                            return d;
                                        },
                                        u = c(a.targetSnapshot.root);
                                    return u.length === 0
                                        ? I(a)
                                        : q(Promise.all(u).then(() => a));
                                }),
                                Dg(() => this.afterPreactivation()),
                                Ee(() => {
                                    let {
                                            currentSnapshot: a,
                                            targetSnapshot: c,
                                        } = r,
                                        u = this.createViewTransition?.(
                                            this.environmentInjector,
                                            a.root,
                                            c.root,
                                        );
                                    return u ? q(u).pipe(H(() => r)) : I(r);
                                }),
                                ft(1),
                                Ee((a) => {
                                    let c = LI(
                                        t.routeReuseStrategy,
                                        a.targetSnapshot,
                                        a.currentRouterState,
                                    );
                                    ((this.currentTransition =
                                        r =
                                        a =
                                            B(g({}, a), {
                                                targetRouterState: c,
                                            })),
                                        this.currentNavigation.update(
                                            (l) => (
                                                (l.targetRouterState = c),
                                                l
                                            ),
                                        ),
                                        this.events.next(new wr()));
                                    let u =
                                        r.beforeActivateHandler.deferredHandle;
                                    return u ? q(u.then(() => a)) : I(a);
                                }),
                                Ae((a) => {
                                    (new Rl(
                                        t.routeReuseStrategy,
                                        r.targetRouterState,
                                        r.currentRouterState,
                                        (c) => this.events.next(c),
                                        this.inputBindingEnabled,
                                    ).activate(this.rootContexts),
                                        s() &&
                                            ((o = !0),
                                            this.currentNavigation.update(
                                                (c) => ((c.abort = Ow), c),
                                            ),
                                            this.lastSuccessfulNavigation.set(
                                                Y(this.currentNavigation),
                                            ),
                                            this.events.next(
                                                new Ue(
                                                    a.id,
                                                    this.urlSerializer.serialize(
                                                        a.extractedUrl,
                                                    ),
                                                    this.urlSerializer.serialize(
                                                        a.urlAfterRedirects,
                                                    ),
                                                ),
                                            ),
                                            this.titleStrategy?.updateTitle(
                                                a.targetRouterState.snapshot,
                                            ),
                                            a.resolve(!0)));
                                }),
                                jr(
                                    qg(i.signal).pipe(
                                        be(() => !o && !r.targetRouterState),
                                        Ae(() => {
                                            this.cancelNavigationTransition(
                                                r,
                                                i.signal.reason + "",
                                                fe.Aborted,
                                            );
                                        }),
                                    ),
                                ),
                                Ae({
                                    complete: () => {
                                        o = !0;
                                    },
                                }),
                                jr(
                                    this.transitionAbortWithErrorSubject.pipe(
                                        Ae((a) => {
                                            throw a;
                                        }),
                                    ),
                                ),
                                Lr(() => {
                                    (i.abort(),
                                        o ||
                                            this.cancelNavigationTransition(
                                                r,
                                                "",
                                                fe.SupersededByNewNavigation,
                                            ),
                                        this.currentTransition?.id === r.id &&
                                            (this.currentNavigation.set(null),
                                            (this.currentTransition = null)));
                                }),
                                Yn((a) => {
                                    if (((o = !0), this.destroyed))
                                        return (r.resolve(!1), ie);
                                    if (zg(a))
                                        (this.events.next(
                                            new je(
                                                r.id,
                                                this.urlSerializer.serialize(
                                                    r.extractedUrl,
                                                ),
                                                a.message,
                                                a.cancellationCode,
                                            ),
                                        ),
                                            BI(a)
                                                ? this.events.next(
                                                      new br(
                                                          a.url,
                                                          a.navigationBehaviorOptions,
                                                      ),
                                                  )
                                                : r.resolve(!1));
                                    else {
                                        let c = new kn(
                                            r.id,
                                            this.urlSerializer.serialize(
                                                r.extractedUrl,
                                            ),
                                            a,
                                            r.targetSnapshot ?? void 0,
                                        );
                                        try {
                                            let u = Q(
                                                this.environmentInjector,
                                                () =>
                                                    this.navigationErrorHandler?.(
                                                        c,
                                                    ),
                                            );
                                            if (u instanceof Sr) {
                                                let {
                                                    message: l,
                                                    cancellationCode: d,
                                                } = la(this.urlSerializer, u);
                                                (this.events.next(
                                                    new je(
                                                        r.id,
                                                        this.urlSerializer.serialize(
                                                            r.extractedUrl,
                                                        ),
                                                        l,
                                                        d,
                                                    ),
                                                ),
                                                    this.events.next(
                                                        new br(
                                                            u.redirectTo,
                                                            u.navigationBehaviorOptions,
                                                        ),
                                                    ));
                                            } else
                                                throw (this.events.next(c), a);
                                        } catch (u) {
                                            this.options
                                                .resolveNavigationPromiseOnError
                                                ? r.resolve(!1)
                                                : r.reject(u);
                                        }
                                    }
                                    return ie;
                                }),
                            );
                        }),
                    )
                );
            }
            cancelNavigationTransition(t, r, o) {
                let i = new je(
                    t.id,
                    this.urlSerializer.serialize(t.extractedUrl),
                    r,
                    o,
                );
                (this.events.next(i), t.resolve(!1));
            }
            isUpdatingInternalState() {
                return (
                    this.currentTransition?.extractedUrl.toString() !==
                    this.currentTransition?.currentUrlTree.toString()
                );
            }
            isUpdatedBrowserUrl() {
                let t = this.urlHandlingStrategy.extract(
                        this.urlSerializer.parse(this.location.path(!0)),
                    ),
                    r = Y(this.currentNavigation),
                    o = r?.targetBrowserUrl ?? r?.extractedUrl;
                return (
                    t.toString() !== o?.toString() &&
                    !r?.extras.skipLocationChange
                );
            }
            static ɵfac = function (r) {
                return new (r || e)();
            };
            static ɵprov = v({ token: e, factory: e.ɵfac, providedIn: "root" });
        }
        return e;
    })();
function Pw(e) {
    return e !== Dr;
}
var om = new D("");
var im = (() => {
        class e {
            static ɵfac = function (r) {
                return new (r || e)();
            };
            static ɵprov = v({
                token: e,
                factory: () => p(kw),
                providedIn: "root",
            });
        }
        return e;
    })(),
    fa = class {
        shouldDetach(n) {
            return !1;
        }
        store(n, t) {}
        shouldAttach(n) {
            return !1;
        }
        retrieve(n) {
            return null;
        }
        shouldReuseRoute(n, t) {
            return n.routeConfig === t.routeConfig;
        }
        shouldDestroyInjector(n) {
            return !0;
        }
    },
    kw = (() => {
        class e extends fa {
            static ɵfac = (() => {
                let t;
                return function (o) {
                    return (t || (t = po(e)))(o || e);
                };
            })();
            static ɵprov = v({ token: e, factory: e.ɵfac, providedIn: "root" });
        }
        return e;
    })(),
    va = (() => {
        class e {
            urlSerializer = p(Zt);
            options = p(Yt, { optional: !0 }) || {};
            canceledNavigationResolution =
                this.options.canceledNavigationResolution || "replace";
            location = p(Ht);
            urlHandlingStrategy = p(ga);
            urlUpdateStrategy = this.options.urlUpdateStrategy || "deferred";
            currentUrlTree = new Ne();
            getCurrentUrlTree() {
                return this.currentUrlTree;
            }
            rawUrlTree = this.currentUrlTree;
            getRawUrlTree() {
                return this.rawUrlTree;
            }
            createBrowserPath({
                finalUrl: t,
                initialUrl: r,
                targetBrowserUrl: o,
            }) {
                let i = t !== void 0 ? this.urlHandlingStrategy.merge(t, r) : r,
                    s = o ?? i;
                return s instanceof Ne ? this.urlSerializer.serialize(s) : s;
            }
            commitTransition({
                targetRouterState: t,
                finalUrl: r,
                initialUrl: o,
            }) {
                r && t
                    ? ((this.currentUrlTree = r),
                      (this.rawUrlTree = this.urlHandlingStrategy.merge(r, o)),
                      (this.routerState = t))
                    : (this.rawUrlTree = o);
            }
            routerState = jg(null, p(G));
            getRouterState() {
                return this.routerState;
            }
            _stateMemento = this.createStateMemento();
            get stateMemento() {
                return this._stateMemento;
            }
            updateStateMemento() {
                this._stateMemento = this.createStateMemento();
            }
            createStateMemento() {
                return {
                    rawUrlTree: this.rawUrlTree,
                    currentUrlTree: this.currentUrlTree,
                    routerState: this.routerState,
                };
            }
            restoredState() {
                return this.location.getState();
            }
            static ɵfac = function (r) {
                return new (r || e)();
            };
            static ɵprov = v({
                token: e,
                factory: () => p(Fw),
                providedIn: "root",
            });
        }
        return e;
    })(),
    Fw = (() => {
        class e extends va {
            currentPageId = 0;
            lastSuccessfulId = -1;
            get browserPageId() {
                return this.canceledNavigationResolution !== "computed"
                    ? this.currentPageId
                    : (this.restoredState()?.ɵrouterPageId ??
                          this.currentPageId);
            }
            registerNonRouterCurrentEntryChangeListener(t) {
                return this.location.subscribe((r) => {
                    r.type === "popstate" &&
                        setTimeout(() => {
                            t(r.url, r.state, "popstate", { replaceUrl: !0 });
                        });
                });
            }
            handleRouterEvent(t, r) {
                t instanceof Wt
                    ? this.updateStateMemento()
                    : t instanceof lt
                      ? this.commitTransition(r)
                      : t instanceof jo
                        ? this.urlUpdateStrategy === "eager" &&
                          (r.extras.skipLocationChange ||
                              this.setBrowserUrl(this.createBrowserPath(r), r))
                        : t instanceof wr
                          ? (this.commitTransition(r),
                            this.urlUpdateStrategy === "deferred" &&
                                !r.extras.skipLocationChange &&
                                this.setBrowserUrl(
                                    this.createBrowserPath(r),
                                    r,
                                ))
                          : t instanceof je && !Lg(t)
                            ? this.restoreHistory(r)
                            : t instanceof kn
                              ? this.restoreHistory(r, !0)
                              : t instanceof Ue &&
                                ((this.lastSuccessfulId = t.id),
                                (this.currentPageId = this.browserPageId));
            }
            setBrowserUrl(t, { extras: r, id: o }) {
                let { replaceUrl: i, state: s } = r;
                if (this.location.isCurrentPathEqualTo(t) || i) {
                    let a = this.browserPageId,
                        c = g(g({}, s), this.generateNgRouterState(o, a));
                    this.location.replaceState(t, "", c);
                } else {
                    let a = g(
                        g({}, s),
                        this.generateNgRouterState(o, this.browserPageId + 1),
                    );
                    this.location.go(t, "", a);
                }
            }
            restoreHistory(t, r = !1) {
                if (this.canceledNavigationResolution === "computed") {
                    let o = this.browserPageId,
                        i = this.currentPageId - o;
                    i !== 0
                        ? this.location.historyGo(i)
                        : this.getCurrentUrlTree() === t.finalUrl &&
                          i === 0 &&
                          (this.resetInternalState(t),
                          this.resetUrlToCurrentUrlTree());
                } else
                    this.canceledNavigationResolution === "replace" &&
                        (r && this.resetInternalState(t),
                        this.resetUrlToCurrentUrlTree());
            }
            resetInternalState({ finalUrl: t }) {
                ((this.routerState = this.stateMemento.routerState),
                    (this.currentUrlTree = this.stateMemento.currentUrlTree),
                    (this.rawUrlTree = this.urlHandlingStrategy.merge(
                        this.currentUrlTree,
                        t ?? this.rawUrlTree,
                    )));
            }
            resetUrlToCurrentUrlTree() {
                this.location.replaceState(
                    this.urlSerializer.serialize(this.getRawUrlTree()),
                    "",
                    this.generateNgRouterState(
                        this.lastSuccessfulId,
                        this.currentPageId,
                    ),
                );
            }
            generateNgRouterState(t, r) {
                return this.canceledNavigationResolution === "computed"
                    ? { navigationId: t, ɵrouterPageId: r }
                    : { navigationId: t };
            }
            static ɵfac = (() => {
                let t;
                return function (o) {
                    return (t || (t = po(e)))(o || e);
                };
            })();
            static ɵprov = v({ token: e, factory: e.ɵfac, providedIn: "root" });
        }
        return e;
    })();
function ya(e, n) {
    e.events
        .pipe(
            be(
                (t) =>
                    t instanceof Ue ||
                    t instanceof je ||
                    t instanceof kn ||
                    t instanceof lt,
            ),
            H((t) =>
                t instanceof Ue || t instanceof lt
                    ? 0
                    : (
                            t instanceof je
                                ? t.code === fe.Redirect ||
                                  t.code === fe.SupersededByNewNavigation
                                : !1
                        )
                      ? 2
                      : 1,
            ),
            be((t) => t !== 2),
            ft(1),
        )
        .subscribe(() => {
            n();
        });
}
var dt = (() => {
    class e {
        get currentUrlTree() {
            return this.stateManager.getCurrentUrlTree();
        }
        get rawUrlTree() {
            return this.stateManager.getRawUrlTree();
        }
        disposed = !1;
        nonRouterCurrentEntryChangeSubscription;
        console = p(Ou);
        stateManager = p(va);
        options = p(Yt, { optional: !0 }) || {};
        pendingTasks = p(Et);
        urlUpdateStrategy = this.options.urlUpdateStrategy || "deferred";
        navigationTransitions = p(ma);
        urlSerializer = p(Zt);
        location = p(Ht);
        urlHandlingStrategy = p(ga);
        injector = p(G);
        _events = new re();
        get events() {
            return this._events;
        }
        get routerState() {
            return this.stateManager.getRouterState();
        }
        navigated = !1;
        routeReuseStrategy = p(im);
        injectorCleanup = p(om, { optional: !0 });
        onSameUrlNavigation = this.options.onSameUrlNavigation || "ignore";
        config = p(jn, { optional: !0 })?.flat() ?? [];
        componentInputBindingEnabled = !!p(qo, { optional: !0 });
        currentNavigation =
            this.navigationTransitions.currentNavigation.asReadonly();
        constructor() {
            (this.resetConfig(this.config),
                this.navigationTransitions
                    .setupNavigations(this)
                    .subscribe({ error: (t) => {} }),
                this.subscribeToNavigationEvents());
        }
        eventsSubscription = new ne();
        subscribeToNavigationEvents() {
            let t = this.navigationTransitions.events.subscribe((r) => {
                try {
                    let o = this.navigationTransitions.currentTransition,
                        i = Y(this.navigationTransitions.currentNavigation);
                    if (o !== null && i !== null) {
                        if (
                            (this.stateManager.handleRouterEvent(r, i),
                            r instanceof je &&
                                r.code !== fe.Redirect &&
                                r.code !== fe.SupersededByNewNavigation)
                        )
                            this.navigated = !0;
                        else if (r instanceof Ue)
                            ((this.navigated = !0),
                                this.injectorCleanup?.(
                                    this.routeReuseStrategy,
                                    this.routerState,
                                    this.config,
                                ));
                        else if (r instanceof br) {
                            let s = r.navigationBehaviorOptions,
                                a = this.urlHandlingStrategy.merge(
                                    r.url,
                                    o.currentRawUrl,
                                ),
                                c = g(
                                    {
                                        scroll: o.extras.scroll,
                                        browserUrl: o.extras.browserUrl,
                                        info: o.extras.info,
                                        skipLocationChange:
                                            o.extras.skipLocationChange,
                                        replaceUrl:
                                            o.extras.replaceUrl ||
                                            this.urlUpdateStrategy ===
                                                "eager" ||
                                            Pw(o.source),
                                    },
                                    s,
                                );
                            this.scheduleNavigation(a, Dr, null, c, {
                                resolve: o.resolve,
                                reject: o.reject,
                                promise: o.promise,
                            });
                        }
                    }
                    kI(r) && this._events.next(r);
                } catch (o) {
                    this.navigationTransitions.transitionAbortWithErrorSubject.next(
                        o,
                    );
                }
            });
            this.eventsSubscription.add(t);
        }
        resetRootComponentType(t) {
            ((this.routerState.root.component = t),
                (this.navigationTransitions.rootComponentType = t));
        }
        initialNavigation() {
            (this.setUpLocationChangeListener(),
                this.navigationTransitions.hasRequestedNavigation ||
                    this.navigateToSyncWithBrowser(
                        this.location.path(!0),
                        Dr,
                        this.stateManager.restoredState(),
                        { replaceUrl: !0 },
                    ));
        }
        setUpLocationChangeListener() {
            this.nonRouterCurrentEntryChangeSubscription ??=
                this.stateManager.registerNonRouterCurrentEntryChangeListener(
                    (t, r, o, i) => {
                        this.navigateToSyncWithBrowser(t, o, r, i);
                    },
                );
        }
        navigateToSyncWithBrowser(t, r, o, i) {
            let s = o?.navigationId ? o : null;
            if (o) {
                let c = g({}, o);
                (delete c.navigationId,
                    delete c.ɵrouterPageId,
                    Object.keys(c).length !== 0 && (i.state = c));
            }
            let a = this.parseUrl(t);
            this.scheduleNavigation(a, r, s, i).catch((c) => {
                this.disposed || this.injector.get(Me)(c);
            });
        }
        get url() {
            return this.serializeUrl(this.currentUrlTree);
        }
        getCurrentNavigation() {
            return Y(this.navigationTransitions.currentNavigation);
        }
        get lastSuccessfulNavigation() {
            return this.navigationTransitions.lastSuccessfulNavigation;
        }
        resetConfig(t) {
            ((this.config = t.map(Ul)), (this.navigated = !1));
        }
        ngOnDestroy() {
            this.dispose();
        }
        dispose() {
            (this._events.unsubscribe(),
                this.navigationTransitions.complete(),
                this.nonRouterCurrentEntryChangeSubscription?.unsubscribe(),
                (this.nonRouterCurrentEntryChangeSubscription = void 0),
                (this.disposed = !0),
                this.eventsSubscription.unsubscribe());
        }
        createUrlTree(t, r = {}) {
            let {
                    relativeTo: o,
                    queryParams: i,
                    fragment: s,
                    queryParamsHandling: a,
                    preserveFragment: c,
                } = r,
                u = c ? this.currentUrlTree.fragment : s,
                l = null;
            switch (a ?? this.options.defaultQueryParamsHandling) {
                case "merge":
                    l = g(g({}, this.currentUrlTree.queryParams), i);
                    break;
                case "preserve":
                    l = this.currentUrlTree.queryParams;
                    break;
                default:
                    l = i || null;
            }
            l !== null && (l = this.removeEmptyProps(l));
            let d;
            try {
                let h = o ? o.snapshot : this.routerState.snapshot.root;
                d = Og(h);
            } catch {
                ((typeof t[0] != "string" || t[0][0] !== "/") && (t = []),
                    (d = this.currentUrlTree.root));
            }
            return Pg(d, t, l, u ?? null, this.urlSerializer);
        }
        navigateByUrl(t, r = { skipLocationChange: !1 }) {
            let o = qt(t) ? t : this.parseUrl(t),
                i = this.urlHandlingStrategy.merge(o, this.rawUrlTree);
            return this.scheduleNavigation(i, Dr, null, r);
        }
        navigate(t, r = { skipLocationChange: !1 }) {
            return (Lw(t), this.navigateByUrl(this.createUrlTree(t, r), r));
        }
        serializeUrl(t) {
            return this.urlSerializer.serialize(t);
        }
        parseUrl(t) {
            try {
                return this.urlSerializer.parse(t);
            } catch {
                return (
                    this.console.warn(it(4018, !1)),
                    this.urlSerializer.parse("/")
                );
            }
        }
        isActive(t, r) {
            let o;
            if (
                (r === !0
                    ? (o = g({}, bg))
                    : r === !1
                      ? (o = g({}, Il))
                      : (o = g(g({}, Il), r)),
                qt(t))
            )
                return pg(this.currentUrlTree, t, o);
            let i = this.parseUrl(t);
            return pg(this.currentUrlTree, i, o);
        }
        removeEmptyProps(t) {
            return Object.entries(t).reduce(
                (r, [o, i]) => (i != null && (r[o] = i), r),
                {},
            );
        }
        scheduleNavigation(t, r, o, i, s) {
            if (this.disposed) return Promise.resolve(!1);
            let a, c, u;
            s
                ? ((a = s.resolve), (c = s.reject), (u = s.promise))
                : (u = new Promise((d, h) => {
                      ((a = d), (c = h));
                  }));
            let l = this.pendingTasks.add();
            return (
                ya(this, () => {
                    queueMicrotask(() => this.pendingTasks.remove(l));
                }),
                this.navigationTransitions.handleNavigationRequest({
                    source: r,
                    restoredState: o,
                    currentUrlTree: this.currentUrlTree,
                    currentRawUrl: this.currentUrlTree,
                    rawUrl: t,
                    extras: i,
                    resolve: a,
                    reject: c,
                    promise: u,
                    currentSnapshot: this.routerState.snapshot,
                    currentRouterState: this.routerState,
                }),
                u.catch(Promise.reject.bind(Promise))
            );
        }
        static ɵfac = function (r) {
            return new (r || e)();
        };
        static ɵprov = v({ token: e, factory: e.ɵfac, providedIn: "root" });
    }
    return e;
})();
function Lw(e) {
    for (let n = 0; n < e.length; n++) if (e[n] == null) throw new m(4008, !1);
}
var Uw = (() => {
        class e {
            router = p(dt);
            stateManager = p(va);
            fragment = z("");
            queryParams = z({});
            path = z("");
            serializer = p(Zt);
            constructor() {
                (this.updateState(),
                    this.router.events?.subscribe((t) => {
                        t instanceof Ue && this.updateState();
                    }));
            }
            updateState() {
                let {
                    fragment: t,
                    root: r,
                    queryParams: o,
                } = this.stateManager.getCurrentUrlTree();
                (this.fragment.set(t),
                    this.queryParams.set(o),
                    this.path.set(this.serializer.serialize(new Ne(r))));
            }
            static ɵfac = function (r) {
                return new (r || e)();
            };
            static ɵprov = v({ token: e, factory: e.ɵfac, providedIn: "root" });
        }
        return e;
    })(),
    Qt = (() => {
        class e {
            router;
            route;
            tabIndexAttribute;
            renderer;
            el;
            locationStrategy;
            hrefAttributeValue = p(new _s("href"), { optional: !0 });
            reactiveHref = $u(() =>
                this.isAnchorElement
                    ? this.computeHref(this._urlTree())
                    : this.hrefAttributeValue,
            );
            get href() {
                return Y(this.reactiveHref);
            }
            set href(t) {
                this.reactiveHref.set(t);
            }
            set target(t) {
                this._target.set(t);
            }
            get target() {
                return Y(this._target);
            }
            _target = z(void 0);
            set queryParams(t) {
                this._queryParams.set(t);
            }
            get queryParams() {
                return Y(this._queryParams);
            }
            _queryParams = z(void 0, { equal: () => !1 });
            set fragment(t) {
                this._fragment.set(t);
            }
            get fragment() {
                return Y(this._fragment);
            }
            _fragment = z(void 0);
            set queryParamsHandling(t) {
                this._queryParamsHandling.set(t);
            }
            get queryParamsHandling() {
                return Y(this._queryParamsHandling);
            }
            _queryParamsHandling = z(void 0);
            set state(t) {
                this._state.set(t);
            }
            get state() {
                return Y(this._state);
            }
            _state = z(void 0, { equal: () => !1 });
            set info(t) {
                this._info.set(t);
            }
            get info() {
                return Y(this._info);
            }
            _info = z(void 0, { equal: () => !1 });
            set relativeTo(t) {
                this._relativeTo.set(t);
            }
            get relativeTo() {
                return Y(this._relativeTo);
            }
            _relativeTo = z(void 0);
            set preserveFragment(t) {
                this._preserveFragment.set(t);
            }
            get preserveFragment() {
                return Y(this._preserveFragment);
            }
            _preserveFragment = z(!1);
            set skipLocationChange(t) {
                this._skipLocationChange.set(t);
            }
            get skipLocationChange() {
                return Y(this._skipLocationChange);
            }
            _skipLocationChange = z(!1);
            set replaceUrl(t) {
                this._replaceUrl.set(t);
            }
            get replaceUrl() {
                return Y(this._replaceUrl);
            }
            _replaceUrl = z(!1);
            isAnchorElement;
            onChanges = new re();
            applicationErrorHandler = p(Me);
            options = p(Yt, { optional: !0 });
            reactiveRouterState = p(Uw);
            constructor(t, r, o, i, s, a) {
                ((this.router = t),
                    (this.route = r),
                    (this.tabIndexAttribute = o),
                    (this.renderer = i),
                    (this.el = s),
                    (this.locationStrategy = a));
                let c = s.nativeElement.tagName?.toLowerCase();
                this.isAnchorElement =
                    c === "a" ||
                    c === "area" ||
                    !!(
                        typeof customElements == "object" &&
                        customElements
                            .get(c)
                            ?.observedAttributes?.includes?.("href")
                    );
            }
            setTabIndexIfNotOnNativeEl(t) {
                this.tabIndexAttribute != null ||
                    this.isAnchorElement ||
                    this.applyAttributeValue("tabindex", t);
            }
            ngOnChanges(t) {
                this.onChanges.next(this);
            }
            routerLinkInput = z(null);
            set routerLink(t) {
                t == null
                    ? (this.routerLinkInput.set(null),
                      this.setTabIndexIfNotOnNativeEl(null))
                    : (qt(t)
                          ? this.routerLinkInput.set(t)
                          : this.routerLinkInput.set(
                                Array.isArray(t) ? t : [t],
                            ),
                      this.setTabIndexIfNotOnNativeEl("0"));
            }
            onClick(t, r, o, i, s) {
                let a = this._urlTree();
                if (
                    a === null ||
                    (this.isAnchorElement &&
                        (t !== 0 ||
                            r ||
                            o ||
                            i ||
                            s ||
                            (typeof this.target == "string" &&
                                this.target != "_self")))
                )
                    return !0;
                let c = {
                    skipLocationChange: this.skipLocationChange,
                    replaceUrl: this.replaceUrl,
                    state: this.state,
                    info: this.info,
                };
                return (
                    this.router.navigateByUrl(a, c)?.catch((u) => {
                        this.applicationErrorHandler(u);
                    }),
                    !this.isAnchorElement
                );
            }
            ngOnDestroy() {}
            applyAttributeValue(t, r) {
                let o = this.renderer,
                    i = this.el.nativeElement;
                r !== null ? o.setAttribute(i, t, r) : o.removeAttribute(i, t);
            }
            _urlTree = Vt(
                () => {
                    (this.reactiveRouterState.path(),
                        this._preserveFragment() &&
                            this.reactiveRouterState.fragment());
                    let t = (o) => o === "preserve" || o === "merge";
                    (t(this._queryParamsHandling()) ||
                        t(this.options?.defaultQueryParamsHandling)) &&
                        this.reactiveRouterState.queryParams();
                    let r = this.routerLinkInput();
                    return r === null || !this.router.createUrlTree
                        ? null
                        : qt(r)
                          ? r
                          : this.router.createUrlTree(r, {
                                relativeTo:
                                    this._relativeTo() !== void 0
                                        ? this._relativeTo()
                                        : this.route,
                                queryParams: this._queryParams(),
                                fragment: this._fragment(),
                                queryParamsHandling:
                                    this._queryParamsHandling(),
                                preserveFragment: this._preserveFragment(),
                            });
                },
                {
                    equal: (t, r) =>
                        this.computeHref(t) === this.computeHref(r),
                },
            );
            get urlTree() {
                return Y(this._urlTree);
            }
            computeHref(t) {
                return t !== null && this.locationStrategy
                    ? (this.locationStrategy?.prepareExternalUrl(
                          this.router.serializeUrl(t),
                      ) ?? "")
                    : null;
            }
            static ɵfac = function (r) {
                return new (r || e)(
                    Z(dt),
                    Z(Be),
                    ho("tabindex"),
                    Z(Do),
                    Z(lr),
                    Z(tt),
                );
            };
            static ɵdir = Ut({
                type: e,
                selectors: [["", "routerLink", ""]],
                hostVars: 2,
                hostBindings: function (r, o) {
                    (r & 1 &&
                        ws("click", function (s) {
                            return o.onClick(
                                s.button,
                                s.ctrlKey,
                                s.shiftKey,
                                s.altKey,
                                s.metaKey,
                            );
                        }),
                        r & 2 &&
                            Is(
                                "href",
                                o.reactiveHref(),
                                gu,
                            )("target", o._target()));
                },
                inputs: {
                    target: "target",
                    queryParams: "queryParams",
                    fragment: "fragment",
                    queryParamsHandling: "queryParamsHandling",
                    state: "state",
                    info: "info",
                    relativeTo: "relativeTo",
                    preserveFragment: [
                        2,
                        "preserveFragment",
                        "preserveFragment",
                        bo,
                    ],
                    skipLocationChange: [
                        2,
                        "skipLocationChange",
                        "skipLocationChange",
                        bo,
                    ],
                    replaceUrl: [2, "replaceUrl", "replaceUrl", bo],
                    routerLink: "routerLink",
                },
                features: [ur],
            });
        }
        return e;
    })();
var Zo = class {};
var sm = (() => {
        class e {
            router;
            injector;
            preloadingStrategy;
            loader;
            subscription;
            constructor(t, r, o, i) {
                ((this.router = t),
                    (this.injector = r),
                    (this.preloadingStrategy = o),
                    (this.loader = i));
            }
            setUpPreloading() {
                this.subscription = this.router.events
                    .pipe(
                        be((t) => t instanceof Ue),
                        Mt(() => this.preload()),
                    )
                    .subscribe(() => {});
            }
            preload() {
                return this.processRoutes(this.injector, this.router.config);
            }
            ngOnDestroy() {
                this.subscription?.unsubscribe();
            }
            processRoutes(t, r) {
                let o = [];
                for (let i of r) {
                    i.providers &&
                        !i._injector &&
                        (i._injector = fr(i.providers, t, ""));
                    let s = i._injector ?? t;
                    i._loadedNgModuleFactory &&
                        !i._loadedInjector &&
                        (i._loadedInjector =
                            i._loadedNgModuleFactory.create(s).injector);
                    let a = i._loadedInjector ?? s;
                    (((i.loadChildren &&
                        !i._loadedRoutes &&
                        i.canLoad === void 0) ||
                        (i.loadComponent && !i._loadedComponent)) &&
                        o.push(this.preloadConfig(s, i)),
                        (i.children || i._loadedRoutes) &&
                            o.push(
                                this.processRoutes(
                                    a,
                                    i.children ?? i._loadedRoutes,
                                ),
                            ));
                }
                return q(o).pipe(Wn());
            }
            preloadConfig(t, r) {
                return this.preloadingStrategy.preload(r, () => {
                    if (t.destroyed) return I(null);
                    let o;
                    r.loadChildren && r.canLoad === void 0
                        ? (o = q(this.loader.loadChildren(t, r)))
                        : (o = I(null));
                    let i = o.pipe(
                        se((s) =>
                            s === null
                                ? I(void 0)
                                : ((r._loadedRoutes = s.routes),
                                  (r._loadedInjector = s.injector),
                                  (r._loadedNgModuleFactory = s.factory),
                                  this.processRoutes(
                                      s.injector ?? t,
                                      s.routes,
                                  )),
                        ),
                    );
                    if (r.loadComponent && !r._loadedComponent) {
                        let s = this.loader.loadComponent(t, r);
                        return q([i, s]).pipe(Wn());
                    } else return i;
                });
            }
            static ɵfac = function (r) {
                return new (r || e)(E(dt), E(G), E(Zo), E(ha));
            };
            static ɵprov = v({ token: e, factory: e.ɵfac, providedIn: "root" });
        }
        return e;
    })(),
    am = new D(""),
    Bw = (() => {
        class e {
            options;
            routerEventsSubscription;
            scrollEventsSubscription;
            lastId = 0;
            lastSource = Dr;
            restoredId = 0;
            store = {};
            urlSerializer = p(Zt);
            zone = p(le);
            viewportScroller = p(tl);
            transitions = p(ma);
            constructor(t) {
                ((this.options = t),
                    (this.options.scrollPositionRestoration ||= "disabled"),
                    (this.options.anchorScrolling ||= "disabled"));
            }
            init() {
                (this.options.scrollPositionRestoration !== "disabled" &&
                    this.viewportScroller.setHistoryScrollRestoration("manual"),
                    (this.routerEventsSubscription = this.createScrollEvents()),
                    (this.scrollEventsSubscription =
                        this.consumeScrollEvents()));
            }
            createScrollEvents() {
                return this.transitions.events.subscribe((t) => {
                    t instanceof Wt
                        ? ((this.store[this.lastId] =
                              this.viewportScroller.getScrollPosition()),
                          (this.lastSource = t.navigationTrigger),
                          (this.restoredId = t.restoredState
                              ? t.restoredState.navigationId
                              : 0))
                        : t instanceof Ue
                          ? ((this.lastId = t.id),
                            this.scheduleScrollEvent(
                                t,
                                this.urlSerializer.parse(t.urlAfterRedirects)
                                    .fragment,
                            ))
                          : t instanceof lt &&
                            t.code === Cr.IgnoredSameUrlNavigation &&
                            ((this.lastSource = void 0),
                            (this.restoredId = 0),
                            this.scheduleScrollEvent(
                                t,
                                this.urlSerializer.parse(t.url).fragment,
                            ));
                });
            }
            consumeScrollEvents() {
                return this.transitions.events.subscribe((t) => {
                    if (!(t instanceof Ir) || t.scrollBehavior === "manual")
                        return;
                    let r = { behavior: "instant" };
                    t.position
                        ? this.options.scrollPositionRestoration === "top"
                            ? this.viewportScroller.scrollToPosition([0, 0], r)
                            : this.options.scrollPositionRestoration ===
                                  "enabled" &&
                              this.viewportScroller.scrollToPosition(
                                  t.position,
                                  r,
                              )
                        : t.anchor && this.options.anchorScrolling === "enabled"
                          ? this.viewportScroller.scrollToAnchor(t.anchor)
                          : this.options.scrollPositionRestoration !==
                                "disabled" &&
                            this.viewportScroller.scrollToPosition([0, 0]);
                });
            }
            scheduleScrollEvent(t, r) {
                let o = Y(this.transitions.currentNavigation)?.extras.scroll;
                this.zone.runOutsideAngular(async () => {
                    (await new Promise((i) => {
                        (setTimeout(i),
                            typeof requestAnimationFrame < "u" &&
                                requestAnimationFrame(i));
                    }),
                        this.zone.run(() => {
                            this.transitions.events.next(
                                new Ir(
                                    t,
                                    this.lastSource === "popstate"
                                        ? this.store[this.restoredId]
                                        : null,
                                    r,
                                    o,
                                ),
                            );
                        }));
                });
            }
            ngOnDestroy() {
                (this.routerEventsSubscription?.unsubscribe(),
                    this.scrollEventsSubscription?.unsubscribe());
            }
            static ɵfac = function (r) {
                Ru();
            };
            static ɵprov = v({ token: e, factory: e.ɵfac });
        }
        return e;
    })();
function Gl(e, ...n) {
    return At([
        { provide: jn, multi: !0, useValue: e },
        [],
        { provide: Be, useFactory: cm },
        { provide: Eo, multi: !0, useFactory: um },
        n.map((t) => t.ɵproviders),
    ]);
}
function cm() {
    return p(dt).routerState.root;
}
function Yo(e, n) {
    return { ɵkind: e, ɵproviders: n };
}
function um() {
    let e = p(ge);
    return (n) => {
        let t = e.get(hr);
        if (n !== t.components[0]) return;
        let r = e.get(dt),
            o = e.get(lm);
        (e.get(ql) === 1 && r.initialNavigation(),
            e.get(pm, null, { optional: !0 })?.setUpPreloading(),
            e.get(am, null, { optional: !0 })?.init(),
            r.resetRootComponentType(t.componentTypes[0]),
            o.closed || (o.next(), o.complete(), o.unsubscribe()));
    };
}
var lm = new D("", { factory: () => new re() }),
    ql = new D("", { factory: () => 1 });
function dm() {
    let e = [
        { provide: fu, useValue: !0 },
        { provide: ql, useValue: 0 },
        Cs(() => {
            let n = p(ge);
            return n.get(Ku, Promise.resolve()).then(
                () =>
                    new Promise((r) => {
                        let o = n.get(dt),
                            i = n.get(lm);
                        (ya(o, () => {
                            r(!0);
                        }),
                            (n.get(ma).afterPreactivation = () => (
                                r(!0),
                                i.closed ? I(void 0) : i
                            )),
                            o.initialNavigation());
                    }),
            );
        }),
    ];
    return Yo(2, e);
}
function fm() {
    let e = [
        Cs(() => {
            p(dt).setUpLocationChangeListener();
        }),
        { provide: ql, useValue: 2 },
    ];
    return Yo(3, e);
}
var pm = new D("");
function hm(e) {
    return Yo(0, [
        { provide: pm, useExisting: sm },
        { provide: Zo, useExisting: e },
    ]);
}
function gm() {
    return Yo(8, [Ll, { provide: qo, useExisting: Ll }]);
}
function mm(e) {
    dr("NgRouterViewTransitions");
    let n = [
        { provide: Vl, useValue: rm },
        {
            provide: Hl,
            useValue: g({ skipNextTransition: !!e?.skipInitialTransition }, e),
        },
    ];
    return Yo(9, n);
}
var vm = [
        Ht,
        { provide: Zt, useClass: Tt },
        dt,
        Ln,
        { provide: Be, useFactory: cm },
        ha,
        [],
    ],
    Qo = (() => {
        class e {
            constructor() {}
            static forRoot(t, r) {
                return {
                    ngModule: e,
                    providers: [
                        vm,
                        [],
                        { provide: jn, multi: !0, useValue: t },
                        [],
                        r?.errorHandler
                            ? { provide: $l, useValue: r.errorHandler }
                            : [],
                        { provide: Yt, useValue: r || {} },
                        r?.useHash ? Hw() : $w(),
                        Vw(),
                        r?.preloadingStrategy
                            ? hm(r.preloadingStrategy).ɵproviders
                            : [],
                        r?.initialNavigation ? zw(r) : [],
                        r?.bindToComponentInputs ? gm().ɵproviders : [],
                        r?.enableViewTransitions ? mm().ɵproviders : [],
                        Gw(),
                    ],
                };
            }
            static forChild(t) {
                return {
                    ngModule: e,
                    providers: [{ provide: jn, multi: !0, useValue: t }],
                };
            }
            static ɵfac = function (r) {
                return new (r || e)();
            };
            static ɵmod = jt({ type: e });
            static ɵinj = gt({});
        }
        return e;
    })();
function Vw() {
    return {
        provide: am,
        useFactory: () => {
            let e = p(tl),
                n = p(Yt);
            return (n.scrollOffset && e.setOffset(n.scrollOffset), new Bw(n));
        },
    };
}
function Hw() {
    return { provide: tt, useClass: Ju };
}
function $w() {
    return { provide: tt, useClass: Os };
}
function zw(e) {
    return [
        e.initialNavigation === "disabled" ? fm().ɵproviders : [],
        e.initialNavigation === "enabledBlocking" ? dm().ɵproviders : [],
    ];
}
var zl = new D("");
function Gw() {
    return [
        { provide: zl, useFactory: um },
        { provide: Eo, multi: !0, useExisting: zl },
    ];
}
var Kt = class e {
    constructor(n) {
        this.http = n;
    }
    apiUrl = "https://elclaquetazo-production.up.railway.app/api";
    getMovies() {
        return this.http.get(`${this.apiUrl}/movies`);
    }
    getMovie(n) {
        return this.http.get(`${this.apiUrl}/movies/${n}`);
    }
    static ɵfac = function (t) {
        return new (t || e)(E(gl));
    };
    static ɵprov = v({ token: e, factory: e.ɵfac, providedIn: "root" });
};
var qw = (e) => ["/movie", e];
function Ww(e, n) {
    if (
        (e & 1 &&
            (b(0, "tr")(1, "td", 18),
            x(2),
            N(),
            b(3, "td")(4, "strong"),
            x(5),
            N()(),
            b(6, "td"),
            x(7),
            N(),
            b(8, "td", 19),
            x(9),
            Bu(10, "slice"),
            N(),
            b(11, "td", 20)(12, "a", 21),
            x(13, " Ver m\xE1s "),
            N()()()),
        e & 2)
    ) {
        let t = n.$implicit,
            r = n.index;
        (de(2),
            ct(r + 1),
            de(3),
            ct(t.title),
            de(2),
            ct(t.year),
            de(2),
            Ts(
                " ",
                Vu(10, 6, t.synopsis, 0, 120),
                "",
                (t.synopsis == null ? null : t.synopsis.length) > 120
                    ? "..."
                    : "",
                " ",
            ),
            de(3),
            Xe("routerLink", Uu(10, qw, t.id)));
    }
}
function Zw(e, n) {
    e & 1 && (b(0, "p", 22), x(1, " No hay pel\xEDculas registradas. "), N());
}
var Da = class e {
    constructor(n) {
        this.movieService = n;
    }
    movie = z(void 0);
    movies = Vt(() => this.movie());
    ngOnInit() {
        this.movieService
            .getMovies()
            .subscribe({
                next: (n) => this.movie.set(n),
                error: (n) => console.error("Error:", n),
            });
    }
    static ɵfac = function (t) {
        return new (t || e)(Z(Kt));
    };
    static ɵcmp = Je({
        type: e,
        selectors: [["app-movies"]],
        decls: 33,
        vars: 2,
        consts: [
            [1, "site-header"],
            [
                1,
                "container-fluid",
                "px-4",
                "py-3",
                "d-flex",
                "align-items-center",
            ],
            ["href", "index.html"],
            [
                "src",
                "assets/logo.png",
                "alt",
                "CineLog",
                1,
                "header-logo",
                "me-3",
            ],
            [1, "text-muted", "fs-5"],
            [1, "container", "py-5"],
            [1, "table-responsive"],
            [1, "table", "table-dark", "table-hover", "align-middle"],
            [2, "border-bottom", "2px solid #e50914"],
            ["scope", "col"],
            ["scope", "col", 1, "text-center"],
            [4, "ngFor", "ngForOf"],
            ["class", "text-muted text-center mt-4", 4, "ngIf"],
            [1, "site-footer"],
            [1, "container", "py-4", "text-center"],
            [1, "footer-materia"],
            [1, "footer-info"],
            ["href", "mailto:jose.chavez9027@alumnos.udg.mx"],
            [1, "text-muted"],
            [1, "text-muted", 2, "max-width", "400px"],
            [1, "text-center"],
            [
                1,
                "btn",
                "btn-sm",
                2,
                "background-color",
                "#e50914",
                "color",
                "#fff",
                3,
                "routerLink",
            ],
            [1, "text-muted", "text-center", "mt-4"],
        ],
        template: function (t, r) {
            (t & 1 &&
                (b(0, "header", 0)(1, "div", 1)(2, "a", 2),
                ye(3, "img", 3),
                N(),
                b(4, "span", 4),
                x(5, "\u2014 Cat\xE1logo de Pel\xEDculas"),
                N()()(),
                b(6, "div", 5)(7, "div", 6)(8, "table", 7)(9, "thead")(
                    10,
                    "tr",
                    8,
                )(11, "th", 9),
                x(12, "#"),
                N(),
                b(13, "th", 9),
                x(14, "T\xEDtulo"),
                N(),
                b(15, "th", 9),
                x(16, "A\xF1o"),
                N(),
                b(17, "th", 9),
                x(18, "Sinopsis"),
                N(),
                b(19, "th", 10),
                x(20, "Detalle"),
                N()()(),
                b(21, "tbody"),
                Bt(22, Ww, 14, 12, "tr", 11),
                N()()(),
                Bt(23, Zw, 2, 0, "p", 12),
                N(),
                b(24, "footer", 13)(25, "div", 14)(26, "p", 15),
                x(
                    27,
                    " Conceptualizaci\xF3n de entornos de desarrollo de aplicaciones y servicios ",
                ),
                N(),
                b(28, "p", 16),
                x(29, "Jos\xE9 Luis Ch\xE1vez Flores | C\xF3digo: 3367"),
                N(),
                b(30, "p", 16)(31, "a", 17),
                x(32, "jose.chavez9027@alumnos.udg.mx"),
                N()()()()),
                t & 2 &&
                    (de(22),
                    Xe("ngForOf", r.movies()),
                    de(),
                    Xe("ngIf", !r.movies() || r.movies().length === 0)));
        },
        dependencies: [$t, Fs, So, Qo, Qt, Xu],
        styles: [
            "[_nghost-%COMP%]{display:block;background-color:#0a0a0a;min-height:100vh}.site-header[_ngcontent-%COMP%]{background-color:#0a0a0af2;border-bottom:2px solid #e50914;position:relative;z-index:10}.header-logo[_ngcontent-%COMP%]{height:48px;width:auto}.brand-name[_ngcontent-%COMP%]{font-family:Playfair Display,serif;font-size:1.8rem;color:#e50914;letter-spacing:2px}.brand-name[_ngcontent-%COMP%]{font-family:Playfair Display,serif;font-size:2rem;color:#e50914;letter-spacing:2px}.table-dark[_ngcontent-%COMP%]{--bs-table-bg: #111111;--bs-table-hover-bg: #95030a;--bs-table-border-color: #222222}thead[_ngcontent-%COMP%]   th[_ngcontent-%COMP%]{color:#e50914;font-weight:600;letter-spacing:1px;text-transform:uppercase;font-size:.85rem}.site-footer[_ngcontent-%COMP%]{background-color:#0a0a0a;border-top:2px solid #e50914;color:#888}.footer-materia[_ngcontent-%COMP%]{font-size:.85rem;color:#e50914;margin-bottom:.3rem;font-weight:400}.footer-info[_ngcontent-%COMP%]{font-size:.85rem;color:#aaa;margin-bottom:.2rem}.footer-info[_ngcontent-%COMP%]   a[_ngcontent-%COMP%]{color:#aaa;text-decoration:none}.footer-info[_ngcontent-%COMP%]   a[_ngcontent-%COMP%]:hover{color:#e50914}.text-muted[_ngcontent-%COMP%]{color:#999!important}",
        ],
    });
};
function Yw(e, n) {
    if ((e & 1 && ye(0, "img", 22), e & 2)) {
        let t = bs(2);
        Xe("src", t.movie().cover, hs)("alt", t.movie().title);
    }
}
function Qw(e, n) {
    if (
        (e & 1 &&
            (b(0, "div", 13)(1, "div", 14),
            Bt(2, Yw, 1, 2, "img", 15),
            b(3, "div", 16)(4, "h2", 17),
            x(5),
            N(),
            b(6, "p", 18),
            x(7),
            N(),
            ye(8, "hr", 19),
            b(9, "p", 20),
            x(10),
            N(),
            b(11, "a", 21),
            x(12, " \u2190 Volver al cat\xE1logo "),
            N()()()()),
        e & 2)
    ) {
        let t = bs();
        (de(2),
            Xe("ngIf", t.movie().cover),
            de(3),
            ct(t.movie().title),
            de(2),
            ct(t.movie().year),
            de(3),
            ct(t.movie().synopsis));
    }
}
function Kw(e, n) {
    e & 1 && (b(0, "p", 23), x(1, "Cargando pel\xEDcula..."), N());
}
var Ea = class e {
    constructor(n, t) {
        this.movieService = n;
        this.route = t;
    }
    movie = z(void 0);
    movies = Vt(() => this.movie());
    ngOnInit() {
        let n = Number(this.route.snapshot.paramMap.get("id"));
        this.movieService
            .getMovie(n)
            .subscribe({
                next: (t) => this.movie.set(t),
                error: (t) => console.error("Error:", t),
            });
    }
    static ɵfac = function (t) {
        return new (t || e)(Z(Kt), Z(Be));
    };
    static ɵcmp = Je({
        type: e,
        selectors: [["app-movie"]],
        decls: 18,
        vars: 2,
        consts: [
            [1, "site-header"],
            [
                1,
                "container-fluid",
                "px-4",
                "py-3",
                "d-flex",
                "align-items-center",
            ],
            ["routerLink", "/"],
            [
                "src",
                "assets/logo.png",
                "alt",
                "CineLog",
                1,
                "header-logo",
                "me-3",
            ],
            [1, "text-muted", "fs-5"],
            [1, "container", "py-5"],
            ["class", "movie-card mx-auto", 4, "ngIf"],
            ["class", "text-muted text-center mt-4", 4, "ngIf"],
            [1, "site-footer"],
            [1, "container", "py-4", "text-center"],
            [1, "footer-materia"],
            [1, "footer-info"],
            ["href", "mailto:jose.chavez9027@alumnos.udg.mx"],
            [1, "movie-card", "mx-auto"],
            [1, "d-flex", "gap-4", "align-items-start"],
            ["class", "movie-cover", 3, "src", "alt", 4, "ngIf"],
            [1, "flex-grow-1"],
            [1, "movie-title"],
            [1, "movie-year", "text-muted", "mb-3"],
            [2, "border-color", "#e50914", "opacity", "1"],
            [1, "movie-synopsis"],
            [
                "routerLink",
                "/movies",
                1,
                "btn",
                "btn-sm",
                "mt-3",
                2,
                "background-color",
                "#e50914",
                "color",
                "#fff",
            ],
            [1, "movie-cover", 3, "src", "alt"],
            [1, "text-muted", "text-center", "mt-4"],
        ],
        template: function (t, r) {
            (t & 1 &&
                (b(0, "header", 0)(1, "div", 1)(2, "a", 2),
                ye(3, "img", 3),
                N(),
                b(4, "span", 4),
                x(5, "\u2014 Detalle de Pel\xEDcula"),
                N()()(),
                b(6, "div", 5),
                Bt(7, Qw, 13, 4, "div", 6)(8, Kw, 2, 0, "p", 7),
                N(),
                b(9, "footer", 8)(10, "div", 9)(11, "p", 10),
                x(
                    12,
                    " Conceptualizaci\xF3n de entornos de desarrollo de aplicaciones y servicios ",
                ),
                N(),
                b(13, "p", 11),
                x(14, "Jos\xE9 Luis Ch\xE1vez Flores | C\xF3digo: 3367"),
                N(),
                b(15, "p", 11)(16, "a", 12),
                x(17, "jose.chavez9027@alumnos.udg.mx"),
                N()()()()),
                t & 2 &&
                    (de(7),
                    Xe("ngIf", r.movie()),
                    de(),
                    Xe("ngIf", !r.movie())));
        },
        dependencies: [$t, So, Qt],
        styles: [
            "[_nghost-%COMP%]{display:block;background-color:#0a0a0a;min-height:100vh}.site-header[_ngcontent-%COMP%]{background-color:#0a0a0af2;border-bottom:2px solid #e50914;position:relative;z-index:10}.header-logo[_ngcontent-%COMP%]{height:48px;width:auto}.movie-card[_ngcontent-%COMP%]{background:#111;border:1px solid #222222;border-radius:12px;padding:2rem;max-width:860px}.movie-cover[_ngcontent-%COMP%]{width:200px;border-radius:8px;border:2px solid #e50914;object-fit:cover}.movie-title[_ngcontent-%COMP%]{font-family:Playfair Display,serif;font-size:2rem;color:#fff;letter-spacing:1px;margin-bottom:.3rem}.movie-year[_ngcontent-%COMP%]{font-size:1rem;color:#999!important}.movie-synopsis[_ngcontent-%COMP%]{color:#ccc;font-size:1rem;line-height:1.8}.text-muted[_ngcontent-%COMP%]{color:#999!important}.site-footer[_ngcontent-%COMP%]{background-color:#0a0a0a;border-top:2px solid #e50914;color:#888}.footer-materia[_ngcontent-%COMP%]{font-size:.85rem;color:#e50914;margin-bottom:.3rem;font-weight:400}.footer-info[_ngcontent-%COMP%]{font-size:.85rem;color:#aaa;margin-bottom:.2rem}.footer-info[_ngcontent-%COMP%]   a[_ngcontent-%COMP%]{color:#aaa;text-decoration:none}.footer-info[_ngcontent-%COMP%]   a[_ngcontent-%COMP%]:hover{color:#e50914}",
        ],
    });
};
var Ca = class e {
    static ɵfac = function (t) {
        return new (t || e)();
    };
    static ɵcmp = Je({
        type: e,
        selectors: [["app-index"]],
        decls: 24,
        vars: 0,
        consts: [
            [1, "site-wrapper"],
            [1, "site-header"],
            [
                1,
                "container-fluid",
                "px-4",
                "py-3",
                "d-flex",
                "align-items-center",
            ],
            ["routerLink", "/"],
            [
                "src",
                "assets/logo.png",
                "alt",
                "CineLog",
                1,
                "header-logo",
                "me-3",
            ],
            [1, "site-main"],
            [1, "overlay"],
            [
                1,
                "container",
                "h-100",
                "d-flex",
                "align-items-center",
                "justify-content-center",
            ],
            [1, "hero-card", "text-center"],
            [1, "hero-title"],
            [1, "hero-description"],
            ["routerLink", "/movies", 1, "btn", "btn-explore", "mt-3"],
            [1, "site-footer"],
            [1, "container", "py-4", "text-center"],
            [1, "footer-materia"],
            [1, "footer-info"],
            ["href", "mailto:jose.chavez9027@alumnos.udg.mx"],
        ],
        template: function (t, r) {
            t & 1 &&
                (b(0, "div", 0)(1, "header", 1)(2, "div", 2)(3, "a", 3),
                ye(4, "img", 4),
                N()()(),
                b(5, "main", 5),
                ye(6, "div", 6),
                b(7, "div", 7)(8, "div", 8)(9, "h1", 9),
                x(10, "Bienvenido a El Claquetazo"),
                N(),
                b(11, "p", 10),
                x(
                    12,
                    " El Claquetazo es tu cat\xE1logo personal de pel\xEDculas. Explora nuestra colecci\xF3n, descubre sinopsis, a\xF1os de estreno y portadas de las mejores pel\xEDculas registradas en nuestra base de datos. Una experiencia visual pensada para los amantes del cine. ",
                ),
                N(),
                b(13, "a", 11),
                x(14, "Ver cat\xE1logo"),
                N()()()(),
                b(15, "footer", 12)(16, "div", 13)(17, "p", 14),
                x(
                    18,
                    " Conceptualizaci\xF3n de entornos de desarrollo de aplicaciones y servicios ",
                ),
                N(),
                b(19, "p", 15),
                x(20, "Jos\xE9 Luis Ch\xE1vez Flores | C\xF3digo: 3367"),
                N(),
                b(21, "p", 15)(22, "a", 16),
                x(23, "jose.chavez9027@alumnos.udg.mx"),
                N()()()()());
        },
        dependencies: [Qo, Qt],
        styles: [
            '@font-face{font-family:Lato;font-style:normal;font-weight:300;font-display:swap;src:url(https://fonts.gstatic.com/s/lato/v25/S6u9w4BMUTPHh7USSwaPGR_p.woff2) format("woff2");unicode-range:U+0100-02BA,U+02BD-02C5,U+02C7-02CC,U+02CE-02D7,U+02DD-02FF,U+0304,U+0308,U+0329,U+1D00-1DBF,U+1E00-1E9F,U+1EF2-1EFF,U+2020,U+20A0-20AB,U+20AD-20C0,U+2113,U+2C60-2C7F,U+A720-A7FF}@font-face{font-family:Lato;font-style:normal;font-weight:300;font-display:swap;src:url(https://fonts.gstatic.com/s/lato/v25/S6u9w4BMUTPHh7USSwiPGQ.woff2) format("woff2");unicode-range:U+0000-00FF,U+0131,U+0152-0153,U+02BB-02BC,U+02C6,U+02DA,U+02DC,U+0304,U+0308,U+0329,U+2000-206F,U+20AC,U+2122,U+2191,U+2193,U+2212,U+2215,U+FEFF,U+FFFD}@font-face{font-family:Lato;font-style:normal;font-weight:400;font-display:swap;src:url(https://fonts.gstatic.com/s/lato/v25/S6uyw4BMUTPHjxAwXjeu.woff2) format("woff2");unicode-range:U+0100-02BA,U+02BD-02C5,U+02C7-02CC,U+02CE-02D7,U+02DD-02FF,U+0304,U+0308,U+0329,U+1D00-1DBF,U+1E00-1E9F,U+1EF2-1EFF,U+2020,U+20A0-20AB,U+20AD-20C0,U+2113,U+2C60-2C7F,U+A720-A7FF}@font-face{font-family:Lato;font-style:normal;font-weight:400;font-display:swap;src:url(https://fonts.gstatic.com/s/lato/v25/S6uyw4BMUTPHjx4wXg.woff2) format("woff2");unicode-range:U+0000-00FF,U+0131,U+0152-0153,U+02BB-02BC,U+02C6,U+02DA,U+02DC,U+0304,U+0308,U+0329,U+2000-206F,U+20AC,U+2122,U+2191,U+2193,U+2212,U+2215,U+FEFF,U+FFFD}@font-face{font-family:Playfair Display;font-style:normal;font-weight:700;font-display:swap;src:url(https://fonts.gstatic.com/s/playfairdisplay/v40/nuFvD-vYSZviVYUb_rj3ij__anPXJzDwcbmjWBN2PKeiunDTbtPY_Q.woff2) format("woff2");unicode-range:U+0301,U+0400-045F,U+0490-0491,U+04B0-04B1,U+2116}@font-face{font-family:Playfair Display;font-style:normal;font-weight:700;font-display:swap;src:url(https://fonts.gstatic.com/s/playfairdisplay/v40/nuFvD-vYSZviVYUb_rj3ij__anPXJzDwcbmjWBN2PKeiunDYbtPY_Q.woff2) format("woff2");unicode-range:U+0102-0103,U+0110-0111,U+0128-0129,U+0168-0169,U+01A0-01A1,U+01AF-01B0,U+0300-0301,U+0303-0304,U+0308-0309,U+0323,U+0329,U+1EA0-1EF9,U+20AB}@font-face{font-family:Playfair Display;font-style:normal;font-weight:700;font-display:swap;src:url(https://fonts.gstatic.com/s/playfairdisplay/v40/nuFvD-vYSZviVYUb_rj3ij__anPXJzDwcbmjWBN2PKeiunDZbtPY_Q.woff2) format("woff2");unicode-range:U+0100-02BA,U+02BD-02C5,U+02C7-02CC,U+02CE-02D7,U+02DD-02FF,U+0304,U+0308,U+0329,U+1D00-1DBF,U+1E00-1E9F,U+1EF2-1EFF,U+2020,U+20A0-20AB,U+20AD-20C0,U+2113,U+2C60-2C7F,U+A720-A7FF}@font-face{font-family:Playfair Display;font-style:normal;font-weight:700;font-display:swap;src:url(https://fonts.gstatic.com/s/playfairdisplay/v40/nuFvD-vYSZviVYUb_rj3ij__anPXJzDwcbmjWBN2PKeiunDXbtM.woff2) format("woff2");unicode-range:U+0000-00FF,U+0131,U+0152-0153,U+02BB-02BC,U+02C6,U+02DA,U+02DC,U+0304,U+0308,U+0329,U+2000-206F,U+20AC,U+2122,U+2191,U+2193,U+2212,U+2215,U+FEFF,U+FFFD}.site-wrapper[_ngcontent-%COMP%]{display:flex;flex-direction:column;min-height:100vh;font-family:Lato,sans-serif}.site-header[_ngcontent-%COMP%]{background-color:#0a0a0af2;border-bottom:2px solid #e50914;position:relative;z-index:10}.header-logo[_ngcontent-%COMP%]{height:58px;width:auto}.brand-name[_ngcontent-%COMP%]{font-family:Playfair Display,serif;font-size:1.8rem;color:#e50914;letter-spacing:2px}.site-main[_ngcontent-%COMP%]{flex:1;position:relative;background-image:url(https://images.unsplash.com/photo-1489599849927-2ee91cede3ba?w=1600&q=80);background-size:cover;background-position:center;display:flex;align-items:center;min-height:70vh}.overlay[_ngcontent-%COMP%]{position:absolute;inset:0;background:#000000b8}.hero-card[_ngcontent-%COMP%]{position:relative;z-index:2;max-width:680px;padding:2.5rem;background:#ffffff0d;border:1px solid rgba(255,255,255,.12);border-radius:12px;-webkit-backdrop-filter:blur(6px);backdrop-filter:blur(6px)}.hero-title[_ngcontent-%COMP%]{font-family:Playfair Display,serif;font-size:3rem;color:#fff;margin-bottom:1.2rem;letter-spacing:1px}.hero-description[_ngcontent-%COMP%]{font-size:1.1rem;color:#ccc;font-weight:300;line-height:1.8}.btn-explore[_ngcontent-%COMP%]{background-color:#e50914;color:#fff;font-size:1rem;font-weight:400;padding:.6rem 2rem;border-radius:4px;text-decoration:none;transition:background .2s}.btn-explore[_ngcontent-%COMP%]:hover{background-color:#b0060f;color:#fff}.site-footer[_ngcontent-%COMP%]{background-color:#0a0a0a;border-top:2px solid #e50914;color:#888}.footer-materia[_ngcontent-%COMP%]{font-size:.85rem;color:#e50914;margin-bottom:.3rem;font-weight:400}.footer-info[_ngcontent-%COMP%]{font-size:.85rem;color:#aaa;margin-bottom:.2rem}.footer-info[_ngcontent-%COMP%]   a[_ngcontent-%COMP%]{color:#aaa;text-decoration:none}.footer-info[_ngcontent-%COMP%]   a[_ngcontent-%COMP%]:hover{color:#e50914}',
        ],
    });
};
var ym = [
    { path: "movies", component: Da },
    { path: "movie/:id", component: Ea },
    { path: "", redirectTo: "index", pathMatch: "full" },
    { path: "index", component: Ca },
];
var Dm = { providers: [Bc(), Gl(ym)] };
var Ia = class e {
    constructor(n) {
        this.movieService = n;
    }
    movies = [];
    ngOnInit() {
        this.movieService.getMovies().subscribe({
            next: (n) => {
                ((this.movies = n), console.log("Conexi\xF3n exitosa:", n));
            },
            error: (n) => {
                console.error("Error:", n);
            },
        });
    }
    static ɵfac = function (t) {
        return new (t || e)(Z(Kt));
    };
    static ɵcmp = Je({
        type: e,
        selectors: [["app-root"]],
        decls: 1,
        vars: 0,
        template: function (t, r) {
            t & 1 && ye(0, "router-outlet");
        },
        dependencies: [$t, Go],
        encapsulation: 2,
    });
};
ll(Ia, Dm).catch((e) => console.error(e));
